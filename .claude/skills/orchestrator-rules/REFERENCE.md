...
- **Stated intent has four homes.** The named initiatives of the operator and the admiral live in the active plan's order, in the dated directives in `.harmonik/context/captain-lanes.md`, in `.harmonik/crew/admiral-initiatives.md`, and in the direction-log RETURN-PATH. A flagship that no ranking surfaces is a signal to re-float it, never a reason to work past it onto grab-bag churn.
...
**Refresh-then-act is LIGHT.** Re-derive the one fact you are about to act on, not everything. Act on the boot digest's live numbers, never on a claim carried in a doc or a handoff — that holds for ALL durable docs, not only HANDOFF. For a one-off in-loop action between boots, re-derive only the single fact you are betting on, for example `br ready --parent <epic> --limit 0` for the lane you are about to staff. A glance, not a re-audit.
