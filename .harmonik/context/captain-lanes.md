...
> - **TRUE ROOT CAUSE CONFIRMED (kynes, comms 019f4ee9 — code-level):** the daemon's DIRECT pi-launch path (handler.Launch → newSessionWithIDs, session.go:196) ALWAYS opens a stdin PIPE via cmd.StdinPipe(), but the only stdin close/EOF is gated behind `if spec.HandlerSpec != nil` (handler.go:246). pi's HandlerSpec is NIL (its prompt is argv, not HC-005 stdin) → the pipe is NEVER closed → pi blocks in libuv kevent on fd0 BEFORE any model POST. StdinDevNull:true (pilaunchspec.go:332) is IGNORED in the direct path (only honored on the tmux/substrate path, tmuxsubstrate.go:924). Confirmed with srt OFF + no tmux (direct daemon child, fd0=UNFED PIPE via lsof). --no-extensions/srt/extensions ALL REFUTED.
> - **FIX (bug hk-y20d2; first attempt a433e669 was NECESSARY BUT INSUFFICIENT — kynes proved still-hangs on scratch, comms 019f4eff):** a433e669 makes handler.Launch honor StdinDevNull, but the flag is DROPPED through the harness conversion chain: buildPiLaunchSpec sets StdinDevNull:true → PiHarness.LaunchSpec (piharness.go:176) → handlercontract.SpawnSpec (HAS NO StdinDevNull field → dropped) → buildCodexRoutedLaunchSpec (harnessregistry.go:113) rebuilds handler.LaunchSpec with StdinDevNull defaulting FALSE → StdinPipe → unfed pipe → hang. **COMPLETE fix = a433e669 PLUS (1) add StdinDevNull to handlercontract.SpawnSpec, (2) set it in PiHarness.LaunchSpec, (3) copy it in buildCodexRoutedLaunchSpec.** Regression test MUST assert fd0=/dev/null end-to-end through the ROUTED path (not handler.Launch in isolation — that's what missed it). Re-dispatched on the CLAUDE harness (self-fix bootstrap trap: daemon was implementing the pi-fix under srt-wrapped pi = hangs). **Redeploy HELD until the COMPLETE fix proves pi green IN-DAEMON on scratch /tmp/hkg0 (captain-authority redeploy trigger).**
...
> - **#3 CODEX-AS-CREW — piter** (piter-q, hk-q3ovr, Opus): captain scoped the childless epic into 6 beads (spike hk-fijwi → hk-l63b9 → hk-lrf30 → hk-0ysh3/6z72r/8efdl). Spike concludes in a SESSION-MODEL choice needing OPERATOR ratification (persistent Codex TUI vs bounded `codex exec resume` loop vs app-server driver) — piter surfaces options, captain routes to operator. PENDING operator decision.
> - **#4 QUALITY — stilgar** (stilgar-q, hk-x2spu+hk-ih5k6+hk-vdqe2): fail-closed gate track, now a PARALLEL quality track (no longer flagship blocker). + new main-lint-debt cleanup bead directed here.
> - **#5 COMMS-HARNESS — yueh** (yueh2-q, hk-7m7o2, Sonnet): systematic context-cancel; holding T4 operator-design bead. (old yueh-q paused → yueh2-q.)
> - DEPRIORITIZED, do NOT staff: eval-program, flywheel, dehardcode.
>
> **PI DECOUPLE (event 019f4ec1, still live):** the pi/flagship redeploy is NO LONGER gated on hk-x2spu + hk-ih5k6. The SOLE redeploy gate is ONE isolated GATE-0 e2e test (repro pi hang on OLD binary d7abf34a → prove green with afa32372; doubles as root-cause confirmation). On GATE-0 green, captain redeploys on own authority.
> - **LANE 1 — kynes** (Opus, kynes-q, epic hk-hcrvb): RE-TASKED 01:25Z off the "stand by for redeploy" idle-wait → now OWNS **GATE-0** (flagship critical path). Builds the isolated scratch-daemon e2e. **⚠️ ROOT-CAUSE COHERENCE GAP — captain CONFIRMED (01:25Z):** the old hk-ttina theory ("pi wedges on extension-load without --no-extensions") is SUSPECT — `.pi/extensions/*` is NOT tracked on current main (flywheel tree deleted Jul2 by 353fc3c1), NO `~/.pi/extensions` in home, NONE in a live worktree, and live binary d7abf34a (Jul8) POSTDATES the deletion. So there is nothing to load; the A/B green may have a DIFFERENT cause. GATE-0 must OBSERVE whether any extension is actually present at launch + report {e2e result, extensions-present y/n}. On GATE-0 green → captain redeploys → kynes re-runs pi seed → hk-hcrvb complete.
> - **LANE 2 — stilgar** (Opus, stilgar-q): hk-x2spu + hk-ih5k6 now a PARALLEL QUALITY TRACK, NO LONGER the flagship/redeploy blocker. hk-x2spu re-scoped (land approved check-hooks.sh+Makefile, prove acceptance in ISOLATED worktree, NO primary-repo lefthook-install); hk-ih5k6 running. hk-vdqe2 (P1 keeper race, HARD e2e-repro merge gate) sequences after. NEW bead directed onto stilgar's lane: containedctx/contextcheck main-lint-debt cleanup — gates the LIVE fail-closed activation of hk-x2spu's hooks (NOT the redeploy anymore).
> - kynes-q stays PAUSED/preserved; ops-monitor flags paused-queue:kynes-q — EXPECTED (scratch-daemon e2e does not use the fleet queue). gb-mbp live re-enable still HELD for operator.

## ⭐⭐ (SUPERSEDED 01:25Z by 019f4ec1 — redeploy decoupled) CURRENT TRUTH 2026-07-11 ~00:30Z — pi:local ROOT-CAUSED; TWO lanes (kynes flagship stand-by + stilgar gate-clear)
> Two active lanes.
> - **LANE 1 — kynes** (Opus, kynes-q, epic hk-hcrvb): flagship core-loop-proof, on STAND-BY. claude:local = GREEN + shipped (5624a948). **pi:local = RED, ROOT-CAUSED (hk-ttina, CONVERGED):** the live fleet daemon (commit **d7abf34a**) launches pi **WITHOUT `--no-extensions`**, so pi wedges in extension-load startup BEFORE any network/model contact → hangs to the ~30min node deadline. NOT srt / NOT backend / NOT agent_ready (pi is CompletionProcessExit, no agent_ready gate) / NOT AllowedDomains. Proven: deterministic 2× in-daemon repro + a controlled A/B (same backend, differ ONLY by adding `--no-extensions` → greens, 131KB NDJSON) + git linchpin (d7abf34a PREDATES the fix commit **afa32372** "pass --no-extensions to pi", PI-022/hk-9s5fx). Reconciles the old "pi green in-daemon" datapoint: that was a NEWER binary (with afa32372). **FIX = redeploy fleet daemon onto a binary with afa32372 (already on main).** kynes-q intentionally PAUSED (preserved diagnostic state); ops-monitor will keep flagging paused-queue:kynes-q — EXPECTED, do NOT resume. On gate-clear + redeploy: kynes re-runs the pi seed to prove GREEN → flagship complete.
> - **LANE 2 — stilgar** (Opus, stilgar-q, epic hk-x2spu): redeploy-GATE-CLEAR lane (flagship critical path — clearing these LIFTS the redeploy gate). Verified comms-online + dispatched both: **hk-x2spu** (P1 lefthook-in-PATH → reviewer-trailer gate fires; IN_PROGRESS) + **hk-ih5k6** (P2 regression tests for 0866a2f0/a63275e1/c3307acc). Next after both close: hk-rfhaw + hk-rg526, then top `kerf next`.
> - **Redeploy authority:** captain redeploys on own authority ONCE both gate beads close (announce HOLD→GREEN); do NOT redeploy before.
> - HELD for operator (unrelated): gb-mbp live re-enable — do NOT auto-flip.
> - Live trace target (optional): hung pi pid 57764 (run 019f4e8a) left alive for operator `sudo fs_usage`; auto-reaps at its 30min deadline.
> - **WATCH RESTORED 2026-07-11 00:39Z** (admiral-directed, KNOWN staffing gap): always-on triage session back (crew watch, session 4ff9724e) after >43h down — resumes filtering ops-monitor/stall/release/paused-queue escalations off the captain. Confirm in `comms who`.
> - **hk-vdqe2 (P1, keeper clear→brief race, cycle.go completeCycleTail) STAFFED onto stilgar** — sequences AFTER the two gate beads (redeploy critical path), AHEAD of rfhaw/rg526. HARD MERGE GATE (operator, non-negotiable): rigorous ACTUAL e2e twin/scenario-harness repro of the slow-handoff/busy-pane race + a PERMANENT regression test failing-on-current / passing-after-fix — NOT unit tests alone.
> - **FLEET MERGE OUTAGE — mitigated 2026-07-11 ~00:57Z (captain):** hk-x2spu's implementer ran `lefthook install` on the PRIMARY repo, activating pre-push=`make check` BEFORE main is green under it → main's real pre-existing lint debt (containedctx/contextcheck in internal/daemon/daemon.go, workloop.go, export_test.go) + 4 stale `.claire/worktrees/agent-*` placeholder dirs failed the gate → push_failed → ALL daemon merges blocked (hk-x2spu rejected despite reviewer APPROVE; hk-ih5k6 would fail same). Violates locked quality-enforcement-A sequence (fail-closed live ONLY on main-green + REDEPLOY). CAPTAIN removed the premature primary-repo pre-push+pre-commit shims (backed up `.harmonik/context/hook-mitigation-backup/`, reinstall via `lefthook install`) — enforcing the locked decision. **NEW redeploy-gate dependency:** main's containedctx/contextcheck lint debt must be cleared (or nolint'd) + `.claire` placeholders purged BEFORE hooks can go fail-closed live. hk-x2spu re-scoped: land approved code, prove acceptance in an ISOLATED worktree, do NOT primary-install. Orphaned `git stash` (settings.json drift) left by the failed run — stilgar to reconcile.

## ⭐⭐ (SUPERSEDED 2026-07-11 00:30Z — pi ROOT-CAUSED above) 2026-07-10 ~20:48Z — TRACK A: flagship core-loop-proof, kynes single lane; rebase ABANDONED (branch superseded)
...
> - **NOW (07-10 ~21:5xZ):** T9 live full-matrix run on MAIN via scratch daemon, per-leg:
>   - **claude:local = GREEN** (proven end-to-end; harness path gap1/gap3/gap5 + re-spec'd gap4/t10 all pass). Root insight: the T9 cells were authored for single-mode but production runs **dot mode** (config workflow_mode:dot; 77MB daemon history has ZERO merge events, terminals = close/consolidate/finalize). Captain APPROVED aligning 3 assertions to dot-parity (gap4 expect dot; t10 = APPROVE terminal + commit_landed==true; gap1 harness_selected) WITH gap3 landing-proof kept REQUIRED (not goalpost-moving — verified against real daemon). Re-spec committed local-on-main, self-test 29/0, push APPROVED (scripts/spec only, NOT a redeploy → gate untouched).
>   - **pi:local = RED, isolated to ornith agent hanging pre-agent_ready the full ~30min node deadline.** kynes classifies (c) real pi-harness defect. Captain RULING (event 019f4e28): DISCRIMINATOR-FIRST before any major-issue-fanout — memory says ornith agent_ready is genuinely ~20min + the ~3min stall is a KNOWN false-positive, and proven-green pi was IN-DAEMON not scratch+srt; the 30min deadline may be CLIPPING a slow agent. Re-run pi:local with extended ~45-60min deadline: greens-with-time = deadline-tuning (not a defect); stays-hung-past-45min = CONFIRMED defect → captain-orchestrated fanout. File P1 pi-hang bead regardless (KNOWN-BLOCKED). pi stays in kynes's lane. **UPDATE:** ruling accepted — bead **hk-ttina** filed; discriminator RUNNING in bg. MECHANISM: the ~30min clip = env var `SCRATCH_BATCH_TIMEOUT` default 1800s (≈1820s observed) — NO rebuild/redeploy needed. Primary discriminator = re-run at `SCRATCH_BATCH_TIMEOUT=3600` (60min). IF-STILL-HANGS arms (admiral-added, captain-adopted): ARM1 = re-run same seed IN-DAEMON (only proven-green config: minimal warm_read [/opt/homebrew,/usr/lib,/System/Library]) = decisive scratch+srt-vs-in-daemon tie-breaker (historical srt egress/fs block is the top suspect); ARM2 = DGX health-check (low prior — engine probed <1s healthy; hang is pre-agent_ready client-side). Operator wants a PING on verdict. Awaiting ~60min verdict.
>   - codex KNOWN-SKIP, remote SKIP-loud (no tcp worker).
...
## ⭐⭐ CURRENT TRUTH (2026-07-05 ~09:03Z — hk-5qp7z PROVEN; gb-mbp RAMPING 3→6, jessica activating+saturating)
> Keeper /clear resume. The concurrent-worktree-create race is DEAD; ramp is GO.
>
> **THE FIX (jessica, LANDED + PROVEN):** hk-5qp7z — serialize worktree-add + HEAD-resolve under `createMu` on the worker (commits d92b16de + fd76a69e, in HEAD 3a712140). Code-gap fork (B): a real mutex around add+resolve + a widened-concurrency-window negative regression test. FINAL VERDICT (jessica): 7-canary concurrent wave (jessica-q5, unique-file collision-proof beads) at gb-mbp max_slots:3 → **7/7 created worktrees concurrently with ZERO empty-HEAD/WorktreeCreationFailed** (the exact wave shape that fast-failed 7/7 in ~7s under the bug), 5/7+ merged clean. This is the same race class twice-"fixed" by RETRY (hk-iaj1w, hk-lt091) and slipped both times — createMu SERIALIZES, so it holds.
> **THE RAMP (captain set target, jessica executes):** captain bumped workers.yaml gb-mbp max_slots 3→6 durably (09:03Z). Registry is boot-only → jessica cycles the daemon at a clean lull (NOT orphaning in-flight duncan WS1e / stilgar hk-hm09z), registers 6 slots, then saturates from the 159-ready backlog. HOLD at 6 until clean under sustained 4-6 CONCURRENT merged-clean load, THEN 6→10 (operator's target) + one more cycle.
> **SUPPLY WATCH:** jessica's earlier finding — only ~4 collision-safe self-contained beads in the top-60 ready; sustained 6-slot remote saturation may outrun the collision-safe supply. jessica to flag if it runs dry before saturation.
>
> **LANES NOW:** jessica = gb-mbp ramp-to-6 activation + saturation (critical path, active). duncan = eval-metrics WS1e (run 019f3186, recovered from a supervisor-revival orphan via queue resume; driving to merge). stilgar = hk-hm09z, driving to merge. watch = bus monitoring, healthy. Local pinned 4. Paused-by-failure queue list = pre-existing cruft. Operator notified of the milestone (no action needed).
...
> **CAPTAIN ACTION (07:26Z):** reverted workers.yaml max_slots 6→1 (durable next-boot-safe) + `harmonik worker disable gb-mbp` LIVE (in-memory, stopped the storm, dispatch routes local cap-4). NO daemon restart (avoided orphaning duncan's in-flight WS1e). Ramp to >1 GATED on the fix.
> **THE FIX (jessica owns):** hk-5qp7z [P1 bug] "Concurrent remote worktree-create race: empty HEAD under max_slots>1 (serialize worktree-add on worker)". Awaiting jessica's FORK report: (A) stale worker binary (fix on main but not deployed to worker repo → redeploy) vs (B) code gap (retry doesn't serialize → needs a mutex/lock + a CONCURRENT regression test). Do NOT re-enable gb-mbp until fix lands + CONCURRENT re-validate passes.
>
> **LANES NOW:** jessica = hk-5qp7z concurrent-worktree-create fix (NEW critical path). duncan = eval-metrics WS1e (attempt 3, run 019f3128). stilgar FREE. watch = healthy. Local pinned 4 (now the ONLY active substrate until the remote fix lands). Operator notified. Backlog finding (jessica): only ~4 safe self-contained beads in top-60 ready — sustained remote saturation will need a real work supply, flag later.

## ⭐⭐ (SUPERSEDED 07:27Z — ramp reverted) CURRENT TRUTH (2026-07-05 ~07:22Z — gb-mbp RAMPED to 6 + MERGED-CLEAN CONFIRMED)
