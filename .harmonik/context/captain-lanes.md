...
> - **FIX (kynes implementing, captain-authorized comms 019f4eea):** honor StdinDevNull in the direct handler.Launch path (fd0→/dev/null, or CloseStdin immediately when HandlerSpec==nil) so argv-driven ProcessExit harnesses get EOF. fixer-in-worktree + agent-review + PERMANENT regression test → prove pi greens IN-DAEMON on scratch /tmp/hkg0 = the REAL GATE-0 green. **That in-daemon green is the redeploy trigger (captain authority) — NOT a redeploy of afa32372 (which would NOT fix it).** kynes filing the bug.
