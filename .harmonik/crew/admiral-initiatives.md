...
| **Hot-reconfigure + concurrent/multi-worker dispatch** — change daemon remote/local config WITHOUT restart, run local + remote AT THE SAME TIME, and (phase 2) multiple remotes | live-toggle: hk-xjbvi (READY, daemon-reliability) · scheduling design: research (build on `plans/2026-06-20-remote-node-telemetry-autoscale/` + its `phase2/`) | **PARKED — research-needed, queued AFTER current set** (operator 2026-06-25) | 3 parts: (1) live worker enable/disable toggle = ALREADY specced as hk-xjbvi (no research; rides daemon-reliability). (2) concurrent local+remote = NOT designed — registry is V1 single-worker (`SelectWorker` binary, `ErrTooManyWorkers`); needs scheduling design. (3) multi-remote = anticipated in autoscale plan phase2 but deferred; pull design forward. Captain to scope a research kerf-work for parts 2+3. |
