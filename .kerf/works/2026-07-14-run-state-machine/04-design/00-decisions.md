...
## M3-D11 — The M2→M3 input/ack seam: the reactor CONSUMES agent-input (AIS), does NOT own it

> **OWNERSHIP CORRECTED (planner, 2026-07-14) + SURFACE CORRECTED to the authoritative bench
> spec.** Two corrections supersede this decision's original form: (1) **M2-1 OWNS** the
> input/ack seam and **M3-4 CONSUMES** it (ROADMAP line 84; the "owned edge" framing was
> backwards). (2) The real seam is the **bench** `agent-input.md` (prefix **AIS**), NOT the
> stale repo-local `.kerf/` mirror that both the earlier design iteration and the dispatch
> brief read (which used a guessed `IN-*` / `Submit(ctx,InputMsg)(InputAck,error)` / `MsgID`
> surface). The reactor consumes the AIS surface verbatim and defines no competing type.

**The fixed seam M3 consumes (M2 owns these — [agent-input.md] AIS-001/AIS-003/AIS-004/AIS-INV-001):**
- `InputPort` with `SubmitInput(ctx, InputRequest) (Ack, error)` and `CloseInput` (AIS-001).
- `Ack` = an acceptance **class** ∈ {`Accepted`, `Rejected`, `Degraded`} + a driver-internal
  monotonic input-sequence id + an optional protocol acceptance token (AIS-003). `Degraded` =
  "written but not positively confirmed" (the interim tmux/paste case returns exactly Degraded).
- Dual delivery: `SubmitInput` returns the `Ack` synchronously AND the driver emits durable
  `agent_input_acked` / `agent_input_stale` events (AIS-004; payloads owned by event-model.md).
- Bounded output-or-stale: every `SubmitInput` reaches exactly one terminal — an `Ack` or an
  `agent_input_stale` — within the agent-input bounded window (AIS-INV-001); the window value is
  owned by the agent-input seam, NOT re-implemented by the reactor.

**How the `Dispatch` machine consumes it (M3 owns THIS side only):**
- Actions: `ActSubmitSeed` / `ActSubmitBrief`, each carrying an `InputRequest`; the shell
  effector calls `InputPort.SubmitInput`. `ActLaunchAgent`, `ActKillAgent`, `ActArm/CancelTimer`
  (agent_ready, kill_reap, …) stay M3's. **No `input_ack` timer is armed by M3** — the ack bound
  lives inside `SubmitInput` (AIS-INV-001).
- Events: the shell converts the synchronous `Ack` + the dual `agent_input_acked`/`stale` events
  into reactor events; correlation uses the `Ack`'s monotonic input-sequence id (NOT an
  M3-minted id). `EvAgentReady` MUST be run_id-stamped and delivered on fresh start AND resume,
  with the `agent_ready` timer as the backstop (this is M3's own liveness edge, D7).
- **Three-valued Ack handling:** `Accepted` advances the dispatch; `Rejected` routes to the
  fail-closed liveness edge (D7); `Degraded` MUST NOT be treated as confirmation — the reactor
  keeps requiring an agent-derived readiness/progress signal and lets the liveness bound
  terminate a never-confirmed Degraded submission. (This is exactly the resume-hang case: a
  Degraded resume seed is a paste written-but-unconfirmed — the liveness edge is the fix.)
- Composition: the per-submission output-or-stale guarantee (AIS-INV-001) feeds M3's per-run
  RSM-INV-001 / SR9 peer — a stale (or Rejected, or never-confirmed Degraded) resume seed drives
  the fail-closed reopen, never silence.

**PLANNER-RECONCILE (still open, escalated):** (a) ownership+surface corrected here to match the
authoritative bench; the sibling **Set-B** design docs in this directory (`runexec-design.md`,
`ports-design.md`, `liveness-parity-design.md`) still carry the pre-correction `EvInputAck` /
`Submit(ctx,payload)(Ack,error)` vocabulary and MUST be reconciled to AIS before they are used —
flagged to the coordinator. (b) M2's spec (AIS-004 PLANNER-RECONCILE) independently asks the
planner to confirm the ownership direction; this decision confirms M2-owns. (c) The agent-input
`InputPort` co-lands with this spec (hard dependency).
