...
> **STATUS: SPEC DRAFT — ALL blockers resolved. A,B,D,E,F locked by 18-reviewer convergence; C resolved to (B) staged-bead generator (operator delegated the pick to the captain 2026-06-16; kynes fresh-analysis concurred). kynes 2026-06-16.**
> Normative write-up of the 18-adversarial-reviewer convergence in
> `.kerf/works/flywheel-motion/notes-blocker-convergence.md`, the design vision in
> `docs/flywheel-self-reinforcing-design.md` §1–§11, and the problem space in `01-problem-space.md`.
> Pinned bead: `hk-0oca` (epic, `codename:flywheel`). Cited primitives are already shipped in this repo
...
`AppendItems` eager-refill pipeline — the file is shared, the code path is distinct), with all four guardrails:
...
4. **At-most-once ledger** — idempotency via `reacted_ledger` keyed on `(target_bead_id, follow_up_class)`,
   so a deploy+verify follow-up is spawned exactly once (no duplicate tails).
...
| Key | Meaning |
|---|---|
| `movement_weights` | per-event-type weights for the §1 metric (terminal events high; starts/chatter 0) |
| `window` | sliding-window duration (default ~30m) |
| `warmup_window` | cold-start watermark before activation is allowed (§1.4) |
| `sustained_windows` | consecutive low windows required to trip (default 2) |
| `suppression_ttl` | decaying TTL for operator-attached / dialogue-recency suppression (§3.2) |
| `attached_inactive_timeout` | attached-but-inactive timeout guarding the operatorAttached-pins-forever bug |
| `phase_flag` (optional) | operator-forced suppression — INVALID without `phase_flag_expiry` |
| `phase_flag_expiry` | mandatory expiry for `phase_flag` |
| `liveness_no_progress_n` | N cycles with no terminal progress → G-liveness self-kill (§6.1) |
| `done_definition` (per-class) | `merged` (default) or a Phase-2 deploy+verify class opt-in (§5.2) |
