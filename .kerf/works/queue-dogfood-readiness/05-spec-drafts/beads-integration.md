...
#### BI-013f — Failed-queue recovery preflight

Before QueueStore changes a `paused-by-failure` queue, it MUST read every
failed item's live record with `br show <bead_id> --format json`. Every record
MUST report Beads status `open`. A missing record, read failure, or non-open
status rejects the whole recovery transaction. The adapter MUST perform no
Beads write. The QueueStore MUST leave the queue unchanged and emit no
`queue_recovered` observation or dispatch wake. The typed result is owned by
...
Before first-canary selection, the gate owner MUST retain a readiness snapshot.
...
run. Planning and selection MUST NOT close, create, or otherwise mutate fleet
Beads state. At implementation triage, a confirmed current condition MUST be a
new scoped open record with current source evidence. Scratch results are
evidence only. They do not change Beads state.
