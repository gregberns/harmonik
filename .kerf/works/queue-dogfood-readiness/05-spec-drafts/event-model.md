...
| 8.10.9 | `queue_recovered` | O | queue | audit, observability, orchestrator-core | `queue_id`, `normalized_name`, `rearmed_bead_ids`, `rearmed_count`, `recovered_at` |
...
A subsystem that wants to emit a new cross-bus event type MUST add the type to §8 via the foundation amendment protocol ([architecture.md §4.6]). The addition amendment MUST provide: type name, emitter, typical consumers, payload fields, four-axis tags, durability class, and evidence satisfying every criterion in §8.9. The addition amendment MUST include (a) the §8 row, (b) the emitter-spec edit adding the emission requirement, (c) at least one consumer cited in another spec, (d) one `allEventTypeCohort` entry unless an explicit carve-out applies, and (e) an updated `TestCrossBusEventTypeCohortCount` baseline in `eventtype_coverage_gjyks_test.go`. That test MUST assert the ordinary cohort length against a named `wantCount` constant. The addition work creates this test if it is absent.
...
Readers of events MUST accept the immediately prior schema version (N-1) for every event type AND for the envelope. Per-type independence means harmonik maintains one independent compatibility contract for each registered event type. The exact total is derived from the current registry and cohort tests. Breaking changes (per §6.4 table) require a migration release scheduled at an operator pause per [operator-nfr.md §4.3].
...
`reason` is an exhaustive enum (`group_failure` is the v0.1 pause-by-failure path; `operator_drain` is the operator-initiated drain path). New variants require an EV-027 amendment. `queue_resumed` remains reserved. Failed-queue recovery emits the distinct `queue_recovered` event.
...
rearmed_bead_ids: <String[]>
rearmed_count: <Integer>
recovered_at: <Timestamp>
```

The queue's durable candidate is the authority for recovery. The queue emitter
MUST attempt this class-O event only after the recovery transaction has
installed the candidate in memory. It MUST attempt the event before it wakes
dispatch. Event loss or append failure is diagnostic and MUST NOT roll back
recovery. Restart MUST NOT synthesize or retry the event.

The concrete payload type is `core.QueueRecoveredPayload`. Its `Valid()` method
MUST reject an empty queue ID or normalized name, a negative re-armed count, or
a count that differs from the bead-ID list length. The queue event registration
MUST use a `mustRegister` constructor and a
`PayloadCompatEntry{CurrentVersion:1, PreviousVersion:0, CompatWindowHolds:true,
AdditiveOnly:true}` entry. `queue_recovered` is a normal cross-bus event. It
MUST be added to `allEventTypeCohort` and to
`TestCrossBusEventTypeCohortCount`. The implementation MUST add the named test
if it is absent and update its `wantCount` baseline by one. The keeper-only
...
The envelope carries a `schema_version` integer; each payload type carries a per-type version maintained by the registry. Per-type versions evolve independently. "N-1 readable" applies per-type. The current registry and cohort tests define the total number of contracts.
...
- Queue-lifecycle events (§8.10): emission rules in [queue-model.md §3, §8].
  ...
  `queue_cancelled_operator`, `queue_recovered`) are queue-emission-owned; this spec owns payload
...
- **§8.10 queue cohort.** Final success requires canonical UUID
  ...
  Recovery tests prove QueueStore commit and memory install precede one
  `queue_recovered` attempt, which precedes dispatch wake. Registry tests
  construct, validate, register, compat-decode, and count `queue_recovered`.
...
Question: §6.3 declares concrete YAML for about 14 event types. Remaining types land per §8 field lists plus the Go registry. The registry and cohort tests define the current total.
Owner: foundation-author
