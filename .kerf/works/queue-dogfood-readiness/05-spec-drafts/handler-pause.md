...
**HP-043 — Resume does not clear queue pause.** Resuming a handler does NOT clear a `paused-by-failure` or `paused-by-drain` `Queue.status`. Queue recovery does NOT call `HandlerPauseController`, clear handler state, or emit `handler_resumed`. These are orthogonal state machines. See §8.3a.
...
- Resuming a handler (§7.3 HP-040) does NOT resume a `paused-by-failure` queue. Queue recovery may re-arm an item while its handler remains paused. The next dispatch check applies the normal held-item behavior. These are independent operator actions.
