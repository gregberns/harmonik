...
`queue-dogfood-readiness` is the required cross-package Kerf work. Its Decompose,
Research, Change Design, and Spec Draft passes are complete. Integration review
is active. Work this order:
...
## Queue dogfood readiness release contract

Before either lane changes the queue/daemon boundary, both lanes MUST review the
release-contract table in `docs/queue-readiness-durability-proofs.md`. Alpha
owns daemon shutdown and daemon-side release tests. Bravo owns failed-item
recovery, queue wiring, CLI recovery, non-daemon proofs, ratchets, and backlog
triage.

The work order is contract, repairs, controlled tests, readiness snapshot,
scratch proof, operator authority, assessor gate, then operator go or no-go.
The fleet daemon remains stopped until the operator gives a later explicit
direction. Each handoff MUST name its owner, tested revision, durable edge,
proof command, artifact path, failure classification, and next operator
decision.
