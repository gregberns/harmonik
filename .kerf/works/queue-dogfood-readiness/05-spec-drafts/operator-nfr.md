...
- Operator dashboard UI — deferred to a future UX spec.
- Full binary signing — deferred per §4.2; the commit-hash check is the gate per locked decisions (see [handler-contract.md §4.10]).
...
- Metrics exposition wire format (Prometheus / OpenTelemetry) — deferred; structured logs + typed events are the observability surface per §4.9.
- Distributed tracing across multiple harmonik instances — deferred; per-project daemon isolation makes multi-instance tracing an OS-process concern per §4.10.
- Per-tenant cost attribution — there is no multi-tenancy per §4.10; shared LLM-budget / shared skill-registry / shared operator-identity concerns acknowledged and deferred.
- Observability overhead budget — deferred.
- Multi-repo workflow support — no ON requirement governs it. **AMENDED v0.5.8 (bead `hk-specs-crossrepo-stale-i1bcg`) — cross-repo dispatch is implemented and gated by an operator-maintained safelist. The superseded rule is quoted at the end of this bullet.** One daemon still supervises one project. A bead MAY name a second repository with `target_repo: <abs-path>` in its `## Branching` section, and the daemon then runs that bead against the named repository. The operator controls this with the `allowed_repos` list in `.harmonik/config.yaml` under the `daemon:` block. A bead whose `target_repo` appears in that list clears the cross-repo gate and runs against the named repository. The other dispatch guards still apply to it. The daemon reopens a bead whose `target_repo` does not appear in that list, and the reopen reason names the safelist. An absent or empty `allowed_repos` list refuses every cross-repo bead, so the capability is off until the operator turns it on. The full contract, including what moves to the target repository and what stays in the supervised project, is stated in [process-lifecycle.md §2.2]. `docs/cross-repo-dispatch.md` still describes the pre-safelist state and is not normative. **SUPERSEDED at v0.5.8 (was correct through v0.5.7):** "deferred; harmonik operates against one repository at a time per problem-space constraints. Known boundary: the daemon lands changes only to the supervised repo; a bead declaring `target_repo: <path>` is now explicitly refused with `CrossRepoUnsupportedError` (guard landed **hk-3r3**); cross-repo fixes are applied out-of-band until full cross-repo dispatch is implemented."
...
- **commit-hash integrity gate** — the binary-integrity check: the to-be-installed binary's source-commit hash must match the operator-supplied expected hash. Full binary signing is deferred. (see §4.2)
...
- **Allowed enumeration:** `{single, dot}`. `review-loop` was retired and is no longer selectable — `core.WorkflowMode.Valid()` in `internal/core/workflowmode.go` rejects it.
- **Default value:** `dot` (the embedded `standard-bead.dot` workflow graph; built-in fallback). On embedded-artifact load failure the daemon MUST fail the run loudly, NEVER fall to `single` and never change the run shape in silence; `single` is reachable ONLY via an explicit per-task `workflow:single` label. See [execution-model.md §4.3 EM-012a] for the resolution walk and EM-012a-FLOOR for the review floor. This restates the owner rule, which changed its floor mechanism from silent demotion to loud failure at execution-model v0.10.0.
- **Precedence layers** (four tiers, evaluated highest-to-lowest at claim time):
  ...
  2. Per-project policy (reserved tier; not yet populated).
  3. Daemon default per [process-lifecycle.md §4.1].
  4. Built-in fallback `dot` (the embedded `standard-bead.dot`); on embedded-artifact load failure fail the run loudly, never fall to `single` (per [execution-model.md §4.3 EM-012a] and EM-012a-FLOOR). `single` is selectable only via an explicit `workflow:single` label at tier 1.
...
- **Iteration cap (review-loop):** hardcoded at 3; NOT operator-tunable. **SCOPE ALERT (raised 2026-07-30):** this entry covers a retired mode. `review-loop` was retired at [execution-model.md §4.3 EM-015d] and `core.WorkflowMode.Valid()` no longer accepts it, so no run can reach this cap. The cap value of 3 carries over to the `dot` path as the per-edge traversal cap of [execution-model.md §4.9 EM-043]. Whether to retire this inventory entry or re-point it at EM-043 is an author decision, not a citation repair, so the entry is left as written.
...
#### ON-005 — Commit-hash integrity gate is the binary-install check
...
#### ON-018 — N-1 compatibility is the compat window
...
step 3 completes. The drain marker MUST record that terminal release result.
This requirement amends ON-027 and ON-030 for committed DOT release.
On crash or restart, the daemon MUST inspect git and Beads again before it
classifies the run. JSONL is diagnostic evidence only.

SIGINT and SIGTERM MUST begin graceful drain. A normal command watchdog MAY
report elapsed time and drain state, but it MUST NOT exit before graceful drain
does. Only `stop --immediate` or SIGKILL may bypass the drain. Any timeout
escalation MUST use the configured drain-timeout path of ON-029. That path MUST
not SIGKILL, exit, or mark step 2 complete while a committed DOT run lacks the
EM-053a close-or-reopen result.
...
Every subsystem MUST emit structured logs. Unstructured log lines (free-form text only) are forbidden at spec-declared emission points. Structured logs are the substrate for observability; Prometheus / OpenTelemetry wire formats are deferred per §4.10.ON-043.
...
#### ON-042 — Multi-tenancy is explicitly deferred

Per-project daemon isolation (one daemon per project per [process-lifecycle.md §4.1]) is the answer to multi-tenancy. Per-tenant cost attribution is out of scope for this spec; running N daemons does not auto-partition costs. The following concerns are acknowledged as real and explicitly deferred, not solved:

- **Shared operator LLM budgets.** The Anthropic quota is a per-account limit; running N daemons does not create N quotas. A machine-level budget coordinator is required and is not yet designed.
...
"Deferred" here means "not solved by per-project-daemon isolation"; it does NOT mean "dismissed." Later work to address these concerns lands as a foundation amendment, not an incremental change.

Tags: mechanism

#### ON-043 — Metrics exposition format is deferred

Prometheus and OpenTelemetry wire formats for metric exposition are deferred. The observability surface is structured logs (§4.9.ON-035) plus typed events (§4.9.ON-034). An implementation MAY additionally expose Prom/OTel endpoints but MUST NOT require them for conformance.

Tags: mechanism

#### ON-044 — Distributed tracing across daemons is deferred

Distributed tracing across multiple harmonik instances is deferred. Per-project daemon isolation means multi-instance tracing is an OS-process-isolation concern, not a harmonik-code concern — each daemon is a separate process with its own event log and its own state. Cross-daemon correlation (if ever needed) is an external-tooling layer, not a foundation spec.

Tags: mechanism

#### ON-050 — `harmonik attach` minimum surface

`harmonik attach` MUST: (a) connect to `.harmonik/daemon.sock` and verify the daemon-instance handshake per ON-013b (when ON-013b lands; until then, accept any daemon); (b) stream a live event tap (subset of `daemon_*`, `run_*`, `node_*`, `operator_*`, `error` events per [event-model.md §8]) to the operator's terminal; (c) present a periodic status snapshot (output of `harmonik status`) every T_attach_status (default 10s, operator-tunable); (d) accept operator commands inline (subset of `pause`, `resume`, `stop`); (e) detach cleanly on operator SIGINT or `:detach` command without affecting the daemon's state. The attach session_id MUST be carried in any operator-command emission for audit-trail correlation per ON-039.
...
> INFORMATIVE: Codes 1–23 are the current surface. Subsystem specs MAY declare additional subsystem-specific exit codes, which are registered against this taxonomy during spec-draft.
...
**Deferred extensions.** Full binary signing (§4.2.ON-006), metrics exposition format (§4.10.ON-043), distributed tracing (§4.10.ON-044), and the multi-tenancy concerns of §4.10.ON-042 are deferred additive extensions; none is required for Core conformance.
...
- **ON-005 — ON-006 (integrity gate).** Upgrade scenario tests with matching and mismatched commit hashes; verify `operator_upgrade_rejected` on mismatch; verify a later signing extension does not break conformance.
...
- **ON-030 — ON-033 (restart RTO).** Restart scenario benchmarks measuring SIGTERM-to-`ready` across representative hardware at the standard-fixture scale; verify 30s p95 nominal and 300s hard ceiling.
...
Resolution: Filesystem-based advisory-lock-guarded shared counter file at `$HOME/.harmonik/machine-agent-count` is the normative shape per §4.10.ON-041c(ii). The coordinator-daemon option is deferred; a coordinator would require a second process-lifecycle contract (startup, crash semantics, upgrade path) that has not been scoped. If contention measurements at scale show unacceptable lock-acquisition latency, a coordinator daemon is the upgrade path — additive to the existing contract, not a breaking change.
Owner: resolved.
...
Default-if-unresolved: The second command observes the state-machine in the post-first-command state and either no-ops (both paused) or errors (if incompatible). No explicit lock. The single-operator assumption makes this acceptable; revisit when multi-operator deployments appear.
...
Question: v0.3 set the RTO target to 30s nominal fixture / 300s ceiling (§4.8.ON-031, §4.8.ON-032). If fixture-based measurement at implementation time shows 30s is unachievable under realistic loads, does the nominal target get relaxed or does the fixture itself shrink?
Owner: foundation-author
Blocks: none (default below).
...
Default-if-unresolved: The inline minimum shape in ON-035 is sufficient for now. Promote to a dedicated `quality-checks.md` if the structured-log surface grows beyond ~100 lines OR if the parser contract acquires consumers beyond the local `harmonik attach` UI.

#### OQ-ON-008 — Daemon-discovery scope for `harmonik list`

Question: §4.10.ON-041 names the daemon-discovery scope as `$HOME` plus `$HARMONIK_PROJECT_ROOTS`. Is this sufficient, or is a system-wide registry needed (e.g., `/var/lib/harmonik/daemons.d/`)?
Owner: foundation-author.
Blocks: none (default is the ON-041 mechanism).
Default-if-unresolved: Stays as ON-041 / E6 mechanism (`$HOME` + `$HARMONIK_PROJECT_ROOTS`). A system-wide registry is deferred; cross-user discovery is not a current need.

#### OQ-ON-009 — Migration-release manual procedure documentation home

Question: ON-019 refers to a "dedicated migration workflow (deferred; not yet specified)" for breaking schema changes; the manual interim procedure (operator-paused boundary, schema-version verification, on-disk state inspection) needs a documentation home. Where does it live?
Owner: foundation-author.
Blocks: none.
Default-if-unresolved: Release notes for any migration release; a dedicated migration playbook document is deferred.
...
Owner: foundation-author.
Blocks: none.
Default-if-unresolved: Operator manually trims; rotation policy is deferred. ON does not at this revision impose a daemon-side cleanup obligation.
...
**Why N-1 and not N-2 or wider.** N-1 is the smallest window that lets operators upgrade without coordinating the daemon with the on-disk state. Wider windows (N-2, N-3) increase reader code complexity without proportional benefit — harmonik is single-operator, single-machine, and the migration cost at a break is a short operator-paused ritual. The window may widen later if multi-operator or fleet-scale deployments appear.
...
**Why multi-tenancy is deferred, not solved.** Per-project daemon isolation is a genuine answer for the common solo-developer case, and it scales gracefully to "a few projects at once on one machine." What it does NOT address — shared LLM quotas, shared skill installations, shared operator identity — is not tractable without a machine-level coordinator that would itself need a process-lifecycle contract, a failure story, and a reconciliation protocol. Deferring is cheaper than committing to a half-designed coordinator. The acknowledgment in §4.10.ON-042 is load-bearing: "deferred ≠ dismissed" is the posture that keeps the door open for a later amendment without re-opening the foundation.
