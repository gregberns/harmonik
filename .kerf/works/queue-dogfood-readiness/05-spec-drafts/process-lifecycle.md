...
response `queue_id` and `prior_status`; resume request `name` and `queue_id`,
with response `queue_id`, `normalized_name`, `rearmed_bead_ids`,
`rearmed_count`, and `status`. Queue-model owns the method-specific
...
the wire names. `queue-remove`, `queue-pause`, and `queue-clear` remain
...
3. **Classify in-flight runs and freeze dispatch.** For every in-flight run (`in_flight(run)` per [operator-nfr.md §3]) at drain entry, the daemon MUST classify the run into one of three sub-states:
   ...
   - **(ii) just-checkpointed** — a checkpoint has just landed and no follow-up node has been dispatched. The daemon withholds dispatch. A committed DOT run whose release is unfinished is not quiescent. It MUST complete the EM-053a drain close-or-reopen result before step 3 completes. A non-DOT run, or a DOT run whose release is already terminal, is quiescent.
   ...
   The step-3-complete signal to step 4 is the watcher-completion aggregation across (i)-class runs, every EM-053a terminal result for committed DOT work, and the quiescence of the remaining (ii)+(iii)-class runs.
4. Wait for agent-subprocess termination (bounded by the operator-configurable drain timeout per [operator-nfr.md §4.7 ON-029]). A normal graceful timeout MUST NOT SIGKILL or exit while committed DOT release work lacks its EM-053a terminal result. It MUST continue that release work with cancellation-free contexts. `stop --immediate` and SIGKILL remain the only bypasses.
...
- **`hk queue recover [--queue <name>] [--queue-id <uuid>]`** — recover one
  ...
  returns a typed failure that names the selected queue and observed state.
  Daemon-not-running → exit code 17 with no local write.
...
> `enqueue` method are retired. The six live-daemon `hk queue <verb>` surfaces
> are submit, append, status, dry-run, cancel, and resume; remove/pause/clear are
> deferred.
...
`status`, `dry-run`, `cancel`, `resume`). Each verb accepts both `--flag value` and
