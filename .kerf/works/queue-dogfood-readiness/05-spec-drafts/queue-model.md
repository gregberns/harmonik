...
- Event-payload field schemas for the nine queue-lifecycle events — owned by [/Users/gb/github/harmonik/specs/event-model.md §8.10].
...
- `queue-remove` and `queue-clear` semantics — deferred to v0.2 (see §A.3).
...
RECORD Item:
  ...
  attempts          : Integer               -- dispatch attempts; reset to zero only by QM-052b recovery
  last_failure_reason : String | None       -- most recent dispatch failure; cleared only by QM-052b recovery
...
`run_id`, `status`, `attempts`, `last_failure_reason`, or
`review_loop_failures` field. Those fields are daemon-owned at accept time.
The daemon MUST ignore a client-supplied value and initialize a new item with
`run_id: None`, `status: pending`, `attempts: 0`,
`last_failure_reason: None`, and `review_loop_failures: 0`.
...
#### RECORD QueueResumeResponse

```
RECORD QueueResumeResponse:
  queue_id          : UUID
  normalized_name   : String
  rearmed_bead_ids  : List<BeadID>
  rearmed_count     : Integer
  status            : QueueStatus  -- always "active"
```
...
(a) Events produced:
  ...
  - `queue_recovered` — emission rule §8.3b QM-052b; payload schema in [/Users/gb/github/harmonik/specs/event-model.md §8.10.9]. Class O.
...
A group MUST NOT re-enter `pending` or `active` once it has reached
`complete-success`. A group in `complete-with-failures` MAY re-enter `active`
only as part of the successful QM-052b recovery transaction. The transaction
must re-arm its failed items in the same candidate before it changes the group
or queue status. No restart, fresh submit, drain release, or handler resume is
a substitute for this recovery operation.
...
| `-32019`            | `queue_name_invalid`         | §2.1 queue-name validation   |

Each code is a stable wire constant. The `message` field of the JSON-RPC error carries the typed-error shape per the PL-003b `<error_type>{...}` convention; the `code` field is the numeric value in this table.
...
No further dispatch occurs while `status == paused-by-failure`. The daemon
remains running and the canonical named file persists with that status. Recovery
is the QM-052b transaction.
...
rejects recovery with no queue mutation, Beads write, observation, or dispatch
wake. This prevents a durable queue recovery that cannot claim its item.

Inside one QueueStore transaction it MUST re-arm every failed item in that
queue: `failed → pending`, `attempts → 0`, and `last_failure_reason → None`.
It MUST retain `run_id` as the prior-run audit reference and retain
`review_loop_failures` as the independent bounded-spend counter. It MUST reopen
each affected group to `active`, and set the queue status to `active`. The
QueueStore MUST durably install the candidate before it returns success. The
transaction MUST leave memory and persistent state at the prior candidate on a
write failure or stale snapshot. It MUST retain quarantine on a failed
replacement write.

After successful transaction and memory installation, the daemon MUST attempt
exactly one `queue_recovered` observation. It MUST then wake dispatch. Event
append failure is diagnostic and MUST NOT roll back recovery. Restart MUST NOT
synthesize or retry the observation. Recovery success means only that the
queue mutation is durable. It does not mean that any item has dispatched.

The operation MUST reject a missing queue, a queue in any status other than
`paused-by-failure`, a quarantined queue, a failed recovery preflight, and every
failed transaction. It MUST return exactly one typed
`queue_recovery_failed{reason, normalized_name, queue_id?, observed_status?,
bead_id?, bead_status?}` response. `reason` is one of `queue_not_found`,
`queue_not_recoverable`, `queue_quarantined`, `recovery_bead_not_open`,
`recovery_read_failed`, `recovery_write_failed`, or `recovery_stale_snapshot`.
Codes `-32030` through `-32036` map to those values in that order. These codes
are the queue-recovery block and do not reuse queue-validation codes
`-32010` through `-32019` or the handler-reservation block `-32020` through
`-32029`. A rejected recovery MUST NOT clear handler pause
state or emit `handler_resumed`. Raw setters and direct persistence MUST NOT
implement recovery, reservation release, or reservation undo. Those operations
require named QueueStore transaction operations that change status and run ID in
one candidate.
...
paused. A persisted `paused-by-failure` queue may recover only through
QM-052b. A persisted `paused-by-drain` queue remains subject to the drain
release path. `queue-clear` remains deferred.
...
|---|---|---|
| [/Users/gb/github/harmonik/specs/event-model.md] | §8.10 | Owns the nine queue-lifecycle event payloads, including `queue_item_reconciled` (row 8.10.7) and `queue_recovered` (row 8.10.9); this spec cites them by name and reason-enum but does not define payload schemas. |
