...
cancellation-free context, skips the normal gate and normal outcome emission, and synchronizes the
run branch before every merge decision. Its close result is `CloseBead` then `bead_closed` then
`run_completed`. Its reopen result is `ReopenBead` then `run_failed`. This includes a close path
that resolves as no change. A missing tip, synchronization failure, merge failure, or close
failure MUST select the reopen result. Drain completion requires exactly one result. The shutdown
edge MUST NOT emit `outcome_emitted`.
