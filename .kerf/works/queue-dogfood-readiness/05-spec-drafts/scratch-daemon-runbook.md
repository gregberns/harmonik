...
Use this procedure only for the scratch readiness canary that precedes the
fleet canary. It does not restrict normal scratch-daemon work.

1. Retain a preflight record. It names the tested revision, host load, allowed
   daemon-suite concurrency, selected bead, canary reason, batch path, and
   intended event-capture path.
2. Use one open repeat-safe item in one stream group. Set
   `SCRATCH_MAX_CONCURRENT=1`.
3. Use a local non-Pi harness. Do not use a remote worker, a cross-repository
   target, a wave queue, or `feedback`.
4. Run the batch. Before scratch cleanup, copy the batch JSON and event capture
   to a retained evidence path outside the scratch tree.
5. Record the matrix result with the branch, commit, load record, and canary
   reason. A result from more than one daemon suite or an uncontrolled host is
   machine-contention evidence until a controlled rerun classifies it.
6. Run `make queue-dogfood-readiness-validate` with the retained preflight,
   batch JSON, event capture, and result matrix. The validator writes one
   decision record and rejects missing artifacts, more than one item, a wave,
   concurrency other than one, a Pi or remote harness, a cross-repository
   target, `feedback`, an uncontrolled host, or more than one daemon suite.
7. An assessor accepts or rejects only a passing validator record plus the
   retained artifacts. Scratch evidence does not change fleet ledger state.
