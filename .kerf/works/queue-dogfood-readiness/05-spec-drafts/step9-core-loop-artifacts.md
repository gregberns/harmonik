...
## Queue dogfood readiness evidence

`make queue-dogfood-readiness` is the local non-Pi readiness target. It accepts
`QUEUE_DOGFOOD_SCRATCH`, `QUEUE_DOGFOOD_EVIDENCE`, and
`QUEUE_DOGFOOD_BEAD` as required inputs. It accepts
`QUEUE_DOGFOOD_REASON` as a required human-readable input. It rejects an empty
or fleet-root scratch path. It runs exactly one seeded repeat-safe item in one
stream group at concurrency one. It rejects a remote worker, cross-repository
target, wave queue, Pi harness, or feedback action. It writes its retained
preflight JSON, batch JSON, event capture, result matrix, and controlled-load
record under `$QUEUE_DOGFOOD_EVIDENCE/<run-id>/` before scratch cleanup.

`make queue-dogfood-readiness-validate` accepts that retained run directory. It
checks the declared input and output shapes and writes
`$QUEUE_DOGFOOD_EVIDENCE/<run-id>/validation.json`. It rejects missing evidence,
more than one item, a wave, concurrency other than one, Pi or remote placement,
cross-repository execution, feedback, uncontrolled host load, or more than one
daemon suite. The validator does not call Beads or the fleet daemon.

The result record MUST classify machine contention separately from product
failure. This target does not change the broader Pi core-loop target. It
precedes the independent assessor gate. The assessor accepts or rejects a
passing validation record and its retained artifacts. It does not consume normal
queue work.
