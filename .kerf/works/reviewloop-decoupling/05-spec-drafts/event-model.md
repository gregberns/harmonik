...
- **TransitionKind** — the enum declared in [execution-model.md §4.10 EM-044] and [execution-model.md §6.1]. Values: `forward | local-patchback | architectural-rollback | policy-rollback | context-restore | context-checkpoint`. Event payloads use the enum by cross-reference; no independent ownership.
...
- **claude_session_id** — the compatibility-named authoritative harness continuity identifier per [execution-model.md §3], including a native ID from a `Captured` harness. Distinct from this spec's per-launch `session_id`. It is optional on pre-launch events when a Captured identity is not yet known and required on continuity-dependent events after durability. The two fields MUST NOT be conflated.
...
| 8.1.13 | `implementer_phase_complete` | O | daemon-core | orchestrator-core, audit, observability | `run_id`, `exit_code`, `stderr_tail_head`, `commit_landed`, `duration_seconds` |
| 8.1.14 | `reviewer_budget_exceeded` | O | daemon-core | operator-observability, audit | `run_id`, `budget_ms`, `elapsed_ms`, `changed_lines`, `reason` |

> **§8.1.12 `implementer_no_work_suspected` (hk-368i4) — EV-027 addition amendment.** Emitted by daemon-core when a process-exit implementer (codex) finishes with NO commit and a CLEAN worktree AND its phase ran for less than the configured no-work duration floor. §8.9 compliance evidence: **(a)** cross-subsystem consumers — operator-observability (the diagnostic an operator reads when a run fails with no explanation), audit, and improvement-loop; **(b)** lifecycle-boundary signal — it is emitted once at the implementer-phase boundary, alongside `implementer_phase_complete`, not per tick; **(c)** single summary event per phase, no re-emit; **(d)** typed payload `ImplementerNoWorkSuspectedPayload` with `Valid()`; **(e)** durability class O — loss costs a diagnostic only, because the run's failure is independently recorded by the no-commit path; **(f)** non-idempotent, single emission per run; **(g)** cited by [handler-contract.md] codex-path consumers and this section as the detector for the silent-implementer failure class; **(h)** not a paired-phase lifecycle — it is a single detector observation, not a pending/resolved pair. **Detector, not gate:** the run is already failing via the no-commit guard when this fires; the event names the shape so a future silent failure of a different cause is still caught. Both conditions are required — duration alone would flag a legitimately trivial bead, and a real fast run still commits and so never reaches the no-change outcome.

> **§8.1.13 `implementer_phase_complete` — logical result publication, not quiescence.** The daemon emits this event once after the implementer session's logical work result and exit information are available. It does NOT assert that the phase's heartbeat loop, hook callbacks, auxiliary observers, pane, or workspace dependents have joined; the invisible phase-close barrier owned by [process-lifecycle.md] is the sole authority for resource quiescence and workspace release. The event is class O because git plus the terminal run event remains authoritative for work-product and run state.

> **§8.1.14 `reviewer_budget_exceeded` — reviewer-watchdog diagnostic.** The daemon emits this event when the reviewer verdict observer terminates a reviewer after its diff-derived verdict allowance plus permitted bounded liveness grace is exhausted. It is NOT a [control-points.md §4.5] Budget event: it has no `budget_ref`, dispatch admission, per-chunk accrual, or replayed counter. The event MUST agree with the authoritative reviewer-budget diagnostic transferred into the run-workspace control archive per [workspace-model.md §4.7 WM-027a]. Its class is O because the F-class `review_loop_cycle_complete{completion_reason=error}` and terminal run event carry routing weight.

> Section Axes (§8.1 Run lifecycle): All §8.1 event emissions are mechanism-tagged. Class F entries (`run_started`, `run_completed`, `run_failed`, `transition_event`, `checkpoint_written`) are fsync-backed; class O entries, including `implementer_phase_complete` and `reviewer_budget_exceeded`, are best-effort. Default per-entry Axes — class F: `llm-freedom=none; io-determinism=deterministic; replay-safety=safe; idempotency=non-idempotent`. Class O: `llm-freedom=none; io-determinism=best-effort; replay-safety=safe; idempotency=non-idempotent`. Replay-safety is `safe` for all: the JSONL append is idempotent at the content level and consumers tolerate duplicate delivery per EV-014b.
...
| 8.1a.4 | `iteration_cap_hit` | O | orchestrator-core | audit, observability, operator-observability, beads-integration | `run_id`, `workflow_mode`, `iteration_count`, `cap_value` (Integer; = 3 at MVH), `final_verdict = REQUEST_CHANGES` |
...
> **Launch identity and truth.** `implementer_resumed` MUST source both identity fields from the resumed implementer's launch artifacts; `reviewer_launched` MUST source them from the fresh reviewer launch artifacts; and `reviewer_verdict` MUST repeat the identities of that same reviewer launch. The payload `run_id` MUST equal the envelope `run_id`, `iteration_count` MUST be monotone within the cycle, implementer `claude_session_id` MUST remain stable across resumes, and reviewer `claude_session_id` MUST be fresh per reviewer launch. A callback-synthesized lifecycle event MUST NOT substitute a cached prior-phase identity.

> **`reviewer_launched` is dispatch commitment.** The event records that reviewer dispatch has been committed with concrete launch artifacts and is emitted before `Handler.Launch` is invoked. It does not assert that the process or pane exists. A launch failure may therefore follow `reviewer_launched` without any `launch_initiated` or `agent_ready`; that trace is truthful and terminates through `review_loop_cycle_complete{completion_reason=error}`.
...
> Emission ordering (§8.1a). For a single **review-loop** iteration, the partial order MUST be: (a) optional `implementer_resumed` intent for every implementer launch after iteration 1; (b) the implementer launch lifecycle reaches `agent_ready`; (c) every submitted input resolves internally as synchronous `Ack{Rejected}`, positively accepted (projected as `agent_input_acked`), or stale (projected as `agent_input_stale`); only positive acceptance permits work progression; (d) `implementer_phase_complete` publishes the logical work result; (e) `reviewer_launched` records committed reviewer dispatch; (f) the reviewer launch lifecycle reaches `agent_ready`; (g) reviewer input follows the same three-way submission resolution; (h) `reviewer_verdict` follows successful verdict validation. The two input events are O projections, not durable advancement authority. `implementer_phase_complete` does not authorize advancement or workspace release; the process-lifecycle phase-close barrier does.
>
> No-progress detection occurs before reviewer dispatch: with a prior REQUEST_CHANGES verdict, emit `review_fixup_stalled` then `review_loop_cycle_complete{completion_reason=fixup_stalled}` and emit no reviewer event; otherwise emit `no_progress_detected` then `review_loop_cycle_complete{completion_reason=no_progress}`. After a verdict: actionable `REQUEST_CHANGES` with iterations remaining loops to the next `implementer_resumed`; actionable `REQUEST_CHANGES` at the cap emits `iteration_cap_hit` then `review_loop_cycle_complete{completion_reason=cap_hit}`; `BLOCK` emits `review_loop_cycle_complete{completion_reason=blocked}`; `APPROVE` emits `review_loop_cycle_complete{completion_reason=approved}`. A raw `REQUEST_CHANGES` verdict with an empty `flags` array remains unchanged in `reviewer_verdict`, but execution normalizes it to approval and the cycle records `completion_reason=approved`; it MUST NOT emit `iteration_cap_hit` or resume the implementer.
>
> Reviewer launch failure, malformed or missing verdict, and cancellation after the cycle has been entered emit no `reviewer_verdict` and terminate through `review_loop_cycle_complete{completion_reason=error}`. Every entered review-loop cycle MUST have exactly one successfully persisted `review_loop_cycle_complete`, including error and cancellation paths. A failed append/fsync is instead the EV-016a durability-failure path and is not a phantom completion. The terminal `run_completed` or `run_failed` MUST follow the successfully persisted cycle-complete event; no cycle event may follow that terminal run event.
>
> For **DOT-mode** no-progress termination: three paths — (1) prior verdict was `REQUEST_CHANGES`: emit `review_fixup_stalled{workflow_mode="dot"}` and terminate via needs-attention; (2) prior verdict was neither `REQUEST_CHANGES` nor `APPROVE` (or no verdict yet): emit `no_progress_detected{workflow_mode="dot"}` and terminate via needs-attention; (3) prior verdict was `APPROVE` AND HEAD advanced past the run baseline `parentSHA` (APPROVED-AND-DONE exemption per hk-8ps7q): emit NO no-progress event and COMPLETE the run as success. In all DOT cases `review_loop_cycle_complete` is NOT emitted, and `run_completed` / `run_failed` follows directly.

> Reviewer-verdict schema reuse (§8.1a.3). The `reviewer_verdict` payload's `schema_version`, `verdict`, `flags`, and `notes` fields conform verbatim to the `agent-reviewer` skill's JSON verdict schema v1 (referenced from the project skill registry). The daemon MUST read `.harmonik/review.json` from the box-A reviewer projection defined by [workspace-model.md §4.7 WM-027a], MUST validate the file against schema v1 (the `schema_version` field MUST equal 1; `verdict` MUST be in `{APPROVE, REQUEST_CHANGES, BLOCK}`; `flags` MUST be a String array; `notes` MUST be a String), MUST transfer the validated artifact to the run-workspace control archive, AND MUST emit `reviewer_verdict` only after those steps succeed. A malformed verdict file or required transfer failure MUST cause the daemon to emit `review_loop_cycle_complete{completion_reason=error}` and route the run to the `needs-attention` close path per [execution-model.md §4.3 EM-015e]; the daemon MUST NOT emit `reviewer_verdict` with a malformed or non-archived payload. The reviewer-hardening rule that `notes` MUST include file:line citations for any REQUEST_CHANGES verdict is enforced upstream of event emission (reviewer prompt + adapter validation per the agent-reviewer skill); event-model records the raw validated verdict, including a flagless REQUEST_CHANGES value that execution later normalizes to approval, and does not rewrite it.
...
> **`agent_ready` callback identity.** When the daemon synthesizes `agent_ready` from a hook or substrate callback, the payload MUST carry the `run_id` and `session_id` from the concrete launch artifacts for the session that became ready. The payload `run_id` MUST equal the envelope `run_id`; a zero, omitted, cached prior-phase, or mismatched identity is invalid and MUST NOT satisfy a ready gate.
...
A producer of an F-class event MUST inspect and propagate the append/fsync result. An append or fsync failure means the lifecycle fact was NOT successfully emitted; the producer MUST route the run through the existing durability-failure and operator-escalation/quarantine path and MUST NOT advance as though the event were durable. In particular, a failed `review_loop_cycle_complete` append is a durability failure, not a completed cycle. O- and L-class producers remain best-effort, but their helpers MUST make emission failure explicit to the diagnostic channel rather than silently treating a failed append as success.
...
```yaml
run_id: <UUID>
exit_code: <Integer>                    # 0 clean, non-zero error, -1 daemon kill
stderr_tail_head: <String>              # first 200 bytes of captured stderr tail; empty when unavailable
commit_landed: <Boolean>                # true iff worktree HEAD advanced past the phase baseline
duration_seconds: <Number>              # wall-clock seconds from implementer launch to logical result
```

The event publishes the implementer's logical work result. It does not prove process, observer, callback, or workspace quiescence and MUST NOT be used as a workspace-release gate.

#### `reviewer_budget_exceeded`

```yaml
run_id: <UUID>
budget_ms: <Integer>                    # positive diff-derived initial allowance
elapsed_ms: <Integer>                   # positive elapsed reviewer-observer duration
changed_lines: <Integer>                # non-negative line count used for the allowance calculation
reason: <String>                        # non-empty terminal reason, e.g. budget-exceeded or hard-ceiling
```

The payload MUST match the authoritative reviewer-budget diagnostic in the run-workspace control archive. This event is a reviewer-watchdog diagnostic, not a §8.4 ControlPoint-budget event.
...
A `REQUEST_CHANGES` verdict with an empty `flags` array remains `REQUEST_CHANGES` in this event. Execution may normalize that flagless verdict to approval for routing, but MUST express the normalization only through `review_loop_cycle_complete{completion_reason=approved}` and MUST NOT rewrite the raw verdict event.
...
final_verdict: <enum: REQUEST_CHANGES>               # the only actionable verdict at the cap-hit boundary
```
...
- Run-lifecycle events (§8.1): emission rules in [execution-model.md §6.5]. The optional `workflow_mode` payload field on `run_started` / `run_completed` / `run_failed` is co-owned: this spec normatively declares its placement and enum values per §3 glossary `WorkflowMode`; [execution-model.md §4.3 EM-012a] declares how the resolved value is computed. `implementer_phase_complete` is daemon-emission-owned at logical work-result publication; `reviewer_budget_exceeded` is daemon-emission-owned when the reviewer observer returns allowance exhaustion after the diagnostic transfer required by [workspace-model.md §4.7 WM-027a].
...
- Declare and register every event type in §8 with payload fields named in the §8 tables and durability classes per EV-016. This includes all seven §8.1a review-loop event types (`implementer_resumed`, `reviewer_launched`, `reviewer_verdict`, `iteration_cap_hit`, `no_progress_detected`, `review_fixup_stalled`, `review_loop_cycle_complete`), the §8.1 daemon events `implementer_phase_complete` and `reviewer_budget_exceeded`, and the §8.8.6 `bead_label_conflict` event type.
...
|---|---|---|---|
| 2026-07-24 | 0.7.2 | agent (codename:reviewloop-decoupling) | **Review-loop event truth, ordering, and reviewer-watchdog diagnostic.** Registers the already-shipped `implementer_phase_complete` and `reviewer_budget_exceeded` payloads in §8.1/§6.3 as O-class daemon events; defines implementer completion as logical result publication rather than resource quiescence; makes `reviewer_launched` a dispatch-commit fact; requires launch-artifact identities and callback-ready identity agreement; narrows `iteration_cap_hit.final_verdict` to actionable `REQUEST_CHANGES`; preserves raw flagless REQUEST_CHANGES while cycle routing normalizes it to approval; extends the §8.1a partial-order oracle with ready/input terminals, error/cancellation completion, and exactly-one successfully persisted cycle-complete; and requires F-class append/fsync failures to enter the durability-failure path rather than silently advancing. No new wire event names are introduced; both newly documented names were already registered in the Go event registry. No prior EV IDs or §8 rows are renumbered or retired. |
