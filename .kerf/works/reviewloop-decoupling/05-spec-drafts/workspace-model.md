...
This spec defines the workspace primitive — one authoritative leased run worktree, its branching convention, its lifecycle, and the durable session-log archive and metadata sidecars it owns — plus the unleased reviewer projections used by review-loop phases. It names the lease-by-run rule (the run workspace is leased by a `Run`, not by individual agents or projections), the three-level branching model (node commits on a task branch, task branch squash-merges to an integration branch, integration merges to main), the merge-conflict resolver (original implementer), and the session-log path + metadata sidecar that the workspace manager (S06) pre-creates before the handler (S04) attaches.
...
- Lease-by-run rule: one leased run workspace per run; agents that execute in that workspace occupy it sequentially, while reviewer phases use mandatory unleased projections.
...
- Run-workspace versus box-A reviewer-projection placement, reviewer-control artifact transfer, and projection reclamation.
...
- **session-log directory** — the per-session directory under the run workspace's `.harmonik/sessions/${session_id}/` archive. A reviewer session first writes to the corresponding path in its projection, then S06 transfers the complete directory into the run-workspace archive before projection cleanup. (see §4.7)
...
- **run workspace** — the authoritative leased workspace for a run. It may be box-A-local or worker-remote; it owns the task branch, implementer work product, and durable review-control archive. (see §4.7.WM-027a)
- **reviewer projection** — a short-lived, unleased, box-A-local detached worktree at the verified implementation SHA used only for reviewer execution and staging. It is not a second run workspace and never receives a lease lock. (see §4.7.WM-027a)
- **review-control archive** — the authoritative `.harmonik/` review artifacts in the run workspace, including per-iteration verdicts, reviewer feedback, the current-verdict alias when present, and reviewer-budget diagnostics. Projection copies are staging files, not archive authority. (see §4.7.WM-027a, §6.2)
- **orphan evidence types** — filesystem conditions detected by §4.3.WM-013c / §4.1.WM-003a and routed to reconciliation Cat 3. The defined types are `bare-worktree-no-lease` (registered run worktree, no lease-lock, no sessions), `sidecar-without-lease` (registered run worktree, sidecar present, no lease-lock), and `reviewer-projection-integrity` (a reviewer-projection candidate whose manifest is missing, malformed, or inconsistent with its deterministic path or registered SHA).
...
(c) Types introduced (cross-subsystem):
  ...
  | `ReviewerProjectionManifest` (§6.1) | mechanism | `io-determinism=deterministic; idempotency=non-idempotent` (on-disk artifact) |
...
(g) NFRs inherited / overridden:
  - Inherited: `ON-018` N-1 schema compatibility (§6.4 applies it to `Workspace`, `SessionMetadataSidecar`, and `ReviewerProjectionManifest`).
...
Discovery MUST distinguish canonical run-workspace paths from reviewer-projection candidates before applying a no-lease classification. A directory matching `<repo>/.harmonik/worktrees/<run_id>-reviewer-<iteration>/` or containing `.harmonik/reviewer-projection.json` is a projection candidate and MUST be evaluated under WM-027a / WM-033; the intentional absence of a lease-lock or the presence of a staged reviewer sidecar MUST NOT classify it as `bare-worktree-no-lease` or `sidecar-without-lease`.

If daemon startup (or reconciliation) observes a canonical run-workspace directory at `<repo>/.harmonik/worktrees/<run_id>/` that `git worktree list --porcelain` reports as a registered worktree but that carries NO lease-lock file (`${workspace_path}/.harmonik/lease.lock` absent per §4.3.WM-013a) AND NO session-log directory (`${workspace_path}/.harmonik/sessions/` absent), it MUST be classified under the evidence type `bare-worktree-no-lease` and routed to reconciliation Cat 3 per [reconciliation/spec.md §8]. If the same run worktree carries a sidecar (`.harmonik/sessions/<session_id>/harmonik.meta.json` present) but NO lease-lock, it MUST be classified under the evidence type `sidecar-without-lease` and routed to reconciliation Cat 3. Both cases arise from a SIGKILL / power loss between `git worktree add` and the lease-lock fsync gate of WM-016; neither the `leased` nor any post-`ready` event has been durably emitted, so the run's workflow state treats the workspace as non-existent while the filesystem retains an orphan. Cat 3 routing gives reconciliation the right to propose `reopen-bead` (discard the partial worktree, fresh run_id per WM-034) or `accept-close-with-note` with operator cleanup.

A projection candidate with a missing, malformed, or tuple/path/SHA-mismatched manifest MUST be classified as `reviewer-projection-integrity` and handed to reconciliation with the observed path, manifest parse result, registered git-worktree SHA (if any), and any surviving owner evidence. It MUST NOT be deleted or reclassified by the generic no-lease branch.
...
A run workspace MUST be leased by exactly one `Run` for the run's full lifetime, defined as the interval that starts on the emission of `workspace_leased` (§4.4.WM-016) and ends on the emission of a terminal workspace event (`workspace_merge_status` with `status=merged`, or `workspace_discarded`) per §4.3.WM-013b. Agents dispatched into the run workspace (planner, researcher, builder, implementer, merge agent) MUST occupy that worktree sequentially across their nodes. A reviewer instead executes in the mandatory per-iteration projection of WM-027a; that projection is daemon-owned staging, is not a `Workspace`, and MUST NOT acquire or reuse the run lease. An agent MUST NOT hold exclusive ownership of the run worktree for the duration of its agent-level session; the centralized-controller principle ([architecture.md §4.9]) requires the run — not the agent — to be the lease holder.

Tags: mechanism

#### WM-011 — One active agent at a time inside a workspace

At any instant, AT MOST ONE agent process MAY be actively writing to any one physical worktree. Agents dispatched into the run workspace run sequentially as the run traverses its workflow graph; parallel nodes within one run MUST NOT share that worktree. Parallel nodes across different runs occupy separate worktrees per WM-002. A reviewer projection is a different physical worktree, but review-loop phase ordering still forbids reviewer/implementer overlap per [execution-model.md §4.3 EM-015d-KERNEL]. Enforcement is delegated to the orchestrator (S01): the orchestrator MUST NOT dispatch a second agent into a worktree already holding a live handler subprocess. The workspace manager's storage-level contribution for the run workspace is the lease-lock file (§4.3.WM-013a), whose content identifies the owning run but does not by itself arbitrate per-agent concurrency; a projection has no lease-lock and is protected by phase ownership and the PL-014b close barrier.

> INFORMATIVE: `review-loop` mode (per [execution-model.md §4.3]) is a concrete instance of the phase-ordering rule across two physical worktrees. The implementer runs in the leased run workspace and reaches a committed checkpoint before the reviewer launches in an unleased exact-SHA projection. The reviewer and every projection observer quiesce, required artifacts transfer to the run workspace, and the projection is removed before the implementer resumes. This introduces a transient staging worktree, not a second run workspace or lease.
...
**One lease per run lifetime — including across multi-session modes.** In `review-loop` mode (per [execution-model.md §4.3]) — and any other workflow mode that launches multiple sessions sequentially within a single run — exactly ONE lease MUST cover the authoritative run workspace for the entire run. The lease is acquired at `workspace_leased` (per WM-016) and released only at the terminal workspace transition (`workspace_merge_status` with `status=merged` or `workspace_discarded`) per WM-013b. Implementer and other run-workspace sessions execute under that lease. Reviewer sessions are run-associated but execute in WM-027a projections outside the lease; their logs and metadata transfer into the run-workspace archive without acquiring a projection lease. The run-workspace lease-lock file is NOT re-acquired or released per session.
...
The workspace manager's startup path MUST first partition `<repo>/.harmonik/worktrees/` entries into canonical run-workspace candidates, reviewer-projection candidates, scratch merge-worktrees, and unknown paths. Reviewer-projection candidates are identified by the deterministic suffix and/or manifest named by WM-027a and MUST bypass lease discovery; they enter the manifest-aware PL-006 / WM-033 branch. For each canonical run-workspace candidate, discovery MUST (a) validate the `<run_id>` path of WM-002; (b) call `git worktree list --porcelain` against `<repo>` and confirm the directory is a registered worktree; (c) read the lease-lock file per WM-013a (if present) to recover the `run_id`, `pid`, and `created_at`; and (d) stat `${path}/.harmonik/sessions/` to detect whether any session was ever started. A canonical run-workspace directory failing (b) but present on disk is an orphan worktree subject to [process-lifecycle.md §4.2 PL-006]. A directory passing (b) with a live lease-lock file whose recorded `pid` is NOT the current daemon is subject to the orphan-sweep rule of §4.8.WM-033 (stale → sweep; live non-orphan → fail-fast coordination with HC-044a; see OQ-WM-005).
...
.harmonik/reviewer-projection.json
...
For every run-associated agent session, the run workspace MUST retain one authoritative session-log directory at `${run_workspace_path}/.harmonik/sessions/${session_id}/`. For a handler launched in the run workspace, that directory exists there before launch. For a reviewer launched in a WM-027a projection, the workspace manager MUST instead pre-create a staging directory at `${reviewer_projection_path}/.harmonik/sessions/${session_id}/`; the handler writes its live log there, and S06 MUST atomically transfer the complete directory into the same-`session_id` run-workspace archive path after reviewer quiescence and before projection cleanup. A projection copy is not the retained archive.

The directory is the join point for handler-written session logs (S04) and CASS-read metadata (S08). The workspace manager OWNS directory creation and projection-to-archive transfer; handlers OWN log-file contents. The handler's `session_log_location` progress-stream emission per [handler-contract.md §4.2 HC-010] announces the pre-existing live path to the watcher. After a reviewer transfer, S06 MUST make the retained run-workspace path available to S08/CASS as the authoritative archive location; a consumer MUST NOT retain the removed projection path as its durable join key.
...
The workspace manager MUST write a metadata sidecar file at the session's live path — `${run_workspace_path}/.harmonik/sessions/${session_id}/harmonik.meta.json` for a run-workspace session or `${reviewer_projection_path}/.harmonik/sessions/${session_id}/harmonik.meta.json` for a reviewer session — before the handler launches. The file MUST carry `run_id`, `node_id`, `agent_type` (the conformance-class identifier per [architecture.md §4.7 AR-024]), `workflow_id`, `launched_at` (RFC 3339 timestamp), `schema_version` (set by the workspace manager per §6.4), and `bead_id` (present iff the run is bead-tied per [execution-model.md §4.3 EM-014]). The metadata sidecar is the authoritative join key for CASS indexing AND the authoritative source for agent-type at merge-time implementer identification (§4.6.WM-022). For runs inside expanded sub-workflows (per [execution-model.md §4.8 EM-034a]), the sidecar's `node_id` MUST carry the namespaced form `<parent_node_id>/<sub_node_id>` so the sidecar locates the expanded node uniquely.
...
An interrupted sidecar write (process death between steps (i) and (iii)) leaves a `.tmp-<pid>` file and no canonical `harmonik.meta.json`. The startup sweep MUST tolerate orphan `.tmp-<pid>` files by removing them; a missing `harmonik.meta.json` with a present run-workspace lease-lock is classified per §4.3.WM-013c. A missing lease-lock with a present sidecar is the `sidecar-without-lease` evidence type of WM-003a only for a canonical run workspace; the same observation inside a reviewer projection is expected staging and is classified through the projection manifest.

For a reviewer session, the complete session directory — including `harmonik.meta.json`, handler logs, and any session-local diagnostics — MUST be transferred without changing `session_id` after PL-014b has joined the handler, watcher, callbacks, and observers. Destination creation and every transferred file MUST use the temp-write + file-fsync + rename + parent-directory-fsync discipline above. The transfer is complete only after S06 verifies the destination directory contains the durable sidecar and all source entries selected at the quiescence boundary. A partial or unverifiable transfer is a phase error and forbids projection cleanup.
...
The workspace manager MUST write the first run-workspace session's metadata sidecar BEFORE emitting `workspace_leased` per WM-016. This ordering ensures that any consumer of `workspace_leased` observing the handler launch can join metadata without racing the sidecar write. For subsequent run-associated sessions, including reviewer sessions staged in projections, the live-path sidecar write precedes handler launch but does NOT re-emit `workspace_leased`; it proceeds as a stand-alone sidecar-write operation covered by WM-026.

Tags: mechanism

#### WM-027a — Reviewer verdict artifact path

For workflows that include a reviewer agent (notably `review-loop` mode per [execution-model.md §4.3]), review execution uses two explicit locations:

- The **run workspace** is the authoritative leased workspace. It may be box-A-local or worker-remote and owns the task branch, implementer work product, and durable review-control archive.
- The **reviewer projection** is a short-lived, unleased, box-A-local detached worktree at the verified implementation SHA. It exists only for one reviewer iteration and is not a second run lease.

Every review-artifact operation MUST name its location explicitly. A caller MUST NOT infer local versus remote placement from a nullable runner, path reachability, or a failed local filesystem operation.

**Projection identity and path.** A reviewer projection is identified by the closed tuple `{run_id, iteration, source_ref, source_sha, location="box-a"}`. Its canonical path is `<repo>/.harmonik/worktrees/<run_id>-reviewer-<iteration>/`; its `.harmonik/reviewer-projection.json` manifest MUST record the complete tuple before reviewer launch. The path and manifest are deterministically reconstructable from the tuple. The projection MUST be created with `git worktree add --detach <projection_path> <source_sha>` and MUST carry no lease-lock, task branch, or `Workspace` lifecycle record.

**Location-aware transfer seam.** Every projection↔run-workspace copy MUST route through one daemon-owned artifact-transfer seam whose request names `{source_location, source_path, destination_location, destination_path, expected_digest, artifact_kind}`. The two location values are explicit members of `{box-a, run-workspace-local, run-workspace-remote}` for this revision. The seam MUST execute remote-side reads, writes, ref fetches, and fsync/rename operations through the configured worker runner; it MUST NOT probe a local path first, interpret a nullable runner as a placement signal, or silently fall back between hosts. A successful response MUST mean that the destination bytes or directory tree are durable and digest-verified. A transport success without destination durability or digest verification is failure. The seam is a workspace/run-shell mechanism owned by S06 and the daemon composition root; it is not an agent-to-agent file channel.

**Lifecycle and authority.**

(a) Before projection creation, the daemon MUST use the location-aware seam to transfer or fetch the committed implementer `source_ref` and `source_sha` to box A and verify that the resolved object equals the tuple's `source_sha`. A projection at any other SHA is invalid and the reviewer MUST NOT launch.

(b) The daemon MUST atomically materialize `review-target.md` and the reviewer session directory + metadata sidecar in the reviewer projection before reviewer launch. Reviewer execution, target reads, live session-log writes, live verdict writes, and verdict-budget observation occur in that projection. The reviewer MUST write `${reviewer_projection_path}/.harmonik/review.json` conforming to the `agent-reviewer` JSON verdict schema v1.

(c) After the reviewer phase, the daemon MUST read and validate the projection's `review.json` locally on box A. Projection files are staging copies only; they are not durable run authority.

(d) After successful validation, the daemon MUST atomically transfer the verdict through the location-aware seam to the run-workspace review-control archive as `${run_workspace_path}/.harmonik/review.iter-<N>.json`, where `<N>` is the completed iteration. If an existing consumer requires the current-verdict path, the daemon MUST atomically update `${run_workspace_path}/.harmonik/review.json` as an alias of that same validated content. The per-iteration file is authoritative. Transfer uses the WM-026 temp-write + rename + file-fsync + parent-directory-fsync discipline at the destination, whether the destination is local or remote.

(e) Before an implementer resume/Retask for iteration `N+1`, the daemon MUST write `${run_workspace_path}/.harmonik/reviewer-feedback.iter-<N>.md` through the location-aware seam and complete its WM-026 durability boundary. The resumed implementer reads the file in its own run-workspace location; a box-A-only feedback copy is not delivery.

(f) When reviewer allowance exhaustion produces `.harmonik/reviewer-budget-exceeded.json` in the projection, the daemon MUST transfer that diagnostic atomically through the location-aware seam to the run-workspace review-control archive before emitting `reviewer_budget_exceeded` or finalizing the phase outcome. The diagnostic file, not the event, is the durable Workspace Model artifact.

(g) After reviewer quiescence, the daemon MUST transfer the complete reviewer session directory and metadata sidecar per WM-025/WM-026 through the same seam into `${run_workspace_path}/.harmonik/sessions/${session_id}/`. A required verdict, feedback, budget-diagnostic, session-log, or metadata transfer failure — including destination digest mismatch or remote durability failure — is a phase error. The daemon MUST NOT emit a verdict or budget outcome that claims an unarchived artifact, MUST NOT continue to implementer Retask without durable feedback, and MUST NOT remove the projection while the retained reviewer session archive is incomplete.

(h) The projection may be removed only after the reviewer session has terminated, all authoritative and auxiliary observers have cancelled and joined, pane/process wait and reap are complete, verdict validation is complete, and all required artifact plus session-directory transfers have completed and verified durable. Cleanup MUST be idempotent and MUST perform `git worktree remove --force --force` followed by `git worktree prune`.

**Crash reclamation.** Projection creation MUST register the identity tuple in the projection manifest before launch. On startup, the workspace manager MAY reclaim a leftover projection only after PL-006 has quiesced or excluded every matching prior-generation process/pane, validating the manifest against the deterministic path and registered git-worktree SHA, proving that no live phase owner, reviewer session, callback, or observer still owns it, and verifying that every recoverable reviewer session directory and required review-control artifact is durable in the run workspace. If a source artifact is complete but not yet archived, startup MAY complete the same digest-verified transfer before reclamation. Path shape alone is not ownership proof. A missing, malformed, or mismatched manifest, an unverifiable live-owner state, or an incomplete/untransferable retained artifact is `reviewer-projection-integrity` evidence handed to reconciliation; it MUST NOT be deleted by a path-pattern sweep.

**Checkpoint exclusion and event authority.** `.harmonik/review.json`, `.harmonik/review.iter-*.json`, `.harmonik/reviewer-feedback.iter-*.md`, `.harmonik/review-target.md`, `.harmonik/reviewer-budget-exceeded.json`, and `.harmonik/reviewer-projection.json` MUST be excluded from checkpoint commits via WM-013e. Class-F events remain routing and audit facts; they do not replace the durable run-workspace files or provide an alternate recovery source.
...
> INFORMATIVE: `review-loop` mode exercises the run-associated session archive more aggressively than `single` mode. A run that reaches the three-iteration cap produces at most six launch-generation directories in the run-workspace archive: three implementer generations (the initial launch plus at most two resumes) and three fresh reviewer launches. Reviewer directories originate in three distinct projections and are transferred under WM-025/WM-027a; they are counted at the run archive, not as retained sessions in the disposable projections. Earlier approval produces fewer directories.
...
Instead, harmonik-managed bead worktrees rely on `--dangerously-skip-permissions` at launch (see [handler-contract.md §4.9 HC-055b] and `isHarmonikManagedWorktree`), which pre-authorizes all tools without materializing a settings-file consent gate. That flag is the sanctioned mechanism for operator-created harmonik worktrees.

On merge with an existing settings.json that already declares a `permissions.allow` key (i.e. the user committed their own into the cloned repo), that user block MUST be preserved exactly as-is — harmonik neither strips, edits, nor augments it. A user who opts into their own `permissions.allow` accepts whatever consent behavior it implies; that is their choice.

Cross-ref: [claude-hook-bridge.md §4.12 CHB-029] (trust pre-seed companion). Bead: hk-5gmkd (issue #25), supersedes hk-53y35. Code: `internal/workspace/claudesettings_wm040a.go`.
...
**Verification and repair.** Atomicity guards against *cooperating* writers only. Claude Code is not one: a live `claude` process rewrites `~/.claude.json` wholesale from its own in-memory snapshot and does not take harmonik's advisory lock, so an entry harmonik has just written may be erased moments later. The daemon MUST therefore RE-READ the config after the write and confirm `projects[worktreePath].hasTrustDialogAccepted == true` actually persisted; a successful write syscall is not evidence that it did. On a failed verification the daemon MUST retry the whole read-modify-write within a bounded attempt budget, re-reading the file FRESH on every attempt so the entry is re-applied on top of whatever the foreign writer left — re-applying a cached snapshot would erase entries committed in between and violate Preservation. A JSON-decode failure on any read is retried within the same budget, because a foreign non-atomic write can be observed torn; a config still undecodable on the final attempt is reported and left untouched. The whole loop (lock waits included) stays inside the existing lock-timeout ceiling. This narrows the race but does not eliminate it: a rewrite landing between the final verification and Claude Code's own startup read still produces the dialog, which no mechanism on harmonik's side of the process boundary can prevent.

**Idempotency.** If the entry is already present and `hasTrustDialogAccepted` is already `true`, no write is performed. This covers the re-attach path (daemon restart mid-session). The check is a lock-free read and is advisory by construction — a rewrite immediately afterwards can still remove the entry, which is why the write path verifies rather than trusting its own success.
...
**Failure.** If the write fails — including the case where the entry has not persisted after the final verification attempt — the daemon MUST NOT exec Claude. The error is surfaced as `ErrStructural` with `sub_reason: trust_seed_failed`; `agent_failed` is emitted. An un-seeded launch blocks silently rather than failing fast, so reporting success while the entry is absent on disk is forbidden: the error message MUST name the non-persistence and its likely cause (a concurrent foreign rewrite).

**Trust vs. permissions — two distinct gates.** WM-040b addresses only the folder-*trust* dialog ("Is this a project you trust?"), suppressed by the `~/.claude.json` `hasTrustDialogAccepted` key. It does NOT address the separate per-tool *permission* gate. For harmonik-managed bead worktrees the per-tool gate is handled by launching with `--dangerously-skip-permissions` per [handler-contract.md §4.9 HC-055b] (see WM-040a's "No permissions pre-authorization" note and `isHarmonikManagedWorktree`); the `.claude/settings.local.json` file in the worktree does not carry trust state that suppresses the startup dialog, and `--permission-mode` remains unused.
...
The prohibition above applies to leased run workspaces, not WM-027a reviewer projections. Projection processing is a separate manifest-aware branch that MUST occur only after PL-006 has completed the relevant process/tmux reaping and owner-proof exclusions. A leftover reviewer projection MAY be reclaimed only through WM-027a's proof: the manifest tuple matches the deterministic path and registered git-worktree SHA; no live phase owner, reviewer session, callback, watcher, or observer remains; and required reviewer logs, metadata, verdict, feedback, and diagnostics are already durable in the run workspace or are successfully transferred before removal.

If that proof fails, S06 MUST hand the projection to reconciliation as `reviewer-projection-integrity` evidence containing the observed path, manifest bytes or parse error, registered SHA, discovered session IDs, archive-presence/transfer result, and surviving owner evidence. The generic no-lock/path-pattern sweep MUST NOT remove or reclassify it. A conforming projection that is reclaimed MUST be removed with WM-027a's idempotent `git worktree remove --force --force`; malformed or ambiguous evidence remains on disk for the reconciliation verdict.
...
Every subsystem observing a run workspace MUST treat its `run_id` as the exclusive lease holder for the run's full lifetime (per WM-010 definition). No subsystem MAY mechanism a per-agent lease acquisition. Agents dispatched into the run workspace sequentially occupy it under one lease; reviewer projections are separately phase-owned, daemon-controlled, and always unleased. This invariant constrains the orchestrator (S01; dispatch), the agent runner (S04; handler-launch preconditions), the workspace manager (S06; storage), reconciliation (lost-lease and projection-integrity detection), and the memory layer (S08; session-metadata consumption). It is the workspace-layer realization of the centralized-controller principle per [architecture.md §4.9 AR-INV-007].
...
```
RECORD ReviewerProjectionManifest:
    run_id         : UUID                    -- owning run
    iteration      : Integer                 -- 1-based reviewer iteration
    source_ref     : String                  -- run/task ref transferred to box A
    source_sha     : CommitSHA               -- verified detached-HEAD object
    location       : String                  -- fixed value "box-a" in this revision
    schema_version : Integer                 -- N-1 readable per §6.4; set by S06
```
...
#### WM-RIA-001 — Reviewer-input artifact path

For every review-loop iteration, the sole structured reviewer-input artifact MUST be staged at `${reviewer_projection_path}/.harmonik/review-target.md` in the exact-SHA box-A projection. It MUST NOT be materialized as the reviewer's live input in the run workspace. The path is projection-scoped and expires only after the PL-014b close barrier and WM-027a archive-transfer gate permit projection removal.
...
| `<repo>/.harmonik/worktrees/<run_id>-reviewer-<N>/` | S06 (transient) | Box-A reviewer projection for iteration `<N>` per WM-027a; detached at the verified `source_sha`, unleased, registered for crash reclamation. |
| `${reviewer_projection_path}/.harmonik/reviewer-projection.json` | S06 | Reviewer-projection identity manifest containing `{run_id, iteration, source_ref, source_sha, location}`; required before launch and validated before reclamation. |
| `${reviewer_projection_path}/.harmonik/sessions/<session_id>/` | S06 creates/transfers; S04 writes logs | Live reviewer session staging directory. The complete directory and sidecar transfer to the same-`session_id` path under the run workspace after quiescence and before projection cleanup. |
| `${reviewer_projection_path}/.harmonik/review-target.md` | S06 writes; reviewer reads | Projection-staging reviewer input for one iteration; atomically materialized before launch; not authoritative run state. |
| `${reviewer_projection_path}/.harmonik/review.json` | reviewer writes; S06 validates/transfers | Live projection-staging verdict; conforms to agent-reviewer schema v1; not authoritative until transferred under WM-027a. |
| `${reviewer_projection_path}/.harmonik/reviewer-budget-exceeded.json` | reviewer observer writes; S06 transfers | Projection-staging allowance-exhaustion diagnostic; must transfer before the observable budget outcome. |
| `${run_workspace_path}/.harmonik/review.json` | S06 | Optional current-verdict alias of the authoritative per-iteration archive; never the sole durable verdict. |
...
| `${run_workspace_path}/.harmonik/review.iter-<N>.json` | S06 (authoritative per-iteration archive) | Validated iteration-`N` verdict atomically transferred from the projection; authoritative durable reviewer record. |
| `${run_workspace_path}/.harmonik/reviewer-feedback.iter-<N>.md` | S06 (authoritative Retask input) | Reviewer feedback written durably in the implementer's run-workspace location before iteration `N+1` resume/Retask. |
| `${run_workspace_path}/.harmonik/reviewer-budget-exceeded.json` | S06 | Authoritative reviewer-allowance diagnostic transferred before `reviewer_budget_exceeded` emission. |
...
- **WM-010 — WM-013e (lease model).** Multi-agent-sequential scenario tests: three run-workspace agents run in sequence under one lease; reviewer launch uses a separate exact-SHA projection with no lease-lock and never reacquires the run lease; one-writer-per-physical-worktree and no reviewer/implementer overlap are enforced; lease-lock file is written exactly once per lease with atomic temp+rename+fsync+parent-dir-fsync; lock JSON content matches WM-013a; lock is released on terminal transitions (WM-013b) with lock absence AND workspace-local `lease_released` marker as release proofs; per-terminal-path release gates exercised (merged, run_failed, post_escalation, verdict_driven); filesystem discovery partitions canonical run paths from projections (WM-013c); canonical-path re-use is rejected (WM-013d); `.gitignore` hygiene enforced at startup with `GitignoreWriteForbidden` surfaced if daemon lacks write permission (WM-013e).
...
- **WM-025 — WM-030, WM-RIA-001 (session-log and reviewer artifacts).** Filesystem tests: metadata sidecar exists before `workspace_leased` for the first session; sidecar atomic write (temp+rename+parent-dir-fsync) verified; `.tmp-<pid>` orphan cleanup by startup sweep; subsequent session sidecars do not re-emit `workspace_leased`; namespaced `node_id` tested inside an expanded sub-workflow; `.gitignore` excluding `.harmonik/sessions/` is detected as a misconfiguration; CASS (S08) reads sidecar + log; post-merge retention default verified. WM-027a scenarios cover local and remote run workspaces, exact-SHA box-A projection creation, no second lease, projection-local WM-RIA-001 target, local verdict validation, location-explicit digest-verified verdict/feedback/budget transfer, full reviewer session-directory + sidecar transfer after watcher/process/pane join, remote destination fsync, transfer-failure fail-closed behavior, and cleanup only after every archive transfer verifies durable.
- **WM-031 — WM-033 (failed-run persistence).** Daemon-restart scenario tests: failed-run worktrees persist across restart; lease-lock file released on terminal failure per WM-013b with marker written before unlink; orphan sweep uses content-first staleness (mtime tiebreaker only) per WM-033; `git worktree prune` invoked after sweep; operator-issued `git worktree lock` respected (prune skips locked entries). Leftover reviewer projections are removed only after process/pane quiescence, manifest/path/SHA identity agreement, no live owner, and durable archive verification or successful recovery transfer; missing/malformed/mismatched manifests, live-owner ambiguity, and incomplete/untransferable archives hand `reviewer-projection-integrity` evidence to reconciliation without deletion.
...
|---|---|---|---|
| 2026-07-24 | 0.4.10 | agent (codename:reviewloop-decoupling) | **Projection integration repair.** Reconciled WM-010/WM-011/WM-013a and WM-INV-001 with one leased run workspace plus mandatory unleased per-iteration reviewer projections; partitioned projection candidates from generic no-lease discovery; added the `reviewer-projection-integrity` reconciliation handoff; required full reviewer session-log/metadata transfer before cleanup; defined the explicit-location, digest-verified local/remote artifact-transfer seam; added the `WM-RIA-001` reviewer-input path anchor; corrected three-iteration archive accounting to six launch generations; and aligned WM-033 reclamation with PL-006 quiescence. No frozen WM-NNN IDs were added or renumbered. **Status:** reviewed. |
| 2026-07-24 | 0.4.9 | agent (codename:reviewloop-decoupling) | **Reviewer projection placement and durable control archive.** WM-027a now distinguishes the authoritative run workspace from the box-A reviewer projection, fixes projection identity and pathing, requires exact-SHA materialization, defines verdict/feedback/budget-diagnostic transfer into the run workspace, fails closed on transfer errors, and specifies idempotent cleanup. WM-033 adds manifest-validated crash reclamation. §6.1 adds `ReviewerProjectionManifest`; §6.2 names projection and run-archive paths; WM-013e lists the new runtime files. No requirement IDs were added or renumbered. **Status:** reviewed. |
...
| 2026-05-14 | 0.4.6 | agent (hk-53y35) | **WM-040a amended: permissions.allow pre-authorization added.** The materialized settings.json MUST now include a top-level `"permissions": {"allow": [...]}` array with the standard Claude Code tool set (Read, Write, Edit, Bash, Glob, Grep, WebFetch, WebSearch, NotebookEdit, TodoWrite, Task). Per-tool permission dialogs block unattended daemon operation; this is the spec-compliant alternative to the deny-listed --dangerously-skip-permissions flag (CHB-007 / HC-055). On merge, existing user `permissions.allow` is preserved. Cross-ref: claude-hook-bridge.md §4.12 CHB-029. Code: internal/workspace/claudesettings_wm040a.go. No prior requirement IDs renumbered. Status remains `reviewed`. **SUPERSEDED 2026-07-08 by hk-5gmkd — see below.** |
| 2026-07-08 | 0.4.7 | agent (hk-5gmkd, #25) | **WM-040a amended: permissions.allow injection RETIRED (supersedes hk-53y35).** harmonik MUST NOT inject a `"permissions"`/`"allow"` block into worktree settings.json. Root cause: Claude Code >= 2.1.204 treats a pre-approved `permissions.allow` in a git-worktree project settings.json as a consent gate — an interactive "pre-approves N tool permissions" modal that `--dangerously-skip-permissions` does NOT bypass, so a daemon-spawned pane wedges and HC-056 (`agent_ready_timeout`) fires. harmonik-managed bead worktrees rely on `--dangerously-skip-permissions` (HC-055b) instead, applied via `isHarmonikManagedWorktree` (now also matching `.harmonik/worktrees/` and crew-worktrees path segments). A user's own committed `permissions.allow` is preserved untouched (never stripped or augmented). Also reconciled: the stale WM-040a/WM-040b "`--dangerously-skip-permissions` MUST NOT be used" lines (the flag IS used for harmonik-managed worktrees per HC-055b). Companion cross-ref reconciled in claude-hook-bridge.md §4.12 CHB-029. Code: internal/workspace/claudesettings_wm040a.go, internal/daemon/claudelaunchspec.go. No prior requirement IDs renumbered. Status remains `reviewed`. |
| 2026-07-21 | 0.4.8 | agent (hk-qx065) | **WM-040b amended: the trust write is VERIFY-AND-REPAIR, not a single write.** EXTENDS §4.7b (it does not contradict any existing clause): Atomicity, Idempotency, Preservation and the no-exec-on-failure Failure rule all stand unchanged, and each repair attempt satisfies them individually. Root cause: the Atomicity clause guards against *cooperating* concurrent writers, but Claude Code is not one — a live `claude` process rewrites `~/.claude.json` wholesale from its own in-memory snapshot and does not honor harmonik's advisory sidecar flock. OBSERVED at max-concurrent 3: two of three workers parked on the folder-trust modal and the failed worktree's `projects[<realpath>].hasTrustDialogAccepted` was ABSENT from the config even though `EnsureWorktreeTrust` had run and returned success; the modal renders before SessionStart, so no `agent_ready` was ever synthesized and HC-056 fired. **New normative mechanism.** After the atomic write, the daemon MUST RE-READ `~/.claude.json` and confirm `projects[worktreePath].hasTrustDialogAccepted == true` actually persisted — a successful write syscall is NOT evidence the key survived. On a failed verification the daemon MUST retry the whole read-modify-write, re-reading the file FRESH each attempt so the key is re-applied on top of whatever the foreign writer left (re-applying a cached snapshot would make harmonik itself the lost-update clobberer of other entries, violating Preservation). The retry is bounded by an attempt budget (`trustWriteMaxAttempts`, currently 4) with a short backoff, and the pre-existing `defaultTrustLockTimeout` is re-scoped to bound the WHOLE loop rather than each attempt, preserving the ~15s ceiling WM-040b inherited from hk-bfvby. A JSON-decode failure on any read is retried within the same budget (a foreign non-atomic write can be observed torn); a config still undecodable on the final attempt is reported as-is and is never overwritten. **New failure mode.** If the key has not persisted after the final attempt the daemon MUST NOT exec Claude and MUST surface `ErrStructural` naming the non-persistence and its likely cause — reporting success while the key is absent is the defect this amendment closes. This narrows the race; it does not eliminate it. A foreign rewrite landing between the final verification and Claude Code's own startup read still produces the modal, and nothing on harmonik's side of the process boundary can prevent that. The idempotent lock-free fast path (already-trusted → no write, no lock) is unchanged and remains advisory by construction. Companion cross-ref: claude-hook-bridge.md §4.12 CHB-029 (unchanged). Code: internal/workspace/claudetrust_wm040b.go. Tests: internal/workspace/claudetrust_hkqx065_test.go, internal/daemon/scenario_trust_clobber_hkqx065_test.go. No prior requirement IDs renumbered. Status remains `reviewed`. |
