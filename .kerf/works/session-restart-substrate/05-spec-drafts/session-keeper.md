...
> INFORMATIVE: `SK` is not yet present in `specs/_registry.yaml`. Per the template's requirement-numbering convention the registry edit reserving `SK: {spec-id: session-keeper, reserved: 2026-07-13, status: draft}` lands in the same commit that introduces this file; the lint check fails otherwise.
