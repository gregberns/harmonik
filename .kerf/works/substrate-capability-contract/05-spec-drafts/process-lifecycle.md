...
the wire names. `queue-remove`, `queue-pause`, `queue-resume`, and
`queue-clear` remain deferred. The retired `enqueue` method has no alias or
compatibility shim.
...
6a. **Declared capability resolution.** Before first dispatch, the daemon
composition root MUST resolve each capability required by the selected hosting
configuration. It MUST pass each consumer only the capability that consumer
needs. When a required construction capability is absent, the daemon MUST
refuse the selected configuration before dispatch. A capability known only
after a run-plan choice or pane spawn MUST be checked immediately before that
operation. Its absence MUST use that operation's declared result. It MUST NOT
be described as a pre-dispatch requirement. Each optional capability MUST have
an explicit, tested degraded result when absent. This rule does not expose
daemon-private interfaces, change the three session-resolution outcomes in item
3, widen the handler substrate or input seams, or change the selected workflow.
Pane capture remains observation only. An unreachable independent run-session
creation path MUST be removed. It MUST NOT remain as a capability.
...
> NOTE (extqueue v0.1). The `harmonik enqueue` subcommand and its JSON-RPC
> `enqueue` method are retired. The five live-daemon `hk queue <verb>` surfaces
> are submit, append, status, dry-run, and cancel; remove/pause/resume/clear are
> deferred.
...
4. **Never fake, never defer.** The daemon MUST NOT report a capability the resolution did not produce, and MUST NOT allow a missing prerequisite to first surface as a dispatch-time failure. A capability that is genuinely gone is announced loudly at boot and is absent thereafter. PL-021b item 6a governs the selected-mode capability result after hosting resolution.
...
`status`, `dry-run`, `cancel`). Each verb accepts both `--flag value` and
...
- **PL-021b item 6a** — composition tests provide a base-only substrate and capability providers. They assert each optional consumer keeps its explicit degraded result, a selected construction requirement refuses before dispatch, and an operation requirement fails only at its declared operation boundary. Tests assert that handler seams do not widen, workflow selection remains sealed, and no unreachable independent run-session path remains.
...
|---|---|---|---|
| 2026-08-02 | 0.7.5 | agent (codename:substrate-capability-contract) | **Declared substrate capabilities.** PL-021b gains item 6a. The daemon composition root resolves configuration requirements before dispatch and checks later run-plan or post-spawn requirements at their operation boundary. A missing optional capability keeps an explicit tested degradation. The handler process-hosting, input, and observation seams do not widen. The unreachable independent run-session path is removed rather than treated as a hidden capability. PL-028b cross-references the selected-mode result. §10.2 adds composition evidence. |
