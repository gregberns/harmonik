...
composition root MUST resolve the capabilities required by the selected hosting
mode. It MUST pass each consumer only the capability that consumer needs. When
a required capability is absent, the daemon MUST refuse the selected mode
before dispatch. Each optional capability MUST have an explicit, tested
degraded result when absent. This rule does not expose daemon-private
interfaces, change the three session-resolution outcomes in item 3, widen the
handler substrate or input seams, or change the selected workflow. Pane capture
remains observation only. An unreachable independent run-session creation path
MUST be removed. It MUST NOT remain as a capability.
...
- **PL-021b item 6a** — composition tests provide a base-only substrate and capability providers. They assert each optional consumer keeps its explicit degraded result and a selected required mode refuses before dispatch. Tests assert that handler seams do not widen, workflow selection remains sealed, and no unreachable independent run-session path remains.
