...
- **`.claude/skills/` is GENERATED OUTPUT for every skill that ships in the binary.** The source is `cmd/harmonik/assets/skills/<name>/` (pulled in by `//go:embed` in `cmd/harmonik/init_skill_assets.go`). **To change a shipped skill: edit the source copy, then mirror it byte-for-byte into the other homes in the same commit.** **There is a THIRD copy, and it wins.** `.harmonik/agents/_skills/` is tracked and holds `agent-comms`, `beads-cli`, `crew-launch`, `harmonik-dispatch` and `boot`; `internal/agentmanifest` `resolveRef` checks `_skills/` BEFORE the type folder, so for any agent launched through a manifest that copy is the one that loads. **You no longer have to remember any of this: `scripts/skill-mirror-check.sh` runs in `make core`, `fast` and `full`, and fails on drift, on a missing `.claude/skills/` mirror, and on a missing source tree.** The shipped set is `ls cmd/harmonik/assets/skills/`; everything else under `.claude/skills/` is project-authored and safe to edit in place.
...
- **Priority** is stated intent first, then the ledger — canonical in the `orchestrator-rules`
  skill (`REFERENCE.md` §Priority). **How an agent finds work belongs in its direction, not here.**
  Which query surfaces the backlog, in what order, and how deep, changes; a rule in this file
  fixes that choice for every agent in the project and outlives the reason it was written. One
  fact about the tool is not a choice and does bite regardless: **`br ready` shows 20 rows and
  does not say that it truncated**, so a short listing is never evidence of a short backlog. The
  `beads-cli` skill owns that and the flag that defeats it.
