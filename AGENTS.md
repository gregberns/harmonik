...
**Doing kernel / bus / restructure / extraction work?** Read [`plans/2026-09-07-harmonik-restructure/07-segmented-structure-and-day1-standards.md`](plans/2026-09-07-harmonik-restructure/07-segmented-structure-and-day1-standards.md) FIRST — it is the direction of record for the platform re-grounding, and several things are settled there: the code lives in a **segmented module structure** (`kernel/`, `tools/<name>/`, `shared/`, `contract/`), **never a `clean/` folder or a "clean room"**; the substrate is the **kernel** (not "transport"); plugins are **subprocesses** (live-reload + crash isolation); **hub/spoke competing-consumers is core, not deferred**; and verification is **fault-injection from commit one**, not a unit suite. Do not re-design or re-name these. The full design of record and its evidence are the two `plans/2026-09-07-harmonik-{bus,restructure}/` folders (the `05`–`08` files supersede the shallow `01`/`02`).
