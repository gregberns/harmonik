...
**Exit codes:** 0 success (JSON to stdout) · 1 validation error (JSON error body) · 2 transport/protocol error or unrecognised verb (also: usage error from `set-concurrency`, and a selector that matches no queue — `pause`, `resume` and `cancel` all refuse a name nothing matches, and `cancel --queue-id` refuses a uuid nothing matches, rather than report a success that changed nothing; `cancel`, `pause`, `resume` and `recover` all refuse a selector GIVEN EMPTY, which an unexpanded shell variable produces. It used to archive the main queue at exit 0 on `cancel`; on `pause` and `resume` an empty name is the daemon's GLOBAL scope, so it drained every queue or cleared a deliberate fleet-wide pause, and on `recover` it re-armed every failed item on `main`. `harmonik supervise pause` / `supervise resume` remain the way to reach global scope on purpose) · 17 daemon not running.
