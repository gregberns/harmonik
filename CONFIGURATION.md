...
| `--workflow-mode MODE` | daemon flag | `builtin` | Workflow dispatch shape applied to each bead: `builtin` (leave unset so the daemon resolves its own default, which is `dot` running the embedded `standard-bead.dot` — implement → review → merge, reviewed by construction), `single` (implement only, NO review; reachable only as an explicit per-bead choice), or `dot` with `--workflow-ref` for an author-defined graph. `review-loop` is RETIRED (execution-model.md §4.3.EM-015d): the flag value is rejected with a message naming `dot`, and a `daemon.workflow_mode: review-loop` in `config.yaml` fails at load rather than being silently coerced. A per-bead `workflow:` label overrides this; a stale `workflow:review-loop` label is treated as an unknown mode — `bead_label_conflict` is emitted and the bead resolves to `dot`. |
