<!-- PP-TRIAL:v2 2026-06-08 main in-sync origin — CLEAN. Daemon SAFE but PINNED to --workflow-mode review-loop (pid 96288) because the DOT-default flip WEDGES. THIS SESSION (named-queues): landed all gate-robustification (hk-4f5ua/hk-u830m/hk-5735k/hk-4c7kw, all reviewed) → the internal/daemon scenario reds are fixed. BUT the flip itself is BLOCKED on a NEW P1: hk-m5axg — the DOT review node spawns a reviewer claude but never DRIVES it (no prompt injected) → run wedges/run_stale after the implementer commits. The flip was tried (~10:28 operator rebuild), wedged, and the daemon was pinned back to review-loop. NEXT AGENT: fix hk-m5axg (internal/daemon/dot_cascade.go vs the legacy pasteinject reviewer path), live-smoke, then un-pin. **Role handoff with full next-steps: HANDOFF-named-queues.md** (read it). -->
