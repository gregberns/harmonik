...
| 15 | FUTURE | Operator interface for the decisions channel | (not yet filed) | **The operator has no way to see or answer a decision.** An agent that needs a human ruling calls `harmonik decisions wait` and blocks. The request lands in a JSONL file that no human reads, and no surface exists to list pending decisions or answer one. So a decision is raised, never seen, and the agent waits or proceeds unanswered. The machinery underneath works and was hardened on 2026-08-04 (`hk-1dwk2`) — what is missing is the human end of it. Until this is built, treat the decisions channel as write-only and do not design a flow that depends on a human answering through it. |
