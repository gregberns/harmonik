...
4. Pick the work. Stated intent comes first — the active plan's order and any dated operator directive. Below that line, order the unclaimed backlog with `br ready --sort priority --limit 0`, and scope it to one lane with `--parent <epic_id>`. Pass `--limit 0`: `br ready` returns 20 rows by default, so a short listing is not evidence of a short backlog. `kerf` plans work and does not rank it — `kerf map` shows which work owns a bead.
