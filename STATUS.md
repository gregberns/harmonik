...
1. Read [PRINCIPLES.md](PRINCIPLES.md), then [`plans/2026-07-27-delete-and-rewrite/CHARTER.md`](plans/2026-07-27-delete-and-rewrite/CHARTER.md). Every role starts with those two. What you read after them depends on your role, and `AGENTS.md` §"Per-role load map" is the statement of it — a captain boot-reads neither this file nor `AGENT_INDEX.md`. A session with no role continues [AGENT_INDEX.md](AGENT_INDEX.md) → [STATUS.md](STATUS.md) → `HANDOFF.md`. **Corrected 2026-07-30:** this step used to put `.harmonik/context/captain-lanes.md` in everyone's reading order. That file is captain-tier. Its own header says a captain loads it at STARTUP Step 0b and crews and implementers do not. Its content is also stale, dated 2026-07-22. `HANDOFF.md` is the most recent `/session-handoff` output. It is gitignored and machine-local, so it is absent from a fresh clone.
2. Read the `orchestrator-rules` skill (`.claude/skills/orchestrator-rules/SKILL.md`) — permanent dispatch/priority directives.
3. Run `harmonik queue status` to confirm the daemon is alive. Exit 17 means the daemon socket is absent. **Corrected 2026-07-30:** this step used to point at CLAUDE.md §"Start the daemon once" for the start command. CLAUDE.md has no such section. Use the `harmonik-lifecycle` skill. **Corrected 2026-08-24:** the "daemon is intentionally DOWN, exit 17 is expected" note that stood here was six weeks stale — the daemon is up and dispatching. Exit 17 means it is down and that is a condition to fix, not the expected state.
4. Pick the work. Stated intent comes first — the active plan's order and any dated operator directive. Below that line, **how you find the rest belongs in your mission, not here**: which listing, in what order, and how deep, changes. Two facts about the tools are not choices. `br ready` returns 20 rows and does not say that it truncated, so a short listing is never evidence of a short backlog — the `beads-cli` skill owns the flag that defeats it. And `kerf` plans work without ranking it; `kerf map` shows which work owns a bead.
