...
**`commit_landed=true` on `implementer_phase_complete` means the implementer committed in the run's ISOLATED worktree — NOT that the work merged.** It signals only that the run's worktree HEAD advanced past its parent SHA. NEVER conclude "the work landed" — and never suppress OR escalate — based on `commit_landed` alone. To confirm a run/bead actually landed on the daemon's target branch, require a **merge event** for that same `run_id`/`bead_id`: `bead_closed` (emitted only after the merge into the target branch on a SUCCESS branch — strongest) or `outcome_emitted{kind:"approved"}` (the daemon merge-sequence event). **That target branch is the integration branch, not `main`.** It comes from `.harmonik/branching.yaml` key `defaults.lands_on`. A merge event never proves that anything reached `main`. Only the integration→`main` pull request does that, and the daemon does not open it. A run that reached `implementer_phase_complete{commit_landed:true}` but then `run_failed` at a review stage with **none** of those merge events is a NORMAL review-stage failure (worktree commit only) — classify it per the `run_failed` row, NOT as a landed-work IMMEDIATE. The most common partial-success shape proves the trap:
...
Old guidance let the watch read `commit_landed:true` and infer the work had landed, then false-escalate an IMMEDIATE when the review-stage failure followed. With the merge-event requirement, the absence of `bead_closed`/`outcome_emitted{approved}` for `R` makes clear nothing landed — so the watch treats it as an ordinary review-stage `run_failed`, not a landing contradiction.
