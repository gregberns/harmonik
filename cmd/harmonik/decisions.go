...
//
// The wait path uses the lighter decisionTerminalInLog (one decision, terminal
// only); this full fold is provided for parity with K3 and for the unit test
// that asserts the open-set semantics match. It is intentionally minimal —
// only the fields a K2-side reader needs (question/options for a future
// raise-side echo) are carried.
...
// forbids importing the daemon type).
