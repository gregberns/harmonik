...
func runPromotePush(ctx context.Context, projectDir, target string, cfg promoteConfig) int {
	...
	// #nosec G204 -- target is passed as a discrete git argument, never through a shell.
	...
	defer func() {
		...
		if rmErr := rmCmd.Run(); rmErr != nil {
			if removeErr := os.RemoveAll(tmpDir); removeErr != nil {
				fmt.Fprintf(os.Stderr, "harmonik promote: remove temporary worktree %s: %v\n", tmpDir, removeErr)
			}
			pruneCmd := exec.CommandContext(ctx, "git", "worktree", "prune")
			pruneCmd.Dir = projectDir
			if pruneErr := pruneCmd.Run(); pruneErr != nil {
				fmt.Fprintf(os.Stderr, "harmonik promote: prune stale worktree registration: %v\n", pruneErr)
			}
		}
	}()

	// #nosec G204 -- tmpDir comes from os.MkdirTemp and remoteTip is git's rev-parse output.
	...
	for _, sha := range cfg.shas {
		...
		if cpOut, cpErr := cpCmd.CombinedOutput(); cpErr != nil {
			...
			if abortErr := abortCmd.Run(); abortErr != nil {
				fmt.Fprintf(os.Stderr, "harmonik promote: abort failed cherry-pick: %v\n", abortErr)
			}
		...
		if beadID != "" {
			// #nosec G204 -- beadID becomes one argument to git commit; no shell is invoked.
	...
	for attempt := 1; attempt <= maxPromotePushAttempts; attempt++ {
		...
		if _, goModErr := os.Stat(filepath.Join(tmpDir, "go.mod")); goModErr == nil {
			for _, buildArgs := range [][]string{
				...
				// #nosec G204 -- buildArgs are the fixed build and vet command argument lists.
		...
		// #nosec G204 -- target is passed as a discrete git refspec argument, never through a shell.
		...
		// #nosec G204 -- target is passed as a discrete git revision argument, never through a shell.
		...
		if rebaseOut, rebaseErr := rebaseCmd.CombinedOutput(); rebaseErr != nil {
			...
			abortCmd.Dir = tmpDir
			if abortErr := abortCmd.Run(); abortErr != nil {
				fmt.Fprintf(os.Stderr, "harmonik promote: abort failed rebase: %v\n", abortErr)
			}
...
func runPromotePR(ctx context.Context, projectDir, target string, cfg promoteConfig) int {
	...
	// #nosec G204 -- ghPath is resolved with LookPath and args are passed directly, not through a shell.
	ghCmd := exec.CommandContext(ctx, ghPath, ghArgs...)
