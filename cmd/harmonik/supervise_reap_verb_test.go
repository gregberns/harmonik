...
func TestSuperviseReapVerb_KillsDeadFlywheel_LeavesDefault(t *testing.T) {
	...
	t.Cleanup(func() {
		killFixtureSession(t, flywheel)
		killFixtureSession(t, def)
	})
	...
	//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
	createOut, err := exec.CommandContext(t.Context(), "tmux", "new-session", "-d", "-s", flywheel, "sleep", "300").CombinedOutput()
	if err != nil {
		t.Skipf("tmux new-session (flywheel) failed (may lack a server): %v: %s", err, createOut)
	}
	if out, setErr := exec.CommandContext(t.Context(), "tmux", "set-option", "-t", flywheel, "remain-on-exit", "on").CombinedOutput(); setErr != nil {
		t.Skipf("could not set remain-on-exit for test fixture: %v (%s)", setErr, strings.TrimSpace(string(out)))
	}
	...
	//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
	pidOut, err := exec.CommandContext(t.Context(), "tmux", "list-panes", "-t", "="+flywheel, "-F", "#{pane_pid}").CombinedOutput()
	if err != nil {
		t.Skipf("could not read flywheel pane_pid: %v: %s", err, pidOut)
	}
	pid, convErr := strconv.Atoi(strings.TrimSpace(string(pidOut)))
	if convErr != nil || pid <= 0 {
		t.Skipf("invalid flywheel pane child PID %q: %v", strings.TrimSpace(string(pidOut)), convErr)
	}
	//nolint:gosec // G204: the PID is validated numeric data read from the controlled fixture's tmux pane.
	if err := exec.CommandContext(t.Context(), "kill", "-KILL", strconv.Itoa(pid)).Run(); err != nil {
		t.Skipf("could not kill flywheel pane child %d: %v", pid, err)
	}
	...
	//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
	if out, err := exec.CommandContext(t.Context(), "tmux", "new-session", "-d", "-s", def, "sleep", "300").CombinedOutput(); err != nil {
	...
	//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
	if err := exec.CommandContext(t.Context(), "tmux", "has-session", "-t", "="+flywheel).Run(); err == nil {
		t.Errorf("flywheel session %q still exists after reap — expected it to be reaped", flywheel)
	}
	// The -default must survive.
	//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
	if err := exec.CommandContext(t.Context(), "tmux", "has-session", "-t", "="+def).Run(); err != nil {
...
func killFixtureSession(t *testing.T, session string) {
	t.Helper()

	out, err := exec.CommandContext(context.WithoutCancel(t.Context()), "tmux", "kill-session", "-t", "="+session).CombinedOutput()
	if err != nil && !strings.Contains(string(out), "can't find session") {
		t.Errorf("kill tmux fixture session %q: %v (%s)", session, err, strings.TrimSpace(string(out)))
	}
}
...
func waitPaneDead(t *testing.T, session string) bool {
	...
	for time.Now().Before(deadline) {
		//nolint:gosec // G204: the tmux command and session name are generated by this test fixture.
		out, err := exec.CommandContext(t.Context(), "tmux", "list-panes", "-t", "="+session, "-F", "#{pane_dead}").CombinedOutput()
...
func waitPaneDeadViaListSessions(t *testing.T, session string) bool {
	...
	deadline := time.Now().Add(10 * time.Second)
	for time.Now().Before(deadline) {
		out, err := exec.CommandContext(t.Context(), "tmux", "list-sessions", "-F", "#{session_name}\x1f#{pane_dead}").CombinedOutput()
		if err == nil {
