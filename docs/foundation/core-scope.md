...
**CI gates (2026-04-24 record):** `go vet`, `staticcheck`, `gofmt -l` empty, unit+property <3min, integration <5min, scenario <10min, crash fast subset <2min, import-allowlist linter, no `t.Skip()` in committed tests. **Two of those never shipped as gates.** Nothing has ever rejected a `t.Skip()`, and the tree holds 233 of them — `tools/testreport` lists every skipped test in its NOT RUN section instead. `testing.md §CI gates` carries the current set.
