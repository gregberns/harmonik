...
**Foundation's correctness is validated against the first buildable slice of harmonik** (see Constraints §"The first buildable slice as forward-reference"). If the slice described in `docs/bootstrap.md §2` is not buildable from foundation specs alone, foundation is incomplete — regardless of how many criteria it formally satisfies.
...
2. **Resolve the cross-cutting architectural questions that block subsystem specs from being written independently.** Concretely, answer:
   ...
   - Queue-format compatibility contract (even if binary signing is deferred)
...
- **Distributed tracing.** Harmonik runs in a single process per bootstrap.md §2. Tracing is deferred; foundation instead specifies the structured-log format every subsystem emits.
- **Binary signing for pause-to-upgrade.** Deferred (Q-R4). Foundation specifies a commit-hash check as the integrity gate — cheaper, and sufficient for the current threat model. Full binary signing is a deferred NFR.
- **Metrics exposition format** (Prometheus scrape endpoint / OpenTelemetry OTLP). The *external wire format* (what a metrics scraper sees) is deferred. Foundation defines the conceptual metric set and requires metrics to be emitted as (a) structured log entries with a `metric_name` field + numeric `value` field, AND (b) typed events of type `metric` in the event log. Support for external scrapers reading Prom/OTel formats is deferred. This resolves the apparent contradiction between "structured-log format is in foundation" and "metrics exposition format defers": the internal emission is in foundation; only the external scrape protocol defers.
...
- **Bootstrapping process detail.** Bootstrap gets its own kerf work once foundation lands. Foundation does not spec the bootstrap process. Foundation *is* validated against the first buildable slice (see Constraints §"The first buildable slice as forward-reference").
...
- **Multi-tenancy / per-tenant cost attribution.** Out of scope for now. Harmonik today is single-user/single-project.
...
| Agent-to-orchestrator trust | **In foundation** (Handler & Process Contract) | Handler subprocess authenticity (is the `claude-twin` we launched the real `claude-twin`?) — resolved via launched-from-known-path-with-commit-hash-check; full process-identity attestation is deferred |
| Twin binary authenticity | **In foundation** (Handler & Process Contract) | Same mechanism as agent-to-orchestrator trust (launched from repo-relative path with commit-hash-check). Twin drift cadence (how often twins are re-verified against real agents) is deferred to S07 scenario-harness |
| Logging overhead budget | **Deferred** | Performance optimization concern; harmonik prioritizes correctness |
...
| Structured-log format | **In foundation** (Cross-cutting NFRs § observability) | Replaces the distributed-tracing concern while harmonik is single-process |
| Queue-format compat window | **In foundation** (Operator Control Semantics) | Required even if binary signing defers |
| Checkpoint-format stability | **In foundation** (Core Execution Data Model) | Required for upgrade across v1→v2 |
...
| Multi-repo workflows | **Deferred** | Harmonik operates against one repository at a time |
| Observability overhead budget | **Deferred** | Not blocking; harmonik optimizes for correctness, not throughput |
...
12. **No DTW (distributed durable workflow) adoption.** Q-R2 resolved 2026-04-20: harmonik does NOT adopt a DTW runtime. Restart recovery is agent-driven deterministic reconciliation against git + Beads; JSONL is observational, not replayed for state reconstruction.
    ...
    **If these conditions change** — specifically if harmonik grows to (a) span machines, (b) include irreversible external-action nodes, or (c) need multi-day external waits — reconciliation becomes insufficient and DTW (Temporal, Restate, or equivalent) must be re-evaluated. The reconciliation model is explicitly scoped to the applicability conditions above; extending it to cover a DTW-shaped need is a foundation amendment, not an incremental change.

    Rationale for current scope: git + Beads already carry the authoritative state, and DTW's replay-on-restart model contradicts the store-authority discipline harmonik already requires. Within those applicability conditions, reconciliation is a strictly simpler answer.
...
### The first buildable slice as forward-reference

Foundation is not complete until the first buildable slice of harmonik — the subsystem cut in `docs/bootstrap.md §2` — is buildable from foundation specs alone, without requiring a foundation revision. Concretely: for each subsystem in that cut (S01–S08, S09 explicitly excluded per bootstrap), foundation answers every cross-cutting question that subsystem's cut depends on. This is operationalized as Success Criterion SC-11.
...
| CASS | Session-indexing and retrieval; the memory layer depends on it. | Version-pinned when S08 implementation begins. |
| CASS Memory | Institutional-memory primitives; deferred per decision #8. | Not pinned yet. |
...
**SC-4. Every cross-cutting decision listed in Goal 2 is resolved.** Each is either answered normatively in a foundation spec, deferred to a named subsystem spec with written reason, or explicitly postponed to later work with written reason. No item is left open.

**SC-5. Every parked question in STATUS.md and every open-questions section of the subsystem docs has a disposition** — answered / deferred-subsystem / deferred-later / explicitly-out-of-scope-with-reason.

**SC-6. Every `missing` NFR in `.kerf/recon/nfr-inventory.md` has a disposition** (not just the top-10). Dispositions: accepted / deferred-subsystem / deferred-later / rejected-with-rationale. Decidable by checklist.
...
**SC-11. The subsystem cut in bootstrap.md §2 is buildable from foundation specs alone.** Operationalized as a **buildability matrix** produced during the integration pass and reviewed in spec-draft and integration passes.

The matrix has:
- **Rows:** the eight subsystems in the bootstrap.md §2 cut (S01 orchestrator minimum, S02 YAML policies, S03 in-process + JSONL, S04 NTM + Claude Code + twin, S05 Claude Code hooks for completion, S06 worktree + adze + sequential-multi-agent, S07 one scenario + twin, S08 CASS pointed at session-log directory). S09 is excluded from that cut per bootstrap.md.
...
- **Cells:** one of `ANSWERED in spec X §Y` / `N/A for this subsystem (rationale)` / `DEFERRED (rationale)` / `ESCALATED (in QUESTIONS.md)`.
...
**What it contains.** **Observability protocol**: structured-log format every subsystem emits, conceptual metric set (without Prom/OTel wire format — that's deferred), health/liveness contract, audit-record requirements (subset of traces). **Security posture**: secrets lifecycle, command-execution sandbox, network egress policy, prompt-injection defense. **Operator control semantics**: the between-task invariant (what "task" means, where the boundary is), pause/stop/upgrade state machine (states, transitions, triggers), queue-format compatibility contract (N-1 supported; breaking change requires migration release). Graceful shutdown ordering. Commit-hash check as the integrity gate (Q-R4 resolution).
...
- The subsystem cut in `docs/bootstrap.md §2` as a forward-reference validation target.
...
- **Foundation is validated against the first buildable slice of harmonik, not against exhaustive theoretical completeness.** SC-11 forces that buildability check. If foundation can be "complete" but the slice can't be built, foundation is wrong.
