...
1. **Conventional Commits** for message format, with a small fixed type set. House style since 2026-08-23: the validator reads trailers, not the subject.
...
**Git hooks are retired.** lefthook, and its self-re-arming `install`, was removed because it re-wired itself on every commit. Validation now runs from the two gate targets, not from a pre-commit, pre-push, or commit-msg hook. That covers the Tier 1 and Tier 2 gates, the credential scan, and the commit-message trailers.

`scripts/commit-msg-gate.sh` runs the commit-message validator over real commits. **It runs in `make full` alone**, through the `commit-msg-check` target, which calls it with `--head-only` — the commit just made. `gate-static-product` does not call it, so `make fast` and the per-bead `make core` read no commit message at all. The script keeps a second mode, with **no argument**, which reads every commit from the **message baseline** named in that script forward to `HEAD`; no target calls that mode any more. It read 236 commits, took 47 seconds, named 109 rejections and exited 0 regardless, so `make full` dropped it on 2026-08-23 and keeps `--head-only`. That mode has no `--range` flag, and passing one is an error. The message baseline is a ratchet: it grandfathers the older commits this validator refuses, and it only moves forward.

**On a branch that does not descend from the message baseline, BOTH modes exit 0.** `--head-only` is the enforcing mode, but it demotes itself to advice when `git merge-base --is-ancestor` says this history does not descend from the baseline: it still prints `REJECTED` and the count, and it still exits 0. Measured 2026-08-12 in a scratch repository, the same invalid message committed twice, only the baseline's ancestry different: exit 1 where the baseline is an ancestor, exit 0 where it is not. **The message baseline IS an ancestor of `main` now, so `--head-only` enforces there.** Re-measured 2026-08-23: `main` and `origin/main` are both `aa423dcc0`, and `git merge-base --is-ancestor 4102b4e2e HEAD` returns 0 against each. It was not an ancestor when this paragraph was first written, because the baseline sat on the integration branch alone; the merge that superseded `main` with that branch carried it over. The demotion rule itself is unchanged — a lane whose history does not descend from the baseline still gets advice rather than a verdict — so read bead `hk-commit-msg-gate-advisory-on-main-ap068` as the record of a condition that has since cleared on `main`, not as a live description of it. The credential scan below does NOT share this: its `--head-only` consults no baseline at all. It reads `HEAD^1..HEAD`, or the whole tree when `HEAD` is a root commit.

`scripts/secret-scan.sh` is wired the same way and for the same reason. `gate-static-product` calls it with `--head-only`, so it too runs in `make fast`, `make full` and `make core`. `make full` calls it with `--range`, which is meant to read every line this branch adds on top of the **credential baseline** named in that script. The credential baseline is a different commit from the message baseline, and **both are ancestors of `main` now** — re-measured 2026-08-23 with `main` == `origin/main` == `aa423dcc0`, `git merge-base --is-ancestor` returns 0 for the message baseline `4102b4e2e` and for the credential baseline `0743e4dec`. An earlier version of this paragraph said neither was, measured 2026-08-12, and that reading is spent: the merge that superseded `main` with the integration branch carried both baselines over. Off that line — on a lane that descends from neither — the two gates still behave differently, which is the part worth carrying: the credential scan keeps an enforcing `--head-only`, and the message gate demotes itself to advice.

**Know what `--range` does when the credential baseline does not resolve.** It does not refuse. It narrows. There are three ways in: the baseline is missing from the repository, the baseline is not an ancestor of `HEAD`, or `HEAD` **is** the baseline. In each the scan falls back to `HEAD^1..HEAD` and reports clean if that is clean, whatever the rest of the tree holds. The fallback's width is not fixed: `HEAD^1..HEAD` is usually one commit, it is the whole merged side when the tip is a merge, and when `HEAD` has no parent at all the base becomes the empty tree and the scan reads the whole tree — the widest scope, not the narrowest. So on any lane that has not merged in a branch carrying the baseline, the range scope is the tip and not the branch. `main` stopped being such a lane on 2026-08-23 — it carries the credential baseline now — but a lane cut from a point before the baseline still is. Measured on the integration branch on 2026-08-12: the wide scope reads over two hundred thousand added lines and the tip fallback reads a few hundred. Neither figure is quoted exactly, because both move with every commit — the tip fallback especially, which is one commit's worth and was measured hours apart at two very different values on that same day. The order of magnitude between them is the point.

Read a green `--range` on such a branch as "the tip is clean", not as "this branch adds no credential". For credentials, `--head-only` is unaffected, because it consults no baseline. It used to be the whole of the protection on `main`, and it is not any more: both baselines reach `main` now, so both range scopes are live there. Do not read "unaffected" as "every commit passes through it". It reads `HEAD` only, so one CI run over a push of five commits reads the tip and never reads the other four. On a pull-request run `actions/checkout` builds a merge commit, so `HEAD^1..HEAD` there is the whole PR diff instead — the coverage depends on how the branch reached CI. Whether the fallback should refuse outright instead is a real decision, and nobody has made it. Refusing turns `make full` red on `main` with no quick legal repair, and a gate in that state gets deleted rather than obeyed. Bead `hk-254ea` is the record of the gap.

The credential range **fails on a finding**, where the commit-message ledger only reported — and no target runs that ledger any more. A bad message on a merged commit has no legal repair. A leaked key has one, so refusing is a demand that can be met.

`scripts/secret-scan.sh` **must not** be wired to a gate with no argument. That scope is the staged index, and every gate in this repository runs after the commit is made, when nothing is normally staged. A gate wired that way would therefore read whatever a developer happened to leave in the index — nothing, in the ordinary flow — rather than the change under test. `scripts/commit-msg-gate.sh` is different. With no argument it reads the message ledger, not the index. `make full` called it that way until 2026-08-23 and no longer does. `scripts/gate-fails-closed-test.sh` asserts that every one of these call sites is in the expanded step list — both credential-scan scopes, and, for the commit message, the gate, its wrapper self-test, the Go tests that hold the rules and the build that produces the binary the gate runs. It does that because the scan sat with no caller at all for the twenty days from 2026-07-23, when lefthook was deleted, to 2026-08-12, and no test of a script can see that. It had no unit test either, for the whole seventy-three days from its creation on 2026-05-31; the first one written against it found it admitting a key. Do not restate this as "nineteen green assertions sat unwired for twenty days" — the assertions were written on the last day of that span, and the scan they were written against did not work.

`scripts/secret-scan.sh` stays callable on its own, which is the index scope, for use before a commit exists. To check a message before you commit it, run `go run ./cmd/harmonik commit-msg validate <file>`: exit 0 on a clean message, exit 1 with every problem numbered on stderr. **Run it from source.** The subcommand landed on 2026-08-23, so an installed `harmonik` built before that date exits 2 with `unknown subcommand "commit-msg"` under a page of general help. **`--no-verify` is moot**, because there is no hook to bypass. To comply, run `/check` after committing — `make fast`, then `make full` — and fix and re-commit if it comes back red.

## Commit conventions

Format: `<type>(<scope>): <subject>` — body optional, trailers optional. Types: `feat`, `fix`, `refactor`, `test`, `docs`, `chore`, `spec`, `build`, `perf`. Scope (prefer, not required): subsystem ID (`s01`, `s04`) or top-level package (`daemon`, `handler`, `workspace`, `cli`). Subject: imperative, short, no trailing period. Body wraps at 100.

**That shape is house style and no gate reads it.** The subject half went out with the shell validator on 2026-08-23: the type set, the 72-character ceiling, the trailing period and the scope charset are now checked by nothing. The validator polices review honesty — the trailer rules below — and nothing else. A `Revert "feat: x"` subject, which the old validator refused for its shape, passes.
...
The JSON trailer is schema-validated by the Go package `internal/commitmsg`, which `harmonik commit-msg validate` exposes and which `scripts/commit-msg-gate.sh` runs from `make full` over the new commit; no git hook is involved. An unparseable JSON trailer fails validation. Prevents prompt-injection that would pass a free-text verdict. `schema_version`, `verdict`, `flags` and `notes` are all required, and `notes` must not be empty — the same **four** the Go reader in `internal/workspace` requires of a verdict file. **The two readers do not agree on WHEN `flags` is required, and the gap is real.** `internal/commitmsg` demands the key of an `APPROVE` or a `CLEAN` verdict and of nothing else; `internal/workspace` `parseReviewVerdict` demands it of every verdict. A `REQUEST_CHANGES` trailer with no `flags` therefore passes the commit gate and is `ErrMalformed` when the same JSON is read as a `review.json`. Write `flags` on every verdict — `[]` when the reviewer raised nothing — and the difference cannot reach you.
...
**An approval is held to more than the other verdicts**, so that the honest form is always the cheaper thing to write. For `APPROVE` (and the config-reviewer's `CLEAN`), the validator also requires that `Reviewed-By:` gives the bare name of a reviewer skill this repo has — `agent-reviewer` or `agent-config-reviewer` — with nothing after it. `agent-reviewer (codex harness)` and `agent-reviewer (APPROVE)` are both refused: the parenthetical used to be free text that no rule read, which let `agent-reviewer (myself)` pass as an approval by a real reviewer. Detail about the run — which harness ran, when, what it checked — belongs in the verdict's own `notes` field, inside the JSON rather than beside it. The name must also be a reviewer skill directory that git tracks (`.claude/skills/<name>/SKILL.md`), so an untracked `mkdir` beside the real skills cannot mint a reviewer identity. The validator further requires that the value does not say "self", and that the JSON carries the `flags` key. This proves the name is a real reviewer. It cannot prove that reviewer ran, and no shell script reading a commit message ever could.
...
**`Trivial: true` bypass trailer.** To opt a single commit out of the `Reviewed-By:` / `Review-Verdict:` requirement, add the trailer `Trivial: true` anywhere in the commit message's trailer block (after the blank line separating the body from trailers). `harmonik commit-msg validate` — which `make full` runs through `scripts/commit-msg-gate.sh` — detects this trailer and skips the reviewer-trailer check. Use ONLY for: typo fixes, whitespace normalization, obvious one-line corrections, and test-infrastructure trivial changes. The `make full` requirement still applies — `Trivial: true` does not bypass linting or tests, only the agent-reviewer trailer.
