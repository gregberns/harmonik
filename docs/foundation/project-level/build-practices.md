...
`scripts/secret-scan.sh` stays callable on its own, which is the index scope, for use before a commit exists. To check a message before you commit it, run `harmonik commit-msg validate <file>`: exit 0 on a clean message, exit 1 with every problem numbered on stderr. **`--no-verify` is moot**, because there is no hook to bypass. To comply, run `/check` after committing — `make fast`, then `make full` — and fix and re-commit if it comes back red.
