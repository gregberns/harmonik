...
The JSON trailer is schema-validated by `scripts/validate-commit-msg.sh`, which `scripts/commit-msg-gate.sh` runs from `gate-static-product` over the new commit and from `make full` over every commit in scope; no git hook is involved. An unparseable JSON trailer fails validation. Prevents prompt-injection that would pass a free-text verdict. `schema_version`, `verdict` and `notes` are all required, and `notes` must not be empty — the same three the Go reader in `internal/workspace` requires of a verdict file.
...
**`Trivial: true` bypass trailer.** To opt a single commit out of the `Reviewed-By:` / `Review-Verdict:` requirement, add the trailer `Trivial: true` anywhere in the commit message's trailer block (after the blank line separating the body from trailers). `scripts/validate-commit-msg.sh` — which the gate targets run through `scripts/commit-msg-gate.sh` — detects this trailer and skips the reviewer-trailer check. Use ONLY for: typo fixes, whitespace normalization, obvious one-line corrections, and test-infrastructure trivial changes. The `make full` requirement still applies — `Trivial: true` does not bypass linting or tests, only the agent-reviewer trailer.
