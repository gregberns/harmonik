...
**Git hooks are retired.** lefthook (and its self-re-arming `install`) was removed because it re-wired itself on every commit. Validation — the Tier 1/Tier 2 gates, secret scan, and commit-message trailers — now runs from the two gate targets, not from a pre-commit / pre-push / commit-msg hook. `scripts/commit-msg-gate.sh` is what runs the commit-message validator over real commits, in two modes: `gate-static` calls it with `--head-only`, which reads the commit just made, so both `make fast` and `make full` check it; `make full` calls it again with no argument, which reads every commit from the grandfather baseline named in the script forward to `HEAD`. The baseline is a ratchet — it grandfathers the older commits this validator refuses, and it only moves forward. `scripts/validate-commit-msg.sh` and `scripts/secret-scan.sh` stay callable on their own. **`--no-verify` is moot** (no hook to bypass); the way to comply is to run `/check` (`make fast`, then `make full`) after committing and fix + re-commit if it comes back red.
