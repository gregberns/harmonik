...
The JSON trailer is schema-validated by `scripts/validate-commit-msg.sh`, which `scripts/commit-msg-gate.sh` runs from `gate-static` over the new commit and from `make full` over every commit in scope; no git hook is involved. An unparseable JSON trailer fails validation. Prevents prompt-injection that would pass a free-text verdict. `schema_version`, `verdict` and `notes` are all required, and `notes` must not be empty — the same three the Go reader in `internal/workspace` requires of a verdict file.
