...
| commit-message trailers / secret scan | `scripts/secret-scan.sh --head-only` from `gate-static-product`, which `make fast`, `make full` and the per-bead `make core` all reach, so all three run it; `make full` adds `scripts/secret-scan.sh --range` (fails) and, through `commit-msg-check`, `scripts/commit-msg-gate.sh --head-only`. The commit-message gate runs in `make full` alone — `make fast` and `make core` read no commit message. | `ci.yml` → *make full* | **Partly** — a credential finding fails the build; the message gate can go advisory off its baseline line. Both are narrower than they look, and not in the way this row used to say: see the note under this table. |
...
The baseline **is** an ancestor of `main` as of 2026-08-23 — `main` == `origin/main` == `aa423dcc0`, and `git merge-base --is-ancestor 0743e4dec HEAD` returns 0 — so the range scope on `main` is the branch. This paragraph said the opposite until the merge that superseded `main` with the integration branch carried the baseline over. The narrowing still bites on any lane cut from a point before the baseline: there the range scope is the tip, not the branch. The credential scan's `--head-only` is unaffected — it consults no baseline — and carries the weight there. Do not read "unaffected" as "every commit passes through it": it reads `HEAD` at the moment a gate runs, so a CI run over a push of five commits reads the tip and never reads the other four, and on a pull-request run `actions/checkout` builds a merge commit so the same flag reads the whole PR diff. What it covers depends on how the branch reached CI. Bead `hk-254ea` is the record of the gap.

**The commit-message gate has the same tip-only gap, and one mode left that blocks.** Its ledger mode reports and never fails, and no target calls it any more. Its `--head-only` mode is the enforcing one. It demotes itself to advice and exits 0 whenever the history does not descend from the message baseline — measured 2026-08-12, the same invalid message exits 1 on the integration branch and 0 on a history that does not descend from the baseline. **That demotion no longer fires on `main`:** re-measured 2026-08-23, `git merge-base --is-ancestor 4102b4e2e HEAD` returns 0, so the message baseline reaches `main` and the gate enforces there. Read bead `hk-commit-msg-gate-advisory-on-main-ap068` as the record of a condition that has cleared on `main` and still applies to a lane cut from before the baseline. What does NOT clear is the tip-only scope above: `--head-only` reads `git rev-parse HEAD` and nothing else, and one push of five commits is one CI run, so four of the five messages are never read. What the gate reads also narrowed on 2026-08-23: the validator is the Go package `internal/commitmsg`, reached as `harmonik commit-msg validate <file>`, and it judges the review trailers alone — no subject shape, no type set, no length ceiling. `docs/foundation/project-level/build-practices.md` §"Git hooks are retired" holds the full statement for both gates.
