...
The "custom hook framework" implementation option enumerated above is the deferred expansion path for non-Claude agent types; each future agent type (pi-code, etc.) gets its own bridge spec mirroring claude-hook-bridge.md's structure.
