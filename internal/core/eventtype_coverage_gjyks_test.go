...
// session_keeper_* families (§8.16 watcher/lifecycle AND §8.20 interior) and the
// §8.21 agent_input_* acceptance events, which are registered + compat-tabled but
// deliberately CARVED OUT of this cross-bus taxonomy cohort per EV-050 (D8;
// §8.21 follows the same precedent). A new session_keeper_* / agent_input_* type
// gets a mustRegister constructor and a mandatory pertypecompat row, but MUST NOT
// be added here.
//
