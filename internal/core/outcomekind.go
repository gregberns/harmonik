...
// OutcomeKind values per execution-model.md §6.1 ENUM OutcomeKind (v0.3.3).
const (
...
// Valid reports whether k is one of the two declared OutcomeKind constants at MVH.
...
func (k OutcomeKind) Valid() bool {
	switch k {
	case OutcomeKindDefault, OutcomeKindReconciliationVerdict:
		return true
	default:
		return false
	}
...
func (k *OutcomeKind) UnmarshalText(text []byte) error {
	...
	if !v.Valid() {
		return fmt.Errorf(
			"outcomekind: unknown value %q; must be one of default, reconciliation_verdict",
			string(text),
		)
	}
	*k = v
