...
// verdict-execution time and compared against the values in Snapshot;
// divergence_reason classifies which store advanced.
//
// # Structural invariants (enforced by Valid)
//
//   - Snapshot passes SnapshotToken.Valid().
...
//
// Note: the field was renamed from SnapshotToken to Snapshot (hk-6x7dw) to
// avoid triggering the EV-036 secret-prefix scanner (HC-031 matches "Token").
type StaleVerdictPayload struct {
	// Snapshot is the SnapshotToken captured at investigator-dispatch time.
