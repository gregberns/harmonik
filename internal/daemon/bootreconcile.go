...
func (bs *bootState) buildReconcileAdapters(ctx context.Context, st *reconcileState) error {
	...
	if cfg.BrPath != "" {
		...
		if brAdapterErr != nil {
			...
			// BI-031b. Non-fatal: bead-reset sweep is best-effort.
			_ = brcli.BrErrReconciliationCategoryWithEmit(ctx, brAdapterErr, "br-new-for-project-sweep", bus)
		} else {
			// BI-024a: explicit br --version handshake before any bead operation.
			// hk-m6243: a version delta is a NOTICE (log + continue); only
			// exec-failure or unparseable output is fatal (exit code 8). Notice
			// (not warning) per operator direction 2026-07-16: a delta is an
			// expected, benign condition, not something wrong. (hk-3pbox, hk-m6243.)
			if versionErr := brAdapter.CheckBrVersion(ctx, release.BeadsVersion); versionErr != nil {
				if errors.Is(versionErr, brcli.ErrBrVersionMismatch) {
					...
					if failedBytes, marshalErr := json.Marshal(failedPayload); marshalErr == nil {
						if emitErr := bus.Emit(ctx, core.EventTypeDaemonStartupFailed, failedBytes); emitErr != nil {
	...
	// Raw queue.json load for the orphan-sweep bead-provenance check (hk-2ty0g):
	// a lightweight read-only load BEFORE the full LoadQueueAtStartup. Non-fatal:
	// a missing/corrupt queue.json yields empty sets and the sweep falls back to
	// intent-log provenance only. (process-lifecycle.md §4.5 PL-006 sixth bullet.)
...
func (bs *bootState) runOrphanSweepAndAdopt(ctx context.Context, daemonStartTime time.Time, st *reconcileState) {
	...
	// Reconcile pre-restart in-flight runs: for any run with run_started but no
	// terminal event, emit run_failed (hk-r73qr). Also source orphans from the
	// live dispatch-tracker (queueDispatched) so a bead crashed-on before any
	// run_started still clears its dispatch-lock (hk-iwu8a); genuinely-live runs
	// (in .harmonik/runs/) are excluded.
	if cfg.JSONLLogPath != "" {
		...
		// hk-hju8n: snapshot the resettable-bead set in two bulk `br list` calls
		// so the reconcile does an O(1) map lookup per bead. On any bulk-list
	...
	// then reconciliation_completed immediately after, so a hung startup
	// reconciliation is detectable (F6/hk-mptxw). Non-fatal. (hk-63oh.21.)
	if startupRunUID, startupUIDErr := uuid.NewV7(); startupUIDErr == nil {
		...
		if startupRecBytes, marshalErr := json.Marshal(startupRecPayload); marshalErr == nil {
			if emitErr := bus.Emit(ctx, core.EventTypeReconciliationStarted, startupRecBytes); emitErr != nil {
		...
		if startupCompBytes, marshalErr := json.Marshal(startupCompPayload); marshalErr == nil {
			if emitErr := bus.Emit(ctx, core.EventTypeReconciliationCompleted, startupCompBytes); emitErr != nil {
