...
func TestCodexNoWorkSuspected_hk368i4(t *testing.T) {
	...
	cases := []struct {
		name     string
		outcome  daemon.ExportedCodexRefsOutcome
		duration time.Duration
		override time.Duration
		want     bool
	}{
		// Every observed no-work run, at its measured duration.
		{"observed no-work 3.27s", daemon.ExportedCodexRefsNoChange, 3270 * time.Millisecond, floor, true},
		{"observed no-work 3.38s", daemon.ExportedCodexRefsNoChange, 3380 * time.Millisecond, floor, true},
		{"observed no-work 4.00s", daemon.ExportedCodexRefsNoChange, 4 * time.Second, floor, true},
		{"observed no-work 5.19s", daemon.ExportedCodexRefsNoChange, 5190 * time.Millisecond, floor, true},
		...
		{"no-change but slow — not flagged", daemon.ExportedCodexRefsNoChange, 40 * time.Second, floor, false},
		{"no-change exactly at floor — not flagged", daemon.ExportedCodexRefsNoChange, daemon.ExportedCodexNoWorkDurationFloorDefault, floor, false},
		...
		{"fast run that committed", daemon.ExportedCodexRefsCommitted, 1 * time.Second, floor, false},
		{"fast run that amended", daemon.ExportedCodexRefsAmended, 1 * time.Second, floor, false},
		{"fast run already carrying the trailer", daemon.ExportedCodexRefsAlreadyPresent, 1 * time.Second, floor, false},
		...
		{"zero duration — unmeasured, never flags", daemon.ExportedCodexRefsNoChange, 0, floor, false},
		{"negative duration — unmeasured, never flags", daemon.ExportedCodexRefsNoChange, -1 * time.Second, floor, false},

		// The floor is tunable, and tuning moves the boundary in both directions.
		{"override raises the floor above a real run", daemon.ExportedCodexRefsNoChange, 20 * time.Second, 30 * time.Second, true},
		{"override lowers the floor below a no-work run", daemon.ExportedCodexRefsNoChange, 4 * time.Second, 2 * time.Second, false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := daemon.ExportedCodexNoWorkSuspected(tc.outcome, tc.duration, tc.override)
			if got != tc.want {
...
func TestCodexNoWorkFloor_hk368i4(t *testing.T) {
	...
	cases := []struct {
		...
		{"zero falls back to default", 0, daemon.ExportedCodexNoWorkDurationFloorDefault},
		{"negative falls back to default", -5 * time.Second, daemon.ExportedCodexNoWorkDurationFloorDefault},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := daemon.ExportedCodexNoWorkFloor(tc.override); got != tc.want {
...
func TestCodexNoWorkFloorDefault_SitsInTheMeasuredGap_hk368i4(t *testing.T) {
	...
	floor := daemon.ExportedCodexNoWorkDurationFloorDefault
