...
// commitbudget_heartbeat_cw0fa_test.go — the commit budget advances on evidence
// of WORK, never on evidence that a process EXISTS (hk-cw0fa).
//
...
//
// Helper prefix: cw0fa* (implementer-protocol.md §Helper-prefix discipline).
...
// cw0faBus records the last payload emitted per event type. It satisfies
// handlercontract.EventEmitter and nothing else.
type cw0faBus struct {
	mu       sync.Mutex
	payloads map[core.EventType][]byte
}

func (b *cw0faBus) Emit(_ context.Context, t core.EventType, payload []byte) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.payloads == nil {
...
func (b *cw0faBus) EmitWithRunID(ctx context.Context, _ core.RunID, t core.EventType, p []byte) error {
	return b.Emit(ctx, t, p)
}

func (b *cw0faBus) last(t core.EventType) []byte {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func TestCW0faHeartbeatDoesNotExtendCommitBudget(t *testing.T) {
	...
	const cw0faBeatsPerBudget = 6
	beat := budget / cw0faBeatsPerBudget
	if beat >= heartbeatStalenessThreshold {
		t.Fatalf("beat interval %v (budget %v / %d) is not shorter than the staleness threshold %v; the staleness kill would fire first and this test would prove nothing",
			beat, budget, cw0faBeatsPerBudget, heartbeatStalenessThreshold)
	}
	...
	killAt := beat * (cw0faBeatsPerBudget + 1)
	if killAt >= ceiling {
	...
	for i := 0; i <= cw0faBeatsPerBudget; i++ {
		cw0faSendBeat(t, events, i)
		clk.Advance(beat)
	}
...
// cw0faSendBeat delivers one agent_heartbeat and fails the test if the loop does
...
func cw0faSendBeat(t *testing.T, events chan<- core.EventEnvelope, n int) {
	t.Helper()
	select {
