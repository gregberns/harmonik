...
func TestGoCacheQuiescent_FreshCacheIsNotReapable(t *testing.T) {
	...
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "trim.txt"), []byte("x"), 0o600); err != nil {
...
func TestGoCacheQuiescent_ShardChurnReadsAsInUse(t *testing.T) {
	...
	if err := os.Mkdir(filepath.Join(dir, "cd"), 0o750); err != nil {
...
func TestReapAgentGoCaches_HonoursQuiescence(t *testing.T) {
	...
	for _, d := range []string{fresh, idle} {
		if err := os.MkdirAll(filepath.Join(d, "ab"), 0o750); err != nil {
...
// TestDiskCheck_HealthyDisk_LeavesAgentCachesAlone guards the property the
// hk-137y6 and hk-gjbpp fixes only have TOGETHER, which is why it is worth a
// test of its own rather than trusting the two halves separately.
//
// hk-137y6 gave every agent its own bounded cache and taught the reclaim to
// reap those caches; hk-gjbpp removed the healthy-disk cadence that used to
// reap on a timer. If a later change reintroduces a cadence — or moves the
// per-agent reclaim out of the below-watermark branch — the daemon goes back
// to deleting other agents' working sets while they build, and the failure is
// silent in both directions (phantom test failures, and green runs that never
// compiled). Neither existing test catches that on its own:
// TestDiskCheck_ReactiveOnly_NoProactiveCadence only watches `go clean -cache`
// through the goCacheCleanFunc seam, which the per-agent reclaim does not go
// through, and TestReapAgentGoCaches_HonoursQuiescence calls the reclaim
// directly and so says nothing about WHEN it is called.
//
// The cache is deliberately back-dated past the quiescence window: it is
// eligible for reclaim on every axis except disk pressure, so a survival here
// can only mean the healthy-disk path never reached the reclaim at all.
func TestDiskCheck_HealthyDisk_LeavesAgentCachesAlone(t *testing.T) {
	t.Parallel()

	project := t.TempDir()
	idle := GoCacheDirFor(project, "some-other-crew")
	shard := filepath.Join(idle, "ab")
	if err := os.MkdirAll(shard, 0o750); err != nil {
		t.Fatalf("seed %s: %v", idle, err)
	}
	old := time.Now().Add(-goCacheQuiescenceWindow - time.Hour)
	for _, p := range []string{shard, idle} {
		if err := os.Chtimes(p, old, old); err != nil {
			t.Fatalf("back-date %s: %v", p, err)
		}
	}
	if !goCacheQuiescent(idle, time.Now()) {
		t.Fatal("precondition: the back-dated cache must read as quiescent, or this test proves nothing")
	}

	deps := &workLoopDeps{
		projectDir:                project,
		diskCheckIntervalOverride: time.Nanosecond,
		// 100 GiB free: healthy by any watermark.
		diskFreeBytesFunc: func(string) (uint64, error) { return 100 * 1024 * 1024 * 1024, nil },
		goCacheCleanFunc:  func() error { t.Error("go clean -cache ran on a healthy disk"); return nil },
	}
	maint := &loopMaintenanceState{}
	// Several ticks: a cadence-based reap would be time-gated, so one tick could
	// pass by luck. Any reintroduced timer with a test override would fire here.
	for range 5 {
		runPeriodicDiskCheck(t.Context(), deps, maint)
	}

	if _, err := os.Stat(shard); err != nil {
		t.Errorf("another agent's cache shard was reaped on a HEALTHY disk (%v) — the per-agent reclaim "+
			"escaped the below-watermark branch, which is the mid-build cache wipe hk-gjbpp removed", err)
	}
	if maint.diskLow {
		t.Error("diskLow set on a healthy-disk probe")
	}
}
