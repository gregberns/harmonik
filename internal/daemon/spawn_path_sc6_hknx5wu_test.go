...
// Four sub-tests:
//
//  1. pre-seal-without-notify-stream: count = 14
...
//     - 2 from QuiesceArbiter              (epic_completed, agent_message; M1 hk-jeby)
//
//  2. pre-seal-with-notify-stream: count = 18
//     - 14 base subscriptions (same as above)
...
func TestSC6_CompositionRootWiringScan_AllPreSealSubscriptionsPresent(t *testing.T) {
	...
	t.Run("pre-seal-without-notify-stream", func(t *testing.T) {
		...
		//   2 QuiesceArbiter              (epic_completed + agent_message; M1 hk-jeby)
		// = 14.
		const wantBase = 14
	...
	t.Run("pre-seal-with-notify-stream", func(t *testing.T) {
		...
		// 14 base + 4 from NotifyStreamConsumer (run_started, workspace_merge_status,
		// run_completed, run_failed) = 18. Base count is the wantBase set above
