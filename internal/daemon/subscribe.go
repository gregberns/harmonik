...
func (h *SubscribeHub) HandleSubscribe(ctx context.Context, conn net.Conn, req SubscribeRequest) {
	...
	// lastReplayedUID is the high-water mark of event_id bytes sent during
	// JSONL replay. Live-channel events with event_id ≤ lastReplayedUID are
	// skipped as duplicates. Zero value means no replay was performed.
	var lastReplayedUID [16]byte

	// Replay historical events when since_event_id is set and we have a log
	// path configured.
	if req.SinceEventID != "" && h.cfg.EventsJSONLPath != "" {
		if sinceUUID, parseErr := uuid.Parse(req.SinceEventID); parseErr == nil {
			...
			for evt := range eventbus.ScanAfter(h.cfg.EventsJSONLPath, sinceID) {
				// Respect client disconnect during replay.
				select {
				case <-streamCtx.Done():
					return
				default:
				}
				// Apply type filter during replay.
				if !wildcard {
