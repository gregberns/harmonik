...
// TestCycler_WaitForNewSID_SkipsUUIDv7 verifies that waitForNewSessionID skips
// UUIDv7 session IDs (daemon implementers) and only returns a UUIDv4 SID.
// Without this guard the cycler binds .managed to the daemon implementer's
// session, making the watcher treat the captain's UUIDv4 session as foreign.
// (Refs: hk-lap — clear->resume latch race fix)
func TestCycler_WaitForNewSID_SkipsUUIDv7(t *testing.T) {
	t.Parallel()

	const (
		agent   = "uuid7-skip-cycler-agent"
		cycleID = "cyc-uuid7-001"
		prevSID = "f7e7210d-0000-4000-8000-000000000001" // captain's old UUIDv4
		v7SID   = "019ebb07-0000-7000-8000-000000000001" // daemon implementer's UUIDv7
		v4SID   = "a0b1c2d3-0000-4abc-8000-000000000002" // captain's new UUIDv4
	)
	...
	// Gauge sequence: prevSID for first 2 calls → v7SID for calls 3-5 (skipped)
	// → v4SID for subsequent calls (accepted).
	var callCount int
	readGaugeFn := func(_, _ string) (*keeper.CtxFile, time.Time, error) {
		callCount++
		switch {
		case callCount <= 2:
			return &keeper.CtxFile{Pct: 95.0, SessionID: prevSID}, time.Now(), nil
		case callCount <= 5:
			return &keeper.CtxFile{Pct: 95.0, SessionID: v7SID}, time.Now(), nil
		default:
			return &keeper.CtxFile{Pct: 95.0, SessionID: v4SID}, time.Now(), nil
		}
	}
	...
	cfg := keeper.CyclerConfig{
		...
		WarnPct:        80.0,
		HandoffTimeout: 500 * time.Millisecond,
		ClearSettle:    500 * time.Millisecond,
		PollInterval:   10 * time.Millisecond,
		CycleIDGen:     func() string { return cycleID },
		IsManagedFn:    func(_, _ string) bool { return true },
		HandoffFilePath: func(_, a string) string {
	...
	// SetManagedSessionFn must receive the UUIDv4 SID, not the UUIDv7.
	if gotSessionID != v4SID {
		t.Errorf("SetManagedSessionFn session_id = %q; want %q (UUIDv4, not UUIDv7 %q)",
			gotSessionID, v4SID, v7SID)
	}
}
