...
func TestWatcher_WarnOnly_EmitsWarnButNoRespawn(t *testing.T) {
	...
	// Write a fresh gauge above the warn threshold (pct ≥ 85 > WarnPct 80).
	...
	cfg := keeper.WatcherConfig{
		...
		// eligible. With WarnOnly=true it must never fire.
		RespawnCmd:   "echo respawn-would-run",
	...
	// Respawn must NOT have fired.
	if rec.count() != 0 {
		t.Errorf("want 0 respawn calls in warn-only mode; got %d", rec.count())
	}

	// No respawn_attempted events should exist.
