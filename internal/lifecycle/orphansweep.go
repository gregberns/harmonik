...
func RunOrphanSweep(
	...
	// (a) Tmux sessions.
	tmuxKilled, err := SweepOrphanTmuxSessions(ctx, projectHash, cfg.TmuxLister, cfg.TmuxKiller, cfg.Logger)
	if err != nil {
	...
	handlersKilled, err := SweepOrphanHandlers(ctx, projectHash, cfg.HandlerLister, cfg.Logger)
	if err != nil {
	...
	brSurvived, err := SweepOrphanBr(ctx, cfg.BrLister, cfg.Logger)
	if err != nil {
	...
	staleIntents, err := EnumerateStaleIntents(projectDir, daemonStartTime)
	if err != nil {
	...
	reconRemoved, err := SweepStaleReconciliationLocks(projectDir, cfg.Logger)
	if err != nil {
	...
	if len(errs) > 0 {
		return result, fmt.Errorf("lifecycle: RunOrphanSweep: %s", strings.Join(errs, "; "))
	}
	return result, nil
}
