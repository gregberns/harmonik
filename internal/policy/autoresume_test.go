...
func TestBackoffDuration(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			params: AutoResumeParams{Base: base, Attempts: 0, MaxBackoff: 5 * time.Millisecond},
			want:   base, // Attempts<=0 short-circuits before the cap
		},
		{
			name:   "negative attempts returns base",
			params: AutoResumeParams{Base: base, Attempts: -3, MaxBackoff: time.Hour},
			want:   base,
		},
		{
			name:   "one attempt doubles",
			params: AutoResumeParams{Base: base, Attempts: 1, MaxBackoff: time.Hour},
		...
		},
		{
			...
			params: AutoResumeParams{Base: base, Attempts: 10, MaxBackoff: 50 * time.Millisecond},
			want:   50 * time.Millisecond,
		},
		{
			name:   "zero MaxBackoff applies default cap",
			params: AutoResumeParams{Base: 20 * time.Minute, Attempts: 8, MaxBackoff: 0},
			want:   DefaultAutoResumeMaxBackoff, // 30m
		},
		{
			name:   "negative MaxBackoff applies default cap",
			params: AutoResumeParams{Base: 20 * time.Minute, Attempts: 8, MaxBackoff: -1},
			want:   DefaultAutoResumeMaxBackoff,
		},
	}
...
func TestBackoffDuration_DefaultCapConstant(t *testing.T) {
	t.Parallel()
	if DefaultAutoResumeMaxBackoff != 30*time.Minute {
		t.Errorf("DefaultAutoResumeMaxBackoff=%s, want 30m", DefaultAutoResumeMaxBackoff)
	}
}
