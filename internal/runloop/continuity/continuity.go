...
// The declared location kinds. A local workspace carries no worker name; a
// remote one requires it.
const (
...
// current state required to construct the durable checkpoint transition that
// persists the mapping into Run.context, and its commit trailers. That
// transition must satisfy the durability rule of execution-model.md §4.5
// EM-023a; the persistence-ordering obligation is claude-hook-bridge.md §4.6
// CHB-023.
//
...
// when needed, of the durable transition (EM-023a) that persists the mapping.
//
...
// a conflicting committed identity, or persist a new mapping through a durable
// transition per EM-023a, before the identity is used (CHB-023). A
// working-tree-only mapping is not durable and must not be treated as reuse.
...
// The declared version-selected dispositions of a successful transaction.
const (
...
// The declared failure stages, in the order Execute can reach them.
const (
...
// validateIdentity enforces the policy-specific identity shape: a minted
// identity must have been known before launch and must match what was observed;
// a captured identity is only known after launch and must carry no pre-launch
// expectation.
