...
// a validated ScenarioFile. It implements the parse_scenario_file operation
// declared in specs/scenario-harness.md §4.a (SH-ENV-001).
//
// Enforcement per spec:
//
//   - The path MUST have a ".yaml" extension (lower-case, byte-exact); ".yml"
//     and other variants MUST be rejected as scenario-load-failure (SH-002).
//   - The file MUST be ≤ 1 MiB; over-size files MUST be rejected (SH-003).
//   - The file MUST NOT begin with a UTF-8 BOM (SH-003).
//   - Forbidden YAML tags (!!python/object, !eval, !!binary) MUST be rejected
//     at parse time per SH-003 and SH-INV-005.
//   - The YAML document MUST be parsed in strict / KnownFields mode so that
//     unknown struct fields cause a parse failure (SH-INV-005).
//   - The parsed-node count MUST NOT exceed 100 000 (SH-003).
//   - The decoded ScenarioFile MUST satisfy ScenarioFile.Valid() (SH-004).
//
// On any violation the returned error describes the failure class. Callers
// that map errors to ScenarioResult should classify any returned error as
// FailureClassScenarioLoadFailure per §8.
//
...
func ParseScenarioFile(path string) (ScenarioFile, error) {
	...
	// Read the file, enforcing the 1 MiB size ceiling (SH-003).
	//nolint:gosec // G304: path comes from the harness suite-load caller, not user input
	f, err := os.Open(path)
	if err != nil {
	...
	for _, tag := range scenarioYAMLForbiddenTags {
		if strings.Contains(rawStr, tag) {
			return ScenarioFile{}, fmt.Errorf("scenario-load-failure: file %q contains forbidden YAML tag %q (SH-003, SH-INV-005)", path, tag)
		}
	}
	...
	// countYAMLNodes follows alias nodes so that YAML alias/anchor bombs
	// (e.g. billion-laughs patterns) are counted by their expanded size, not
	// by the number of alias references. SH-INV-005 explicitly lists
	// "anchors that reference unbound aliases" as a forbidden construct;
	// the node-count ceiling here is the backstop that catches alias bombs
	// whose anchors are valid but whose expansion exceeds safe limits.
	// The counter terminates as soon as the limit is reached to bound cost.
	nodeCount := countYAMLNodes(&doc, scenarioFileNodeLimit+1)
	if nodeCount > scenarioFileNodeLimit {
		return ScenarioFile{}, fmt.Errorf("scenario-load-failure: file %q exceeds 100 000-node ceiling (%d nodes) (SH-003, SH-INV-005)", path, nodeCount)
	}
...
// countYAMLNodes returns the expanded node count for a yaml.Node tree, up to
// limit nodes. When the running total reaches or exceeds limit the function
// returns immediately so the caller can detect the ceiling without paying the
// full traversal cost for pathological inputs.
//
// Alias nodes are followed through to their anchor target so that YAML
// alias/anchor bombs (billion-laughs patterns) contribute their expanded size
// to the count, not just 1 per alias reference. This implements the
// SH-INV-005 / SH-003 backstop for alias-based denial-of-service inputs.
//
// The nil or empty document yields 0.
func countYAMLNodes(n *yaml.Node, limit int) int {
	if n == nil {
		return 0
	}
	// Follow alias nodes so that each alias contributes its fully-expanded
	// size. This is what makes the 100 000-node ceiling an effective guard
	// against alias/anchor bombs (SH-INV-005).
	if n.Kind == yaml.AliasNode && n.Alias != nil {
		return countYAMLNodes(n.Alias, limit)
	}
	count := 1
	if count >= limit {
		return count
	...
	for _, child := range n.Content {
		count += countYAMLNodes(child, limit-count)
		if count >= limit {
			return count
		}
	}
