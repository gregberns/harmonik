...
|---|---|---|---|---|---|
| D1 | Is `failure_class` a permitted LHS on edge `condition` expressions? | C1 | C2, C3, C5 | G2, G5 | yes (sugar over Outcome meta) |
| D2 | `failure_class` placement on Outcome (top-level field vs. notes vs. new `kind`) | C3 | C1 (LHS whitelist), C2 (cascade input) | G5 | Option A — top-level field on FAIL |
| D3 | Q1 — control-point as node-type vs. policy primitive vs. discriminant | C1 + C4 (symmetric) | C2 (dispatch table), C3 (control-point payload), C5 (bead-process) | G1 | Framing A (drop from catalog) or C (single type w/ discriminant); reject B |
| D4 | Edge-condition LHS whitelist enumeration | C1 | C2 (validator), C3 (Outcome surface), C5 (review-loop verdict) | G2, Q6 | Outcome fields + whitelisted context keys |
| D5 | Edge-condition dialect (CP §6.4 predicate language vs. restricted `key=value` mini-language) | C1 | C2 (evaluator), C4 (guard-vs-edge boundary) | G2 | (open — research did not lean) |
| D6 | Verdict surfacing for review-loop (status-mapping vs. context_updates key vs. first-class field) | C3 + C1 | C5 (review-loop.dot), C2 (cascade) | G1 partial, G4 partial | (open) |
