...
- **claude_session_id** — the Claude Code session identifier captured from `claude -p ... --output-format json` (the JSON `session_id` field) and consumed by `claude --resume <id>` to continue the same Claude session across subprocess launches. Stored in `Run.context.claude_session_id` under `review-loop` mode per §4.3.EM-015d. Distinct from harmonik's internal `session_id` event field per [event-model.md §8.3], which is a UUIDv7 generated by the handler per [handler-contract.md §4.1] and is opaque to non-handler consumers. The two identifiers MUST NOT be conflated: `claude_session_id` is owned and minted by Claude Code; `session_id` is owned and minted by the handler watcher.
...
- **Same Claude session resumed across implementer iterations.** The daemon MUST capture the implementer's Claude Code session identifier from the first iteration's launch (via `claude -p ... --output-format json` per [handler-contract.md §4.1]) and persist it into `Run.context.claude_session_id`. On every subsequent implementer iteration (every `REQUEST_CHANGES` cycle), the daemon MUST resume the same Claude session via `claude --resume <claude_session_id>`. The implementer subprocess is fresh per iteration; the Claude session is logically continuous.
...
- **Per-iteration events.** Each implementer resume MUST emit `implementer_resumed` per [event-model.md §8.1]; each reviewer launch MUST emit `reviewer_launched`; the verdict-file read MUST emit `reviewer_verdict` carrying the agent-reviewer schema v1 fields verbatim plus `run_id`, `session_id`, and `iteration_count`. On no-progress detection per §4.3.EM-015e, the daemon MUST emit `no_progress_detected`. On cap-hit per §4.3.EM-015e, the daemon MUST emit `iteration_cap_hit`. On cycle termination (by any path), the daemon MUST emit exactly one `review_loop_cycle_complete` per [event-model.md §8.1] carrying the `completion_reason`.
...
- **Cap-hit (REQUEST_CHANGES at iteration 3).** On a reviewer verdict of `REQUEST_CHANGES` AND `iteration_count = 3`, the daemon MUST emit `iteration_cap_hit` per [event-model.md §8.1] and MUST terminate the run via the `needs-attention` close path. Bead-side, the run's terminal-transition write per [beads-integration.md §4.4 BI-010] MUST apply the `needs-attention` label per [operator-nfr.md §4.3].
...
- Run lifecycle — `run_started` (on dispatch against a bead or standalone input), `run_completed` (on success terminal state), `run_failed` (on failure terminal state; payload includes the failure class per §8). All three carry the resolved `workflow_mode` (per §4.3.EM-012a) on their payloads per [event-model.md §8.1].
...
**Core MVH.** An implementation conforming to Core MVH MUST pass every requirement in EM-001 through EM-046 (including sub-requirements EM-012a, EM-015a, EM-015b, EM-015c, EM-015d, EM-015e, EM-017a, EM-018a, EM-020a, EM-023a, EM-024a, EM-025a, EM-031a, EM-034a, EM-034b, EM-034c, EM-036a, EM-041a, EM-042a, EM-043a, EM-046a, EM-046b) and invariants EM-INV-001, EM-INV-004, and EM-INV-005 (the three invariants surviving the §5 selection test; EM-INV-002, EM-INV-003, EM-INV-006 are retired). For `workflow_mode = single` (the default per §4.3.EM-012a built-in fallback), the cascade (§4.10), checkpoint cadence (§4.5), and sub-workflow composition (§4.8) apply unchanged. For `workflow_mode = review-loop`, the hardcoded two-node cycle of §4.3.EM-015d MUST be observed; the cap and early-exit rules of §4.3.EM-015e MUST be enforced; the six review-loop events of §6.5 MUST be emitted. `workflow_mode = dot` is reserved for post-MVH and is NOT required for Core MVH conformance.

**Post-MVH extensions.** Failure-commit emission (deferred per §4.5.EM-025) and `recoverable-non-idempotent` node-type defaults (§4.2.EM-010) are additive extensions to Core MVH; neither is required to claim Core MVH conformance. The `workflow_mode = dot` general workflow-graph walker is a post-MVH extension; the EM-012a precedence walk's tolerance for `workflow:dot` labels is normative at MVH (label resolution must not crash), but no MVH dispatcher is obligated to drive a `dot` run.
