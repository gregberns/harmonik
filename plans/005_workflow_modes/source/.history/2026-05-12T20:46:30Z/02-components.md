...
- Five new event types in §8.1:
  ...
  - `reviewer_verdict` (class F — upgraded in change-design pass; gates terminal routing). Payload conforms to the existing `agent-reviewer` JSON schema v1 (`schema_version`, `verdict ∈ {APPROVE, REQUEST_CHANGES, BLOCK}`, `flags[]`, `notes`). Also includes `run_id`, `session_id`, `iteration_count`.
