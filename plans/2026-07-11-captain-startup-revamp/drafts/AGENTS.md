<!-- DRAFT (2026-07-11 startup-revamp, Stage 2): proposed replacement for AGENTS.md. Do not deploy standalone — lands with companion edits to AGENT_INDEX.md, STATUS.md, HANDOFF.md, and cmd/harmonik/assets/templates/AGENTS.template.md. -->
...
Standing behavioral rules: the **`orchestrator-rules` skill** (`.claude/skills/orchestrator-rules/SKILL.md`) is canonical. On conflict: **orchestrator-rules skill > AGENTS.md prose > per-domain skills own their detail**. Operational state lives in `.harmonik/context/` (captain tiers — surfaced to the captain through `harmonik agent brief`, never read as a boot chain) and per-agent handoffs — **never in this file**. AGENTS.md is a ROUTER: it points you at the right contract; it does not restate one. Skill tree: **`.claude/skills/` is canonical** (project-local under this repo — do NOT resolve skill names against the global `~/.claude/skills/`); `.harmonik/agents/_skills/` is a generated mirror — never edit the mirror.

## Booting

**Booting as a manifest agent (admiral/captain/crew/watch/assessor)?** Run `harmonik agent brief` (`--wake fresh|keeper-restart|trigger:<id>`). Its output IS your complete boot context — identity → wake reason → operating+skills → triggers → handoff. Do not read STARTUP.md or any doc chain.

**Daemon-dispatched implementer/reviewer beads:** your worktree's `agent-task.md` (+ seed / `review-target.md`) IS your complete boot context — do not run `agent brief`, do not follow this map.

**Solo / implementer-orchestrator (a plain main session — not a manifest agent, not a bead)?** Boot = `/session-resume` (reads `HANDOFF.md`) + the **`orchestrator-rules` skill** (standing rules) + the **harmonik-dispatch** skill. No doc-chain ritual — AGENT_INDEX.md / STATUS.md are on-demand references, not boot reads.

**Exploring the knowledge base (not booting)?** [AGENT_INDEX.md](AGENT_INDEX.md) is the map — load on demand.
...
br update <id> --status=in_progress   # SOLO OPERATOR ONLY — see Workflow Pattern
br close <id> --reason="Completed"    # SOLO OPERATOR ONLY — daemon owns terminal transitions
br close <id1> <id2>  # Close multiple issues at once (solo operator only)
...
2. **Claim** — **SOLO OPERATOR ONLY**: `br update <id> --status=in_progress`. Daemon-dispatched agents (implementer/reviewer beads, queue workers) NEVER issue status transitions — the daemon owns them.
3. **Work**: Implement the task
4. **Complete** — **SOLO OPERATOR ONLY**: `br close <id>`. Daemon-dispatched agents NEVER close beads; the daemon issues terminal transitions on merge.
