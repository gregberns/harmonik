<!-- DRAFT — proposed replacement for /AGENT_INDEX.md (repo root)
     (startup-doc revamp companion, per 02-cutover-and-open-questions.md §2.2[B] + Step 0.3 +
     00-SYNTHESIS.md §6 "AGENT_INDEX.md KB map + skills directory | KEEP as on-demand map; CUT
     L7 boot-order banner; fix dup Reviews section"). Lands WITH the amended AGENTS.md at
     Step 4.1. Two fixes only — this file is otherwise KEPT as the on-demand knowledge-base map:
     (1) the old "Reading order on boot" banner assumed the AGENT_INDEX → STATUS → captain-lanes
     → HANDOFF ritual manifest agents no longer follow (`harmonik agent brief` is the one boot
     command; beads boot from `agent-task.md`) — replaced with an on-demand-map framing plus a
     boot pointer; (2) the file carried two separate "### Reviews" sections (stale duplication
     from an earlier edit) — merged into one. This DRAFT comment is removed on deploy. -->

# Harmonik -- Agent Discovery Index

> **On-demand map, not a boot doc.** This is the master index for the harmonik knowledge base
> product/`docs/` — every document in the project is reachable from this file within two hops.
> Manifest agents (captain, crew, admiral, watch) do NOT read this at boot: their boot context is
> `harmonik agent brief`, whose output IS the complete boot context. Beads boot from the
> daemon-injected `agent-task.md`, not this map. Come here when you need to FIND a doc — problem,
> goal, concept, component, subsystem, spec, or protocol — not as a startup ritual.
>
...
### Reviews
- [docs/reviews/README.md](docs/reviews/README.md) -- *ARCHIVED* (Apr 2026 foundation/spec persona reviews)
- [docs/reviews/2026-04-23-foundation-phase3/](docs/reviews/2026-04-23-foundation-phase3/) -- six-persona Phase 3 review of foundation (see also [README.md](docs/reviews/2026-04-23-foundation-phase3/README.md) for the synthesis + round-2 amendment plan)
...
- [plans/2026-07-11-captain-startup-revamp/](plans/2026-07-11-captain-startup-revamp/) -- Manifest-boot cutover: `harmonik agent brief` replaces the STARTUP.md + reading-order ritual for captain/crew/admiral/watch
