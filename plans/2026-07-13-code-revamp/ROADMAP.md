...
| **M5 — daemon-decompose** | god-package breakup: 3 real cuts (`hook`→`policy`→`orchestrator`) + 2 debt retirements (socket dispatch, boot wiring); NOT "≥8 packages" | rides plan docs (no-beads) | `M5-PROBLEM-SPACE.md` | M3 (mergeq) + M4 (agentrunner files settled) | **BUILD-READY** — slice 1 `internal/hook` GO (COORD c042) |
| **M6 — controlled-testing harness** | make controlled-E2E a REAL gate (flip CI, fix `check-full` gap, kill review stub); **docker + subprocess E2E**; **twin↔real parity — Claude is the priority (replay + property/fuzz the twin)**, codex fresh-capture-diff, pi twin-from-scratch; revive the **live core-loop check** (`core-loop-proof`, PR #20, formerly "acceptance oracle") in an isolated/reproducible subprocess+docker env; **wire the assessor as an adversarial (critic+QA+architect) release-readiness AGENT that delegates verification to subagents, JUDGES, then discusses signoff with the admiral — who makes the FINAL call (NOT a bead-count)**. Gate scales with change risk (trivial skips; critical/large = rigorous). Split CI/local, made explicit. **GATES the M4 real-box proof — no live-remote until M6 lands.** | rides plan docs (no-beads) | `M6-PROBLEM-SPACE.md` | M4 code-complete | **PROBLEM-SPACE drafted + reviewed + operator-aligned** (COORD c046–c048, 2026-07-16); awaiting operator go |
| **DOGFOOD** | rebuilt core passes the **live core-loop check** offline → daemon back on (built by **M6 WS4**) | — | — | M2 + M3 (+ M4 for remote) + **M6** | gate |
...
| **Better pi interaction seam** — pi is open/modifiable; instead of driving it via tmux keystrokes, **fork pi or write a plugin for a codex-style structured handler** (more scalable + testable) | operator direction 2026-07-16 | **future investigation — placeholder only**, NOT part of M6; scope later (relates to M6 WS3 pi-twin + M4 pi remote path) |
