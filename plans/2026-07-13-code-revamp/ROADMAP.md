...
| **M6 — controlled-testing harness** | make controlled-E2E a REAL gate (flip CI, fix `check-full` gap, kill review stub); **docker + subprocess E2E**; **twin↔real parity — Claude is the priority (replay + property/fuzz the twin)**, codex fresh-capture-diff, pi twin-from-scratch; revive the acceptance oracle (`core-loop-proof`, PR #20); **wire the assessor as a release-readiness AGENT that JUDGES (reconciles beads↔commits↔reviews), NOT a bead-count**. Split CI/local, made explicit. **GATES the M4 real-box proof — no live-remote until M6 lands.** | rides plan docs (no-beads) | `M6-PROBLEM-SPACE.md` | M4 code-complete | **PROBLEM-SPACE drafted + independently reviewed** (COORD c043, 2026-07-16); awaiting operator go |
| **DOGFOOD** | rebuilt core passes acceptance oracle offline → daemon back on (oracle built by **M6 WS4**) | — | — | M2 + M3 (+ M4 for remote) + **M6** | gate |
