...
5. ✅ **Landed 2026-07-31, both halves.** They were built in parallel by two agents and merged
   together, so each half's own note below was written without knowledge of the other.

   **The refusal half.** The second copy of the report — a closure in the remote block of
   `beadRunOne` that wrote its own stderr line, `worker_tunnel_failed` event and reopen — is gone.
   The three gates (port allocation, the socket-path check, the readiness gate) now build a
   `runPlanRefusal` through one `tunnelRefusal` builder and report it through `refuseRunPlan`,
   which is the reporter the plan's five decisions already used. The two copies agreed on wording,
   so nothing about the report moved. Guarded by
   ...
   **The nested-scope half, for the two resources a launch can hold.** The per-launch set is on a
   ...
   **The substrate spawn slot stays where it is, and that is a decision, not an omission.** Four
   ...
   With the refusal half above, step 6 has nothing left open.
