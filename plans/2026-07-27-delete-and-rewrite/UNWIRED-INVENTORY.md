...
- **`core.Subscription.Since` and `OffsetCheckpointEventID`** — read by `busImpl.Seal`, whose switch
  ...
  all 20 production subscription registrations.** *(Count them by registration, not by composite
  literal — `internal/daemon/notifystream.go` registers four consumers from one
  `[]core.Subscription{}` slice header, which is why a naive grep says 17.)* Every consumer takes
...
> reproduces it from the repo alone: **307 orphans of 1,174 requirement IDs, 26.1%**. The method
> reproduces `SPEC-TRIAGE.md`'s per-spec table exactly on 12 of 27 rows, including all six files
> that hold 62% of the orphans, so that report's 25.2% was sound and the drift is tree movement.
> The 27.5% figure is the outlier and should be dropped. See CHARTER.md.
>
...
> assigned in none of the 20 production subscription registrations, so the EV-014d startup replay
