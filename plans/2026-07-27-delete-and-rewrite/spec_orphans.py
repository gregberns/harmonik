...
WHAT IT COUNTS
  ...
  lowercase suffix, for example EM-031a or SS-002fold. The PREFIX must be
  registered in specs/_registry.yaml. An ID belongs to the spec that owns its
  prefix, not to the file the occurrence sits in.
...
HOW IT CLASSIFIES
  ...
  An ID is CITED by a file when the ID occurs in that file AS A WHOLE TOKEN.
  The Go side uses the SAME word-bounded pattern as the spec side. This is
  load-bearing and it was wrong once: a bare substring test made OQ-EM-006 in
  a Go file mark EM-006 as implemented, and PL-021a mark PL-021, which broke
  this docstring's own promise to exclude open questions. 38 open-question and
  kerf-task IDs occur in Go files, so the effect was not small.
...
WHAT THIS NUMBER IS NOT
  It measures CITATION, not implementation. Three limits follow, and a reader
  who wants a coverage figure must not use this one without them.
  1. A requirement ID can never be a Go identifier, because it contains a
     hyphen. Every citation is therefore in a comment or a string literal. A
     cited rule is one a human wrote the number next to, which is weaker than
     a tested rule and much weaker than an enforced one. Some citations sit
     inside a TODO or an explicit "not implemented" note and still count.
  2. A RETIRED rule usually counts as an orphan, because nothing cites a rule
     that no longer applies. Retiring a rule therefore tends to RAISE this
     rate, and the figure is only comparable between two commits if you say
     which. "Usually" is doing real work: QM-058 is retired with its number
     burned, yet two scenario tests cite it as a whole token, so it lands in
     CITED-ONLY-IN-TESTS rather than in the orphan bucket.
  3. Some rules are implemented outside Go and read as orphans for that
     reason: agent skills under cmd/harmonik/assets/skills/, lint rules in
     .golangci.yml, and prompt files. These are real implementations that this
     method cannot see.
...
# The suffix is [a-z]* and not [a-z]?, because SS-002fold and SS-014slot are
# real numbered headings that a single-letter suffix drops from the universe
# entirely while still letting a substring test wire bare SS-002.
...
def cite_counts(paths, ids, pattern):
    """Return {id: number of files in paths that name it as a whole token}.

    The pattern is the one used on the spec side. Matching both sides the same
    way is the point: a substring test here reports an open question or a
    lettered sibling as a citation of the base rule.
    """
    counts = defaultdict(int)
    wanted = set(ids)
    for path in paths:
        text = read(path)
        for rid in {m.group(0) for m in pattern.finditer(text)} & wanted:
            counts[rid] += 1
    return counts
...
def main():
    ...
    prod_hits = cite_counts(prod, ids, pattern)
    test_hits = cite_counts(test, ids, pattern)
