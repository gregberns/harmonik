...
# Window names inside the captain session. The captain claude runs in the
# `agent` window; the keeper runs in a sibling `keeper` window of the SAME
# session (window-nesting, hk-z036). The shell can't import the Go constants.
# MUST match tmux.WindowAgent/WindowKeeper
WIN_AGENT="agent"
WIN_KEEPER="keeper"

# 1) Launch the captain in its own tmux session with the MINTED --session-id,
#    in a window named `agent`. Interactive, remote-controllable (operator can
#    watch at claude.ai/code).
...
tmux new-session -d -s "$CAP_TMUX" -n "$WIN_AGENT" -e "HARMONIK_AGENT=$CAP_NAME" \
...
CAPTAIN_PID="$(tmux display-message -p -t "${CAP_TMUX}:${WIN_AGENT}" "#{pane_pid}")"
...
#    via `sh -c`. It respawns ONLY the captain `agent` WINDOW (not the whole
#    session, hk-z036 invariant I1), resuming the SAME session-id (--resume "$SID",
#    NOT a fresh --session-id that would fork a new conversation), and refreshes
#    captain.pid so the daemon orphan sweep keeps skipping the freshly-relaunched
#    session. The sibling `keeper` window — and the keeper process running there —
#    survives untouched, so it re-binds to the freshly respawned agent window. It
#    deliberately does NOT arm a second keeper: the keeper that runs this script is
#    the only one (no dup keeper).
#    The script is generated (not inlined into the keeper argv) to avoid nesting the
#    respawn command's quotes inside the keeper's own tmux string.
RESPAWN_SCRIPT="$COGNITION_DIR/captain-respawn.sh"
cat > "$RESPAWN_SCRIPT" <<EOF
#!/usr/bin/env bash
# captain-respawn.sh — GENERATED by captain-launch.sh (hk-opuv / hk-z036). DO NOT EDIT.
# Dead-pane self-heal: respawn ONLY the captain \`agent\` WINDOW, resuming the SAME
# session-id. The sibling \`keeper\` window survives. Does NOT arm a keeper
# (the keeper that runs this is the only one).
set -euo pipefail
tmux respawn-window -k -t "$CAP_TMUX:$WIN_AGENT" -e "HARMONIK_AGENT=$CAP_NAME" 'claude --dangerously-skip-permissions --remote-control $CAP_NAME --resume $SID'
tmux display-message -p -t "$CAP_TMUX:$WIN_AGENT" '#{pane_pid}' > "$COGNITION_DIR/captain.pid"
EOF
...
# 4) Arm the session-keeper AFTER the captain is up, in a sibling `keeper` WINDOW
#    of the SAME captain session (window-nesting, hk-z036). The keeper drives the
#    in-process handoff→/clear→/session-resume cycle against the captain.
#    --tmux is the WINDOW target "$CAP_TMUX:$WIN_AGENT": slice K taught the keeper
#    to accept session:window and inject/gauge that named window's active pane, so
#    the keeper never pastes into its own `keeper` window.
...
#    once the gauge goes stale and the agent pane is at a shell prompt (90s cooldown).
#    The keeper lives in the captain session, so it is excluded from reaping by the
#    same captain.sentinel that protects the agent window.
tmux new-window -d -t "$CAP_TMUX" -n "$WIN_KEEPER" \
  "harmonik keeper --agent \"$CAP_NAME\" --tmux \"$CAP_TMUX:$WIN_AGENT\" --warn-abs-tokens $CAP_WARN_ABS --act-abs-tokens $CAP_ACT_ABS --respawn-cmd \"$RESPAWN_SCRIPT\""

echo "captain-launch: captain up in tmux '$CAP_TMUX:$WIN_AGENT' (session_id $SID, pane_pid $CAPTAIN_PID); keeper armed in sibling window '$CAP_TMUX:$WIN_KEEPER' targeting '$CAP_TMUX:$WIN_AGENT' (warn_abs $CAP_WARN_ABS / act_abs $CAP_ACT_ABS)."
...
echo "captain-launch: self-heal respawn wired — keeper will respawn the '$CAP_TMUX:$WIN_AGENT' window via $RESPAWN_SCRIPT (--resume $SID, agent window only, keeper window survives, no dup keeper) if the captain pane dies."
