    ...
    for job in data.get('jobs', []):
        for step in job.get('steps', []):
            if step.get('name', '') == 'make test-scenario':
                concls.append(step.get('conclusion', '') or '')
    # Collect EVERY matching step rather than keeping the last one: should this
    # job ever become a matrix, a later green leg must not overwrite an earlier
    # red one. Any graded non-success wins, so the probe fails toward noise
    # instead of toward the silence it exists to prevent.
    graded = [c for c in concls if c]
    if not graded:
        print('unknown')
    elif all(c == 'success' for c in graded):
        print('green')
    else:
        print('not-green')
except Exception:
