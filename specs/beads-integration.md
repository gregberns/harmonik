...
Harmonik MUST adopt `github.com/Dicklesworthstone/beads_rust` (the SQLite-backed fork) as its task-ledger dependency. Harmonik MUST NOT adopt the Dolt-backed Beads variant (`gastownhall/beads`). Harmonik MUST NOT fork or modify the Beads codebase; Beads is consumed as an external binary via the `br` CLI.
...
Bead IDs are scoped to a single Beads store per project; cross-project bead references are deferred and tracked under OQ-BI-005. The adapter MUST treat `bead_id` as opaque (no parsing, no minting, no rewriting per BI-008).
...
> **RETIRED TEXT (v0.8.x BI-009a), superseded by the above:** "A bead MAY carry an optional label of the form `workflow:<mode>` where `<mode> ∈ {single, review-loop, dot}`. … The built-in fallback carries a hard review-floor (EM-012a-FLOOR): on embedded-artifact load failure (parse failure, missing artifact, or schema-version incompatibility) the daemon MUST fall back to `review-loop`, NEVER to `single`."
...
The allowed-mode enum (`{single, dot}`; `review-loop` RETIRED v0.9.0) is owned by [execution-model.md §4.3]; this requirement cites it by reference.
...
| `run_failed` with `failure_class = canceled` | (no Beads write; OQ-BI-004 tracks operator-cancel routing) | — | daemon emits `run_failed`; OQ-BI-004 tracks whether to reopen |
...
| Operator cancel / `ErrCanceled` (per [handler-contract.md §4.5]) | (no Beads write) | — | OQ-BI-004 tracks whether to reopen |
...
**`deferred` and `tombstone` are operator-facing states harmonik does NOT write.**
...
The label is set by the daemon when a `dot` run terminates at `close-needs-attention` per [workflow-graph.md §8 WG-022] — for example when a capped fix-loop edge is exhausted per [execution-model.md §4.9 EM-043] (and by analogous operator-drain semantics per [operator-nfr.md §4.3]). Its presence asserts that operator triage is required before re-dispatch. An operator who clears the label restores the bead to the submittable set on the next queue-submit.
...
Between the dispatcher's selection of a queue item and the `claim` write to Beads, the daemon MUST re-read the bead's status via `br show <bead_id>` and confirm `status = open`. If the re-read returns a non-open status, the daemon MUST skip the claim, emit `bead_claim_skipped{bead_id, observed_status, reason="status_changed_between_select_and_claim"}` per [event-model.md §8.8], and return the item to its group with status `deferred-for-ledger-dep` per [queue-model.md §6 QM-022] (when the change indicates an external claim by another daemon process — disallowed per BI-009 and §4.8a BI-025e operator note — or a status flip to `deferred`).

A re-read that returns `closed` or `tombstone` is NOT one of those cases. The bead has finished, so the queue item is advanced to `completed` per [queue-model.md §3.2b QM-002b Class A], NOT held at `deferred-for-ledger-dep`. The skip and the `bead_claim_skipped` emission above are unchanged; only the queue-side disposition differs. Amended 2026-08-12 (hk-rern1) — see the revision history for why this is a supersession rather than an additive clarification.

INFORMATIVE — what BI-013c does NOT cover. The cases named above are an external claim by another daemon process, a status flip to `deferred`, and the `closed` / `tombstone` case carved out beneath them. A SIBLING QUEUE inside this same daemon holding the same bead is none of the three: no external process claimed anything, and the ledger status may still read `open`. That case is governed by [queue-model.md §9.8a QM-067a], which requires a per-tick refusal rather than a durable failure, and it changes nothing in the paragraph above. Added 2026-08-12; refs hk-nsion.

INFORMATIVE — why `closed` / `tombstone` was removed from the clause above. This is a SUPERSESSION, and it is worth stating plainly rather than dressing up as a clarification. The old clause was a MUST that named `closed` and `tombstone` itself and prescribed `deferred-for-ledger-dep` for them; it delegated nothing. [queue-model.md §3.2b QM-002b Class A] gave a different answer — advance the item to `completed` — for the same observation. Two rules, two dispositions, one queue item. Class A wins on the merits: a finished bead never moves again, so `deferred-for-ledger-dep` only postpones the same answer until the next daemon start runs Class A over it; and the disposition the shipped daemon actually used, `failed`, parks the queue permanently, because [queue-model.md §8.3b QM-052b] refuses to recover a queue whose bead is not `open`. The `deferred` case is untouched — a deferred bead CAN still change, so holding the item is right for it.

Nothing observable changed with this amendment. The shipped daemon has never written `deferred-for-ledger-dep` at this site: it wrote `failed` before hk-rern1 and writes `completed` after. This paragraph records reality rather than altering it. Added 2026-08-12; refs hk-rern1.
...
> INFORMATIVE — **a bare `Refs: <bead_id>` commit-message match is not evidence that the work is done.** BI-022 makes git authoritative for *completion*. A commit that names a bead makes a weaker claim than that. A bead worked in several parts leaves an earlier partial commit carrying the same ID, and a bare commit-message grep matches it.
>
> Measured, not theorized (`hk-f38n`): the daemon ran that grep before dispatch, read the match as "this bead is finished", and closed the bead while the remaining work had not started. Bead `hk-cmry` closed wrongly, and its outstanding work was refiled as `hk-zmpd` to escape the poisoned match. The pre-dispatch check was removed for that reason. Both surviving close paths in the run driver — the no-change timeout and the no-commit guard — first check whether the work is present, so crash-restart recovery does not rest on the grep alone.
>
> [execution-model.md §4.13 EM-063] Phase 2 and [execution-model.md §4.14 EM-064] tier 2 both still mandate the same bare match as an "already landed" test — EM-063 in the daemon's eager-refill pre-screen, EM-064 in the orchestrator's guard before it submits. They carry the same exposure. Narrowing a normative test belongs to that spec, so this is flagged here and not amended there. Until it is, read a match at those two points as "a commit names this bead", never as "this bead is done".
...
**Core.** An implementation conforming to Core MUST pass every requirement BI-001 through BI-032 (including BI-010d, BI-013b, BI-013c) and every invariant BI-INV-001 through BI-INV-004. BI-010d (orphan-sweep reset) depends on PL-006 extension (hk-iuaed.2); implementation is deferred per the imrest codename label, but the spec language is normative now.

**Deferred extensions.** None declared at this time. Future additions (e.g., a harmonik-side Beads read-through cache with bounded staleness, a second adapter for a non-Beads ledger) are additive and would be tracked as new requirements.
...
Owner: foundation-author
Blocks: none (harmonik assumes POSIX filesystems that allow colons; encoding is local to the adapter)
Default-if-unresolved: Assume POSIX behavior. Revisit when a Windows-host operator scenario emerges.
...
Question: BI-008a declares bead-IDs project-scoped; cross-project bead references are deferred. What surface (federated lookup, ID rewriting, etc.) handles cross-project work composition when the time comes?
Owner: ingestion-spec author (spec not yet written)
Blocks: nothing (single-project assumption)
...
Question: When the task-ingestion agent (not yet built) translates external task sources into beads with typed edges, does it write edges via the §4.8 adapter, or does ingestion get a separate write surface?
Owner: ingestion-agent spec (not yet written)
Blocks: nothing
...
Default-if-unresolved: convention-based; the skill package documents the discipline. Promotion to a mechanical wrapper-fence is deferred.
...
Blocks: nothing
Default-if-unresolved: stay aligned with EV's existing vocabulary; emit `divergence_inconclusive` for adapter-side divergences and structured-log records for adapter-recovery observability. The cross-spec amendment to EV is deferred.
...
~~Blocks: nothing~~
~~Default-if-unresolved: exact-match; semver-range adoption deferred to a later revision.~~
...
Owner: foundation-author
Blocks: nothing (single-daemon-per-project model)
Default-if-unresolved: operator discipline; mechanical detection is deferred.
