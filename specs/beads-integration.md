...
The compatibility window between a pinned Beads version `pinned` and an observed Beads version `observed` is: all observable versions are treated as compatible for startup purposes. A version delta (`observed != pinned`) is a loud WARNING — the daemon logs prominently and continues; it is NOT a fatal startup condition. Hard-failure (exit code 8) is reserved for cases where `br` is genuinely unavailable: exec failure, non-zero exit from `br --version`, or version output that cannot be parsed. This policy supersedes the MVH exact-match rule (`isCompatible(pinned, observed) ≡ pinned == observed`), which proved operationally harmful: the fleet executed 754 successful runs on br=0.2.10 while pinned at 0.1.45, yet the exact-match check bricked every daemon restart during that period. OQ-BI-011 (semver-range widening) is resolved by this amendment.

The check is performed by BI-024a `--version` handshake; only exec/parse failures fail daemon startup with §8 code 8 (`beads-unavailable`) per [operator-nfr.md §8]; a version delta emits a warning only.
