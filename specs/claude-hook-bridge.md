...
**Test isolation and concurrency.** The implementation MUST honor `HARMONIK_CLAUDE_CONFIG_PATH` (full file path) or `CLAUDE_CONFIG_HOME` (directory; config file is `<dir>/.claude.json`) environment variable overrides so that unit and integration tests can redirect writes to a temp path and never touch the real `~/.claude.json`. Precedence: `HARMONIK_CLAUDE_CONFIG_PATH` > `CLAUDE_CONFIG_HOME` > `~/.claude.json`.

A remote run writes this file on the worker. The implementation MUST apply the same precedence there. It MUST evaluate `CLAUDE_CONFIG_HOME` and `~` on the worker, because `CLAUDE_CONFIG_HOME` names a directory on the machine that sets it, and `~` names the home of the user that runs the write. The daemon MUST NOT send its own value of `CLAUDE_CONFIG_HOME` to a worker. It MAY send `HARMONIK_CLAUDE_CONFIG_PATH`, because that variable is harmonik's own and it names one file.

The implementation MUST hold an exclusive advisory flock (`LOCK_EX`) on a sidecar lockfile (`<cfgPath>.lock`) across the entire read-modify-write cycle to serialize concurrent daemon instances writing to the same config file. The wait for that lock MUST be bounded. The implementation MUST retry a non-blocking acquire (`LOCK_EX|LOCK_NB`) until it gets the lock or a timeout ends. On timeout it MUST fail with a structural error, and the error MUST name the config file and the lockfile.

A blocking wait is not permitted. With a blocking wait one stuck holder starves every later run, and the run shows no diagnostic until some later and unrelated deadline ends it. Bead `hk-g8d5x` is that failure: a worker program waited without limit on `~/.claude.json.lock`, and three daemon tests each stalled about 50 seconds while the reported failure named an innocent subsystem. The in-process Go writer has been bounded since `hk-bfvby`. This clause makes the spec agree with that code, and it extends the same rule to the worker programs, which were still blocking.

The sidecar pattern is required: locking the target file directly would interfere with the atomic rename. The sidecar MUST be created with `O_CREATE|O_RDWR` (mode `0600`) if it does not yet exist; the lock is released when the file descriptor is closed after the rename completes.
