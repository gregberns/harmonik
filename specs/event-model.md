...
- The typed event taxonomy for the foundation (complete cross-subsystem emission surface).
...
- **JSONL rotation policy.** Unbounded append works at the current scope per [docs/foundation/core-scope.md §2]; rotation is deferred, tracked at OQ-EV-001.
...
| 8.1.12 | `implementer_no_work_suspected` | O | daemon-core | operator-observability, audit, improvement-loop | `run_id`, `bead_id`, `duration_seconds`, `floor_seconds` |
| 8.1.13 | `run_worktree_churn_edits_discarded` | O | daemon-core (run-merge) | operator-observability, audit | `run_id`, `bead_id`, `worktree_path`, `paths`, `recovery_patch?` |
| 8.1.14 | `run_worktree_untracked_files_removed` | O | daemon-core (run-merge) | operator-observability, audit | `run_id`, `bead_id`, `worktree_path`, `paths`, `unsaved_paths?`, `recovery_patch?` |

> **§8.1.12 `implementer_no_work_suspected` (hk-368i4) — EV-027 addition amendment.** Emitted by daemon-core when a process-exit implementer finishes with NO commit and a CLEAN worktree AND its phase ran for less than the configured no-work duration floor. The emitter covers EVERY harness whose completion mode is process exit — codex and Pi today — in BOTH workflow modes. The trigger is the harness commit fallback's outcome, and the codex and Pi fallbacks return the same outcome enum from the same primitives, so one detector reads either. Reading the trigger as codex-only is what left a `single`-mode Pi run with no record at all (hk-3ywqv). §8.9 compliance evidence: **(a)** cross-subsystem consumers — operator-observability (the diagnostic an operator reads when a run fails with no explanation), audit, and improvement-loop; **(b)** lifecycle-boundary signal — it is emitted once at the implementer-phase boundary, alongside `implementer_phase_complete`, not per tick; **(c)** single summary event per phase, no re-emit; **(d)** typed payload `ImplementerNoWorkSuspectedPayload` with `Valid()`; **(e)** durability class O — loss costs a diagnostic only, because the run's failure is independently recorded by the no-commit path; **(f)** non-idempotent, single emission per run; **(g)** cited by [handler-contract.md] process-exit-path consumers and this section as the detector for the silent-implementer failure class; **(h)** not a paired-phase lifecycle — it is a single detector observation, not a pending/resolved pair. **Detector, not gate:** the run is already failing via the no-commit guard when this fires; the event names the shape so a future silent failure of a different cause is still caught. Both conditions are required — duration alone would flag a legitimately trivial bead, and a real fast run still commits and so never reaches the no-change outcome.

> **§8.1.13 `run_worktree_churn_edits_discarded` and §8.1.14 `run_worktree_untracked_files_removed` (hk-nqvqr, hk-4q6ah) — EV-027 addition amendment.** Both are emitted by the daemon's run-merge path immediately before a step that destroys uncommitted work in the run worktree. `git rebase` refuses to start in a dirty worktree, so the merge cleans the worktree first: it reverts the churn paths with `git checkout -- <path>`, which restores the index content and so destroys the unstaged edit, and it deletes the untracked non-ignored files with `git clean -fd`. Both steps are correct and both stay. Each one now saves the doomed content to a recovery artifact under `.harmonik/recovery/`, warns on standard error, and emits its event. The emission WHEN, the artifact rules, and the apply recipes are owned by [execution-model.md §4.12 EM-072, EM-073]. **Cadence differs between the two, and a consumer MUST NOT assume they match:** the churn revert runs once per rebase attempt, so one merge can emit §8.1.13 more than once with a different artifact each time; the untracked clean runs at most once per merge. §8.9 compliance evidence: **(a)** cross-subsystem consumers — operator-observability (the person who decides whether to recover the destroyed work, reading the stream through the read-only `subscribe` surface of [operator-nfr.md §4.9 ON-055] or `events.jsonl`) and audit (the record that a merge destroyed uncommitted work at all); the emitting subsystem itself consumes neither; **(b)** lifecycle-boundary signals — each fires once per cleanup step at the pre-rebase boundary of the merge phase, not per file and not per tick; **(c)** per-boundary access is required — a run-level summary cannot say which paths went, which artifact holds them, or which of several rebase attempts wrote it, and the artifact path is the only route back to the content; **(d)** typed payloads `RunWorktreeChurnEditsDiscardedPayload` and `RunWorktreeUntrackedFilesRemovedPayload`, each with `Valid()`, and each declared in §6.3; **(e)** four-axis tags plus the mechanism tag per the §8.1 class-O default, and durability class **O** — the artifact on disk and the stderr warning are written at the same moment, so loss of the event costs the report and not the rescued content; **(f)** replay classification — observational per §4.5 EV-021 and `replay-safety=safe`; a replay reader MUST NOT re-run a cleanup and MUST NOT apply a recovery artifact, and it MUST tolerate an artifact path that no longer exists, because nothing prunes `.harmonik/recovery/` on a schedule; emission is non-idempotent, so a consumer MUST dedupe on `event_id` rather than on `run_id`; **(g)** cited by name in [execution-model.md §4.12 EM-072 and EM-073] and in that spec's §6.5 co-ownership map; **(h)** not paired-phase lifecycles — each is one terminal observation of a step that already happened, with no pending phase to pair with. **`recovery_patch` is OPTIONAL on both, on purpose.** The event fires even when the artifact could not be written, and carries an empty `recovery_patch` in that case. A cleanup that destroyed work and saved none of it is the case an operator most needs to hear about, so the report MUST NOT depend on the file.
...
| 8.1a.4 | `iteration_cap_hit` | O | orchestrator-core | audit, observability, operator-observability, beads-integration | `run_id`, `workflow_mode`, `iteration_count`, `cap_value` (Integer; = 3), `final_verdict ∈ {REQUEST_CHANGES, BLOCK}` |
...
| 8.6.14 | `bead_terminal_transition_recovered` **(deferred)** | O | beads-adapter | observability, audit | `bead_id`, `op` (enum: `claim` / `close` / `reopen`), `idempotency_key`, `recovered_at` |

> **(deferred)** §8.6.14 `bead_terminal_transition_recovered` is reserved for a future revision per OQ-BI-008. The adapter today emits a structured-log record per [operator-nfr.md §4.9 ON-035] for adapter-recovery observability rather than this event; the entry is reserved here so the type identifier is burned for future use by the BI adapter and not reused for any other purpose. No conformance obligation attaches to §8.6.14.
...
The `agent_output_chunk` and `budget_accrual` types remain fine-grained because the improvement-loop subsystem requires per-chunk cost attribution and mid-run signals; collapsing them to summary events would lose information. A log-level / filter mechanism for suppressing chunk events at consumer boundaries is a future refinement slot.
...
| 8.10.3 | `queue_group_completed` | F | queue | audit, observability, orchestrator-core | `queue_id`, `group_index`, `final_status` (enum: `complete-success` / `complete-with-failures`), `success_count`, `fail_count`, `completed_at`, `completion_receipt_id?` |
...
> Emission ordering (§8.10). For a single queue's uninterrupted normal path the last item's terminal `run_completed`/`run_failed` MUST precede durable queue group/state commit, which precedes the `queue_group_completed` attempt. Non-final success then attempts `queue_group_started`; failure then attempts `queue_paused`. Final success requires the exact completion receipt/root durable before the one `queue_group_completed` attempt, followed by queue-model CAS cleanup. Restart never retries or synthesizes that event and its outcome never gates cleanup, release, retention, admission, or same-name reuse. Successful live operator cancellation attempts `queue_cancelled_operator` only after archive/legacy/intent cleanup and QueueStore/refusal release, then returns RPC success; graceful shutdown emits none.

> **Queue authority (§8.10).** Canonical queue bytes, queue-owned intents,
> completion receipts, and completion-release markers decide commit,
> completion, cleanup, restart, release, retention, and admission. Queue events
> are derived observations. EV-021 and EV-022 apply without exception: restart
> does not walk JSONL, and no queue intent/receipt/marker contains event
> delivery state.
...
> **Emission rule (§8.13.1).** Daemon MUST emit `epic_completed` at most once per parent epic per daemon session (at-most-once guard keyed on `epic_id`; in-process `emittedEpics` map). Emission is triggered after a successful `CloseBead` call: if the closed bead has a parent epic AND every sibling child shows a **terminal** status in the Beads ledger, the guard is claimed (under `emittedEpicsMu`) and the event is emitted. Terminal means `status=closed` OR `status=tombstone` — the set `core.CoarseStatus.IsTerminal()` defines. A tombstoned child is finished and never becomes closed, so a rule that reads `closed` alone holds the epic back forever. Zero-emit conditions: (a) closed bead has no parent (AC-4); (b) at least one sibling child is not terminal (AC-3); (c) `emittedEpics` already contains the parent ID. Cross-process sibling-race (AC-2) and boot-survival (AC-5) are out-of-scope for C1 and exercised by the T4 scenario bead.
...
The `source_subsystem` field MUST be a Go-package-identifier string. The envelope schema MUST NOT enumerate a fixed set, keeping the layout open for later reorganization. Subsystem identifiers are declared in each subsystem's envelope per [architecture.md §4.4].
...
#### `run_worktree_churn_edits_discarded`

```yaml
run_id: <UUID>
bead_id: <String>
worktree_path: <String>                  # the run worktree the revert ran in
paths: <String[]>                        # churn paths whose unstaged edit the revert destroyed; MUST NOT be empty
recovery_patch: <String>                 # path of the artifact holding those edits; "" when none was written
```

Pre-rebase churn-revert report per [execution-model.md §4.12 EM-072]: emitted before the merge reverts the daemon-owned and agent-owned churn paths with `git checkout -- <path>`. That command restores the index content, so `paths` names the unstaged edits the revert destroys and nothing else. `recovery_patch` names a `git diff --binary` of those edits under `.harmonik/recovery/` in the project root. It is OPTIONAL: an empty value means the artifact could not be written, and the event fires anyway, because a revert that saved nothing is what an operator most needs to hear about. The artifact is cut against the run worktree INDEX, so a reader applies it with `git apply -3` on the machine that ran the merge — a clone can lack the preimage blob. One merge MAY emit this event more than once, because the revert runs once per rebase attempt, and each emission names its own artifact. Class O: the artifact and the stderr warning are written at the same moment, so loss of the event costs the report and not the rescued content.

#### `run_worktree_untracked_files_removed`

```yaml
run_id: <UUID>
bead_id: <String>
worktree_path: <String>                  # the run worktree the clean ran in
paths: <String[]>                        # every untracked non-ignored file the clean deletes; MUST NOT be empty
unsaved_paths: <String[]>                # the entries of paths the artifact does NOT hold; usually empty
recovery_patch: <String>                 # path of the artifact recreating the rest; "" when none was written
```

Pre-rebase untracked-clean report per [execution-model.md §4.12 EM-073]: emitted before the merge deletes the untracked non-ignored files with `git clean -fd`. Those files are the ones the residual-delta commit did not stage — mainly the `.claude/` and `.harmonik/` paths its pathspec excludes. `paths` names every file the clean deletes. `unsaved_paths` names the entries of `paths` that the rescue could not read, so a non-empty `recovery_patch` holds the files in `paths` that are not in `unsaved_paths`. An empty `recovery_patch` holds nothing at all, whatever `unsaved_paths` says. Both fields are required to read the event: without `unsaved_paths` a deleted file sits beside a non-empty `recovery_patch` that does not hold it, and a partial save reads as a full one. `recovery_patch` is a set of add-this-file patches against `/dev/null`, so it needs no base and a reader applies it with plain `git apply`, never with `git apply -3`. It is OPTIONAL for the same reason it is optional on `run_worktree_churn_edits_discarded`. One merge emits this event at most once, because only the first rebase preparation cleans untracked files. Class O, for the same reason.
...
iteration_count: <Integer>                           # = cap_value
cap_value: <Integer>                                 # = 3 per [execution-model.md §4.3 EM-015e]
...
run_id: <UUID>
workflow_mode: <enum: single | review-loop | dot>   # always "review-loop"
final_iteration_count: <Integer>                     # 1..3; = iteration_count at termination
...
completion_receipt_id: <String | absent>          # canonical lowercase UUIDv7
```

`completion_receipt_id` is REQUIRED exactly for final `complete-success` and
MUST equal the queue-model intent-bound receipt ID. It MUST be absent for
non-final `complete-success` and every `complete-with-failures`. The field is
additive/optional for N-1 readers; when present, new readers MUST validate
canonical lowercase UUIDv7 form. Payload construction occurs before final
replace-intent durability. Construction failure is `not_committed`; append
failure after receipt durability is diagnostic.
...
```yaml
parseable:                              # exactly one of parseable/corrupt
  queue_id: <String>                    # canonical UUIDv7
  normalized_name: <String>
  prior_status: <String>
  archive_identity: <String>
  archive_path: <String>
  request_id: <String | absent>
corrupt:
  normalized_name: <String>
...
Exactly one of `parseable` and `corrupt` MUST be present; both or neither is
invalid. Parsed-queue-only fields MUST be absent from the corrupt variant.
Payloads contain no raw canonical bytes or secrets. This Class O event is
attempted only after successful live operator cancellation has durably
archived/cleaned queue state, removed the archive intent, and released
QueueStore/refusal ownership. It is outside replace/archive intents and
completion receipts, is never replayed, and is absent on graceful shutdown.
Append failure is diagnostic and cannot change cancellation success, block
same-name reuse, or suppress the RPC response.
...
- **[beads-integration.md §4.10, OQ-BI-008]** — deferred reservation slot for `bead_terminal_transition_recovered` (§8.6.14) and deferred `divergence_kind` adapter-specific values.
...
**Core.** An implementation conforming to Core MUST:
...
**Deferred extensions.** JSONL rotation policy (OQ-EV-001), quality-checks.md cross-reference (OQ-EV-002), testing.md migration (OQ-EV-003), and per-payload version field (OQ-EV-004) are deferred.
...
- **§8.10 queue cohort.** Final success requires canonical UUID
  `completion_receipt_id`; non-final success and every failure omit it.
  Construction failure precedes durable intent; append failure continues
  receipt-governed cleanup; restart with receipt/no event emits nothing.
  `queue_cancelled_operator` constructor/decode/registry/cohort/count/Class-O
  tests accept exactly one parseable/corrupt variant. Audit failure preserves
  cancellation success and graceful shutdown emits no audit. Existing
  JSONL/replay golden behavior is otherwise unchanged.
...
`harmonik subscribe --since-event-id` is IMPLEMENTED as of 2026-05-27 (hk-a5sil, sha 994c6d2); the CLI help in `cmd/harmonik/subscribe.go` `subscribeUsage` previously read "NOT YET IMPLEMENTED — daemon rejects; hk-a5sil", which was STALE. This spec is normative for the behavior contract; the CLI help is the operator-facing surface for the same contract. The two MUST agree. As part of adopting this revision, both help surfaces in `cmd/harmonik/subscribe.go` — the file-header flag reference and `subscribeUsage` — MUST be updated to remove the "NOT YET IMPLEMENTED" annotation and describe the replay semantics (e.g. "Resume cursor: replay events strictly after this event_id before delivering live stream"). This note closes the obligation; the corrections were applied in the same commit as this spec revision.

## 11. Open questions

#### OQ-EV-001 — JSONL rotation policy

Question: Should `events.jsonl` rotate by size, age, or remain unbounded?
Owner: foundation-author
Default-if-unresolved: Unbounded append; revisit before production deployment.
...
Owner: foundation-author
Default-if-unresolved: No per-payload version field. **Note:** downgrade is lossy — a v2 producer's JSONL consumed by a v1 binary cannot distinguish payload versions from the wire alone. Acceptable today (single-daemon, operator-gated upgrade); revisit when external tooling needs Go-registry-free consumption.
...
Question: §8.7 carries 15 events, several of which (`operator_pause_status`, `operator_resuming`, `operator_stopped`, `operator_upgrading`, `operator_upgrade_completed`, `operator_upgrade_rejected`, `operator_command_rejected`, `dispatch_deferred`, `daemon_orphan_sweep_completed`) are close to "the daemon is in some state now" signals. A unified `operator_state_changed{from_state, to_state, detail}` could fold 4–5 of these into one if the consumer-branching cost of the merge is acceptable. Deferred so current subscribers can be audited for actual branch-shape needs before collapsing.
Owner: foundation-author
Default-if-unresolved: §8.7 shape remains granular; revisit after first implementer pass once subscriber branch-shape is measurable.

#### OQ-EV-007 — Consumer-goroutine panic policy — machine-checkable enforcement

Question: Subscription.on_panic (§6.1) offers three options (`recover_and_log` / `quarantine_consumer` / `fail_daemon`) but the bus-internal enforcement is unwritten. Default is `recover_and_log`: the bus recovers the panic, emits `consumer_failed` (§8.8.2) with `error_category=panic`, and continues dispatching to other consumers. `quarantine_consumer` additionally suspends dispatch to the panicking consumer for the rest of the daemon cycle. `fail_daemon` escalates to `daemon_startup_failed` (inappropriate as a default). Deferred: testing.md enforcement and the `quarantine_consumer`/`fail_daemon` semantics.
Owner: foundation-author
Default-if-unresolved: Implement `recover_and_log`; `quarantine_consumer` and `fail_daemon` are declared in the Subscription record but implemented post-testing.md.

## 12. Revision history

| Date | Version | Author | Summary |
|---|---|---|---|
| 2026-08-24 | 0.7.10 | agent (hk-nqvqr, hk-4q6ah) | **Taxonomy addition (2 new §8.1 rows): §8.1.13 `run_worktree_churn_edits_discarded` (Class O) and §8.1.14 `run_worktree_untracked_files_removed` (Class O).** Both are emitted by the daemon's run-merge path immediately before a step that destroys uncommitted work in the run worktree, per the new [execution-model.md §4.12 EM-072, EM-073]. `git rebase` refuses to start in a dirty worktree, so the merge reverts the churn paths with `git checkout -- <path>` and deletes the untracked non-ignored files with `git clean -fd`. Both steps are correct and both stay; what changes is that each one saves the doomed content to a recovery artifact under `.harmonik/recovery/`, warns on standard error, and names the loss in an event. **Payloads:** the churn event carries `run_id`, `bead_id`, `worktree_path`, `paths`, `recovery_patch`; the untracked event adds `unsaved_paths`, the entries of `paths` the artifact does not hold, so a partial save is legible as one. `recovery_patch` is OPTIONAL on both — the event fires even when the artifact could not be written, because a cleanup that saved nothing is what an operator most needs to hear about. **§6.3 payload schemas** added after `transition_event`, and each one states its own apply recipe: the churn artifact is a diff against the run worktree index and takes `git apply -3` on the machine that ran the merge, while the untracked artifact adds files against `/dev/null` and takes plain `git apply`. **§6.5 co-ownership map** updated: both entries are daemon-run-merge-emission-owned. **Cadence differs and the §8.1 note says so:** the churn revert runs once per rebase attempt, so one merge can emit §8.1.13 more than once, while §8.1.14 fires at most once per merge. Class O for both: the artifact and the warning are written at the same moment, so loss of the event costs the report and not the content. §8.9 evidence for every criterion (a) through (h) is recorded in the §8.1 note. No EV requirement IDs added, renamed, or retired; no pre-existing §8 entries renumbered. Companion: [execution-model.md] v0.11.1. Refs: hk-nqvqr, hk-4q6ah. |
...
**Why the three-class consumer taxonomy with default-observer.** Without class distinction, consumers drift into either "fail-the-producer" or "swallow-failures" behaviors. Default-observer prevents accidental coupling. `synchronous` is retained for future bus-level halt-on-invariant-violation surfaces; the acyclicity clause (EV-010) prevents reentrant deadlock. No consumer registers synchronous today; removing and re-adding the class later costs more than keeping it. Per EV-014c, observer dispatch uses per-observer bounded queues on the same shed semantics as async.

**Why durability-class-driven fsync (EV-016) over hardcoded names.** The first draft hardcoded four types, which rots on every addition. The class framing ties fsync to a semantic property (loss cost > sync cost) declared per §8 row. `workspace_merge_status` is F because losing a merge forces git-DAG reconstruction; `transition_event` is F because it carries divergence-evidence for Cat 3/4 reconciliation; `agent_output_chunk` is L because chunks are statistical aggregates. Per EV-016a, atomicity across multiple boundary events is NOT guaranteed; producers requiring paired durability must emit a single event or resolve via git. Group-commit batching is deferred.
