...
- **workflow_mode** — the resolved execution shape in a `Run` record. New runs use `dot`, the general workflow-graph walker defined in §7.5. Historic `single` values remain readable. A legacy `workflow:single` bead label is a graph-selection request, not an imperative dispatch mode. It resolves the named no-review DOT graph and records `dot` as the resolved execution mode. (see §4.3.EM-012, §4.3.EM-012a, §7.5)
- **WorkflowDescriptor** — the selected workflow identity pair `{workflow_id, workflow_version}`. `workflow_id` comes from the selected DOT graph. The accepted graph-source grammar is named ID `^[A-Za-z][A-Za-z0-9-]*$` or canonical UUID text. UUID text preserves existing graphs and historic records. A new execution does not generate a workflow UUID. The descriptor records identity only. It does not carry graph data, selection source, mode, or policy.
...
- **last_iteration_head_sha** — under `review-loop` mode, the task-branch HEAD recorded after the prior implementer phase and before its reviewer launch. It is the routing baseline for the next post-`REQUEST_CHANGES` implementer: an unchanged value means the fix-up stalled. Stored durably in `Run.context.last_iteration_head_sha`; distinct from `last_diff_hash`, which is diagnostic evidence only. (see §4.3.EM-012, §4.3.EM-015d, §4.3.EM-015e)
- **needs-attention** — under `review-loop` mode, the operator-drained close-path marker selected by the normalized cycle outcome, not by the raw verdict alone. It applies to `cap_hit`, `blocked`, `fixup_stalled`, and `error` outcomes, but not to `approved`, including a raw flagless `REQUEST_CHANGES` normalized to approval. Operator-drained only; no auto-retry. The Beads label and write semantics are owned by [operator-nfr.md §4.3] and [beads-integration.md §4.4]; this spec names when the close path is selected.
- **active queue** — any named queue in the daemon/project `QueueStore` snapshot whose status is `active` per [queue-model.md §3 QM-002]. More than one normalized name may be active; §7.4 selects among eligible active queues under [queue-model.md §9 QM-062, QM-066, QM-067].
- **queue group** — a group of queue items within one named queue per [queue-model.md §5]. A group is the per-queue unit of group-advance gating per §4.3.EM-015f.
...
A `Workflow` MUST be a named, versioned directed graph with a stable `workflow_id`, `name`, `version`, a set of `nodes`, a set of `edges`, a declared `start_node_id` and non-empty `terminal_node_ids` list (see §6.1), an ordered reference list of policies resolved at workflow-load time (cited from [control-points.md §6.3]), and a `metadata` map. The `workflow_id` is a required graph-level identity. It uses the grammar defined in §3 and [workflow-graph.md §10 WG-055]. On-disk representation is a DOT document per [architecture.md §4.10] three-artifact separation.
...
Absent a policy override, the following baseline per-node-type idempotency-class defaults MUST apply: reviewer, researcher, lint, test, typecheck, and analysis nodes default to `idempotent`; builder and merge nodes default to `non-idempotent`. A YAML policy MAY override any of these defaults. Node types introduced later MAY register `recoverable-non-idempotent` defaults with a declared resume protocol.
...
A `Run` MUST carry a stable `run_id`, one resolved `WorkflowDescriptor`, `input` (a workspace reference per [workspace-model.md §4.1], not inline payload), `workflow_mode = dot`, current `state`, `context` (a shared key-value map updated per §4.10.EM-041a), `start_time`, and optional `end_time`. The daemon resolves and validates the selected graph before it creates the observable run start. It seals the descriptor, resolved mode, review policy, selection source, and input reference for the run's lifetime. A run executes EXACTLY ONE workflow invocation against EXACTLY ONE input; multi-workflow or multi-input runs are not permitted. Transition records for the run are discoverable via the task-branch commit range whose commits carry the run's `Harmonik-Run-ID` trailer; no separate `transitions` field on the `Run` record is required.

When `workflow_mode = review-loop`, the run's `context` map MUST carry, at minimum, the reserved keys `iteration_count` (Integer, 1..3, incremented per §4.3.EM-015d), `last_verdict` (one of `{APPROVE, REQUEST_CHANGES, BLOCK}` per the agent-reviewer JSON schema v1, or `null` before iteration 1's reviewer returns), `claude_session_id` (the Claude Code session identifier captured from the implementer's first launch per §4.3.EM-015d; reused on subsequent implementer launches via `claude --resume`), `last_diff_hash` (String, the SHA-256 of `git diff <parent>..<head>` output on the run's task branch at the end of the prior iteration; diagnostic evidence only — it is NOT a review-loop routing predicate per §4.3.EM-015e), and `last_iteration_head_sha` (String, the task-branch HEAD resolved at the end of the prior iteration and before that iteration's reviewer launch; the routing baseline for the fix-up-stalled early-exit per §4.3.EM-015e). These keys are normative for `review-loop`; their absence in a `review-loop` run is an authoring error detected at claim time. For `workflow_mode = single`, no `context` keys are reserved by this spec. For `workflow_mode = dot`, the per-workflow context-key registry per [workflow-graph.md §10 WG-031] is the authoritative source of allowed `context.<key>` LHS references; this spec reserves no context keys for `dot` mode.

> DECLARED IMPLEMENTATION GAP (dated 2026-07-28). Four of the five reserved keys above are not durably persisted today, so their MUST is aspirational. `internal/core/runcontextkeys.go` declares four key constants (`RunContextKeyIterationCount`, `RunContextKeyLastVerdict`, `RunContextKeyClaudeSessionID`, `RunContextKeyLastDiffHash`); **none of the four constants has a non-comment reference anywhere in the tree.** The live driver carries the values as in-memory fields on `reviewLoopState` (`iterationCount`, `claudeSessionID`, `lastVerdictNotes` / `lastVerdictFlags`, `lastDiffHash`, `lastIterHeadSHA`), and `last_iteration_head_sha` has no constant at all. **`claude_session_id` is the one exception and IS durable:** `persistClaudeSessionID` in `internal/daemon/sessioncontext_chb023.go` writes it to `.harmonik/run-context/<run_id>/context.json` and commits it on the task branch per [claude-hook-bridge.md §4.6 CHB-023] / §4.5.EM-023a — it simply reaches durability through that file rather than through the key constant. The other four (`iteration_count`, `last_verdict`, `last_diff_hash`, `last_iteration_head_sha`) have no durable carrier at all, so a review-loop run's iteration state does not survive a daemon restart, and the "durably write" obligations in §4.3.EM-015d / §4.3.EM-015e plus the restart-reconstruction obligation in §10.2 are targets, not conformance claims for those four. Recorded here rather than left implicit so that no reader infers conformance from the MUST. The `last_iteration_head_sha` key is newly reserved at v0.9.6 and joins the same pre-existing gap; it does not open a new one. **SUPERSEDED IN PART at v0.10.0:** `review-loop` is retired (§4.3.EM-015d) and its driver deleted, so the four keys with no durable carrier now have no writer either and this gap closes by removal rather than by repair. The `claude_session_id` exception outlives the mode and became a finding in its own right: with the review-loop driver gone, `persistClaudeSessionID` has no production caller at all, and no code anywhere reads `.harmonik/run-context/<run_id>/context.json` or the `claude_session_id_persisted` event back — the write was never wired to a crash-recovery reader. `internal/runmerge/stripruncontext.go` reaches the same conclusion independently and exists solely to strip those commits back off the target branch. Tracked as `hk-50li9`, which must resolve it one way or the other: implement the missing EM-031 reader, or retire the persist and the strip together.
...
0. **Per-item queue input.** A queue item may carry `workflow_mode` and
   `workflow_ref`. This is the CLI and persisted-record tier. When its mode is
   `single`, the resolver MUST map it to the registered embedded
   `no-review-bead` graph before graph validation. It MUST set resolved mode to
   `dot`, select descriptor `{workflow_id="no-review-bead",
   workflow_version="1.0"}`, and record
   `workflow_selection_source = queue_item_single_mode`. The persisted item
   retains raw `workflow_mode = single` for legacy audit. A tier-0 `dot` value
   follows the ordinary DOT path. This tier does not authorize `single` as an
   executor mode or a self-selected no-review policy.
1. **Per-task.** The bead's `workflow:<mode>` label per [beads-integration.md §4.3 BI-009a]. `workflow:dot` selects the ordinary DOT resolution path. Legacy `workflow:single` selects the registered embedded `no-review-bead` graph and records `workflow_selection_source = legacy_single_label`. It does not select an imperative dispatcher. Both resolve to execution mode `dot`. `workflow:review-loop` is RETIRED v0.10.0. If the bead carries more than one `workflow:<mode>` label, the daemon MUST treat tier 1 as absent AND MUST emit `bead_label_conflict` per [event-model.md §8.8] before continuing the walk.
2. **Per-project.** The project-level configuration default. The resolution function MUST tolerate this tier's absence and pass through to tier 3; the tier slot is reserved for later operator configuration and is non-normative for conformance.
3. **Per-daemon.** The daemon configuration default per [process-lifecycle.md §4.1 PL-004a].
4. **Built-in fallback.** `dot`. The default DOT workflow is the embedded `standard-bead.dot` artifact (the canonical exemplar registered with the daemon per §7.5.1.EM-055 and §7.5.5.EM-059). A legacy `workflow:single` label is not a mode fallback. It is a tier-1 selection of the named no-review DOT graph. The resolved mode remains `dot`, and the run-start payload records `workflow_selection_source = legacy_single_label`. A tier-0 item `single` uses the same graph with `queue_item_single_mode`. The daemon MUST NOT resolve `single` at tier 3 or tier 4.

   **Review-floor guarantee (EM-012a-FLOOR). AMENDED v0.10.0 (amendment `review-loop-retirement`) — the floor's MECHANISM changes from silent demotion to loud failure; the floor's GUARANTEE is unchanged.** The retired text is quoted at the end of this sub-clause so a reader can tell a deliberate reversal from an omission.

   The tier-4 resolution to `dot` carries a hard review-floor: **a bead resolved via tier 2, tier 3, or tier 4 MUST NEVER be dispatched without a review gate.** That guarantee is delivered by two obligations:

   (a) **Reviewed by construction.** The embedded `standard-bead.dot` artifact MUST carry a reviewer node on the sole inbound edge to `close` (the topology contract of [workflow-graph.md §17 WG-047..WG-052]), so a default-resolved `dot` run is review-gated by the graph itself. This half is UNCHANGED by this amendment and remains the primary mechanism of the floor.

   (b) **Loud failure, never a silent change of shape.** If the embedded `standard-bead.dot` artifact fails to load — parse failure, missing artifact, schema-version incompatibility per §7.5.3.EM-057 check 1, or template-parameter substitution failure — the daemon MUST fail the run with a diagnostic naming the artifact and the underlying load error. The daemon MUST NOT dispatch the bead under any other workflow shape. `single` remains unreachable by default resolution exactly as before; `review-loop` is no longer a fallback target because it is no longer a workflow mode (see §4.3.EM-015d, RETIRED v0.10.0).

   *Rationale for changing the mechanism.* Three observations, each verified against the implementation at the time of this amendment:

   1. **The demotion was not a reachable production state for the shipped artifact.** The artifact is bound at compile time (`//go:embed` in `internal/daemon/standardgraph.go`), so a missing artifact is a build error, not a runtime one; and `internal/daemon/standardgraph_sync_test.go` asserts both that the shipped bytes parse and validate, and that they are byte-identical to `specs/examples/standard-bead.dot`. A parse failure of the shipped graph is therefore a red test suite, not a condition the daemon must survive at dispatch. The one residual reachable case is template-parameter substitution failure, which is an authoring error in the dispatching request — precisely the case where an actionable failure serves the operator better than a silent change of execution shape.
   2. **The demotion was not honestly observable.** `run_started` stamps `workflow_mode` before the demotion is applied, so the run's own start event records a mode the run did not execute. (A stderr diagnostic was written, and later per-iteration events did stamp the real mode, so a demoted run was *detectable* by cross-checking — but the one event field whose purpose is to answer "which mode did this run use?" answered wrongly.)
   3. **It matches the project's stated posture for an unavailable capability.** `plans/2026-07-27-delete-and-rewrite/CHARTER.md` states, of the tmux requirement, that a capability genuinely unavailable "should be announced loudly at boot and degraded honestly — never faked, and never deferred to a crash on first dispatch." That sentence is scoped to tmux in its original context; this amendment applies the same reasoning to the embedded graph.

   > **RETIRED TEXT (v0.9.x EM-012a-FLOOR), superseded by the above:** "The tier-4 resolution to `dot` carries a hard review-floor: if the embedded `standard-bead.dot` artifact fails to load (parse failure, missing artifact, or schema-version incompatibility per §7.5.3.EM-057 check 1), the daemon MUST fall back to `review-loop`, NEVER to `single`. A bead resolved via tier-3 or tier-4 MUST NEVER be dispatched without a review gate: the resolution chain's floor is `review-loop`, and `single` (the no-review one-handler-per-node shape) is unreachable by default resolution. The fallback ordering on embedded-artifact load failure is therefore `dot → review-loop` (and the daemon MUST emit a diagnostic recording the fallback), NOT `dot → single`. This floor is the structural guarantee that the system never silently bypasses review for default-resolved work."

Resolution MUST run exactly once per run at claim time. Before `run_started`, the daemon MUST locate and parse the selected DOT graph, substitute typed attributes, validate the result, then produce a `WorkflowDescriptor` from that graph's `workflow_id` and `version`. The resolver returns the descriptor beside the input reference, selected graph, resolved mode, resolved review policy, and selection source. The descriptor is identity only. It does not carry the selected graph or the other launch values. The resolved launch result MUST be sealed into the Run record (§6.1) before any node in the run is dispatched and MUST NOT be re-evaluated for the lifetime of the run. The event payload MUST surface the descriptor, `workflow_mode = dot`, review policy, and selection source per [event-model.md §8.1].

The resolver, not DOT input, derives `review_policy`. It MUST yield `no_review`
only for the registered embedded descriptor `{workflow_id="no-review-bead",
workflow_version="1.0"}` when its selection source is
`legacy_single_label` or `queue_item_single_mode`. It MUST yield `reviewed` for
every other graph. A graph attribute, a custom graph, or an explicit graph
reference MUST NOT claim `no_review`. A descriptor, policy, and provenance
tuple that breaks this binding is a pre-start validation error.

A tier-1 input that names an unknown mode (a `workflow:<mode>` label whose `<mode>` value is not in `{single, dot}`) MUST cause tier 1 to be treated as absent AND MUST emit `bead_label_conflict` with the offending label recorded in the payload. As of v0.10.0 this includes the retired value `review-loop`: a bead still carrying `workflow:review-loop` is treated as an unknown mode — tier 1 absent, `bead_label_conflict` emitted — and resolves onward to `dot`. This is the deliberate migration path for beads labelled before the retirement; it degrades to the reviewed default rather than failing the bead.

> NORMATIVE CROSS-REFERENCE: A default-resolved `dot` run executes the embedded `standard-bead.dot` graph through the §7.5 `dot`-mode binding (input contract §7.5.1.EM-055, dispatch equivalence §7.5.2.EM-056, validator obligations §7.5.3.EM-057, node-type dispatch table §7.5.4.EM-058). The `standard-bead.dot` exemplar MAY contain `sub-workflow`-type nodes; their in-place expansion and dispatch are governed by §4.8.EM-034 (family) and the dedicated sub-workflow-node-dispatch contract in [sub-workflow-dispatch.md] (sub-workflow nodes within DOT workflows), consumed by §7.5.2.EM-056 clause 3 unchanged.
...
2. **Per-project.** `.harmonik/config.yaml` at the project root, loaded once at daemon startup and cached for the lifetime of the process (restart required to reload). The normative schema is:
   ...
   Validation rules (all enforced by `LoadProjectConfig` in `internal/daemon/projectconfig.go`, the reference implementation):
...
The daemon MUST emit the `run_started` event (per §6.5 and [event-model.md §8.1]) after `create_run` has allocated the run's `run_id`, after the Beads atomic-claim write per [beads-integration.md §4.3 BI-009] has persisted the claim, and after §4.3.EM-012a has resolved and validated the selected DOT graph. It MUST emit before any node in the run is dispatched. The version-2 payload MUST carry the `run_id`, `WorkflowDescriptor` flat keys `workflow_id` and `workflow_version`, `workflow_mode = dot`, resolved review policy, selection source, `input_ref`, and required start observations `started_at`, `worker_name`, and `worker_os`. The worker fields are null for a local run. When the run is bead-tied per §4.3.EM-014, it MUST carry `bead_id`. When the run originated from a queued dispatch per [queue-model.md §4 QM-010..012], it MUST additionally carry optional `queue_id` and `queue_group_index` fields per [event-model.md §8.10]. These fields are absent for non-queued dispatch. A legacy `workflow:single` label MUST resolve its named no-review graph and record its selection source before this event. It MUST NOT enter an imperative single dispatcher. If the daemon crashes between `run_id` allocation and `run_started` emission, the run is reconstructable from the Beads atomic-claim record per §4.7.EM-031a (no orphan runs).
...
> **Amended 2026-08-13 (hk-jqz61).** The closing sentence used to order the terminal-transition
> bead write AFTER the terminal event. It is replaced, not relaxed: every other obligation in this
> requirement is unchanged. Two independent defects made it wrong. (1) It contradicted §4.12.EM-052
> steps 6-8 and §4.12.EM-053 steps 1-3 in this same spec, the §10.2 conformance obligation for
> EM-052, [run-state-machine.md §7 RSM-021], and the `internal/twinparity` equivalence spine, whose
> `TerminalKinds` order and `bead_closed -> run_completed` causal edge fail a stream carrying the
> reversed order. (2) It attributed the rule to [beads-integration.md §4.4 BI-010], which does not
> state it: BI-010 says WHEN a write is legal in terms of run STATE, and BI-010a classifies writes
> without ordering them against the terminal event. Read literally the retired sentence forbade the
> hk-wfbxf split-brain fix, because a terminal event emitted first cannot take its type from a close
> result that has not happened yet, so an implementer following it would reintroduce that bug.

The daemon MUST emit exactly one of `run_completed` or `run_failed` (per §6.5 and [event-model.md §8.1]) when the run reaches terminal state per §4.3.EM-015c. `run_completed` emits when the run enters a node in `terminal_node_ids` with `outcome.status ∈ {SUCCESS, PARTIAL_SUCCESS}`. `run_failed` emits when the classifier (§8) produces a terminal verdict, when the cascade returns `FAIL` (per §4.10.EM-046a or §4.10.EM-043), or when an operator cancel is observed per §7.1. The `run_failed` event payload MUST carry the failure class per §8 and the `last_checkpoint` SHA per §4.5.EM-025. When the run originated from a queued dispatch per [queue-model.md §4 QM-010..012], `run_completed` and `run_failed` event payloads MUST additionally carry the optional `queue_id` and `queue_group_index` fields per [event-model.md §8.10]. These fields are absent for non-queued dispatch. The truth-claim terminal-transition write per [beads-integration.md §4.4 BI-010] — the `close` or `reopen` a run performs as it terminates — MUST precede the terminal event emission, so the emitted event type can carry the write's result. §4.12.EM-052 step 6 owns the rule that a HARD close failure yields `run_failed` rather than `run_completed`; this requirement cites it and does not restate it. That rule does NOT reach the transient `BrUnavailable` close, which deliberately leaves the bead `in_progress` for [beads-integration.md §4.10 BI-031] intent-log recovery and still terminates the run as a success. The ordered sequence for the merge paths is normative in §4.12.EM-052 steps 6-8 on success and §4.12.EM-053 steps 1-3 on failure; a terminal arising from the §8 classifier, from a cascade `FAIL` per §4.10.EM-046a, or from an operator cancel per §7.1 is ordered by this requirement alone, because those two requirements do not cover it. This orders a run's own close/reopen against its own terminal event and governs nothing else; in particular it does not reach the activity-marker `reset` writes of [process-lifecycle.md §PL-006h], where the daemon emits `run_failed` for an abandoned run and resets the bead afterwards, because a `reset` is an activity-marker write per [beads-integration.md §3] and not a truth-claim write.

> **RETIRED TEXT (v0.10.8), superseded by the above:** "The terminal-transition bead write per
> [beads-integration.md §4.4 BI-010] MUST follow the terminal event emission; it MUST NOT precede
> it."
...
#### EM-015d — Review-loop mode lifecycle — RETIRED v0.10.0

> **RETIRED v0.10.0 (amendment `review-loop-retirement`).** `workflow_mode = review-loop` is no longer a selectable workflow mode; the requirement body below is retained for provenance and is NON-NORMATIVE. `review-loop` described a two-node implementer→reviewer cycle written by hand. `dot` mode (§7.5) is the general graph walker of which that cycle is one particular graph, and the embedded `standard-bead.dot` exemplar expresses the same implementer→reviewer→close shape with a richer node set. The hand-written driver was therefore a redundant second implementation of a shape the general engine already executes in production; it is removed rather than maintained in parallel. Every guarantee EM-015d carried that is still required — the review gate on the sole inbound edge to `close`, iteration/traversal capping, no-progress detection, reviewer-verdict archival, and reviewer-feedback delivery on re-entry — is carried by the `dot` binding (§7.5) and the exemplar topology contract ([workflow-graph.md §17 WG-047..WG-052]). The sub-clauses EM-015d-RFD (reviewer-feedback delivery) and EM-015d-RIA (reviewer input artifact) are retired **as review-loop clauses only**; their obligations survive unchanged for the `dot` reviewer node, which implements both. Beads still carrying a `workflow:review-loop` label resolve per the amended §4.3.EM-012a unknown-mode rule (tier 1 treated as absent, `bead_label_conflict` emitted, resolution continues to `dot`).

Tags: mechanism

A run with `workflow_mode = review-loop` (per §4.3.EM-012) MUST execute a hardcoded two-node cycle: `implementer → reviewer → {APPROVE: close, actionable REQUEST_CHANGES: implementer, BLOCK: close-needs-attention, iteration-cap: close-needs-attention, fixup-stalled: close-needs-attention}`. A `REQUEST_CHANGES` verdict is **actionable** when its `flags` array is non-empty; a flagless `REQUEST_CHANGES` is normalized for routing to approval while its raw verdict remains unchanged in durable artifacts and events (§4.3.EM-015e). The graph is fixed by this spec and is NOT obligated to be expressed as a DOT document; the v1 driver is mode-specific code on the daemon's claim and dispatch path. This sub-graph shape is representable as an instance of the general workflow-graph model defined for `dot` mode (see §7.5 — `dot` mode binding).
...
- **Per-iteration state.** After the implementer phase and before launching the reviewer, the daemon MUST resolve the current task-branch HEAD, compute `last_diff_hash`, and durably write both into `Run.context.last_iteration_head_sha` and `Run.context.last_diff_hash`. HEAD is the routing predicate per §4.3.EM-015e; the diff hash is evidence only. After the reviewer's verdict file lands, the daemon MUST read `.harmonik/review.json`, validate it against the `agent-reviewer` JSON schema v1, archive the file to `.harmonik/review.iter-<N>.json` (where `<N>` is the current `iteration_count`), and update `Run.context.last_verdict` to the raw verdict value.
- **Per-iteration events.** Each implementer resume MUST emit `implementer_resumed` per [event-model.md §8.1a]; each reviewer launch MUST emit `reviewer_launched`; the verdict-file read MUST emit `reviewer_verdict` carrying the raw agent-reviewer schema v1 fields verbatim plus `run_id`, `session_id`, and `iteration_count` — including a `REQUEST_CHANGES` verdict later normalized to approval per §4.3.EM-015e, which MUST NOT be rewritten to `APPROVE` in the artifact or the event. On an unchanged post-`REQUEST_CHANGES` HEAD per §4.3.EM-015e, the daemon MUST emit `review_fixup_stalled` per [event-model.md §8.1a.7]. On cap-hit per §4.3.EM-015e, the daemon MUST emit `iteration_cap_hit`. On cycle termination (by any path), the daemon MUST emit exactly one `review_loop_cycle_complete` per [event-model.md §8.1a] carrying the normalized `completion_reason`. The historical `no_progress_detected` event remains readable for existing traces and is retained for the separately owned `dot`-cascade no-progress path; it is NOT the built-in review-loop post-`REQUEST_CHANGES` route.
...
- **Reviewer-feedback delivery to implementer-resume (EM-015d-RFD).** Before launching an `implementer-resume` (i.e., any implementer dispatch for iteration N ≥ 2), the daemon MUST execute the following ordered steps:
  ...
  1. **Write the reviewer-feedback file.** The daemon MUST write `${workspace_path}/.harmonik/reviewer-feedback.iter-<N-1>.md` (where `<N-1>` is the just-completed iteration's `iteration_count`) containing:
     ...
     - **(EM-015d-RFD-ORIGIN.)** The `dot` cascade re-enters the implementer for causes no reviewer produced: a red commit gate (`GATE_FAIL`) and a pass that advanced no commit (`NO_COMMIT`). Both are delivered through THIS file, because it is the file the resume instruction reads. When the `verdict` is not one of the three schema-v1 reviewer verdicts above, the daemon MUST NOT head the file `# Reviewer feedback` and MUST state, before the verdict line, that no reviewer read the change and that the daemon wrote the file. The origin MUST be derived from the verdict value, not from a separate caller-supplied field, so that a new daemon-produced cause cannot omit the declaration. A document that claims a review nobody performed is the defect this clause closes: an implementer that reads one reasonably acts on it (`hk-2f3v4`, observed live 2026-08-10).
  ...
  2. **Deliver the read instruction via the structured input port.** Only AFTER the file from step 1 exists on disk (i.e., the atomic rename in step 1 has completed), the daemon MUST deliver the following instruction to the resumed implementer via the structured input port of [agent-input.md §4.1 AIS-001] (`InputPort.SubmitInput`), which returns a real `Ack` — a delivery outcome `Delivered`/`Rejected` whose positive acceptance arrives asynchronously as `agent_input_acked` (on the tmux/Claude path SOURCED from the Claude-hook-bridge `outcome_emitted`/`agent_ready` signal per [claude-hook-bridge.md §4.5 CHB-013], not a `capture-pane` scrape), or `agent_input_stale` on the bounded-liveness timeout — per [agent-input.md §4.2 AIS-003] / [agent-input.md §4.3 AIS-004]. For daemon runs this input-delivery mechanism SUPERSEDES the tmux `load-buffer` / `paste-buffer` paste-inject formerly mandated here (the "PL-021b-PASTE" mechanism label is DEMOTED for the daemon-run path per [process-lifecycle.md §4.3 PL-021d] — retained only for the session-keeper and interactive CLI nudge, not for review-loop resume). What is delivered — the read instruction below — is unchanged; only the delivery channel changes:

     > Before continuing, read `.harmonik/reviewer-feedback.iter-<N-1>.md` in your worktree. It says what routed iteration `<N-1>` back to you, and its first line says who produced it — a reviewer, or the daemon because the commit gate went red or no commit landed. Address every point it raises before proceeding.

     The instruction MUST NOT assert that a reviewer examined the work, per EM-015d-RFD-ORIGIN: the daemon delivers the same file for the two non-reviewer causes, and the instruction is composed before the cause is known.

     The input submission MUST occur AFTER `claude --resume <claude_session_id>` has reattached to the implementer's transcript (i.e., after the session is live and has resumed, not during or before the process spawn). The ordering invariant is unchanged: **file exists → session is live → input delivered**. The daemon MUST NOT submit the input before the resumed session is observable per [agent-input.md §4.1 AIS-001] (the port is obtained by structural assertion at the process-spawn seam). If the input submission does not reach a positive acceptance within the bounded window (a synchronous `Rejected` ack, or no `agent_input_acked` before the emitted `agent_input_stale` timeout per [agent-input.md AIS-INV-001]), the daemon MUST log the failure and route the run per [handler-contract.md §4.6] failure handling.
...
- **Reviewer input artifact (EM-015d-RIA).** Before launching the reviewer pane for any iteration (including iteration 1), the daemon MUST write the reviewer input artifact at `${workspace_path}/.harmonik/review-target.md` (path normative per [workspace-model.md §6.2 WM-RIA-001]). This file is the reviewer's sole structured context source; the reviewer is told, via the structured input port of [agent-input.md §4.1 AIS-001] (`InputPort.SubmitInput` → real `Ack` per AIS-003/AIS-004; the former "PL-021b-PASTE" paste-inject is DEMOTED for the daemon-run path per [process-lifecycle.md §4.3 PL-021d]), to read it and produce `review.json` per [claude-hook-bridge.md §4.5 CHB-014]. The daemon MUST execute the following ordered steps:
  ...
  2. **Spawn the reviewer pane.** Only AFTER the file from step 1 exists on disk (i.e., the atomic rename in step 1 has completed), the daemon MUST spawn the reviewer pane via `tmux new-window` per [process-lifecycle.md §4.3 PL-021b]. The pane MUST be observable (i.e., `WindowPanePID` returns a non-zero PID) before step 3 executes.
     ...
     **This step makes the review loop a tmux-hosting-dependent capability (DECLARED 2026-07-28).** [process-lifecycle.md §4.7 PL-021b] item 3 / §4.10 PL-028b now allow the daemon to boot with tmux hosting unresolved. This step does not become optional there: a daemon without tmux hosting cannot satisfy it, so the review loop is one of the capabilities PL-028b item 2 obliges the daemon to announce as lost at startup. Substituting a non-tmux substrate for the reviewer here is the "silently degrading to non-tmux mode" that [process-lifecycle.md §4.7 PL-021a] forbids — the reviewer's harness is pinned deliberately, and a substitution changes which harness reviews the work. The daemon MUST refuse or announce, never substitute.

     > DECLARED IMPLEMENTATION GAP (dated 2026-07-28). On a no-tmux boot the composition root leaves the reviewer substrate nil and the work loop treats nil as "fall back to the general substrate", which performs exactly the substitution this clause forbids; the boot announcement does not name the review loop among the lost capabilities. Filed as `hk-p01zm`. Recorded, not fixed, so that no reader infers conformance.

  3. **Deliver the start instruction via the structured input port.** Only AFTER the reviewer session from step 2 is live, the daemon MUST deliver the following instruction to the reviewer via the structured input port of [agent-input.md §4.1 AIS-001] (`InputPort.SubmitInput` → real `Ack` per [agent-input.md §4.2 AIS-003] / [agent-input.md §4.3 AIS-004]). For daemon runs this SUPERSEDES the tmux `load-buffer` / `paste-buffer` paste-inject formerly mandated here (the "PL-021b-PASTE" mechanism label is DEMOTED for the daemon-run path per [process-lifecycle.md §4.3 PL-021d]). What is delivered — the start instruction below — is unchanged; only the delivery channel changes:
     ...
     The **ordering invariant** is unchanged: **file exists → session live → input delivered**. No step may execute before its predecessor completes. If the input submission does not reach a positive acceptance within the bounded window (a synchronous `Rejected` ack, or no `agent_input_acked` before the emitted `agent_input_stale` timeout per [agent-input.md AIS-INV-001]), the daemon MUST log the failure and route the run per [handler-contract.md §4.6] failure handling.
  ...
  > **Input-delivery mechanism note (M2 agent-input-substrate).** Across the whole `review-loop` lifecycle, every daemon-run instruction delivered to a live implementer or reviewer session — the reviewer start instruction (EM-015d-RIA step 3), the implementer-resume read instruction (EM-015d-RFD step 2), and the resume brief they carry — is DAEMON-RUN INPUT and MUST be delivered via the structured input port of [agent-input.md §4.1 AIS-001] (`SubmitInput` → real `Ack`, AIS-003/AIS-004; bounded-liveness per AIS-INV-001). The "PL-021b-PASTE" mechanism label (the tmux `load-buffer` / `paste-buffer` paste-inject) is SUPERSEDED by AIS for the daemon-run input path; [process-lifecycle.md §4.3 PL-021d] survives only as a carve-out for the session-keeper ([session-keeper.md §4.1 SK-002]) and the interactive CLI nudge / boot-seed paths — review-loop resume is NOT one of those carve-outs, so it MIGRATES to AIS rather than being preserved. Only the delivery MECHANISM changes: the WHAT (the read/start instruction, the review-target, the resume brief) and the review-loop control flow, iteration semantics, ordering invariants, and all non-input-delivery requirements of EM-015d are unchanged. Only spawn verbs (`tmux new-window` at EM-015d-RIA step 2, per [process-lifecycle.md §4.3 PL-021b]) and human-facing observation (`capture-pane`, per [agent-input.md §4 AIS-011]) remain on the tmux surface for daemon runs; the input direction does not.

  Tags (sub-clause EM-015d-RIA): mechanism
  Axes: llm-freedom=none; io-determinism=deterministic; replay-safety=safe; idempotency=non-idempotent

- **Implementer commit obligation is review-loop-scoped.** Under `workflow_mode = review-loop`, the implementer phase MUST advance the worktree HEAD (produce a commit) before the reviewer is launched; a clean implementer exit that does not advance HEAD is a cycle-internal failure. On iteration 1 it routes per [handler-contract.md §4.6] and yields `completion_reason = error`; after an actionable `REQUEST_CHANGES` it routes through the fix-up-stalled early-exit of §4.3.EM-015e, measured against `Run.context.last_iteration_head_sha`. This commit obligation is specific to `review-loop` mode and does NOT apply to `workflow_mode = dot`. A `dot`-mode `agentic` node MAY relax the HEAD-advance requirement via the per-node `non_committing` attribute per [workflow-graph.md §4 WG-041]; that relaxation is gated on the per-node attribute and never reaches the `review-loop` path.
...
#### EM-015e — Review-loop iteration cap and early-exit — RETIRED v0.10.0

> **RETIRED v0.10.0 (amendment `review-loop-retirement`), with EM-015d.** NON-NORMATIVE, retained for provenance. The cap-and-early-exit semantics survive on the `dot` path as per-edge traversal caps (§4.10.EM-043 / §4.10.EM-043a) and the exemplar's declared caps ([workflow-graph.md §17]). One note for the record, which the v0.9.6 catch-up amendment established independently one version earlier and this retirement does not contradict: the review-loop never implemented a `no_progress` early exit. Its only HEAD-advance check routed to `review_fixup_stalled`, and the `no_progress_detected` emitter in that driver had zero callers. v0.9.6 recorded the same fact from the other direction by demoting `no_progress` to a "compatibility value ... NOT emitted by the built-in review-loop post-`REQUEST_CHANGES` route" and adding `fixup_stalled` as the value the code actually produces. So the clause being retired here is one the code never satisfied. The `dot` path DOES implement no-progress detection (emitting `no_progress_detected` behind a configurable guard), and that implementation is untouched by this retirement.

A run with `workflow_mode = review-loop` (per §4.3.EM-012) MUST observe a hardcoded iteration cap of **3**. The cap is not operator-tunable for v1 conformance. The cap and the early-exit conditions below collectively determine the run's terminal-state classification per §4.3.EM-015c:
...
- **Flagless REQUEST_CHANGES normalization.** On a validated `REQUEST_CHANGES` verdict whose `flags` array is empty, the daemon MUST preserve and archive the raw verdict and MUST emit `reviewer_verdict{verdict=REQUEST_CHANGES, flags=[]}`, but MUST normalize the routing decision to `APPROVE`. The run terminates as `completed` with `review_loop_cycle_complete{completion_reason=approved}`; no further implementer iteration is dispatched and no `needs-attention` marker is applied. The raw verdict plus the normalized cycle completion together are the audit representation; the daemon MUST NOT rewrite the verdict artifact or the `reviewer_verdict` event to `APPROVE`. Rationale: a flagless `REQUEST_CHANGES` names no actionable item, so re-dispatching the implementer spends an iteration on a fix-up that has nothing to fix and then wedges the run on the fix-up-stalled exit below. This normalization is `review-loop`-scoped; the `dot` cascade routes the raw `REQUEST_CHANGES` label unconditionally and MUST NOT adopt it without its own amendment.
- **Actionable REQUEST_CHANGES with iterations remaining.** On a reviewer verdict of `REQUEST_CHANGES` with a non-empty `flags` array AND `iteration_count < 3`, the daemon MUST increment `iteration_count` per §4.3.EM-015d, durably record the current task-branch HEAD as `Run.context.last_iteration_head_sha` and the current diff hash as `Run.context.last_diff_hash` (evidence), and re-dispatch the implementer via Claude-session resume per §4.3.EM-015d. The run MUST NOT terminate.
- **Cap-hit (actionable REQUEST_CHANGES at iteration 3).** On a reviewer verdict of `REQUEST_CHANGES` with a non-empty `flags` array AND `iteration_count = 3`, the daemon MUST emit `iteration_cap_hit` per [event-model.md §8.1a] and MUST terminate the run via the `needs-attention` close path. Bead-side, the run's terminal-transition write per [beads-integration.md §4.4 BI-010] MUST apply the `needs-attention` label per [operator-nfr.md §4.3].
...
- **Fix-up-stalled early-exit.** Before each reviewer launch from iteration 2 onward, the daemon MUST resolve the current task-branch HEAD and compare it with the durably recorded prior `Run.context.last_iteration_head_sha`. Reaching iteration 2 or later is structural proof that the prior routing verdict was an actionable `REQUEST_CHANGES` (APPROVE, normalized-flagless, and BLOCK all return before the counter increments). If HEAD is unchanged — i.e. the implementer-resume produced no new commit in answer to the prior verdict — the daemon MUST emit `review_fixup_stalled` per [event-model.md §8.1a.7], carrying the prior reviewer flags and the diagnostic current/prior diff hashes, AND MUST terminate the run via the `needs-attention` close path with `completion_reason = fixup_stalled` BEFORE launching the reviewer for the current iteration. If HEAD advanced, the cycle proceeds **even when the cumulative diff hash equals the prior hash**: diff-hash equality is not a review-loop routing predicate, because a genuine new commit whose net `parent..HEAD` diff collides with a prior one is progress, not a stall. The superseded v1 detector was the SHA-256 hash of `git diff <parent>..<head>`; it is retired for this path and MUST NOT be reinstated without an amendment.

The terminal `run_completed` or `run_failed` event per §4.3.EM-015b MUST be preceded by exactly one `review_loop_cycle_complete` event per §4.3.EM-015d. The `review_loop_cycle_complete` payload's `completion_reason` MUST match the terminal routing decision: `approved` for a raw `APPROVE` **or** a normalized flagless `REQUEST_CHANGES`, `cap_hit` for an actionable `REQUEST_CHANGES` at the cap, `blocked` for `BLOCK`, `fixup_stalled` for unchanged HEAD after an actionable `REQUEST_CHANGES`, and `error` for daemon-side failures observed within the cycle (e.g., verdict-file malformed or missing per §4.3.EM-015d). The value `no_progress` remains in the compatibility grammar for historical traces and for the separately owned `dot`-cascade path; the built-in review-loop post-`REQUEST_CHANGES` route MUST use `fixup_stalled`.
...
The selected named queue's active group MUST NOT advance to the next group
until every item in that group has reached terminal status (`completed` or
`failed`) per [queue-model.md §5 group state machine]. The daemon MUST evaluate
the named group's terminal condition after every `run_completed` or
`run_failed` emission per §4.3.EM-015b whose `queue_id` and
`queue_group_index` payload fields name that group. A terminal group in one
queue MUST NOT advance, pause, or otherwise mutate a sibling queue.

When the named group's terminal-count equals its item-count AND zero failures,
the group transitions to `complete-success` and the queue advances. After the
authoritative queue group/state commit, the uninterrupted normal producer path
MUST attempt `queue_group_completed` with
`final_status = complete-success`, then `queue_group_started` when a successor
exists, per [event-model.md §8.10]. For final success, [queue-model.md §3,
§8 QM-053] additionally requires the authoritative completion receipt/root to
be durable before the final group-completed attempt.

When the named group's terminal-count equals its item-count AND ≥1 failure,
one queue transaction MUST commit the group transition to
`complete-with-failures` and the queue transition to `paused-by-failure`
together per [queue-model.md §8 QM-052]. No observer may see one without the
other. After that single authoritative commit the uninterrupted normal
producer path MUST attempt
`queue_group_completed` with `final_status = complete-with-failures`, then
`queue_paused`, per [event-model.md §8.10]. The queue MUST NOT advance.

In this requirement, “MUST emit” defines the normal-path producer obligation;
it does not guarantee append survival across process crash or append failure.
Queue state and, for final success, the queue-model completion receipt are the
authoritative landmarks. Append failure does not change committed state.
Restart MUST NOT synthesize or retry `queue_group_completed`. When attempts
occur, the last item terminal event still precedes the authoritative
group/state commit and group-completed attempt; successor advance and failure
pause semantics are unchanged.

v0.1 has no `resume` operation; `complete-with-failures` is effectively
terminal for the queue. The queue persists at its canonical named path per
[queue-model.md §3 QM-001]; recovery is daemon-restart + re-submit.
...
#### EM-025 — Failed transitions MUST NOT create checkpoint commits

A failed transition (per §4.5.EM-023a, i.e., `outcome.status = FAIL` or a classifier verdict of `transient|structural|deterministic|canceled|budget_exhausted|compilation_loop` per §8) MUST emit a failure event per [event-model.md §8] (see §8 for the per-class event mapping) but MUST NOT create a checkpoint commit for Core conformance. The failure event MUST reference the last successful checkpoint commit's SHA as its `last_checkpoint` correlation field, providing an anchor to the git trail. Later introduction of failure commits (to support `git bisect` over failures for the improvement loop) is an additive change and does not break the current contract.
...
merged paths then remain stale in the working tree.
...
The artifact does not promise a clean restore everywhere, and what decides it is
whether the object store still holds the preimage blob.

On the machine that ran the merge, it always restores. The revert puts the index
content back, and the artifact is cut against that same index, so the file
matches the preimage of the diff. Both `git apply` and `git apply -3` restore the
edit cleanly, whether or not the path also carried a staged edit.

Anywhere else it depends on the case. When a churn path carried an unstaged edit
only, the preimage is the committed blob, so every clone has it and both commands
restore the edit. When the path carried a staged edit as well, the preimage is a
blob that no commit reaches. A clone taken over a transport does not carry it:
`git apply` fails on context and `git apply -3` refuses with "repository lacks
the necessary blob". A clone taken from a path on the same machine hardlinks the
whole object store, keeps that blob, and `git apply -3` applies WITH CONFLICTS,
which leaves the destroyed content in the file between the markers. The event
names the loss. It does not promise the loss is undone.
...
FUNCTION eager_refill_eval():
    fleet = queue_store.snapshot_all()                    # complete immutable named set
    IF fleet.named_queues IS EMPTY: RETURN
    global_available = max_concurrent - in_flight_count() # §4.11 EM-049
    IF global_available <= 0: RETURN                      # global WIP cap full
    targets = []
    FOR queue IN fleet.named_queues SORTED BY normalized_name:
        IF queue.status != active: CONTINUE
        group = queue.active_group()
        IF group IS None OR group.kind != "stream": CONTINUE
        queue_available = queue.workers - in_flight_count_for_queue(queue.name)
        IF queue_available <= 0: CONTINUE
        pending = COUNT(item FOR item IN group.items WHERE item.status == "pending")
        deficit = min(global_available, queue_available) - pending
        IF deficit > 0: targets.APPEND((queue, group, deficit))
    IF targets IS EMPTY: RETURN
    queue, group, deficit = targets[0]                    # deterministic normalized-name target
    ...
    IF eligible IS NOT EMPTY:
        queue_append(queue_name=queue.name, active_stream_group=group,
                     bead_ids=eligible)                    # [queue-model.md §7 QM-040]
```
...
The complete immutable snapshot is one QueueStore generation. The deterministic
refill target is the lexicographically first eligible active stream queue with
a positive per-queue deficit in that generation. Phase 1 of EM-063 scans every
queue in that same snapshot before any append; a paused, completed, full, or
otherwise ineligible queue contributes no target but does not block an eligible
sibling.
...
**Phase 1 — already-in-queue (complete QueueStore authority, fastest).** For
each candidate `bead_id`, inspect every named queue in one complete immutable
QueueStore snapshot. If the bead is present in any queue with
`status ∈ {pending, dispatched, completed, failed}` → SKIP. A partial,
selected-queue-only, or directory-order scan is non-conforming.
...
|---|---|---|---|
| 1 | complete immutable QueueStore snapshot (or an equivalent all-names status read) | X present in any named queue, `status ∈ {pending, dispatched, completed, failed}` | yes |
...
Tier 1 is fastest (one complete QueueStore-generation scan); MUST be first and
MUST cover all normalized names, not only a proposed target. Tier 2 uses
`origin/main` NOT local `main` (rationale per EM-063). Tiers 3+4 are
supplementary; a conforming impl covering 1+2 satisfies the guard at v1
correctness. Tiers 3+4 SHOULD be checked in long-running orchestrator sessions
to catch cross-session drift. The read order is normative; MUST NOT reverse
(the QueueStore snapshot is the fastest and most current in-memory mirror).
Tags: mechanism

#### EM-065 — Submit/append is per-name after an all-queue duplicate pre-screen

After EM-064's all-queue duplicate pre-screen, the orchestrator MUST resolve a
normalized target name. If that target name is unoccupied, it MAY
`queue-submit` a new queue even while distinct names are live. If that target
name is occupied, it MUST either `queue-append` to that named queue's active
stream group per [queue-model.md §7 QM-040] or reject when the target is not
appendable; it MUST NOT replace the live target. [queue-model.md §6 QM-027] is
per normalized name, not a project-wide singleton. Occupancy of another name
does not reject a valid submit, while finding the bead in any name during
EM-064 always rejects the duplicate.
...
```
RECORD Workflow:
    workflow_id        : WorkflowID         -- required graph identity; named ID or legacy UUID text per §3
    ...
    workflow_class     : String | None      -- optional class tag; the only accepted value is "reconciliation" (flags the §4.5.EM-026 exception); absence means ordinary workflow. Validator per §4.9.EM-038 MUST reject any other non-None value.
...
RECORD Run:
    ...
    workflow_id        : WorkflowID           -- selected graph identity
    workflow_version   : String               -- pinned version at dispatch time
    workflow_mode      : WorkflowMode         -- `dot` for new runs; historic `single` remains readable
    review_policy      : ReviewPolicy         -- resolved graph policy: reviewed or no_review
    workflow_selection_source : WorkflowSelectionSource -- audit provenance for graph selection
    ...
    context            : Map<String, Any>     -- shared context; updated per §4.10.EM-041a; reserved keys under workflow_mode=review-loop per §4.3.EM-012: iteration_count, last_verdict, claude_session_id, last_diff_hash, last_iteration_head_sha
    start_time         : Timestamp            -- RFC 3339 wall clock
    end_time           : Timestamp | None     -- set on terminal transition
    template_params    : Map<String, String> | None  -- per-run template-param map supplied at launch (--param KEY=VALUE per §7.5); sealed at claim time, applied once to typed attributes after parse per [workflow-graph.md §4 WG-045]; None when no params supplied; sealing makes a replay re-substitute identically
```

```
ENUM WorkflowMode:
    single                      -- legacy read compatibility only; new runs resolve a named no-review DOT graph instead
    dot                         -- general workflow-graph walker; required for new runs
```

```
TYPE WorkflowID = String  -- accepted graph-source grammar: ^[A-Za-z][A-Za-z0-9-]*$ or canonical UUID text

RECORD WorkflowDescriptor:
...
ENUM ReviewLoopCompletionReason:
    approved                    -- cycle ended with a raw APPROVE verdict, or a flagless REQUEST_CHANGES normalized to approval; run terminates SUCCESS per §4.3.EM-015e
    cap_hit                     -- actionable REQUEST_CHANGES at iteration_count = 3; run terminates needs-attention per §4.3.EM-015e
    blocked                     -- BLOCK verdict at any iteration; run terminates needs-attention per §4.3.EM-015e
    no_progress                 -- compatibility value: historical traces and the separately owned dot-cascade no-progress path; NOT emitted by the built-in review-loop post-REQUEST_CHANGES route per §4.3.EM-015e
    fixup_stalled               -- post-REQUEST_CHANGES implementer left task-branch HEAD unchanged; run terminates needs-attention before the reviewer launch per §4.3.EM-015e
...
FUNCTION orchestrator_main_loop():
    ...
    -- Steady-state loop. Dispatch input is one complete immutable QueueStore
    -- named-set snapshot per [queue-model.md §3 QM-002, §9 QM-060].
    WHILE NOT shutdown_requested():
        ...
        fleet = queue_store.snapshot_all()          -- complete named set, one immutable generation
        IF fleet.named_queues IS EMPTY:
            IF no_auto_pull():                      -- §4.11.EM-066 sealed startup flag (default ON; use --auto-pull to opt in to br-ready)
                idle_wait_for_queue_submission(); CONTINUE   -- quiet daemon: MUST NOT fall back to br ready
            -- br-ready fallback is legal only while the complete named set is empty.
        ...
        -- Existing-but-ineligible named queues suppress br-ready fallback.
        -- Build candidates without allowing a paused/completed/full sibling to block.
        IF in_flight_count() >= max_concurrent:     -- §4.11.EM-049 capacity gate
            wait_for_run_slot(); CONTINUE
        candidates = []
        FOR queue IN fleet.named_queues:
            IF queue.status != active: CONTINUE
            IF in_flight_count_for_queue(queue.name) >= queue.workers: CONTINUE -- [queue-model.md §9 QM-066]
            group = queue.active_group()
            IF group IS None: CONTINUE
            item = group.next_dispatchable()        -- [queue-model.md §5] head eligible item
            IF item IS None: CONTINUE
            candidates.APPEND((queue, group, item))
        IF candidates IS EMPTY:
            idle_wait(); CONTINUE                   -- named set exists: never call br_ready()
        queue, group, item = select_name_ordered_round_robin(
            candidates, queue_round_robin_cursor)   -- [queue-model.md §9 QM-067]
        queue_round_robin_cursor += 1               -- advance on every queue selection; never reset per tick
        -- Both capacity checks are revalidated after selection and before claim.
        IF in_flight_count() >= max_concurrent:
            wait_for_run_slot(); CONTINUE
        IF in_flight_count_for_queue(queue.name) >= queue.workers:
            wait_for_queue_slot(queue.name); CONTINUE
        IF ledger_blocks_open(item.bead_id):        -- [beads-integration.md §4.3 BI-005]
            ...
            CONTINUE
        resolved = resolve_workflow(item)           -- includes item mode, ref, and params; EM-012a tier 0
        IF NOT validator.validate(resolved.workflow): -- §4.9.EM-038
            queue.mark_item_failed(item, validation_failed); CONTINUE
        acquire_claim_token()                       -- §4.11.EM-050 claim-write serialization
        run = create_run(item.bead_id, resolved,
...
FUNCTION finalize_run(run):
    IF run.state == terminal_success:
        close_result = write_terminal_bead_transition(run)      -- [beads-integration.md §4.4 BI-010]; MUST precede the terminal event per §4.3.EM-015b
        IF close_result IS error:                               -- §4.12.EM-052 step 6: a failed close yields run_failed, NEVER run_completed
            emit_event(run_failed, run, class=run.failure_class, last_checkpoint=run.last_checkpoint_sha)
        ELSE:
    ...
    ELSE IF run.state == terminal_failure:
        write_terminal_bead_transition(run)                     -- [beads-integration.md §4.4 BI-010]; MUST precede the terminal event per §4.3.EM-015b
...
1. **Locate the `.dot` artifact.** The artifact path is derived from the workflow-mode resolution chain per §4.3.EM-012a. The reviewed built-in fallback is the registered embedded `standard-bead.dot` graph. The one audited no-review binding is the registered embedded `internal/daemon/no-review-bead.dot` graph, whose byte-identical canonical exemplar is `specs/examples/no-review-bead.dot`. It declares `workflow_id="no-review-bead"` and `version="1.0"`. The daemon MUST register it in the same embedded-graph registry as `standard-bead.dot`. It may be selected only by the legacy `workflow:single` label or tier-0 queue-item `single` mapping. Per-bead override of the artifact path via a bead-schema extension is deferred to a future BI-NNN requirement in [beads-integration.md]; at v1 the path is sourced from per-daemon configuration only.
1a. **Accept per-run param map.** The `harmonik run` CLI (and the queue-item launch context) accepts repeated `--param KEY=VALUE` flags; the resulting `map[string]string` is the run's `template_params` (§6.1), sealed at claim time alongside `workflow_ref`. A run with no `--param` flags has `template_params = None`. It MUST still execute the typed substitution walk in step 3a with no replacements and perform the WG-045 residual-token check.
2. **Read raw source text.** The daemon MUST read the `.dot` artifact as raw text before parsing.
3. **Parse to AST.** The daemon MUST parse the raw source text to an in-memory graph AST conforming to the DOT subset defined by [workflow-graph.md §10 WG-031, §11 WG-033]. The parser library is an implementation detail; the parsed AST's surface MUST be sufficient to produce a §6.1 `Workflow` record without consulting any other store.
3a. **Apply WG-045 substitution pass.** The daemon MUST apply the template-param substitution pass of [workflow-graph.md §4 WG-045] to typed AST attributes after parsing. It MUST replace each `__KEY__` placeholder with the corresponding value from the sealed `template_params`. A `tool_command` value uses the context-specific quoting rule of WG-045. Every other attribute receives the value verbatim. After substitution, any residual `__UPPER_SNAKE__` token is a launch-time error. The daemon MUST refuse to start the run and report the offending token or tokens. A run with `template_params = None` MUST execute the same typed walk with no replacements and perform the residual-token check. This is the WG-046 ordering invariant: read source → parse → substitute typed attributes → validate → dispatch.
4. **Convert AST to §6.1 `Workflow` record.** Conversion is mechanical: graph attributes (e.g., `workflow_id`, `version`, `schema_version`, `start_node`, `terminal_nodes`, `goal`) populate the §6.1 `Workflow` header; node attributes populate §6.1 `Node` fields per [workflow-graph.md §4 WG-001/WG-002]; edge attributes populate §6.1 `Edge` fields per [workflow-graph.md §6 WG-013]. The conversion produces the record that §4.10 (cascade), §4.8 (sub-workflow), and §4.9 (validation) consume.
...
7. **Return the resolved launch result.** The result contains the `Workflow` record, its `WorkflowDescriptor`, the input reference, selected graph, resolved mode, resolved review policy, and selection source. The resolver derives policy after identity validation. It may return `no_review` only for the registered `no-review-bead` version `1.0` descriptor selected through `legacy_single_label` or `queue_item_single_mode`. All other results carry `reviewed`. Only after steps 1–7 succeed MAY the daemon emit `run_started` and then begin §7.4 `execute_workflow`. A failure at any step routes the run via §7.4's `queue.mark_item_failed(item, validation_failed)` path; no `run_started` event per §4.3.EM-015a is emitted.
...
**Resume semantics.** On daemon restart, a run MUST re-execute steps 1–5 against the same selected `.dot` artifact. The daemon MUST NOT trust a serialized prior parse tree; reparse is cheap and removes a serialization surface. The `WorkflowDescriptor` produced by the post-restart reparse MUST be identical to the descriptor sealed on the Run record per §6.1. A mismatch (artifact mutated under-foot between claim time and restart) MUST route to reconciliation per [reconciliation/spec.md §8.4 Cat 3] (artifact-state divergence); the daemon MUST NOT silently proceed with the post-restart parse.
...
1. **Condition-evaluation site.** Condition evaluation runs **inside §7.3 `select_next_edge` (daemon-side)**, per the existing §7.3 pseudocode line `evaluate_condition(...)`. Handlers do not evaluate edge conditions; the cascade does. This is the same evaluation site `single` runs use. `dot` introduces no handler-side condition obligation. (Restatement only; no new requirement.)
...
The §4.9.EM-038 pre-run validator, when invoked per §7.5.1.EM-055 step 5 against a `dot`-ingested `Workflow` record, MUST enforce the following nine static checks. Any check failing causes the validator to return `false`; §7.4's `queue.mark_item_failed(item, validation_failed)` path runs and the run never starts.
...
8. **Graph identity.** The graph-level `workflow_id` MUST be present and satisfy [workflow-graph.md §10 WG-055]. The parser MUST expose it as the typed `Workflow.workflow_id` field. It MUST NOT retain it only as an unknown attribute. A missing or invalid identity prevents `run_started` and node dispatch.

9. **Review-policy binding.** A DOT attribute MUST NOT set `review_policy`.
   The resolver MUST reject that attribute. It MUST derive `no_review` only for
   the registered embedded `no-review-bead` version `1.0` descriptor selected
   through `legacy_single_label` or `queue_item_single_mode`. Every other
   graph yields `reviewed`. The run MUST NOT start if the descriptor, policy,
   and source tuple is not valid.
...
>
> **Terminal-classification sub-note (component C, precondition).** "A clean agent exit" is a decision the daemon MUST make, and it MUST make it before the HEAD derivation above. The daemon MUST classify the agent's exit with the branch rules of [claude-hook-bridge.md §4.7 CHB-020], in this order. A node whose Stop hook reported `WORK_COMPLETE` or `REVIEWER_VERDICT` has exited cleanly. A node that reported no Stop-hook outcome, exited 0, and left no progress-stream watcher error has also exited cleanly — this is the auto-close case for a harness that emits no Stop hook, such as a `CompletionProcessExit` harness per [handler-contract.md §2 N5]. Every other exit is a node failure, and the daemon MUST NOT derive `SUCCESS` from a HEAD advance after one. That covers a reported `FAILURE_SIGNAL`, a non-zero exit with nothing reported, and a watcher that could not read the progress stream. The rule is the same one the `single` path applies at its terminal switch; a `dot` node that skipped it recorded an agent which committed and then reported failure as `SUCCESS`, and the work was merged.
...
**Conformance lift.** The §10.1 carve-out that made `dot` mode a deferred extension (no dispatcher was obligated to drive a `dot` run) is removed per §10.1 / EM-061 below. Once the full phase-3-dot bundle (C1 + C2 + C3 + C4 + C5 + C6) lands, conforming implementations MUST drive `dot` mode per §7.5.1 through §7.5.4. The lift is gated on the C2-related schema bumps being recorded per §6.4: the §4.1.EM-005 `Outcome.failure_class` additive bump (per the C3 amendment) and the §4.2.EM-006 node-type-catalog breaking bump (per the C1 amendment collapsing `control-point` into `gate`/`non-agentic`).

**Parallel fan-out reservation.** `dot` mode dispatches a run's nodes sequentially; parallel-node-type semantics (a node whose dispatch fans out to multiple concurrent sub-dispatches) are deferred to a later amendment per [architecture.md §4.6]. The deferral is consistent with [workspace-model.md §4.5] and [workspace-model.md §4.7]'s one-agent-per-worktree invariant; any future parallel-fan-out amendment to §7.5 will require a coordinated amendment to workspace-model.md to lift that invariant. No `.dot` schema field, no §6.1 record field, and no §7.4 dispatch primitive is reserved for parallel-fan-out at v1.

**Cross-reference resolutions.** The placeholder text in §4.3.EM-015d ("(cross-ref pending dot-mode spec)") is rewritten to "(see §7.5 — `dot` mode binding)." The §4.3.EM-012 clause that put `dot`-mode reserved context keys out of scope is rewritten to "For `workflow_mode = dot`, the per-workflow context-key registry per [workflow-graph.md §10 WG-031] is the authoritative source of allowed `context.<key>` LHS references; this spec reserves no context keys for `dot` mode."
...
> INFORMATIVE: "Compilation loop" is named after the revision-loop pattern: fixes introduce new regressions, forming a cycle the system must bound. The term is retained despite being narrower than the phenomenon; alternate naming is permitted later via the amendment protocol per [architecture.md §4.6].
...
- **[queue-model.md §3 QM-001, QM-002]** — canonical per-name persistence and complete immutable QueueStore named-set snapshots; consumed by EM-062 through EM-065 and §7.4. Queue-model owns identity and lifecycle; this spec only consumes snapshots.
- **[queue-model.md §4 QM-010..012]** — queue identity (`QueueID`, normalized name, `QueueGroupIndex`); fields surfaced on Run records per §6.1 and on `run_*` event payloads per §4.3.EM-015a, §4.3.EM-015b.
- **[queue-model.md §5]** — per-name queue-group state machine; consumed by §4.3.EM-015f and the §7.4 dispatch loop.
- **[queue-model.md §7]** — per-name queue append semantics; consumed by EM-062 and EM-065, with `queue_appended` event emission per §6.5.
- **[queue-model.md §8]** — queue lifecycle (`paused-by-failure`, `paused-by-drain`, `completed`) and eligible active-queue filtering consumed by §7.4.
- **[queue-model.md §9 QM-060, QM-062, QM-066, QM-067]** — QueueStore serialization, global/per-queue capacity composition, per-queue `workers`, and name-ordered round-robin arbitration; §7.4 consumes these with §4.11's global capacity primitives.
...
- **EM-015d — EM-015e (review-loop routing and early-exit) — RETIRED v0.10.0, obligation WITHDRAWN.**
  These test obligations were written at v0.9.6 against the shipping review-loop driver and were
  accurate for it. The mode was retired one version later and the driver deleted, so the tests they
  require have no subject. The text is retained NON-NORMATIVE below because it is the most precise
  surviving description of how that driver actually routed verdicts, and because the fix-up-stalled /
  `no_progress` distinction it records is the evidence behind the EM-015e retirement note. Nothing
  here transfers to `dot` automatically: per §4.3.EM-015e the flagless-`REQUEST_CHANGES` normalization
  is review-loop-scoped and `dot` MUST NOT adopt it without its own amendment.
  Retired obligation text: Decision-table tests MUST cover, at minimum: raw `APPROVE`; flagless `REQUEST_CHANGES` normalized to approval (assert `completion_reason = approved`, no `needs-attention` marker, no further implementer dispatch, AND that the archived verdict artifact plus the `reviewer_verdict` event still read `REQUEST_CHANGES` with an empty `flags` array); actionable `REQUEST_CHANGES` below the cap (re-dispatch, no termination); actionable `REQUEST_CHANGES` at the cap (`iteration_cap_hit` then `cap_hit`); `BLOCK`; and malformed/missing verdict (`error`). Fix-up-stalled tests MUST assert that an unchanged task-branch HEAD after an actionable `REQUEST_CHANGES` emits `review_fixup_stalled` and terminates with `completion_reason = fixup_stalled` BEFORE any reviewer launch, and — the regression this amendment records — that a HEAD that DID advance proceeds normally even when the cumulative `parent..HEAD` diff hash is unchanged. A negative test MUST assert that the review-loop post-`REQUEST_CHANGES` stall emits `review_fixup_stalled` and NOT `no_progress_detected`, and terminates with `fixup_stalled` and NOT `no_progress`. Restart tests MUST reconstruct `iteration_count`, `last_verdict`, `claude_session_id`, `last_diff_hash`, and `last_iteration_head_sha` from durable state — see the EM-012 declared implementation gap; until it closes, this obligation is a target, not a passing claim.
...
- **EM-055 — EM-059 (`dot`-mode binding §7.5).** The following test obligations apply to the `dot`-mode ingestion and dispatch path:
  ...
  - **Validator negative cases — all nine obligations (EM-057).** For each of the nine validator checks in §7.5.3: (1) schema-version out of N-1 window fails; (2) unknown `type` value fails; (3) edge-condition LHS outside the whitelist fails; (4) unreachable node fails; (5) cycle with no capped edge fails; (6) sub-workflow reference cycle fails; (7) missing required attribute by node type fails (separate case per type: missing `handler_ref` on `agentic`; missing `handler_ref` on `non-agentic`; missing `gate_ref` on `gate`; missing `sub_workflow_ref` on `sub-workflow`; forbidden `handler_ref` on `sub-workflow`); (8) missing or invalid `workflow_id` fails; (9) a DOT `review_policy` attribute or an invalid descriptor, policy, and source tuple fails. Each negative case MUST route to `queue.mark_item_failed(item, validation_failed)` and MUST NOT emit `run_started`.
...
Owner: foundation-author
Blocks: none (current decision: no failure commits per §4.5.EM-025)
...
Owner: foundation-author
Blocks: none (defaults to additive-only)
...
| 2026-07-28 | 0.10.0 | agent (amendment `review-loop-retirement`) | **`workflow_mode = review-loop` RETIRED; the EM-012a-FLOOR mechanism changes from silent demotion to loud run failure.** The enum narrows to `{single, dot}`. **EM-012a-FLOOR AMENDED** — the floor's GUARANTEE ("a bead resolved at tier 2/3/4 is never dispatched without a review gate") is UNCHANGED; its MECHANISM is not. The floor is now delivered by (a) reviewed-by-construction — the embedded `standard-bead.dot` carries a reviewer node on the sole inbound edge to `close` per [workflow-graph.md §17 WG-047..WG-052], unchanged — and (b) LOUD FAILURE: on embedded-artifact load failure (parse, missing, schema-incompat per §7.5.3.EM-057 check 1, or template-param substitution failure) the daemon MUST fail the run with a diagnostic naming the artifact and the error, and MUST NOT dispatch under any other shape. Rationale recorded in-clause, each point verified against the implementation: the artifact is compile-time `//go:embed` and `internal/daemon/standardgraph_sync_test.go` asserts the shipped bytes parse and are byte-identical to `specs/examples/standard-bead.dot`, so a parse failure is a red suite rather than a production state; the demotion was not honestly observable because `run_started` stamps `workflow_mode` before the reassignment, so the run's start event named a mode the run did not execute; and the charter's tmux fail-fast reasoning ("announced loudly … degraded honestly — never faked") applies. **EM-015d and EM-015e RETIRED** (bodies retained NON-NORMATIVE for provenance): `review-loop` was a hand-written particular of the general `dot` graph walker (§7.5), which is the production default and already executes the same implementer→reviewer→close shape via the exemplar; the sub-clauses EM-015d-RFD and EM-015d-RIA are retired as review-loop clauses only and survive unchanged for the `dot` reviewer node. Recorded honestly: EM-015e's no-progress early-exit was never implemented by the review-loop driver (its `no_progress_detected` emitter had zero callers; the only HEAD-advance check routed to `review_fixup_stalled`), while the `dot` path does implement it — so this retirement drops a clause the code never satisfied. **Migration:** a bead still carrying `workflow:review-loop` is treated as an unknown mode per the amended EM-012a — tier 1 absent, `bead_label_conflict` emitted, resolution continues to `dot`; it degrades to the reviewed default rather than failing. Restatement-sync: §3 glossary `workflow_mode`, EM-012 Run clause, §6.1 `RECORD Run.workflow_mode`, and the §6.1 `WorkflowMode` ENUM. Companion amendments: [process-lifecycle.md] PL-004a v0.7.0, [beads-integration.md] BI-009a v0.9.0. **Relationship to the v0.9.6 catch-up amendment (landed concurrently, retained in full):** v0.9.6 documented the review loop that actually shipped — flagless-`REQUEST_CHANGES` normalization, the fix-up-stalled early exit, the `last_iteration_head_sha` reserved key, `fixup_stalled` in the completion-reason enum, and the declared gap that four of five reserved context keys have no durable carrier. None of it is reverted here; it is the accurate record of a mode that existed, and this retirement is the reason it now reads as history. The two amendments AGREE on every point of fact, including the one that matters most to this retirement — that the review-loop's `no_progress` exit was never implemented. Where a v0.9.6 clause described the retired mode it is marked retired in place rather than deleted: the §10.1 Core review-loop obligation is WITHDRAWN, and the §10.2 EM-015d/EM-015e test obligations are retained NON-NORMATIVE as the most precise surviving description of that driver's routing. v0.9.6's flagless-normalization clause already stated that the rule is review-loop-scoped and that `dot` MUST NOT adopt it without its own amendment, so `dot` routing the raw `REQUEST_CHANGES` label after this retirement is the behaviour v0.9.6 specified, not a regression introduced by it. |
| 2026-07-28 | 0.9.6 | agent (spec-drift catch-up / hk-togxq, hk-thbbv) | **CATCH-UP AMENDMENT — EM-015d/EM-015e are aligned to review-loop behavior that shipped months ago without an amendment.** Two behaviors were in the daemon and not in this spec, so a reader treating `specs/` as the oracle would have "fixed" correct code back to a replaced design. Verified against the code before amending, not taken on report. **(1) The no-progress detector was replaced by HEAD advancement (`hk-togxq`).** The `reviewed` text required the SHA-256 of `git diff <parent>..<head>` to be compared against `Run.context.last_diff_hash` and `no_progress_detected` to be emitted on equality. `emitNoProgressDetected` in `internal/daemon/reviewloop.go` has **zero call sites anywhere in the tree**; the live driver compares the current task-branch HEAD against `reviewLoopState.lastIterHeadSHA` and emits `review_fixup_stalled` (`emitReviewFixupStalled`), terminating with `ReviewLoopCompletionReasonFixupStalled` — a completion reason this spec did not contain. The rationale is that a genuine new commit whose net diff collides with a prior one is progress, and the hash test false-flagged it. `no_progress_detected` / `no_progress` are RETAINED as compatibility vocabulary: `emitDotNoProgressDetected` (`internal/daemon/dot_cascade_helpers.go`, called from `dot_cascade_core.go`) is live, so the event is real — it is only the review-loop emitter that is dead. New reserved context key `last_iteration_head_sha` (EM-012, §3 glossary, §6.1 `RECORD Run`); `last_diff_hash` DEMOTED to diagnostic evidence and explicitly non-routing. New `ENUM ReviewLoopCompletionReason` value `fixup_stalled`. Cross-spec: [event-model.md §8.1a.7] already defined `review_fixup_stalled` at v0.6.1 (`hk-m1wqp`, 2026-06-10) — this spec was the one lagging, and the two now agree. **(2) A flagless `REQUEST_CHANGES` routes to approval (`hk-thbbv`).** The `reviewed` text routed every `REQUEST_CHANGES` back to the implementer. The live driver treats `len(verdict.Flags) == 0` as an implicit APPROVE and returns `ReviewLoopCompletionReasonApproved`, because a flagless verdict names nothing to change: the implementer finds no work, quits without a commit, and the next iteration's stall guard wedges a run that is not actually wrong. Actionability is now DEFINED (non-empty `flags`) and threaded through the EM-015d cycle graph, the EM-015e cap-hit and re-dispatch clauses, and the `needs-attention` glossary entry. Normalization is ROUTING-ONLY and `review-loop`-SCOPED: the raw verdict MUST survive unrewritten in the archived artifact and the `reviewer_verdict` event, and the `dot` cascade (which ranks `REQUEST_CHANGES` at severity 1 unconditionally in `verdictSeverity`) does NOT adopt it. §10.2 gains an EM-015d/EM-015e obligation row, including the negative test that the post-`REQUEST_CHANGES` stall emits `review_fixup_stalled` rather than `no_progress_detected`, and the advanced-HEAD/colliding-hash regression. §10.1 stops asserting a fixed count of review-loop events. **Two scope limits recorded rather than papered over.** (i) The `no_progress_detected` prohibition is narrowed to the post-`REQUEST_CHANGES` path: [event-model.md §8.1a] step (c) still carries an `otherwise` branch routing a review-loop no-progress detection with no prior `REQUEST_CHANGES` to `no_progress_detected`. That branch is unreachable — the stall check runs only from iteration 2, and reaching iteration 2 proves a prior actionable `REQUEST_CHANGES` — but retiring it is an event-model amendment, so §6.5 names it as dead rather than contradicting a clause this spec does not own. (ii) A dated DECLARED IMPLEMENTATION GAP is added under EM-012: four of the five `review-loop` reserved context keys have no durable carrier at all — the four constants in `internal/core/runcontextkeys.go` have no non-comment reference anywhere, and the driver keeps the values on `reviewLoopState` — so review-loop iteration state does not survive a daemon restart. `claude_session_id` is the exception and IS durable, via `persistClaudeSessionID` committing `.harmonik/run-context/<run_id>/context.json` per CHB-023. The "durably write" MUSTs are therefore targets for the other four. `last_iteration_head_sha` joins that pre-existing gap; it does not open a new one. No requirement IDs added, renumbered, or retired; amendatory over v0.9.5; status stays `reviewed`. Prose harvested by hand from `88f36c15d` on the abandoned branch `integration/phase-reviewloop-20260725` — the clauses only, NOT that commit's `reviewed`→`draft` / v0.10.0 front-matter demotion and NOT its unlanded EM-015d-KERNEL / EM-015d-CONT / EM-015d-RVA material. Refs: hk-togxq, hk-thbbv, hk-m1wqp; `plans/2026-07-27-delete-and-rewrite/PRIOR-ART.md` §3. |
| 2026-07-28 | 0.9.5 | agent (tmux-optional-boot / hk-0cjb8) | **EM-015d-RIA step 2 declared tmux-hosting-dependent, with the substitution escape closed.** Companion to [process-lifecycle.md] v0.6.2, which withdrew the daemon's hard `$TMUX` fail-fast (operator direction 2026-07-28 narrowing locked decision #4). Step 2 spawns the reviewer pane via `tmux new-window` as an unconditional MUST; now that PL-021b item 3 / PL-028b let the daemon boot with tmux hosting unresolved, that MUST needed a stated disposition or it would have been read either as newly optional or as a contradiction. It is neither: **the step does NOT become optional, and the review loop is therefore one of the capabilities PL-028b item 2 obliges the daemon to announce as lost at startup.** Substituting a non-tmux substrate for the reviewer is named explicitly as the "silently degrading to non-tmux mode" that PL-021a forbids — the reviewer's harness is pinned deliberately, and a substitution changes which harness reviews the work — so the daemon MUST refuse or announce, never substitute. A dated DECLARED IMPLEMENTATION GAP records that a no-tmux boot currently leaves the reviewer substrate nil and the work loop treats nil as "fall back to the general substrate", performing exactly that substitution, while the boot announcement names neither (`hk-p01zm`); recorded, not fixed. Steps 1 and 3, the ordering invariant (file exists → session live → input delivered), the AIS input-delivery migration, and every other EM-015d obligation are UNCHANGED. No EM IDs added, renumbered, or retired. Refs: `1f8781730`, `hk-0cjb8`, `hk-p01zm`. |
| 2026-07-27 | 0.9.4 | agent (`queue-transaction-contract`) | EM-015f clarifies the normal-path `queue_group_completed` attempt after authoritative state commit, with no crash-surviving delivery or restart synthesis; EM-062 through EM-065 align execution consumption with a complete QueueStore named set: all-queue duplicate pre-screen, per-name submit, empty-set-only `br ready`, global plus per-queue capacity gates, and QM-067 round-robin cursor advance. EM-066 and EM-067 now bind fallback ownership to the complete named-set-empty branch; §10.2 enables its pause fixture with `--auto-pull` and adds the nonempty-ineligible no-fallback case. |
...
| 2026-07-14 | 0.9.2 | agent (agent-input-substrate / M2) | **EM-015d review-loop input delivery MIGRATES from tmux paste-inject to the AIS structured input port.** M2 agent-input-substrate makes tmux observation-only for daemon runs and moves ALL daemon-run input delivery to the new `specs/agent-input.md` (AIS) structured `InputPort`. Review-loop resume instructions ARE daemon-run input, so they migrate — they are NOT a preserved carve-out (only the session-keeper and the interactive CLI nudge survive on PL-021d). **EM-015d-RFD step 2** (retitled "Deliver the read instruction via the structured input port"): the implementer-resume read instruction is now delivered via [agent-input.md §4.1 AIS-001] `InputPort.SubmitInput` returning a real `Ack` (AIS-003/AIS-004, bounded-liveness AIS-INV-001) instead of the tmux `load-buffer` / `paste-buffer` paste-inject; the failure path keys off a non-positive ack / `agent_input_stale` rather than a tmux paste error; ordering invariant restated as **file exists → session is live → input delivered**. **EM-015d-RIA** intro + **step 3** (retitled "Deliver the start instruction via the structured input port"): the reviewer start instruction likewise moves to `SubmitInput` → `Ack`. A consolidated **input-delivery mechanism note** is added to EM-015d-RIA: the "PL-021b-PASTE" mechanism label is SUPERSEDED by AIS for the daemon-run input path (cross-ref the [process-lifecycle.md §4.3 PL-021d] demotion clause — PL-021d retained only for keeper (SK-002) + interactive CLI nudge); spawn verbs (`tmux new-window` at RIA step 2 / PL-021b) and `capture-pane` observation (AIS-011) remain, the input direction does not. Only the delivery MECHANISM changes: the WHAT delivered (read/start instruction, review-target, resume brief), the review-loop control flow, iteration semantics, and all non-input-delivery requirements are UNCHANGED. Front-matter version 0.9.0→0.9.2 (catching up past the un-front-matter'd 0.9.1 EM-068 row), last-updated → 2026-07-14. No EM IDs added, renumbered, or retired. Refs: agent-input-substrate M2 integration-sweep finding F2 (D5/D6/D10). |
| 2026-06-13 | 0.9.1 | agent (hk-2j90) | **New EM-068 (§7.5.6) deny-side outcome-derivation gate (C1 work-product inspection + C2 validated marker; deny-side only; daemon authority EM-005c/HC-059; no reviewer-loop re-entry; mechanism-tagged zero-LLM per EM-039/AR-006). EM-027 (§4.6) Outcome-spine additive sentence. Refs: hk-2j90.** |
| 2026-06-11 | 0.9.0 | agent (kerf work `standard-bead-dot` / epic hk-o7j) | **Tier-4 built-in workflow_mode default flips `single` → `dot` (with the embedded `standard-bead.dot` graph); adds the EM-012a-FLOOR review-floor guarantee.** Amended **§4.3.EM-012a** tier 4: the v1.0 built-in fallback is now `workflow_mode = dot`, resolving the embedded `standard-bead.dot` canonical exemplar (cross-ref [workflow-graph.md §12 WG-036 — canonical example workflows], the on-disk `specs/examples/standard-bead.dot`, and the build-embedded `internal/daemon/standard-bead.dot`). `single` is no longer the tier-4 fallback; it is reachable ONLY via an explicit tier-1 `workflow:single` per-bead label (audited via `review_bypassed`) and MUST NOT resolve at tier 3 or tier 4. New sub-requirement **EM-012a-FLOOR** (review-floor guarantee): on embedded-`standard-bead.dot` load failure (parse failure, missing artifact, or §7.5.3.EM-057 check-1 schema incompatibility) the daemon MUST fall back to `review-loop`, NEVER to `single` — a default-resolved bead is NEVER dispatched without a review gate; fallback ordering is `dot → review-loop`, not `dot → single`. Tiers 1–3 preserved verbatim. Added a NORMATIVE CROSS-REFERENCE note binding default-`dot` execution to the §7.5 `dot`-mode binding and to [sub-workflow-dispatch.md] (sub-workflow nodes within DOT workflows) for sub-workflow-node dispatch inside the exemplar. Restatement-sync (not new content): EM-012 Run-clause and §6.1 `RECORD Run.workflow_mode` comment updated from "defaults to `single`" to the v1.0 `dot` default + `review-loop` floor, matching the now-authoritative EM-012a. This makes NORMATIVE the dot-default intent that the landed engine already implements (`internal/daemon/moderesolve.go` tier-4 returns `WorkflowModeDot`; `internal/daemon/workloop.go` embedded-`standard-bead.dot` pre-load with the review-loop floor); prior dot-default intent recorded under kerf work `dot-default` / hk-30vlb. No prior requirement IDs renumbered or retired; new sub-clause EM-012a-FLOOR assigned under EM-012a. Refs: hk-o7j, hk-30vlb. |
...
**Why failure commits are deferred.** Harmonik does not have an improvement loop that needs `git bisect` over failures. Adding failure commits pre-emptively would bloat the checkpoint trail on routine failure (timeouts, rate limits) and complicate the reconciliation detectors' "last durable state" discovery. The design slot is preserved per §4.5.EM-025 so the additive change is cheap if needed.
