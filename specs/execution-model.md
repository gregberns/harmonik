...
- **workflow_mode** — a `Run`-record field carrying one of `{single, dot}` (`review-loop` RETIRED v0.10.0 per §4.3.EM-015d). Resolved at claim time per §4.3.EM-012a; immutable for the run's lifetime. Selects the run's dispatch shape: `single` is the historical one-handler-per-node default, reachable only via an explicit per-bead `workflow:single` label; `dot` is the general workflow-graph walker defined in §7.5 (binding document for `workflow_mode = dot`) and the default. (see §4.3.EM-012, §4.3.EM-012a, §7.5)
...
A `Run` MUST carry a stable `run_id`, `workflow_id`, `workflow_version`, `input` (a workspace reference per [workspace-model.md §4.1], not inline payload), `workflow_mode ∈ {single, dot}` (`review-loop` RETIRED v0.10.0 per §4.3.EM-015d; resolved at claim time per §4.3.EM-012a; immutable for the run's lifetime; the tier-4 built-in default is `dot` at v1.0 per §4.3.EM-012a, whose review-floor is delivered by the exemplar's reviewer node plus a LOUD RUN FAILURE — never `single`, never a silent change of shape — on embedded-artifact load failure per EM-012a-FLOOR), current `state`, `context` (a shared key-value map updated per §4.10.EM-041a), `start_time`, and optional `end_time`. A run executes EXACTLY ONE workflow invocation against EXACTLY ONE input; multi-workflow or multi-input runs are not permitted. Transition records for the run are discoverable via the task-branch commit range whose commits carry the run's `Harmonik-Run-ID` trailer; no separate `transitions` field on the `Run` record is required.
...
1. **Per-task.** The bead's `workflow:<mode>` label per [beads-integration.md §4.3 BI-009a]. Accepted values: `workflow:single`, `workflow:dot` (`workflow:review-loop` RETIRED v0.10.0 — see §4.3.EM-015d). If the bead carries more than one `workflow:<mode>` label, the daemon MUST treat tier 1 as absent AND MUST emit `bead_label_conflict` per [event-model.md §8.8] before continuing the walk.
...
4. **Built-in fallback.** `dot`. At v1.0 the tier-4 built-in fallback workflow_mode is `dot` — NOT the historical `single`. The default DOT workflow this fallback resolves is the embedded `standard-bead.dot` artifact (the canonical exemplar registered with the daemon per §7.5.1.EM-055 step 1 and §7.5.5.EM-059; the normative topology contract for this exemplar — its six-node catalog, ten edges, traversal caps, and the SOLE-inbound-edge-to-`close` review-floor invariant — is [workflow-graph.md §17 WG-047..WG-052 — canonical exemplar: `standard-bead.dot`]; cross-ref also [workflow-graph.md §12 WG-036 — canonical example workflows] and the on-disk artifact at `specs/examples/standard-bead.dot`, whose build-embedded copy at `internal/daemon/standard-bead.dot` is the runtime default). `single` is NOT the tier-4 fallback at v1.0 and is reachable ONLY via an explicit tier-1 per-bead `workflow:single` label (which the daemon MUST audit per the `review_bypassed` event); the daemon MUST NOT resolve `single` at tier 3 or tier 4. This flips the historical `single` tier-4 default fixed in v0.4.0 (EM-012a); the prior dot-default intent is recorded under kerf work `dot-default` / hk-30vlb, and this amendment makes that default NORMATIVE.
...
RECORD Run:
    ...
    workflow_mode      : WorkflowMode         -- one of {single, dot} (review-loop RETIRED v0.10.0); resolved at claim per §4.3.EM-012a; immutable for run lifetime; tier-4 built-in default is `dot` at v1.0 (review-floor: reviewed-by-construction, and LOUD FAILURE — never `single`, never a silent change of shape — on embedded-artifact load failure) per §4.3.EM-012a / EM-012a-FLOOR
...
```
ENUM WorkflowMode:
    single                      -- one-handler-per-node default (Core); applies to ordinary workflow graphs
    -- review-loop RETIRED v0.10.0 (was: hardcoded two-node implementer→reviewer cycle per §4.3.EM-015d)
    dot                         -- general workflow-graph walker; a deferred extension; out of scope for Core conformance
```
...
4. **Convert AST to §6.1 `Workflow` record.** Conversion is mechanical: graph attributes (e.g., `workflow_id`, `version`, `schema_version`, `start_node`, `terminal_nodes`, `goal`) populate the §6.1 `Workflow` header; node attributes populate §6.1 `Node` fields per [workflow-graph.md §4 WG-001/WG-002]; edge attributes populate §6.1 `Edge` fields per [workflow-graph.md §6 WG-013]. The conversion produces the same §6.1 record shape that §4.10 (cascade), §4.8 (sub-workflow), and §4.9 (validation) already consume for `single` runs.
