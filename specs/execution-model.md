...
A `Run` MUST carry a stable `run_id`, `workflow_id`, `workflow_version`, `input` (a workspace reference per [workspace-model.md §4.1], not inline payload), `workflow_mode ∈ {single, review-loop, dot}` (resolved at claim time per §4.3.EM-012a; immutable for the run's lifetime; the tier-4 built-in default is `dot` at v1.0 per §4.3.EM-012a, with a `review-loop` review-floor — NEVER `single` — on embedded-artifact load failure per EM-012a-FLOOR), current `state`, `context` (a shared key-value map updated per §4.10.EM-041a), `start_time`, and optional `end_time`. A run executes EXACTLY ONE workflow invocation against EXACTLY ONE input; multi-workflow or multi-input runs are not permitted. Transition records for the run are discoverable via the task-branch commit range whose commits carry the run's `Harmonik-Run-ID` trailer; no separate `transitions` field on the `Run` record is required.
...
RECORD Run:
    ...
    workflow_mode      : WorkflowMode         -- one of {single, review-loop, dot}; resolved at claim per §4.3.EM-012a; immutable for run lifetime; tier-4 built-in default is `dot` at v1.0 (review-floor `review-loop`, never `single`, on embedded-artifact load failure) per §4.3.EM-012a / EM-012a-FLOOR
