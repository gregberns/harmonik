...
- **durable transition** — a transition satisfying the decision procedure of §4.5.EM-023a: `transition_kind ∈ {forward, local-patchback, architectural-rollback, policy-rollback, context-restore, context-checkpoint}` AND `outcome.status ∈ {SUCCESS, PARTIAL_SUCCESS}`. Durable transitions MUST be checkpointed per §4.5. (see §4.5.EM-023a)
...
- **claude_session_id** — the compatibility-named authoritative harness continuity identifier consumed to resume or Retask the same implementer session across subprocess launches; for a non-Claude harness it still carries that harness's native continuity ID. A `Minted` harness knows the identifier before first work; a `Captured` harness learns it from the initial process after launch, using the exact policy enum of [harness-contract.md §4 HN-008]. Both policies persist the identifier in `Run.context.claude_session_id` through the continuity-checkpoint transaction of §4.3.EM-015d-CONT before any later resume or Retask. Distinct from harmonik's internal `session_id` event field per [event-model.md §8.3], which identifies one handler launch. The two identifiers MUST NOT be conflated.
- **iteration_count** — under `review-loop` mode, the 1-based current implementer→reviewer iteration. Stored in `Run.context.iteration_count`; initialized to 1 and capped at 3 per §4.3.EM-015e. (see §4.3.EM-015d, §4.3.EM-015e)
...
- **needs-attention** — under `review-loop` mode, the operator-drained close-path marker selected by the normalized cycle outcome, not by the raw verdict alone. It applies to `cap_hit`, `blocked`, `fixup_stalled`, and `error` outcomes (including cancellation, malformed/missing verdict, and phase failure), but not to `approved`, including a raw flagless `REQUEST_CHANGES` normalized to approval. No auto-retry occurs. The Beads label and write semantics are owned by [operator-nfr.md §4.3] and [beads-integration.md §4.4]; this spec names when the close path is selected.
...
When `workflow_mode = review-loop`, the run's `context` map MUST carry, at minimum, the reserved keys `iteration_count` (Integer, 1..3, incremented per §4.3.EM-015d), `last_verdict` (the raw validated value from `{APPROVE, REQUEST_CHANGES, BLOCK}`, or `null` before iteration 1's reviewer returns), `claude_session_id` (the authoritative implementer continuity identifier obtained and persisted per §4.3.EM-015d-CONT), `last_diff_hash` (String, the SHA-256 of `git diff <parent>..<head>` output at the end of the prior implementer phase; diagnostic/event evidence only), and `last_iteration_head_sha` (String, the task-branch HEAD recorded after the prior implementer phase; the progress-routing baseline per §4.3.EM-015e). These keys are normative for `review-loop`; their absence in a `review-loop` run is an authoring error detected at claim time. Every change to these keys MUST enter the next durable review-loop checkpoint before a dependent implementer resume, reviewer launch, or terminal transition proceeds; an in-memory-only value does not authorize resume or restart reconstruction. For `workflow_mode = single`, no `context` keys are reserved by this spec. For `workflow_mode = dot`, the per-workflow context-key registry per [workflow-graph.md §10 WG-031] is the authoritative source of allowed `context.<key>` LHS references; this spec reserves no context keys for `dot` mode.
...
A run with `workflow_mode = review-loop` (per §4.3.EM-012) MUST execute a hardcoded two-node cycle: `implementer → reviewer → {APPROVE: close, actionable REQUEST_CHANGES: implementer, BLOCK: close-needs-attention, iteration-cap: close-needs-attention, fixup-stalled: close-needs-attention}`. A `REQUEST_CHANGES` verdict is actionable when `flags` is non-empty; a flagless `REQUEST_CHANGES` is normalized for routing to approval while its raw verdict remains unchanged in durable artifacts and events. The graph is fixed by this spec and is NOT obligated to be expressed as a DOT document at MVH; the v1 driver is mode-specific code on the daemon's claim and dispatch path. This sub-graph shape is representable as an instance of the general workflow-graph model defined for `dot` mode (see §7.5 — `dot` mode binding).

**EM-015d-KERNEL — Deterministic cycle projection.** Review-loop routing MUST be expressible as a pure projection

`CycleState + Observation -> CycleDecision{next_state, ordered_cycle_intents, terminal_result?}`.

`CycleState` contains only iteration/cap, authoritative implementer continuity identity, prior work-product HEAD, current/prior diff hashes as evidence, raw prior verdict/flags/notes, and the exact initial work-product baseline SHA. `Observation` contains only validated phase result, resulting HEAD and work-product advancement, diff hash, validated verdict, cancellation, or typed failure. The projection MUST NOT perform git, filesystem, event-emitter, clock, process, network, goroutine, cleanup, checkpoint, or bead mutation. Those effects remain with their owning shells. After cycle entry, every terminal observation MUST produce exactly one terminal cycle decision; no later observation may advance a terminal decision.
...
- **Same authoritative implementer continuity identity across iterations (EM-015d-CONT).** The daemon MUST obtain and persist the implementer's continuity identity according to the harness policy declared by [harness-contract.md §4 HN-008, HN-011], then reuse exactly that identity on every subsequent implementer resume or Retask. The implementer subprocess and handler `session_id` are fresh per iteration; the harness continuity identity is stable.

  For `Minted`, the sequence MUST be: (1) obtain the authoritative identity before first work; (2) inspect committed `Run.context` state; (3) reuse an identical committed mapping or reject a different mapping as a typed continuity conflict; (4) persist a new mapping through an EM-023a `context-checkpoint` durable transition and return that exact checkpoint SHA as the no-commit baseline; (5) only after checkpoint success, send the negotiated `version_selected` control message, when the selected protocol has one, and permit first work. The selected negotiated protocol version MUST be an explicit transaction input and `version_selected` MUST carry that same version.

  For `Captured`, the sequence MUST be: (1) launch the initial process without substituting a tracking or minted UUID; (2) observe the first authoritative native identity; (3) inspect and persist the mapping through the same continuity-checkpoint transaction; (4) return the resulting checkpoint SHA as the baseline; (5) complete persistence before any resume or Retask. Captured persistence cannot gate initial work because the identity does not exist before launch. A Captured driver has no CHB `version_selected` message unless its own selected protocol defines one.

  The continuity-checkpoint transaction MUST be awaitable and location-explicit. Its result MUST contain the authoritative identity, checkpoint SHA/exact baseline, whether an identical committed mapping was reused, and `version_selected` disposition (`not_applicable` or `sent`). Working-tree-only state is not durable. Missing identity, checkpoint failure, or conflicting committed identity MUST fail closed; checkpoint failure MUST withhold `version_selected` and first work on Minted paths. `version_selected` write or finalization failure is a terminal handshake failure. `HookRelayAck` and Agent Input `Ack` are separate contracts. Event projection is outside transaction success and cannot invalidate an already-committed mapping. Local and remote run workspaces MUST provide identical transaction semantics.
- **Reviewer launches are fresh per iteration.** Each reviewer launch is its own Claude session; the reviewer MUST NOT be resumed across iterations. The reviewer runs in the mandatory box-A projection and writes `${reviewer_projection_path}/.harmonik/review.json` (placement normative per [workspace-model.md §4.7 WM-027a]); this spec consumes the validated verdict for routing only after its required archive transfer.
- **Per-iteration state.** After the implementer phase and before launching the reviewer, the daemon MUST resolve current task-branch HEAD, compute `last_diff_hash`, and durably write the values into `Run.context.last_iteration_head_sha` and `Run.context.last_diff_hash`. HEAD is the routing predicate; diff hash is evidence only. After the reviewer verdict lands, the daemon MUST read and validate `${reviewer_projection_path}/.harmonik/review.json` locally against the `agent-reviewer` JSON schema v1, atomically transfer it to the authoritative `${run_workspace_path}/.harmonik/review.iter-<N>.json` archive (where `<N>` is the current `iteration_count`) per WM-027a, and durably update `Run.context.last_verdict` to the raw verdict value before a dependent resume or terminal transition.
- **Per-iteration events.** Each implementer resume MUST emit `implementer_resumed` per [event-model.md §8.1a]; each reviewer launch MUST emit `reviewer_launched`; the verdict-file read MUST emit `reviewer_verdict` carrying the raw agent-reviewer schema v1 fields verbatim plus `run_id`, `session_id`, and `iteration_count`. On an unchanged post-`REQUEST_CHANGES` HEAD per §4.3.EM-015e, the daemon MUST emit `review_fixup_stalled`; on cap-hit it MUST emit `iteration_cap_hit`. On cycle termination after cycle entry, the daemon MUST emit exactly one `review_loop_cycle_complete` per [event-model.md §8.1a] carrying the normalized `completion_reason`. The historical `no_progress_detected`/`no_progress` vocabulary remains readable for existing traces and separately owned DOT/generic paths but is not the post-`REQUEST_CHANGES` review-loop route.
...
- **Reviewer input artifact (EM-015d-RIA).** Before launching the reviewer pane for any iteration (including iteration 1), the daemon MUST create the exact-SHA box-A reviewer projection required by [workspace-model.md §4.7 WM-027a], then write the reviewer input artifact at `${reviewer_projection_path}/.harmonik/review-target.md` (path normative per [workspace-model.md §6.2 WM-RIA-001]). The projection is short-lived, unleased, and distinct from the authoritative run workspace. This file is the reviewer's sole structured context source; the reviewer is told, via the structured input port of [agent-input.md §4.1 AIS-001] (`InputPort.SubmitInput` → real `Ack` per AIS-003/AIS-004; the former "PL-021b-PASTE" paste-inject is DEMOTED for the daemon-run path per [process-lifecycle.md §4.3 PL-021d]), to read it and produce projection-staging `review.json` per [claude-hook-bridge.md §4.5 CHB-014]. The daemon MUST execute the following ordered steps:

  1. **Write `review-target.md` atomically.** The daemon MUST write `${reviewer_projection_path}/.harmonik/review-target.md` containing:
     ...
     - **Diff section** (`## Diff range`): the base SHA (`Run.context.parent_commit` or the run's task-branch creation point) and the head SHA (current `HEAD` of the task branch at the moment the reviewer is launched), formatted as:
       ...
       head: <head_sha>
       ```
     - **Prior verdicts section** (`## Prior verdicts`) (OMITTED when `iteration_count = 1`): for each prior iteration `k` from 1 to `iteration_count - 1`, a sub-heading `### Iteration <k>` followed by the authoritative archive reference `Verdict file: run-workspace:.harmonik/review.iter-<k>.json` and the one-line summary `verdict: <VERDICT>  flags: <flags-list or "(none)">  notes: <first 200 chars of notes field, truncated with "…" if longer>`. The full text remains in the run-workspace archive; the summary line is reviewer context and a navigation aid.
  ...
  `review-target.md` MUST be excluded from checkpoint commits via the [workspace-model.md §4.7 WM-013e] `.gitignore` hygiene set; it is workflow-control state, not work product. It lives only in the reviewer projection and is removed with that projection after the reviewer session, observers, validation, and required archive transfers quiesce per WM-027a. The run-workspace `review.iter-<N>.json` files are the durable record; `review-target.md` is staging input for one reviewer phase.
...
- **Reviewer-verdict allowance (EM-015d-RVA).** The reviewer watchdog is review-loop execution and process-lifetime policy, not a ControlPoint budget. Its deterministic policy MUST distinguish:

  1. **Diff allowance** — an initial size-derived window beginning when reviewer brief delivery completes. With shipped defaults, `base = 10 minutes`, `per_kline = 10 minutes`, and `hard_ceiling = 60 minutes`; for `changed_lines > 0`, `allowance = clamp(base + per_kline × changed_lines / 1000, base, hard_ceiling)`, using proportional integer-safe duration arithmetic. An unknown, zero, or negative changed-line count yields `min(base, hard_ceiling)`.
  2. **Liveness grace** — bounded postponement after the diff allowance expires. A live pane may extend in increments of `base` up to the absolute hard ceiling. A recent `agent_heartbeat` without a live-pane observation may extend in increments of `base` only up to `min(2 × allowance, hard_ceiling)`. “Recent” means within the shipped `10 minute` heartbeat-active window. Pane presence and heartbeat, regardless of emitter, are liveness evidence only: neither resets/replenishes the diff allowance nor counts as reviewer-work progress.
  3. **Reviewer-work activity** — affirmative agent-derived work evidence. At this revision the only work-completion evidence consumed by this allowance is a complete, valid verdict artifact; no speculative intermediate progress signal is introduced.

  If no valid verdict exists after the applicable ceiling, the observer MUST record `budget-exceeded`, `heartbeat-ceiling`, or `hard-ceiling` as appropriate, write the reviewer-budget sentinel, request graceful quit, and apply the existing bounded kill policy. A valid verdict observed in the same policy evaluation wins over exhaustion. The sentinel and `reviewer_budget_exceeded` diagnostic MUST agree on allowance, changed-line count, elapsed time, and reason.

  The shipped one-shot reviewer reseed remains part of this policy: after `75 seconds` from allowance start, when no valid verdict exists and the reviewer pane is active or liveness is not observable, the observer MAY redeliver the reviewer brief exactly once; reseeding does not reset any allowance or ceiling.

  The policy MUST be expressible as a pure transition over configuration, changed-line count, current time, verdict presence, pane liveness, heartbeat recency, and reseed state, returning one of `continue`, `extend_to_deadline`, `reseed_once`, `verdict_complete`, or `budget_exceeded` with diagnostic values. The phase-owned imperative observer owns timers, event subscription, verdict I/O, pane probing, reseed delivery, quit/kill, sentinel/event emission, cancellation, and join. It MUST quiesce under the phase-close contract before reviewer-workspace release. DOT reuse and ControlPoint registration are outside this clause.

  Tags (sub-clause EM-015d-RVA): mechanism
  Axes: llm-freedom=none; io-determinism=deterministic; replay-safety=safe; idempotency=non-idempotent

- **Implementer work-product commit obligation is review-loop-scoped.** Under `workflow_mode = review-loop`, the implementer phase MUST advance the worktree HEAD with at least one work-product commit before the reviewer is launched. For iteration 1, advancement is measured from the exact checkpoint SHA returned by EM-015d-CONT; the continuity/control checkpoint itself does not satisfy this obligation. For later iterations, advancement is measured from `Run.context.last_iteration_head_sha`. A clean implementer exit without such advancement is a cycle-internal failure: on iteration 1 it routes through `error`; after actionable `REQUEST_CHANGES` it routes through `fixup_stalled` per §4.3.EM-015e. This obligation is specific to `review-loop` mode and does NOT apply to `workflow_mode = dot`. A `dot`-mode `agentic` node MAY relax the HEAD-advance requirement via the per-node `non_committing` attribute per [workflow-graph.md §4 WG-041]; that relaxation is gated on the per-node attribute and never reaches the `review-loop` path.
...
- **Flagless REQUEST_CHANGES normalization.** On a validated `REQUEST_CHANGES` verdict whose `flags` array is empty, the daemon MUST preserve and archive the raw verdict and MUST emit `reviewer_verdict{verdict=REQUEST_CHANGES, flags=[]}`, but MUST normalize the routing decision to `APPROVE`. The run terminates as `completed` with `review_loop_cycle_complete{completion_reason=approved}`; no further implementer iteration is dispatched. The raw verdict plus normalized cycle completion are the audit representation; the daemon MUST NOT rewrite the verdict artifact or event to `APPROVE`.
- **Actionable REQUEST_CHANGES with iterations remaining.** On a reviewer verdict of `REQUEST_CHANGES` with a non-empty `flags` array AND `iteration_count < 3`, the daemon MUST durably increment `iteration_count`, retain the prior diff hash as evidence, and re-dispatch the implementer with the authoritative continuity identity per §4.3.EM-015d-CONT. The run MUST NOT terminate.
- **Cap-hit (actionable REQUEST_CHANGES at iteration 3).** On a reviewer verdict of `REQUEST_CHANGES` with a non-empty `flags` array AND `iteration_count = 3`, the daemon MUST emit `iteration_cap_hit{final_verdict=REQUEST_CHANGES}` per [event-model.md §8.1a] and MUST terminate the run via the `needs-attention` close path. Bead-side, the run's terminal-transition write per [beads-integration.md §4.4 BI-010] MUST apply the `needs-attention` label per [operator-nfr.md §4.3].
...
- **Fix-up-stalled early-exit.** Before each reviewer launch from iteration 2 onward, the daemon MUST resolve current task-branch HEAD and compare it with the durably recorded prior `Run.context.last_iteration_head_sha`. Reaching iteration 2 or later is structural proof that the prior routing verdict was actionable `REQUEST_CHANGES`. If HEAD is unchanged, the daemon MUST emit `review_fixup_stalled` carrying the prior reviewer flags and diagnostic current/prior diff hashes, then MUST terminate through the `needs-attention` close path with `completion_reason=fixup_stalled` BEFORE launching a reviewer for the current iteration. If HEAD advanced, the cycle proceeds even when the cumulative diff hash equals the prior hash; diff-hash equality is not a review-loop routing predicate.
- **Initial work-product failure.** Before iteration 1's reviewer launch, task-branch HEAD MUST have advanced beyond the exact baseline SHA returned by EM-015d-CONT with at least one work-product commit. A control/checkpoint-only advance does not satisfy the obligation. Failure routes through the cycle `error` path without launching the reviewer.

The terminal `run_completed` or `run_failed` event per §4.3.EM-015b MUST be preceded by exactly one `review_loop_cycle_complete` event per §4.3.EM-015d. The `review_loop_cycle_complete` payload's `completion_reason` MUST match the terminal decision: `approved` for raw APPROVE or normalized flagless REQUEST_CHANGES, `cap_hit` for actionable REQUEST_CHANGES at the cap, `blocked` for BLOCK, `fixup_stalled` for unchanged HEAD after actionable REQUEST_CHANGES, and `error` for daemon/phase/cancellation/verdict failures observed after cycle entry. `no_progress` remains in the compatibility grammar for historical traces and separately owned DOT/generic paths; the built-in review-loop post-REQUEST_CHANGES path MUST use `fixup_stalled`.
...
|---|---|
| `transition_kind` | `forward`, `local-patchback`, `architectural-rollback`, `policy-rollback`, `context-restore`, `context-checkpoint` |
...
`context-restore`, `context-checkpoint`, and reconciliation-directed transitions are daemon-produced, not handler-produced. For these, the daemon synthesizes an `Outcome` with `status = SUCCESS` and `actor_role ∈ {daemon, reconciliation}` per §4.10.EM-046; EM-023a applies unchanged. The synthesized Outcome is recorded in the `Transition` record's evidence map under `evidence.synthesized_outcome=true`. A `context-checkpoint` records a context-only durability boundary without relocating graph state; its evidence MUST include `evidence.context_checkpoint=true` and a `checkpoint_purpose` discriminator. For EM-015d-CONT the discriminator is `session_continuity`, and the committed SHA is the exact baseline returned by the continuity transaction.
...
A `Transition` MAY carry a `transition_kind ∈ {forward, local-patchback, architectural-rollback, policy-rollback, context-restore, context-checkpoint}` field and an optional `rollback_to_state_id` field. Forward transitions MUST omit `rollback_to_state_id`; `architectural-rollback` and `policy-rollback` MUST populate it with the target earlier state ID. The `local-patchback`, `context-restore`, and `context-checkpoint` kinds MUST omit `rollback_to_state_id` (they do not relocate the run's graph position). The hybrid shape (kind tag + optional pointer) remains the canonical rollback representation; `context-checkpoint` is a durability boundary, not a fifth rollback type.
...
A `context-checkpoint` transition durably records a context-only update without changing the run's graph state and without claiming a restore or rollback. It MUST set `from_state_id == to_state_id`, omit `rollback_to_state_id`, use a daemon-synthesized `SUCCESS` outcome, and carry the evidence required by §4.5.EM-023a. After the durable reference advance it emits the normal checkpoint and `transition_event` facts, but MUST NOT emit `state_exited` or `state_entered`. EM-015d-CONT uses this kind to persist implementer continuity before `version_selected`/resume and to establish the exact no-commit baseline.
...
ENUM ReviewLoopCompletionReason:
    ...
    no_progress                 -- compatibility value for historical traces and separately owned DOT/generic paths; not emitted for the built-in review-loop post-REQUEST_CHANGES route
    fixup_stalled               -- post-REQUEST_CHANGES implementer left task-branch HEAD unchanged; run terminates needs-attention before reviewer launch per §4.3.EM-015e
...
- Review-loop cycle (when `workflow_mode = review-loop`, with DOT reuse only where Event Model explicitly permits it) — `implementer_resumed` (on every implementer-launch after the first), `reviewer_launched` (on every reviewer-launch), `reviewer_verdict` (after `.harmonik/review.json` is read and validated; carries the raw agent-reviewer JSON schema v1 fields verbatim), `iteration_cap_hit` (only on actionable REQUEST_CHANGES at the cap), `no_progress_detected` (compatibility/DOT-generic no-progress signal; not the built-in post-REQUEST_CHANGES review-loop route), `review_fixup_stalled` (unchanged HEAD after actionable REQUEST_CHANGES), and `review_loop_cycle_complete` (exactly once per entered cycle, before the terminal `run_completed` / `run_failed`).
...
4. **Review-loop artifacts are not applicable.** The `${run_workspace_path}/.harmonik/reviewer-feedback.iter-<N>.md` artifact per §4.3.EM-015d (sub-clause EM-015d-RFD) and the `${reviewer_projection_path}/.harmonik/review-target.md` artifact per §4.3.EM-015d (sub-clause EM-015d-RIA) are `review-loop`-mode-only. A `dot` run MUST NOT produce them; their absence on a `dot` run is not an authoring error and MUST NOT be flagged by the §7.5.3 validator. (The EM-015d cross-ref placeholder is resolved by §7.5.)
...
**Core MVH (amended — EM-061).** An implementation conforming to Core MVH MUST pass every requirement in EM-001 through EM-046 (including sub-requirements EM-012a, EM-015a, EM-015b, EM-015c, EM-015d, EM-015d-KERNEL, EM-015d-CONT, EM-015d-RFD, EM-015d-RIA, EM-015d-RVA, EM-015e, EM-015f, EM-017a, EM-018a, EM-020a, EM-023a, EM-024a, EM-025a, EM-031a, EM-034a, EM-034b, EM-034c, EM-036a, EM-041a, EM-042a, EM-043a, EM-046a, EM-046b) and EM-049 through EM-054 (concurrency primitives §4.11, merge-to-main §4.12), **and EM-055 through EM-059 (`dot`-mode binding §7.5)**, **and EM-066 through EM-067 (no-auto-pull topology + operator-pause fallback gate §4.11)**, plus invariants EM-INV-001, EM-INV-004, and EM-INV-005 (the three invariants surviving the §5 selection test; EM-INV-002, EM-INV-003, EM-INV-006 are retired). For `workflow_mode = single` (the default per §4.3.EM-012a built-in fallback), the cascade (§4.10), checkpoint cadence (§4.5), and sub-workflow composition (§4.8) apply unchanged. For `workflow_mode = review-loop`, the deterministic cycle projection, policy-specific continuity checkpoint, reviewer allowance, cap, and early-exit rules of §4.3.EM-015d/e MUST be observed; all seven §6.5 review-loop event types MUST follow their Event Model emission conditions. **For `workflow_mode = dot`, the input contract (§7.5.1.EM-055), dispatch equivalence (§7.5.2.EM-056), validator obligations (§7.5.3.EM-057), and node-type dispatch table (§7.5.4.EM-058) MUST be observed.** Dispatch input MUST be the active queue per §7.4. Queue-only is the default for all topologies (per §4.11.EM-066 — hk-8vy18); a bare boot with no submitted queue MUST dispatch zero runs. When `--auto-pull` is set (the historical single-daemon topology opt-in), the `br ready` fallback is a conforming opt-in, gated on operator-pause state per §4.11.EM-067. The `--no-auto-pull` flag is accepted as a no-op back-compat alias. The EM-007 amendment per §4.2 (admitting `handler_ref` on `non-agentic` and `gate` nodes) is normative at Core MVH.
...
- **EM-015d — EM-015e (review-loop semantics, continuity, and allowance).** Exhaustive table tests MUST cover initial no-work-product, resumed unchanged-HEAD stall, raw APPROVE, flagless REQUEST_CHANGES normalized to approval, actionable REQUEST_CHANGES retry and cap, BLOCK, malformed/missing verdict, phase error, and cancellation. Property tests MUST prove monotone `iteration_count ∈ 1..3`, at most one reviewer launch per iteration, exactly one terminal cycle decision after entry, no transition after terminal, stable implementer continuity identity, and fresh reviewer identities. Restart tests MUST reconstruct `iteration_count`, raw `last_verdict`, `claude_session_id`, `last_diff_hash`, and `last_iteration_head_sha` from durable state. Minted-policy crash cuts MUST cover failure before checkpoint, after checkpoint/before ACK, and ACK failure; Captured-policy tests MUST cover successful native capture, missing capture, persistence failure, and resume withheld until durability. Committed-map tests MUST prove identical mapping reuse, conflicting mapping rejection, and working-tree-only state is not durable; local and remote adapters MUST return the exact checkpoint SHA used by the no-commit baseline. Reviewer-allowance tests MUST cover the size formula/clamps and unknown-diff fallback, pane and heartbeat ceilings, handler/daemon heartbeat equivalence, no mutation of work allowance by liveness, valid-verdict precedence at deadline, one-shot reseed without deadline reset, sentinel/event agreement, and observer cancellation/join.
...
|---|---|---|---|
| 2026-07-24 | 0.10.0 | agent (kerf `reviewloop-decoupling`) | **Review-loop semantics, continuity, and reviewer-allowance amendment draft.** EM-012 adds durable `last_iteration_head_sha` and makes diff hash evidence-only. EM-015d adds the pure cycle projection (EM-015d-KERNEL), Minted/Captured continuity transaction (EM-015d-CONT), and reviewer allowance (EM-015d-RVA). The continuity transaction uses new EM-023a `context-checkpoint`, returns the exact no-commit baseline, and orders Minted identity → durable checkpoint → selected-version ACK; Captured identity persists after native observation and before resume/Retask. EM-015e adopts HEAD advancement and `fixup_stalled`, preserves raw flagless REQUEST_CHANGES while normalizing routing to approval, and retains `no_progress` for compatibility. Reviewer timing remains the shipped diff allowance plus bounded pane/heartbeat liveness grace and one-shot reseed; heartbeat is not work progress. §6.1 and §10 synchronized. Status `draft`; no existing IDs renumbered or retired. Source: Pass 3 research and Pass 4 C1/C5/C7 designs. |
