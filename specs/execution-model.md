...
**Default.** The supervised (flywheel / cognition-loop) topology launched via [process-lifecycle.md §4.9] and [operator-nfr.md §4.3] defaults `--no-auto-pull` ON. The historical single-daemon topology preserves the `br ready` fallback by NOT setting the flag. The flag's effective default is topology-scoped: a daemon launched outside the supervised topology retains the fallback unless `--no-auto-pull` is passed.
...
FUNCTION orchestrator_main_loop():
    ...
    WHILE NOT shutdown_requested():
        ...
        IF queue IS None:
            IF no_auto_pull():                      -- §4.11.EM-066 sealed startup flag (default ON for supervised topology)
...
**Core MVH (amended — EM-061).** An implementation conforming to Core MVH MUST pass every requirement in EM-001 through EM-046 (including sub-requirements EM-012a, EM-015a, EM-015b, EM-015c, EM-015d, EM-015e, EM-015f, EM-017a, EM-018a, EM-020a, EM-023a, EM-024a, EM-025a, EM-031a, EM-034a, EM-034b, EM-034c, EM-036a, EM-041a, EM-042a, EM-043a, EM-046a, EM-046b) and EM-049 through EM-054 (concurrency primitives §4.11, merge-to-main §4.12), **and EM-055 through EM-059 (`dot`-mode binding §7.5)**, **and EM-066 through EM-067 (no-auto-pull topology + operator-pause fallback gate §4.11)**, plus invariants EM-INV-001, EM-INV-004, and EM-INV-005 (the three invariants surviving the §5 selection test; EM-INV-002, EM-INV-003, EM-INV-006 are retired). For `workflow_mode = single` (the default per §4.3.EM-012a built-in fallback), the cascade (§4.10), checkpoint cadence (§4.5), and sub-workflow composition (§4.8) apply unchanged. For `workflow_mode = review-loop`, the hardcoded two-node cycle of §4.3.EM-015d MUST be observed; the cap and early-exit rules of §4.3.EM-015e MUST be enforced; the six review-loop events of §6.5 MUST be emitted. **For `workflow_mode = dot`, the input contract (§7.5.1.EM-055), dispatch equivalence (§7.5.2.EM-056), validator obligations (§7.5.3.EM-057), and node-type dispatch table (§7.5.4.EM-058) MUST be observed.** Dispatch input MUST be the active queue per §7.4. When `--no-auto-pull` is set (per §4.11.EM-066 — the default for the supervised topology), daemon fallback to `br ready` is non-conforming, and a bare boot with no submitted queue MUST dispatch zero runs. When `--no-auto-pull` is NOT set (the historical single-daemon topology), the `br ready` fallback is a conforming opt-in, gated on operator-pause state per §4.11.EM-067. The EM-007 amendment per §4.2 (admitting `handler_ref` on `non-agentic` and `gate` nodes) is normative at Core MVH.
