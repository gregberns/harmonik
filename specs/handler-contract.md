...
**HC-063-SIG — output-signature reclassification of a non-zero exit (AMENDED 2026-08-10; AMENDED 2026-08-11).** The table above is the DEFAULT. The daemon MAY reclassify a non-zero exit when the command's own output carries a signature that names a cause outside the change under test. The reclassification is daemon-side per §4.5 HC-020, mechanism-tagged (no LLM, decided from bytes the daemon already holds), and MUST be derived from the output rather than from the exit code — a `make` recipe reports make's own status, so the failing command's code does not reach the daemon at all. Three signatures are recognised at v1:
...
| the gate was KILLED — make's recipe line names a signal, e.g. `*** [full] Terminated: 15` | `canceled` | a signal stopped the gate before it reached a verdict, so the gate found no fault in the change (AMENDED 2026-08-11) |
...
The `canceled` case exists because a killed gate says nothing about the code. The default sends it back to the implementer with "the build/test gate did not pass, fix the failure", so the implementer is resumed to fix a fault nobody saw. Measured live 2026-08-11: `make full` ran about 19 minutes of a 3600s node budget, passed the build, the lint and the subprocess tier, and was killed by SIGTERM in the scenario tier (`make: *** [full] Terminated: 15`). Nothing failed. `canceled` matches no back-edge condition in `standard-bead.dot`, so the graph's unconditional fallback carries the run to `close-needs-attention`. The run stops for triage. It does not retry, because nobody has yet identified what killed the gate (`hk-killed-gate-read-as-red-0hi5z`). This row records the OUTPUT half of the detection only. A signal that reaches the gate process itself is already `canceled` by the exit-state table above.
...
**Front-stop composition, NOT replacement (HC-056 / HC-057 relationship).** The synchronous input ack is an earlier, per-input **delivery-handoff observation** that **composes in front of** the existing process-liveness signals; it is NOT positive acceptance — that is the async `agent_input_acked` event above. It does NOT gate first-work dispatch (`agent_ready` per §4.9 HC-056 / §5 HC-INV-004 still does) and does NOT replace the heartbeat / commit watchdogs (§4.9 HC-057, §7.1).
...
The implementation in `internal/harness/claude/launchspec.go` (`BuildLaunchSpec`) reflects current behavior and is cited as evidence below. The spec is authoritative; the code follows.
...
> **SCOPE ALERT — this table is scoped to a retired mode (raised 2026-07-30).** `review-loop` was
> retired at [execution-model.md §4.3 EM-015d]. `core.WorkflowMode.Valid()` in
> `internal/core/workflowmode.go` now accepts only `single` and `dot`, so no run can resolve to the
> mode this table declares as its scope. The `HARMONIK_WORKFLOW_MODE` row below still requires the
> literal value `review-loop`, which the same code rejects. The table is therefore unsatisfiable as
> written. It also has no conformance sensor — see the test-hookpoint note below.
>
> This is left as written on purpose. Retiring the table or re-scoping it to the multi-node `dot`
> path would change what handlers must do, and that is an author decision, not a citation repair.
> The HC-006 enum above and the §6.1 `LaunchSpec` record were narrowed to `{single, dot}` because
> they restate the wire enum the code enforces. This table was not.
...
| **`session_id` source (harmonik-side `shared.LaunchArtifacts.HandlerSessionID`)** | Fresh UUIDv7 minted by `BuildLaunchSpec` at call time | Fresh UUIDv7 minted at each `BuildLaunchSpec` call (distinct from the Claude session ID being reused) | Fresh UUIDv7 minted at each `BuildLaunchSpec` call | §4.2.HC-006; [event-model.md §4.1] |
...
**Test-hookpoint sensor:** This rule has NO conformance sensor today. No test drives all three phases of `BuildLaunchSpec` end to end. The partial sensor this note used to name is gone: `TestON035a_PhaseSetIsComplete` checked the `phase` field on the operator-visible status event surface, and the whole `internal/operatornfr/` package that held it was deleted. No test took its place. A dedicated test that asserts the per-phase `argv` and `claude_session_id` distinctions of `BuildLaunchSpec` would close the gap. Track it as a follow-up bead against `internal/harness/claude/` (target: `TestHC006a_PerPhaseLaunchSpecInvariants`).
...
| **gate** | `SUCCESS` for EVERY evaluated verdict (`allow`, `deny`, AND `escalate-to-human`) per [control-points.md §6.1.8 CP-058] — a successfully-evaluated Gate is `SUCCESS` regardless of the decision. `FAIL` is emitted ONLY when the Gate could NOT be evaluated (evaluator dispatch failure, missing input, escalation backend down). `RETRY` and `PARTIAL_SUCCESS` MUST NOT be emitted — gate evaluation is a single-shot decision per [control-points.md §4.2 CP-006]. | `default` OR `gate_decision` on `SUCCESS`; MUST be `default` (no payload) on `FAIL`. `kind = gate_decision` is REQUIRED on a SUCCESS verdict when the gate's audit posture (per [control-points.md §4.8 CP-040]) requires structured rationale capture; otherwise `default` is acceptable. A `gate_decision` payload MUST NOT appear on a `FAIL` (eval-failure) Outcome per CP-058. | OPTIONAL on `FAIL`; MUST be absent on non-`FAIL`. `failure_class` and `kind = gate_decision` are MUTUALLY EXCLUSIVE on a gate Outcome: a *policy* deny is `SUCCESS` + `gate_decision` + NO `failure_class`; a Gate that could NOT evaluate (e.g. `budget_exhausted`) is `FAIL` + `failure_class` + NO payload. They are never co-emitted. | The verdict-routing carrier: `preferred_label` = the decision string (`"allow"`, `"deny"`, or `"escalate-to-human"`) on `SUCCESS`. Because every evaluated gate is `SUCCESS`, routing distinguishes the verdicts via `preferred_label`, NOT `status`. `payload` is the audit carrier. | Gate evaluators SHOULD NOT emit `context_updates` (a gate's job is to decide, not to mutate context); when emitted, the same registered-key discipline applies. |
...
The five `GateDecisionPayload` fields (per CP-058) are: `policy_id`, `decision` (one of `{allow, deny, escalate-to-human}` per [control-points.md §6.1.6 GateAction]), `decision_actor`, `decision_evidence_ref` (optional), `resolution_signal_id` (optional; required when `decision = escalate-to-human`; correlates the EM-042a gate-pending resolution signal). Handlers MUST populate `policy_id` and `decision` whenever they emit `kind = gate_decision`; the other fields are populated as available. A `kind = gate_decision` Outcome MUST have `status = SUCCESS` regardless of the `decision` value — `allow`, `deny`, and `escalate-to-human` are all successfully-evaluated verdicts per CP-058. Emitting a `gate_decision` payload on a `FAIL` Outcome is non-conforming; `status = FAIL` is reserved for a Gate that could not be evaluated and carries NO payload. The cascade distinguishes the verdicts by routing on `outcome.preferred_label` (the decision string), not on `status`.
...
**Front-stop composition (HC-070).** The per-input synchronous input ack of §4.1a HC-070 composes in front of this timeout as an earlier, per-input delivery-handoff observation — NOT positive acceptance, which is the async `agent_input_acked` event per HC-070; it does NOT replace or satisfy the `agent_ready` gate — first-work dispatch still requires `agent_ready` per HC-INV-004, and this `agent_ready` timeout remains the process-liveness guard for ready-state.
...
1. Resolve `shared.LaunchCtx.WorkspacePath` via `os.EvalSymlinks` to obtain `canonicalWorkspace`.
...
#### HC-048b — Fail provisioning on egress-policy or workspace-escape violation

Before provisioning any resolved skill package, the handler MUST apply two pre-provisioning checks in order:

1. **Egress-policy check.** If `LaunchSpec.egress_whitelist` is non-`None`, the handler MUST inspect the skill package's manifest for a declared `egress_domains[]` list. If the manifest declares one or more egress domains, the handler MUST verify each declared domain against the patterns in `LaunchSpec.egress_whitelist[]`. A declared domain that does not match any pattern in the whitelist MUST cause that skill's provisioning to fail as a structural failure per §4.5.HC-022 (`ErrSkillProvisioningFailed`). If the manifest declares no `egress_domains[]`, the egress check is a no-op for that skill. If `LaunchSpec.egress_whitelist` is `None`, the egress check is a no-op for all skills (unrestricted). Domain-pattern matching follows the glob semantics declared in [control-points.md §4.11.CP-059].

2. **Workspace-escape check.** The handler MUST verify that the skill package's provisioned file paths — including all file-drop targets, CLI binary destinations, and MCP registration paths — resolve to paths within `LaunchSpec.workspace_path`. Any provisioning path that resolves outside the workspace boundary (via symlink resolution, path-traversal patterns, or absolute paths) MUST fail provisioning as a structural failure per §4.5.HC-022 (`ErrSkillProvisioningFailed`). This enforces [operator-nfr.md §4.7.ON-024] at the provisioning layer.

On a check failure for any skill, the handler MUST: (a) skip provisioning of the failing skill; (b) emit a `skills_provisioned` event per HC-049 listing ONLY the skills successfully installed before the failure; (c) fail-launch with `ErrSkillProvisioningFailed` wrapping `ErrStructural`. The `skills_provisioned` event MUST be emitted even when the installed set is empty (e.g., the very first skill failed the egress check). Until the `agent-configuration` spec lands, skill manifests are read from the `manifest.yaml` sidecar in the resolved skill-package directory; `egress_domains[]` and provisioning file paths are fields declared there (bootstrap; migrates to `specs/agent-configuration.md` when finalized).
...
RECORD LaunchSpec:
    ...
    workflow_mode         : Enum | None             -- {single, dot}; present iff non-default mode resolved per §4.2.HC-006; observational only per §4.1.HC-003a
...
|---|---|---|---|
| 2026-07-30 | 0.8.2 | agent (spec citation cleanup) | **Rotted pointers repaired across `specs/`. No obligation changed by this pass.** Deleted files that were cited as implementation evidence now name the symbol that carries the behavior today. Line-number citations became symbol names, per the repo rule to cite symbols and never line numbers. The retired `review-loop` workflow mode was dropped from every list that presented it as a live selectable mode, because `core.WorkflowMode.Valid()` accepts only `single` and `dot`. Rules that name `review-loop` as a RETIRED value to reject are unchanged, and so are the event `review_loop_cycle_complete` and the review-loop-failure budget, whose symbols still exist. Where a spec named a test as its conformance sensor and that test no longer exists, the text now says so instead of claiming cover it does not have. |
| 2026-07-27 | 0.8.1 | agent (codename: input-ack-contract) | **Front-stop wording reconciled with the binary `Ack` (coordinated drift correction; co-landed with [agent-input.md] 0.1.1 and [run-state-machine.md] 0.2.1).** Three "Front-stop composition" paragraphs — in **HC-070** (§4.1a), **HC-056** (§4.9), and **HC-057** (§4.9) — still called the SYNCHRONOUS input ack a "positive … acceptance signal", contradicting HC-070's own body and the §6.1 `Ack` record, both of which land the outcome as binary `{Delivered, Rejected}` with positive acceptance decoupled onto the async `agent_input_acked` event. All three now describe it as an earlier, per-input DELIVERY-HANDOFF observation and name `agent_input_acked` as the positive acceptance. Their front-stop rule is preserved in substance: the input ack composes IN FRONT of, and does not replace or satisfy, the `agent_ready` gate (HC-056 / HC-INV-004) or the heartbeat / silent-hang liveness guard (HC-057, §7.1). No requirement renumbered or retired; the `Ack` record, `InputPort` surface, event contract, HC-INV-008 window, and all unrelated semantics are unchanged. Status remains `reviewed`. |
...
| 2026-05-27 | 0.5.1 | agent (hk-lt0w7) | **HC-058/HC-060 gate-deny semantics aligned to CP-058 (deny→SUCCESS, not FAIL).** Resolved the same deny-routing contradiction fixed in execution-model.md v0.7.1: a successfully-evaluated Gate is `status = SUCCESS` for ALL verdicts (`allow`, `deny`, `escalate-to-human`); `FAIL` is reserved for a Gate that could NOT be evaluated and carries NO `gate_decision` payload. Rewrote the §4.2a HC-058 gate table row (status column: SUCCESS for every verdict, FAIL only on eval-failure; payload column: no payload on FAIL; failure_class column: `failure_class` and `kind=gate_decision` are now MUTUALLY EXCLUSIVE — replaced the wrong co-emitted budget_exhausted example; preferred_label column: carries the decision string `allow`/`deny`/`escalate-to-human`, routing is decision-based not status-based). Rewrote the HC-060 five-field paragraph: corrected the `decision` enum to `{allow, deny, escalate-to-human}`, dropped the wrong `permit→SUCCESS / deny→FAIL` correlation, stated the SUCCESS-regardless-of-decision rule and the no-payload-on-FAIL rule. No HC IDs renumbered. Refs: hk-lt0w7. |
