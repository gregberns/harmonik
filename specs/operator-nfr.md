...
Review-loop attention outcomes (`completion_reason ∈ {cap_hit, blocked, fixup_stalled, error}`) MUST NOT be assigned a new top-level exit-code category. These are run-level terminations, not daemon-level exits; the operator-observable signal is the bead's `needs-attention` label per §4.3.ON-009a plus the durable `review_loop_cycle_complete` event's normalized `completion_reason` field per [event-model.md §8.1a]. `no_progress` / `no_progress_detected` is historical compatibility vocabulary and a separately owned DOT/generic path; a current built-in review-loop producer MUST NOT use it for post-`REQUEST_CHANGES` termination. Implementations MUST NOT extend §8 to accommodate these termination paths.
...
The normalized `review_loop_cycle_complete.completion_reason` is the sole current review-loop input to needs-attention policy. The raw reviewer verdict remains unchanged in `reviewer_verdict`; in particular, a flagless `REQUEST_CHANGES` is normalized to `approved` only at the cycle-decision layer per [execution-model.md §4.3 EM-015e].

| Normalized completion reason | Source condition | Required run terminal | Beads effect |
|---|---|---|---|
| `approved` | Raw `APPROVE`, or flagless `REQUEST_CHANGES` normalized by EM-015e | `run_completed` after the successful merge path | Ordinary BI-010a success close; MUST NOT add `needs-attention` |
| `cap_hit` | Actionable `REQUEST_CHANGES` at iteration 3 | `run_failed` after durable cycle-complete | Idempotent BI-010a `attention-close` |
| `blocked` | Raw reviewer `BLOCK` | `run_failed` after durable cycle-complete | Idempotent BI-010a `attention-close` |
| `fixup_stalled` | HEAD unchanged after actionable `REQUEST_CHANGES`, before another reviewer launch | `run_failed` after durable cycle-complete | Idempotent BI-010a `attention-close` |
| `error` | Launch, input, ready, phase, verdict, allowance, transfer, or cancellation failure after cycle entry | `run_failed` after durable cycle-complete | Idempotent BI-010a `attention-close` |

For every attention outcome, the daemon MUST first successfully persist `review_loop_cycle_complete` with the normalized reason, then emit the required run terminal, then execute the idempotent Beads `attention-close` transaction defined by [beads-integration.md §4.4 BI-010a] and §4.10. That transaction closes the bead and ensures the `needs-attention` label; repeating it after a crash MUST converge to the same `(status=closed, label present)` postcondition.

A failed append/fsync of `review_loop_cycle_complete` is an Event Model durability failure, not
the normalized `error` row: no run terminal or attention-close is authorized until recovery
successfully persists or reconciles the missing cycle decision.

`no_progress` / `no_progress_detected` remains readable for historical review-loop traces and separately owned DOT/generic paths only. It MUST NOT appear as a current built-in review-loop row in this table and MUST NOT be substituted for `fixup_stalled`.

The daemon's submit-time validation per [beads-integration.md §4.5 BI-013a/BI-013b] MUST treat `needs-attention`-labeled beads as out-of-scope for automatic dispatch. There MUST NOT be an auto-retry path: no subsystem MAY re-dispatch a `needs-attention`-labeled bead without operator intervention. Operators drain the set manually by either (a) removing the `needs-attention` label after triage, which restores submit eligibility, or (b) closing the bead as `wontfix`. Phantom auto-retry logic — any code path that removes the label or re-dispatches the bead without an operator-issued command — is a structural violation of this requirement.

**Terminology note.** The "queue" in this requirement is the *needs-attention bead set* — a Beads-side concept enforced by [beads-integration.md §4.5 BI-013a]. It is NOT the daemon's execution queue defined in [queue-model.md §1] and persisted at `.harmonik/queue.json`. The two are layered: the needs-attention set governs which beads an orchestrator MAY enqueue into the execution queue; the execution queue governs which queued beads the daemon dispatches. Operator drain actions in this requirement (label removal, `wontfix` closure) mutate the bead set, not the execution queue.
...
Operator visibility into `review-loop` cycle progression MUST be supplied entirely via the existing JSONL event-consumption path (`harmonik status`, `harmonik logs`, `jq` against `events.jsonl`). The cycle's observability surface is the seven event types declared in [event-model.md §8.1a] — `implementer_resumed`, `reviewer_launched`, `reviewer_verdict`, `iteration_cap_hit`, compatibility/DOT-generic `no_progress_detected`, `review_fixup_stalled`, and `review_loop_cycle_complete`. Current built-in review-loop status rendering MUST use `review_fixup_stalled` / `completion_reason=fixup_stalled` for the post-`REQUEST_CHANGES` zero-HEAD-advance path; `no_progress_detected` is retained only for historical and separately owned DOT/generic records. No new operator command surface (e.g., `harmonik review-status`) MUST be introduced; review-loop information is rendered inline in `harmonik status` when a run's resolved `workflow_mode` is `review-loop`. The operator's diagnostic recipe for a stuck cycle is: filter `events.jsonl` by `run_id` and intersect with the named event types. Pause-reason discriminators reported by `harmonik status` per §4.3.ON-054 MUST NOT add a `review-loop`-specific reason; review-loop pause checkpoints (per §4.3.ON-008 amendment) reuse the existing `operator-pause` reason with the iteration-boundary checkpoint observable in the `drain_summary` field.
...
- **ON-007 — ON-014 (operator-control semantics).** State-machine scenario tests enumerating every transition in §7.1; verify reconciliation-carve-out queueing of pause during `reconciling`; verify improvement-pause auto-resumes; verify `stop --immediate` aborts in-flight runs and emits `run_failed` with class `canceled` on next restart. ON-009a table tests cover approved, cap-hit, blocked, fixup-stalled, and error; assert cycle-complete precedes the run terminal, attention outcomes converge to closed-plus-label through BI-010a, approved never receives the label, and current built-in review-loop output never uses `no_progress`.
...
|---|---|---|---|
| 2026-07-24 | 0.5.7 | agent (kerf `reviewloop-decoupling`) | **Normalized review-loop operator outcomes and needs-attention discipline.** ON-002 now treats `{cap_hit, blocked, fixup_stalled, error}` as run-level outcomes with no daemon exit-code extension and reserves `no_progress` for historical/DOT-generic compatibility. ON-009a adds the authoritative normalized outcome table, requires durable cycle-complete before the run terminal, and binds every attention outcome to BI-010a's idempotent closed-plus-`needs-attention` transaction; approved never receives the label. Citations now point to BI-013a submit-time exclusion. No ON IDs or exit codes were added, retired, or renumbered. |
