...
- Multi-repo workflow support — no ON requirement governs it. **AMENDED v0.5.11 — durable dispatch now separates safelist authority from execution identity.** One daemon still supervises one project. A bead MAY name a second repository with `target_repo: <abs-path>` in its `## Branching` section. The operator controls this with the `allowed_repos` list in `.harmonik/config.yaml` under the `daemon:` block. The daemon compares the declared string to that list before it normalizes or resolves the path. A match authorizes the path. The daemon then binds the clean absolute canonical path as the durable repository identity. The other dispatch guards still apply. The daemon reopens a bead whose declared string does not appear in the list, and the reopen reason names the safelist. An absent or empty list refuses every cross-repo bead. A cross-repo bead cannot use a remote worker until the operator can bind a worker to that repository. A normal remote run binds the worker name, transport, host, and repository path. Replay refuses a change to that route. The full contract is stated in [process-lifecycle.md §2.2]. `docs/cross-repo-dispatch.md` still describes the pre-safelist state and is not normative. **SUPERSEDED at v0.5.8 (was correct through v0.5.7):** "deferred; harmonik operates against one repository at a time per problem-space constraints. Known boundary: the daemon lands changes only to the supervised repo; a bead declaring `target_repo: <path>` is now explicitly refused with `CrossRepoUnsupportedError` (guard landed **hk-3r3**); cross-repo fixes are applied out-of-band until full cross-repo dispatch is implemented."
...
|---|---|---|---|
| 2026-08-14 | 0.5.11 | agent (Charlie) | **The operator-facing multi-repo rule now separates safelist authority from durable execution identity.** Harmonik tests the original declared string against `allowed_repos`. It then binds the clean canonical repository path. Remote route identity includes the worker name, transport, host, and repository path. Cross-repository remote execution stays disabled until the operator can bind workers to repositories. No ON requirement changes. Companion: [process-lifecycle.md] v0.7.9. |
