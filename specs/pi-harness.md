# Pi + OpenRouter Harness

```yaml
---
title: Pi + OpenRouter Harness
spec-id: pi-harness
requirement-prefix: PI
status: draft
...
> Codename pilot · Epic hk-ag97p · Normative build contract.
> Companion design: `04-design/pi-harness-design.md` (kerf bench). Requirement IDs use the `PI-` prefix.
...
1. Pi claims a mechanical bead labelled `harness:pi` on a dedicated Pi queue with an explicit `Workers` cap; `pibillingguard` asserts the key named by `harnesses.pi.api_key_env` is present in the operator environment and no persisted credential exists at `~/.pi/auth.json` (fail-closed, PI-040/PI-042); Pi reads `.harmonik/agent-task.md`, implements the bead's change in the worktree, and commits with a `Refs: <bead-id>` trailer — or `ensurePiRefsTrailer` adds one via fallback (PI-030/PI-031); the DOT test+review gate passes; the daemon squash-merges the task branch and closes the bead (PI-010/PI-014/PI-069/PI-070).
