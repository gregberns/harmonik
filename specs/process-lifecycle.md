...
(e) State owned:
  ...
  - `workflow_mode_default` (in-memory enum value in `{single, dot}` — `review-loop` RETIRED v0.7.0; loaded once at PL-005 step 0 per PL-004a; observable via `harmonik status`).
...
The daemon's project-level configuration MAY carry an optional `workflow_mode` field whose value MUST be one of the enum `{single, dot}` (`review-loop` RETIRED v0.7.0, tracking [execution-model.md §4.3 EM-015d]). A config file naming `workflow_mode: review-loop` MUST fail at load with an actionable diagnostic that names `dot` as the replacement; the daemon MUST NOT silently coerce it. This is deliberately stricter than the per-bead label path, which degrades a retired label to `dot` with a `bead_label_conflict` — a config file is operator-authored and re-read only at boot, so a wrong value there is worth stopping for, whereas a stale bead label is data the operator may not control and must not wedge the queue. When the field is absent, the daemon's default workflow mode MUST be `dot`, resolving to the embedded `standard-bead.dot` canonical graph (the build-embedded copy at `internal/daemon/standard-bead.dot`, registered with the daemon per [execution-model.md §7.5.1 EM-055] and [execution-model.md §7.5.5 EM-059]). This flips the historical `single` default per the `standard-bead-dot` kerf work (epic hk-o7j) and the `dot-default` precedent (hk-30vlb). **Review floor.** The floor's guarantee — a bead resolved from the daemon-level default or the built-in fallback is NEVER dispatched without a review gate — is unchanged, and is delivered by the exemplar graph's own reviewer node ([workflow-graph.md §17 WG-047..WG-052]). If the embedded `standard-bead.dot` graph fails to load (parse failure, missing artifact, schema-version incompatibility per [execution-model.md §7.5.3 EM-057] check 1, or template-parameter substitution failure), the daemon MUST fail the run with a diagnostic naming the artifact and the load error, per [execution-model.md §4.3 EM-012a-FLOOR]; it MUST NOT dispatch the bead under any other workflow shape. The no-review `single` shape remains unreachable through this daemon-level default and through any tier other than an explicit per-bead `workflow:single` label (which the daemon MUST audit per the `review_bypassed` event); the daemon MUST NOT resolve `single` from the daemon-level default or the built-in fallback. The field MUST be read exactly once during PL-005 step 0 (composition-root bootstrap) and MUST be cached in-process as `workflow_mode_default` for the daemon's lifetime; the daemon MUST NOT re-read the field after bootstrap. Mid-run mode change via this surface is FORBIDDEN; an operator-initiated rotation of the daemon-level default requires a daemon restart (or a `harmonik upgrade` exec-replacement per PL-027, which re-runs step 0).

The daemon-level `workflow_mode_default` is the second-lowest-precedence tier of the four-tier workflow-mode resolution chain owned by [execution-model.md §4.3 EM-012a]: per-task (bead `workflow:<mode>` label per [beads-integration.md §4.3 BI-009a]) → per-project → daemon-level (this requirement) → built-in fallback (`dot`, with the [execution-model.md §4.3 EM-012a-FLOOR] review-floor: reviewed by construction, and a loud run failure — never `single`, never a silent change of shape — on embedded-graph load failure). The daemon MUST NOT override higher-precedence tiers (per-project or per-task) when both are present; resolution is the responsibility of the dispatch path per [execution-model.md §4.3 EM-012a]. EM-012a cites this requirement (PL-004a) by name as its tier-3 daemon-level config anchor; the two MUST agree on the dot-default and on the floor's mechanism.
