...
- Agent-subprocess spawning, spawn-regime parentage, daemon-ward socket communication, and exclusive process-wait ownership.
- Review-loop phase-owned process/session/callback/observer lifetime and the quiescence barrier before dependent advancement or workspace release.
...
- Handler launch mechanics, authoritative lifecycle observation, subprocess error classification, and per-operation cancellation timing — owned by [handler-contract.md §4.1, §4.3, §4.4, §4.6]. This spec owns composition of those operations into phase-scoped cancellation, completion, and teardown ordering.
...
- **phase lifetime** — for an implementer or reviewer phase, the interval from its first phase-scoped asynchronous acquisition until every phase-owned resource reaches observable completion. (see §4.5 PL-014b)
- **phase-owned resource** — a process/session, authoritative lifecycle watcher, callback registration, input-delivery task, heartbeat task, event subscription, auxiliary commit/verdict/artifact/budget observer, or phase-only workspace whose use is scoped to one implementer or reviewer phase. (see §4.5 PL-014b)
- **logical phase outcome** — the result fact sufficient for existing event publication and routing; it does not prove that phase-owned resources are quiescent. (see §4.5 PL-014b)
- **quiescent phase completion** — the close barrier after which no phase-owned resource can deliver input, publish a phase event, access its workspace, or retain unjoined asynchronous work. (see §4.5 PL-014b)
- **phase ownership transfer** — an explicit typed transfer naming the recipient owner and its cancellation, completion, and retained-workspace responsibilities. (see §4.5 PL-014b)
- **reviewer projection** — the unleased, box-A-local, exact-SHA phase-only worktree owned by S06 under [workspace-model.md §4.7 WM-027a]. It is reclaimed only after the phase close barrier or the manifest-aware startup branch of PL-006.
...
- **Run-workspace locks and candidate partition.** The daemon MUST enumerate worktrees by filesystem scan of `<repo>/.harmonik/worktrees/*/` per [workspace-model.md §4.1 WM-002]. No in-memory registry is required at sweep time. Before applying a lease rule, it MUST partition canonical run workspaces, reviewer-projection candidates, scratch merge-worktrees, and unknown paths per [workspace-model.md §4.3 WM-013c]. For each canonical run workspace, lease-lock files meeting the staleness criterion of [workspace-model.md §4.3 WM-013a] / §4.8 WM-033 MUST be removed. A reviewer-projection candidate is intentionally unleased and MUST NOT enter the generic lock/no-lock cleanup path. (Canonical lease-lock path alignment across HC-044a, WM-013a, and this spec is tracked as OQ-PL-004.)
...
- **Reviewer projections — after process/tmux quiescence.** Only after the tmux-session and subprocess-cleanup branches above have completed (including bounded polling and live-owner exclusions) MAY the daemon process reviewer-projection candidates. For each candidate it MUST invoke S06's manifest-aware WM-027a / WM-033 classifier: validate the manifest tuple against the deterministic path and registered git-worktree SHA; prove that no live phase owner, reviewer session, callback, watcher, observer, pane, or process remains; and verify that required reviewer logs, metadata, verdict, feedback, and diagnostics are durable in the run workspace. If complete source artifacts remain only in the projection, the daemon MAY use WM-027a's explicit-location, digest-verified transfer seam to finish the archive before removal.

  A projection satisfying every proof MAY be reclaimed idempotently with `git worktree remove --force --force` followed by `git worktree prune`. A missing/malformed/mismatched manifest, live-owner ambiguity, or incomplete/untransferable archive MUST be handed to reconciliation as `reviewer-projection-integrity` evidence with the observed path, manifest parse result, registered SHA, discovered session IDs, archive/transfer result, and surviving owner evidence. It MUST remain on disk and MUST NOT be deleted by path shape, absence of a lease, or a generic orphan rule. Startup has no prior-generation goroutine to join; successful completion of the marker-based process/tmux sweeps plus the manifest/archive proof is the required startup equivalent of the normal PL-014b join barrier.
...
- **Event.** On completion, the daemon MUST emit `daemon_orphan_sweep_completed` (per [event-model.md §8.7.14]) with counts of tmux sessions killed, locks cleared, handler subprocesses killed, `br` subprocesses killed, reconciliation lock files removed, stale intents observed, `bead_in_progress_reset` (count of `in_progress` beads reset to `open` by this sweep), reviewer projections reclaimed, and reviewer projections handed to reconciliation. The two reviewer-projection counts and `bead_in_progress_reset` are additive payload extensions to §8.7.14 — consistent with the precedent set by the `tmux_windows_killed` and `tmux_kill_window_survivors` additions (PL-021c); consumers MUST tolerate unknown integer fields per [event-model.md §6.3] N-1 compatibility. If [event-model.md §8.7.14] requires a schema bump rather than treating an addition as additive-tolerated, the companion EV revision must absorb these fields before finalization.
...
The orphan sweep MUST be deterministic given the filesystem + process state AND the project-scoped provenance marker of §4.2a PL-006e. After the sweep completes, no harmonik-owned process bearing this project's provenance marker from a prior daemon instance is alive **and reachable by this sweep** (the coverage boundary is stated in PL-006 and PL-021b §7), no harmonik-owned run workspace is locked by a prior-instance lease, and every reviewer-projection candidate has either been manifest/archive-proven and reclaimed or durably handed to reconciliation as `reviewer-projection-integrity` evidence. The git walk (PL-005 step 5) and Beads query (step 6) operate against a quiescent project-scoped filesystem.
...
#### PL-014 — Agent subprocess parentage follows the spawn regime

The daemon MUST initiate agent-process creation through one of the spawn regimes of PL-INV-005. A direct-exec handler process is initially a child of the daemon. A substrate-hosted handler process is initially a child of the multiplexer server that the daemon directs to create the pane. Post-crash re-parenting to init is legal for either regime and is recovered by PL-006.

Every spawn MUST carry the provenance marker of PL-006e unless explicitly exempted by the PL-006g spawn-site register. Subprocess lifecycle classification is owned by the authoritative watcher of [handler-contract.md §4.3 HC-011]; process wait and reap are separate lifecycle responsibilities owned here.

Every direct-exec spawn MUST have exactly one owner of its `*exec.Cmd`, and that owner MUST call `cmd.Wait()` exactly once to reap the child's exit status. The authoritative watcher MAY be that owner but is not required to be. Failure to call `cmd.Wait()` produces a zombie that persists until daemon exit and MUST NOT occur on any code path; it is a conformance violation under PL-INV-005 regardless of whether `kill(pid, 0)` reports the zombie as alive. Substrate-hosted sessions use the wait/reap discipline of PL-021b rather than a daemon-owned `*exec.Cmd`.
...
#### PL-014b — Phase-owned resources reach quiescence before dependent advancement

Each implementer or reviewer phase MUST establish one phase-lifetime ownership scope before acquiring its first phase-scoped asynchronous resource. The scope MUST own, when present, the primary session/process, authoritative watcher, hook/callback registration, input-delivery workers, heartbeat task, event subscriptions, auxiliary commit/verdict/artifact/budget observers, and any workspace used only by that phase.

Every asynchronous phase-owned resource MUST expose, directly or through the scope, both (a) an idempotent bounded stop or cancellation operation and (b) observable completion. An auxiliary observer is not an authoritative handler watcher and MUST NOT publish a competing handler-lifecycle terminal event.

A logical phase outcome MAY be published before the scope reaches quiescent phase completion. Outcome publication, including an event whose existing name contains `phase_complete` or `cycle_complete`, MUST NOT authorize the coordinator to launch the next dependent phase, reuse or remove a dependent workspace, or return from the review loop. Those transitions require successful completion of the phase close barrier.

The close barrier MUST preserve the following partial order:

1. Seal new input delivery and nonterminal callback admission.
2. Cancel heartbeat and auxiliary observers.
3. Initiate session termination under the existing Handler Contract cancellation and Process Lifecycle signal/escalation policy without destroying a lifecycle source still required for terminal classification.
4. Preserve and consume any required terminal hook state, then seal the authoritative lifecycle source.
5. Await authoritative watcher completion and every auxiliary completion, and complete the substrate's process wait/reap and pane release obligations.
6. For a reviewer projection, validate the verdict and transfer every required review-control artifact plus the complete reviewer session-log directory and metadata sidecar through [workspace-model.md §4.7 WM-027a]'s explicit-location seam. Destination durability and digest verification MUST complete before dependent event publication that claims an archived artifact, implementer Retask, or projection removal.
7. Release a phase-only workspace only after every process, callback, watcher, and observer that may access it is quiescent and every required transfer is durably verified.

The exact middle ordering is substrate-specific:

- For direct execution, process termination, the exclusive `cmd.Wait`, lifecycle-source closure, and watcher drain MAY proceed concurrently where the transport requires it. The close barrier MUST await both the process-wait owner and the watcher before workspace release.
- For interactive execution, the owner MUST preserve/consume terminal hook state, seal callback admission, and join the authoritative observer before destructive pane release. A tmux session with no authoritative observer is a Handler Contract conformance defect, not permission to omit observer completion.

Callback unregistration is a quiescence barrier. It MUST prevent new callback admission and MUST wait for, or otherwise account for, callbacks admitted before unregistration began. After unregistration returns, no callback from that phase may publish into a later phase. Required terminal hook state MUST remain available until the phase owner has consumed it.

Phase close MUST be bounded, idempotent, safe under repeated or concurrent invocation, and best-effort across all teardown steps. Failure of one step MUST NOT suppress later cleanup attempts. If the bound expires, the coordinator MUST route the result through the existing failure path and MUST retain any workspace still depended upon by an unquiesced resource or incomplete archive transfer. This requirement adds no new failure class or event.

A phase-owned resource MAY outlive the phase only through an explicit typed ownership transfer that names the resource set, successor owner, cancellation and completion obligations, and any retained workspace dependency. Independent-session adoption and intentional retained-failure workspaces remain valid only when represented by such a transfer.

This requirement does not alter process provenance, process classes, signal timing, kill escalation, tmux policy, input protocol, or public event order. Event-model and execution-model remain authoritative for logical outcome names and ordering.
...
Agent-subprocess failure (crash, hang, policy violation) MUST be observed by the daemon's authoritative lifecycle watcher per [handler-contract.md §4.3 HC-011] and MUST produce typed events (`agent_failed`, etc., per [event-model.md §8.3]). The exclusive process-wait owner of PL-014 is responsible for obtaining and exposing the process exit result; the watcher is responsible for lifecycle classification and publication. They MAY be the same execution owner but the contract does not require that identity. Routing of the resulting event to retry / re-plan / terminal paths is owned by [execution-model.md §4.10 EM-046b] (RETRY outcome re-dispatch) and [reconciliation/spec.md §4.2] (reconciliation-routed paths).
...
This spec NAMES the silent-hang detection obligation owned by [handler-contract.md §4.6]. A silent hang is an agent subprocess that remains alive but produces no output, no heartbeat, and no lifecycle signal for longer than a bounded interval. The handler-contract spec owns the detection rule, the wall-clock ceiling, and the cleanup path; this spec requires that the daemon's authoritative watcher (§PL-016) implement the handler-contract detection rule and route silent-hang outcomes into reconciliation per [reconciliation/spec.md §4.2].
...
§PL-006's orphan sweep MUST complete before any reconciliation detector (per [reconciliation/spec.md §4.3]) runs. This invariant is load-bearing for reconciliation correctness: detectors scope on runs ([reconciliation/spec.md RC-INV-005] detectors filter by `run_id`, never `bead_id`), and a run with a live orphan subprocess, stale worktree lock, or unclassified reviewer projection cannot be classified correctly until live ownership is cleared and projection evidence is either reclaimed or handed off.
...
- **PL-005, PL-006, PL-006a, PL-007, PL-INV-003, PL-INV-005** — a scenario test that seeds tmux sessions, stale run-workspace locks, conforming reviewer projections, malformed/mismatched reviewer projections, incomplete reviewer archives, re-parented handler subprocesses AND re-parented `br` subprocesses (with and without the provenance marker), stale `.harmonik/reconciliation-locks/*.lock` files (with and without the `Harmonik-Verdict-Executed: true` commit on the investigator branch per RC-002b), and stale intent files; then starts the daemon and asserts the orphan-sweep event payload matches expected counts (including `br_subprocesses_killed`, `reconciliation_locks_removed`, projections reclaimed, and projections handed to reconciliation). The trace MUST show tmux/process reaping before projection classification; conforming projections reclaim only after archive verification/recovery transfer; malformed, live-owner-ambiguous, or incomplete/untransferable projections remain on disk with `reviewer-projection-integrity` evidence; projections never enter generic no-lease cleanup. The test also asserts that subprocesses lacking the marker are NOT killed; locks under active acquisition by a concurrent process (simulated via `flock(LOCK_EX)` from a fixture process) are NOT removed; and `orphan_sweep_complete_at` is set before any reconciliation detector runs. A companion test seeds `.harmonik/queue.json` with a recognized schema and asserts the in-memory queue is loaded at step 8a, including persisted per-group / per-item statuses and the `paused-by-failure` / `paused-by-drain` queue statuses; an additional test seeds a forward-incompatible `schema_version` and asserts the daemon refuses startup with code 2 and emits `daemon_startup_failed{failure_mode="queue-format-unsupported"}`; an additional test seeds a corrupt `.harmonik/queue.json` and asserts the daemon proceeds without an in-memory queue and emits the structured-log warning per ON-035.
...
- **PL-014, PL-014b, PL-016, PL-017 (process and phase lifetime).** Ordered-trace scenarios MUST cover implementer and reviewer success, launch failure, ready timeout, cancellation, no-commit completion, approval, actionable `REQUEST_CHANGES`, malformed/missing verdict, and budget-triggered termination. Each trace MUST prove that logical outcome publication may precede close but callback/input sealing and heartbeat/auxiliary cancellation and completion precede dependent advancement; direct process wait and authoritative watcher completion precede dependent workspace release; interactive watcher completion precedes destructive pane release; required terminal hook state is consumed before registration close; reviewer process/watcher/pane join precedes digest-verified transfer of verdict/control artifacts and the complete session-log directory + sidecar; transfer precedes projection cleanup and implementer Retask; and outer run-workspace cleanup remains last. Race tests MUST cover callback admission versus unregister, repeated ready delivery, lifecycle-source close versus watcher completion, observer cancellation versus terminal artifact detection, heartbeat cancellation versus publication, terminal hook outcome versus close, transfer versus late log write, and concurrent repeated close. Leak tests MUST repeat implementer/reviewer transitions and assert zero surviving callbacks, subscriptions, heartbeat tasks, auxiliary tasks, watcher tasks, spawn slots, panes, and reviewer workspaces after each phase. A teardown/transfer-timeout test MUST assert that remaining cleanup steps are attempted, the existing failure path is used, and a still-dependent or incompletely archived workspace is retained. An ownership-transfer scenario MUST preserve the independent-session and retained-failure exceptions. Public lifecycle-event order MUST remain unchanged.
...
|---|---|---|---|
| 2026-07-24 | 0.7.1 | agent (reviewloop-decoupling integration repair) | **Projection cleanup and startup ordering.** PL-006 now partitions leased run workspaces from intentionally unleased reviewer projections, processes projections only after process/tmux quiescence, reclaims only manifest/SHA/archive-proven projections, and hands ambiguous/incomplete cases to reconciliation as `reviewer-projection-integrity` evidence. PL-014b now places digest-verified review-control and complete reviewer session-log/metadata transfer after watcher/process/pane join and before projection release or implementer Retask. PL-007, PL-INV-003, and conformance traces were aligned. No requirement IDs were added or renumbered. |
| 2026-07-24 | 0.7.0 | agent (reviewloop-decoupling C4) | **Per-phase resource ownership and quiescent advancement added.** NEW PL-014b defines implementer/reviewer phase lifetime, phase-owned resources, logical outcome versus quiescent completion, bounded stop plus observable completion for all asynchronous resources, callback-unregistration quiescence, ordered best-effort/idempotent teardown, dependent-workspace retention on timeout, and explicit typed ownership transfer. The coordinator may publish existing logical outcome events before close but may not launch a dependent phase, release/reuse its workspace, or return from the review loop before the close barrier. Direct and interactive substrate ordering preserves current process provenance, signal escalation, tmux policy, input protocol, and public event order while preventing authoritative watchers from outliving destructive pane release. PL-014 now states the already-established direct-exec and substrate-hosted parentage regimes, uses PL-006e provenance, and assigns exactly one `cmd.Wait` owner without requiring it to be the watcher. PL-016 separates process exit ownership from authoritative lifecycle classification. Conformance adds ordered success/failure/cancellation/review traces, callback/watcher/observer/heartbeat races and joins, leak repetition, timeout retention, and ownership-transfer coverage. |
