...
- Multi-repo dispatch — no PL requirement governs it. This spec states the boundary only, so that a reader does not mistake the absence of a requirement for a refusal. **AMENDED v0.7.9 — durable dispatch now separates safelist authority from execution identity.** A bead MAY declare `target_repo: <abs-path>` in its `## Branching` section. The value is bead-body only. No project-level tier and no spec-level tier supplies a default, so an absent `target_repo` means the bead lands in the supervised repo. The daemon resolves an **active repo** for every run. When `target_repo` is empty, or equals the daemon's `projectDir`, the active repo is `projectDir` and the run is a normal local run. When `target_repo` names a different path, the daemon first tests the declared string against the `allowed_repos` safelist in `.harmonik/config.yaml` under the `daemon:` block. This test uses exact string equality. It does no path normalization and no symlink resolution. A listed string authorizes the path. The daemon then resolves a clean absolute canonical path and binds that path as the durable repository identity. The worktree, the branching resolution, the no-commit guard, the merge, and the push all use this bound repository. The daemon's own protected-branch list is not applied to a cross-repo run, because that list names branches in the supervised repo and says nothing about the target repo. A path that is NOT on the safelist is refused. The daemon reopens the bead with `CrossRepoUnsafeError`, and the reason names the safelist. An absent or empty `allowed_repos` list matches nothing, so every cross-repo bead is refused until the operator adds the path. The bead ledger, the daemon socket, the queue files, and the event log stay under `projectDir` for a cross-repo run. A cross-repo bead cannot use a remote worker. The daemon must refuse that combination until a worker-to-repository mapping and identity check are specified. A normal remote run binds the worker name, transport, host, and repository path as one durable route. Replay must refuse a route change. Implementation evidence: `internal/daemon/workloop_runplan.go` `resolveRunPlanPlace`, `internal/daemon/branching.go` `CrossRepoUnsafeError` and `isInAllowedRepos`, `internal/projectconfig/projectconfig.go` `AllowedRepos`. Background: `docs/cross-repo-dispatch.md`, which still describes the pre-safelist state and is not normative. **SUPERSEDED at v0.7.2 (was correct through v0.7.1):** "the daemon lands only changes to the repo it supervises. Worktrees are rooted at `<projectDir>/.harmonik/worktrees/`; a bead touching another repo cannot satisfy the managed-worktree prefix check. A bead may declare `target_repo: <abs-path>` in its `## Branching` section; when set and differing from `projectDir`, the daemon reopens the bead with `CrossRepoUnsupportedError` so the operator applies the fix out-of-band (guard landed **hk-3r3**)." `CrossRepoUnsupportedError` still exists in the tree but is deprecated and no live path returns it.
