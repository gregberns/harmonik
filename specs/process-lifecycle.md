...
- Multi-repo dispatch — no PL requirement governs it. This spec states the boundary only, so that a reader does not mistake the absence of a requirement for a refusal. **AMENDED v0.7.2 (bead `hk-specs-crossrepo-stale-i1bcg`) — cross-repo dispatch is implemented and gated by a safelist. The superseded rule is quoted at the end of this bullet.** A bead MAY declare `target_repo: <abs-path>` in its `## Branching` section. The value is bead-body only. No project-level tier and no spec-level tier supplies a default, so an absent `target_repo` means the bead lands in the supervised repo. The daemon resolves an **active repo** for every run. When `target_repo` is empty, or equals the daemon's `projectDir`, the active repo is `projectDir` and the run is a normal local run. When `target_repo` names a different path, the daemon first tests that path against the `allowed_repos` safelist in `.harmonik/config.yaml` under the `daemon:` block. A path on the safelist becomes the active repo. The worktree, the branching resolution, the no-commit guard, the merge, and the push all use the active repo. This list also named an escape-detection check until 2026-08-04. That check is deleted, so no run is failed because the main checkout is dirty. [run-state-machine.md] RSM-008 records the decision. The daemon's own protected-branch list is not applied to a cross-repo run, because that list names branches in the supervised repo and says nothing about the target repo. A path that is NOT on the safelist is refused: the daemon reopens the bead with `CrossRepoUnsafeError` and the reason names the safelist. The comparison is exact string equality between the declared path and a configured path. The daemon does no symlink resolution and no path normalization, so an equivalent path spelled differently is refused. An absent or empty `allowed_repos` list matches nothing, so every cross-repo bead is refused until the operator adds the path. The bead ledger, the daemon socket, the queue files, and the event log stay under `projectDir` for a cross-repo run. The combination of a cross-repo bead and a remote worker is not settled, and this spec makes no claim for it. Implementation evidence: `internal/daemon/workloop_runplan.go` `resolveRunPlanPlace`, `internal/daemon/branching.go` `CrossRepoUnsafeError` and `isInAllowedRepos`, `internal/projectconfig/projectconfig.go` `AllowedRepos`. Background: `docs/cross-repo-dispatch.md`, which still describes the pre-safelist state and is not normative. **SUPERSEDED at v0.7.2 (was correct through v0.7.1):** "the daemon lands only changes to the repo it supervises. Worktrees are rooted at `<projectDir>/.harmonik/worktrees/`; a bead touching another repo cannot satisfy the managed-worktree prefix check. A bead may declare `target_repo: <abs-path>` in its `## Branching` section; when set and differing from `projectDir`, the daemon reopens the bead with `CrossRepoUnsupportedError` so the operator applies the fix out-of-band (guard landed **hk-3r3**)." `CrossRepoUnsupportedError` still exists in the tree but is deprecated and no live path returns it.
...
- **orchestrator-agent** — a separate Claude Code session (cognition-bearing) that drives the daemon via its CLI. Deferred; NOT a component of the daemon. (see §4.6)
...
`default_harness`; append `queue_id`, `name`, `group_index`, and `bead_ids`;
status request `name`, `queue_id`, and optional zero-capable
`watched_group_index`, with response `queue`, `max_concurrent`, and the
receipt-completion fields; and cancel request shipped `queue`, additive
`queue_id`, and shipped `force`, with
response `queue_id` and `prior_status`. Queue-model owns the method-specific
selector precedence and default-main behavior; PL owns transport without
renaming or narrowing those records.

Queue submit/append/status/dry-run payload schemas and validation error codes
(`-32010..-32019`) are owned by [queue-model.md §6] and
...
the wire names. `queue-recover` is REGISTERED (recovery-error codes
`-32030..-32036`, owned by [queue-model.md §8.3b QM-052b]); it replaces the
`queue-resume` name that earlier revisions reserved for the same job.
`queue-remove`, `queue-pause`, and `queue-clear` remain deferred. The retired `enqueue` method has no alias or
compatibility shim.
...
The daemon's per-project file surface includes: `.harmonik/daemon.pid`
(PL-002, PL-002b); `.harmonik/daemon.sock` (PL-003);
`.harmonik/daemon.instance-id` (PL-005 step 0); `.harmonik/daemon.upgrading`
(PL-027(iv)); `.harmonik/daemon.state` (ON-030a); canonical named queues,
replace/archive intents, sibling transaction temps, the flat completion
receipt root, receipt temps, and completion-release markers under
`.harmonik/queues/` per [queue-model.md §3]; migration-only legacy input
`.harmonik/queue.json` for `main` (read/reconcile/remove only, never a new-write
target); the event log and dead-letter files under `.harmonik/events/`; the
event-ID high-water mark `.harmonik/event_id_hwm`; per-consumer spill files;
`.harmonik/beads-intents/`; `.harmonik/reconciliation-locks/`; and per-run
`<workspace_path>/.harmonik/lease.lock`. QueueStore is the sole live writer of
the queue-owned surface. PL reads it only in the bounded PL-005 startup phase
and capability handoff; no lifecycle, CLI, workloop, or legacy singleton path
may write or overwrite `.harmonik/queue.json` or a canonical queue directly.
The daemon MUST NOT read or write harmonik-owned state outside this surface;
subsystem-scoped state is owned by its respective spec.
...
**AMENDED v0.7.0 (amendment `review-loop-retirement`) — the enum narrows to `{single, dot}` and the review floor's mechanism changes from silent demotion to loud failure. The floor's guarantee is unchanged.** The retired text is quoted at the end of this requirement so a reader can tell a deliberate reversal from an omission.

The daemon's project-level configuration MAY carry an optional `workflow_mode` field whose only accepted new value is `dot` (`review-loop` is RETIRED and `single` is not a daemon default). A config file naming `workflow_mode: review-loop` or `workflow_mode: single` MUST fail at load with an actionable diagnostic that names `dot`; the daemon MUST NOT silently coerce it. When the field is absent, the daemon's default workflow mode MUST be `dot`, resolving to the embedded `standard-bead.dot` canonical graph (the build-embedded copy at `internal/daemon/standard-bead.dot`, registered with the daemon per [execution-model.md §7.5.1 EM-055] and [execution-model.md §7.5.5 EM-059]). **Review floor.** A bead resolved from this default or the built-in fallback is NEVER dispatched without a review gate. The exemplar graph provides the reviewer node ([workflow-graph.md §17 WG-047..WG-052]). If it fails to load, the daemon MUST fail the run with a diagnostic. It MUST NOT dispatch another workflow shape. A legacy per-bead `workflow:single` label is a no-review request. It MUST resolve the registered named `no-review-bead` DOT graph, emit `run_started{workflow_mode="dot", workflow_selection_source="legacy_single_label"}`, and then dispatch that graph. A persisted tier-0 queue item with `workflow_mode="single"` MUST resolve the same graph before validation and `run_started`, but record `workflow_selection_source="queue_item_single_mode"`. The queue item retains its raw legacy value for audit. Neither request may enter an imperative single dispatcher. The field MUST be read exactly once during PL-005 step 0 and cached for the daemon lifetime. Mid-run mode change via this surface is forbidden.

The daemon-level `workflow_mode_default` is the second-lowest-precedence tier of the Tier 0–4 workflow-mode resolution chain owned by [execution-model.md §4.3 EM-012a]: tier 0 persisted queue-item input → tier 1 per-task bead `workflow:<mode>` label per [beads-integration.md §4.3 BI-009a] → tier 2 per-project default → tier 3 daemon-level default (this requirement) → tier 4 built-in fallback (`dot`, with the [execution-model.md §4.3 EM-012a-FLOOR] review-floor: reviewed by construction, and a loud run failure — never `single`, never a silent change of shape — on embedded-graph load failure). The daemon MUST NOT override a higher-precedence tier when it is present. Resolution is the responsibility of the dispatch path per [execution-model.md §4.3 EM-012a]. EM-012a cites this requirement (PL-004a) as its tier-3 daemon-level config anchor. The two MUST agree on the dot default and the floor mechanism.

> **RETIRED TEXT (v0.6.x PL-004a), superseded by the above:** "The daemon's project-level configuration MAY carry an optional `workflow_mode` field whose value MUST be one of the enum `{single, review-loop, dot}`. … **Review floor.** If the embedded `standard-bead.dot` graph fails to load (parse failure, missing artifact, or schema-version incompatibility per [execution-model.md §7.5.3 EM-057] check 1), the daemon's default MUST fall back to `review-loop`, NEVER to `single` — the fallback ordering is `dot → review-loop`, and the daemon MUST emit a diagnostic recording the fallback."
...
which the daemon-level default resolves to a no-review graph. A `config.yaml`
`daemon.workflow_mode: single` is therefore invalid.

The two legacy no-review requests are distinct from the config default. A persisted tier-0 queue
item with `workflow_mode = single` MUST select the registered named no-review DOT graph before
validation and `run_started`. It MUST record
`workflow_selection_source = queue_item_single_mode`. The persisted item MUST retain its raw
`workflow_mode = single` value for audit. A tier-1 bead `workflow:single` label MUST select the
same graph and record `workflow_selection_source = legacy_single_label`. Both requests resolve
to `workflow_mode = dot`. Neither request may enter an imperative single dispatcher.

The config read is the concrete realization of the PL-004a obligation that the field "MUST be
read exactly once during PL-005 step 0". It supplies only the tier-3 daemon default in the
Tier 0–4 resolution chain owned by [execution-model.md §4.3 EM-012a]: tier 0 per-item queue
input, tier 1 per-task label, tier 2 per-project default, tier 3 daemon default, then tier 4
built-in fallback. PL-004b MUST NOT change that order. Resolution against the higher-precedence
tiers remains owned by EM-012a.
...
8a. **Read durable startup markers and recover queue-owned state before
readiness.** Read `.harmonik/daemon.state` (per [operator-nfr.md §4.7
ON-030a]) and `.harmonik/daemon.upgrading` (per [operator-nfr.md §4.6 ON-020a]
and PL-027(iv)). Establish/type-check
`.harmonik/queues/.completion-receipts`, then classify canonical named queues,
the migration-only legacy `.harmonik/queue.json`, replace/archive intents,
completion receipts, completion-release markers, and their selected temps per
[queue-model.md §3 QM-002, QM-005..QM-007]. Queue recovery MUST finish or
retain exact per-name refusal before QueueStore installation and step 9.
Marker semantics:
   ...
   - **Queue-owned state present.** A supported exact classifier MUST converge
     each queue/intent/receipt/marker/migration state or retain its affected
     name/identity refusal. Unsupported protocol schema, wrong receipt-root
     type, or an incapable whole-capability target refuses readiness and
     preserves bytes. Valid missing post-release markers may be created only
     after the queue-model proof of durable old canonical/intent absence and
     absent old ownership. Startup MAY run eligible receipt GC after readiness;
     unresolved completion cleanup MUST resolve before the affected name is
     available. Startup MUST NOT inspect JSONL or synthesize/retry any queue
     event.
   - **All markers absent.** The daemon proceeds to step 9 and transitions to `ready` per the normal path.
   - **Marker file unreadable / corrupt.** For `daemon.state` and
     `daemon.upgrading`, retain the existing ON-035 behavior. Queue-owned
     canonical/protocol records are different: preserve them and apply the
     exact queue-model corrupt/unsupported classifier; never treat them as
     absent or use events as repair.
...
> NOTE (window-naming carve-out): the `hk-<hash6>-` WINDOW-name prefix used when the daemon spawns into a session it does not own (`owns_session=false`) per [workspace-model.md §4.1 WM-002a] / PL-021b §4 is a SESSION-INTERNAL window sentinel and is UNAFFECTED by this amendment. PL-006a clause (a) concerns SESSION names only.
...
`project_hash` to shell fallbacks such as `hk-supervise.sh`. Native launch tooling
(`harmonik start captain`, `harmonik start crew <name>`) and this subcommand MUST share
the same `internal/lifecycle` hash accessor; no caller may reimplement SHA-256.
Normative contract:
...
(ii) **Captain / keeper (NEW; C3-written).** A sentinel file `.harmonik/cognition/captain.sentinel` is present AND `kill(captain_pid, 0) == 0` for the PID in `.harmonik/cognition/captain.pid`. These sentinel files are written by the native captain launcher (`harmonik start captain`) — generalizing the existing `supervisor.sentinel`/`supervisor.pid` probe. If no `captain.sentinel`/`captain.pid` exists, the probe finds nothing, and this owner-proof is a NO-OP (no false skip, no regression).
...
#### PL-006h — Dispatched-bead reconcile before the queue load

The (a-queue) exclusion of §PL-006 leaves every bead that a queue records as `dispatched` untouched by the sweep. This requirement is what pays for that exclusion. Without it, a crash between the durable bead claim and the run launch leaves the bead `in_progress` and the queue item `dispatched`, and no rule repairs either one.

At §PL-005 step 3, after the orphan sweep and BEFORE the queue load of step 8a, the daemon MUST reconcile the beads of any prior dispatch against the durable run records under `.harmonik/runs/` and the durable event log. The set MUST include every bead that a canonical named queue records as `dispatched`, and every bead that the event log records a run for. The daemon MUST apply the following rules.

1. **Dead run session.** A run record names a tmux session that no longer exists. The daemon MUST reset the bead `in_progress → open` and MUST then remove the run record.
2. **Orphaned run.** The event log holds a `run_started` for a run on this bead and no terminal event for that run, and no live run record names the bead. The daemon MUST emit `run_failed` for that run and MUST reset the bead.
3. **Claim with no run.** A queue records the bead as `dispatched`, no still-orphaned `run_started` names it, and no live run record names it. The daemon MUST reset the bead. This is the crash window between the durable claim write and the run launch, and this rule is the only one that reaches it.
4. **Terminated run that still holds the bead.** The event log holds a terminal event for a run on this bead, no queue records the bead as `dispatched`, and no live run record names it. The daemon MUST reset the bead. This releases a dispatch lock that a lost queue-item update left set.
5. **Live run.** A run record names a live session. The daemon MUST NOT reset the bead, and MUST NOT change the queue item. Adoption of the live run owns this case, and a reset here is the double dispatch that the (a-queue) exclusion exists to prevent.

Every reset above MUST use the same intent-logged `reset` write as the sixth bullet of §PL-006 (BI-010d op: `in_progress → open`), with that bullet's idempotency key. Every reset in rules 2, 3 and 4 MUST first read the bead's current status, and MUST proceed only when the bead reads `open` or `in_progress`. A bead that has landed — closed, tombstone, blocked, or deferred — MUST NOT be reset by those rules. Rule 1 MUST hold the same guard. It does not hold it today. See the second declared gap below.

**The order is the requirement.** The cross-check of [queue-model.md §3.2a QM-002a] reverts a `dispatched` item only when the bead reads `open`. This reconcile is what makes the bead read `open` by the time that check runs. A daemon that loads the queues first, or that drops these passes, breaks no other rule in this section on its face, and it strands the work: the item stays `dispatched`, the dispatch loop never selects it again, and the scheduled Class B repair of [reconciliation/spec.md §4.3 RC-020a] skips it because a queue records it as dispatched.
...
> DECLARED IMPLEMENTATION GAP (2026-08-11). Rule 1 does not hold the landed-bead
> guard. `daemon.adoptDeadRunSessions` calls `ResetBead` directly, without first
> reading the bead. `brcli.Adapter.ResetBead` does not guard either. So a daemon
> killed after a bead closed but before the run was torn down finds a run record
> with a dead session on the next start and reopens finished work. If a queue
> still records the item as `dispatched`, the cross-check of
> [queue-model.md §3.2a QM-002a] then reverts the item to `pending` and the
> finished work is dispatched a second time. Rule 1 also treats a run record with
...
**PL-013 — Retired in extqueue v0.1.** The prior requirement covered the
Beads-ready-poll dispatch model. Under [queue-model.md], dispatch input is the
QueueStore in-memory set reconstructed from every supported canonical named
queue and queue-owned transaction record at PL-005 step 8a, or admitted by
`queue-submit`; `.harmonik/queue.json` is only the migration input for `main`.
Idle (no active group across the named set) does not terminate the daemon.
Queue absence or completion MUST NOT cause exit; exit occurs only on explicit
stop, operator upgrade, or crash. The retired re-query cadence remains absent.
...
> NOTE: PL-019 is not required today: an implementation MAY conform to Core without ever running a supervisor process. §10.1 retains the deferred-extensions carve-out. The normative text above is the stable contract for any implementation that does run a supervisor; its presence here (rather than only in cognition-loop.md) is because the daemon-side obligations — refusing to start the supervisor, honoring the orphan-sweep exclusion, and keeping the `.harmonik/cognition/` subtree unowned — are daemon-lifecycle requirements.
>
...
All cross-subsystem registries declared by foundation specs — including the event bus ([event-model.md §4.3]), the control-point registry ([control-points.md §4.1]), the handler registry and skill registry ([handler-contract.md §4.1, §4.11]), and the policy registry — MUST be instantiated inside the composition root (`internal/daemon`) on startup per §PL-005 step 0. No out-of-daemon registry is permitted per [architecture.md AR-INV-007]. Later process-geometry changes that split registries across processes are tracked by architecture's deferred surface per [architecture.md §4.5 AR-019] and do NOT require a foundation amendment to PL per [architecture.md §4.5].
...
The daemon MUST version-pin `ntm` per the external-inputs protocol (parallel pattern to [operator-nfr.md §4.4 ON-017]); supported `ntm` versions MUST be declared in the release manifest. An `ntm` version outside the supported set MUST be detected during §PL-005 step 4 Cat 0 pre-check and MUST produce ON §8 code 22 (`ntm-unavailable`) per PL-008a plus `infrastructure_unavailable{failed_prerequisite=ntm_unavailable}` per [event-model.md §8.7.15]. `ntm` not on PATH, tmux missing, or failing the version probe MUST classify identically as Cat 0 per [reconciliation/spec.md §8.1].

**Fail-fast obligation — scoped to the substrate that needs tmux (AMENDED v0.6.2).** The clause below previously read as an unconditional prohibition on running the daemon at all without tmux. It is now conditioned on which spawn substrate is wired at the composition root:

- **When the tmux-hosting substrate is the selected substrate** (PL-021b, the default), the daemon MUST NOT attempt to spawn handler subprocesses without working tmux hosting, and MUST refuse at startup rather than reaching the dispatch loop. Every spawn on that substrate routes through `tmux new-window`, so booting is a claim the daemon cannot honor.
- **When a substrate that hosts agents outside tmux is selected**, tmux absence MUST NOT refuse the boot. The daemon MUST instead resolve tmux hosting as a capability and, when it is unavailable, announce the loss at startup per PL-028b, enumerating the specific capabilities that are gone. Continuing is permitted **only** with that announcement.

**"Silently degrading to non-tmux mode" remains forbidden.** What this amendment changes is the set of honest responses, not the prohibition on dishonest ones. The forbidden behaviors are unchanged and are restated so they cannot be read as relaxed: the daemon MUST NOT report a tmux-hosted capability it does not have, MUST NOT defer discovery of a missing prerequisite to the first dispatch, and MUST NOT reach a `ready` state whose announced capability set is wider than the resolved one.

**The dependency is the tmux BINARY at spawn time, not an ambient tmux CLIENT at boot.** A daemon started with `$TMUX` unset is NOT a fail-fast condition under any substrate; the daemon creates and owns its own session (PL-021b item 3). Inspectability is preserved through that session, not through the operator's ambient one.

**SCOPE — this amendment frees the DAEMON boot path, and it does not (yet) describe the CLI entry points.** Stated explicitly because the natural over-reading is "harmonik runs without tmux", and that is not true of the shipped system today. `harmonik daemon` conforms to the clauses above. The CLI verbs that reach work by other routes have their own `$TMUX` gates, and those were not touched:

> DECLARED SURFACE GAP (dated 2026-07-28, measured rather than assumed). `harmonik run` self-exec-replaces itself with `tmux new-session` when `$TMUX` is unset, or exits when tmux is not on PATH — **and that gate sits ahead of its daemon-up check**, so even an invocation that is nothing more than a thin socket client to an already-running daemon refuses to operate outside tmux. Note precisely what that is and is not. It does not violate PL-028b item 4, whose subject is the daemon throughout and which forbids deferring a missing prerequisite to dispatch — `harmonik run` refuses up front rather than deferring. What it does is work the constraint around silently, by relocating the operator's process instead of stating the constraint, which is the shape this requirement's own silent-degradation prohibition exists to forbid. It would fail item 4's honesty rule if these clauses bound the CLI surface. They do not yet, which is the point of this section. The guard's own stated rationale ("agent windows are created inside the caller's session") is additionally false in the current code, which resolves the deterministic per-project session and explicitly ignores the ambient one. A sweep of `cmd/` for `$TMUX` reads finds this is the ONLY such pre-daemon gate; the sole other read is the capability probe of PL-028b. Filed as `hk-o3aj5`. **A future amendment extending these clauses to the CLI surface MUST close this gap rather than inherit it.**

**Amendment provenance.** Operator direction 2026-07-28 reopened locked decision #4 ("Agent runner (S04): NTM-wrapped Go. Inspectability via tmux is a requirement, not a preference") and narrowed it: *"we should probably be able to run it any way."* The narrowing is recorded in [STATUS.md](../STATUS.md) §"Decisions in force" and in `plans/2026-07-27-delete-and-rewrite/CHARTER.md` §3. tmux is NOT removed — it stays the default host and the operator's inspection surface. The solo-dev ergonomics carve-out this clause cited survives intact for the tmux substrate, which is the default.

This is a narrowing, not a reversal, and it has an operator-ratified precedent inside `specs/`: [agent-input.md] OQ-AIS-003 already resolved that "D5 (tmux inspectability) holds LITERALLY for Claude, which runs interactive-in-tmux. Only the structured Codex driver … observes via the capture tee … and the process need not be hosted under tmux for the structured-driver path." The degradation shape likewise has one: [digest-command.md §DC-009] already specifies that "absence of tmux or an idle server degrades to an empty fleet, not an error."

Tags: mechanism
Axes: llm-freedom=none; io-determinism=deterministic; replay-safety=safe; idempotency=idempotent

#### PL-021b — Direct-tmux substrate (current alternative to ntm adapter)

The daemon MUST consume a direct-tmux substrate in place of the ntm adapter described in PL-021. The direct-tmux substrate is implemented by package `internal/lifecycle/tmux` and exposes the following obligations:
...
2. **Tmux availability check.** The daemon MUST probe tmux at PL-005 step 4 (Cat 0 pre-check) by invoking `tmux -V` and asserting major version ≥ 3.0. **Probe failure is fatal only when this substrate is the selected one (AMENDED v0.6.2).** When it is, the daemon MUST exit with ON §8 code 22 (`tmux-unavailable`, retitled from the v0.4.x ntm-unavailable). When a substrate that hosts agents outside tmux is selected, probe failure MUST NOT exit; it resolves to unavailable tmux hosting and is announced per PL-028b. This obligation supersedes PL-021a's `ntm`-targeted absence-detection until an ntm adapter ships; PL-021a remains in force as the long-term contract once it does.

   > DECLARED IMPLEMENTATION GAP (dated 2026-07-28, and stated so that no reader infers conformance). `cmd/harmonik/tmuxhosting.go` `reportNoTmuxHosting` returns exit code **1**, not 22, on the fatal branch. The divergence predates this amendment (the deleted `$TMUX` guard also returned 1) and is inherited, not introduced by it. It is recorded here rather than fixed because the fix belongs to the ON §8 taxonomy pass, not to a boot-behavior change. Filed as `hk-0cjb8`.
3. **Session resolution (AMENDED v0.6.2).** At startup, before dispatching the first handler subprocess, the daemon MUST resolve the tmux session it will host windows in. The resolution has exactly three outcomes and MUST NOT have a fourth:
   - **Ambient session usable.** `TMUX` is set at daemon startup AND the live session it names is not one of the system sessions that must never receive implementer windows (the supervisor session and the flywheel session per PL-006a / PL-019). The daemon MUST use that session. Windows the daemon creates in it MUST carry a sentinel prefix `hk-<hash6>-` where `<hash6>` is the first 6 hex chars of the project hash. The daemon does NOT create or kill the operator's session.
   - **No usable ambient session, tmux working.** `TMUX` is unset, OR it is set but names a system session, OR the live session name cannot be read. The daemon MUST resolve the deterministic per-project session `harmonik-<project_hash>-default` (PL-006a naming, the same name PL-028 refinement step 2.ii computes), MUST create it if absent, and MUST keep it alive for the daemon's whole lifetime because it now owns it. This is a supported, first-class boot path — **not** a degradation and **not** a defect. Inspectability is fully preserved: the operator attaches with `tmux attach -t harmonik-<project_hash>-default`. What is lost is only the convenience that the operator is already looking at the session.

     The daemon MUST NOT consult `tmux display-message` to discover the live session when `TMUX` is unset. With no ambient client that command answers with whatever session the tmux server most recently made current — possibly a crew or captain session — so trusting it would scatter implementer windows into sessions the daemon does not own.
   - **tmux unusable.** The item-2 probe fails, or the deterministic session cannot be created. There is no tmux hosting. Under this substrate that is fatal per item 2; the announcement obligation is PL-028b's.

   **`owns_session` is a function of who created the session, not of whether `$TMUX` was set.** The second outcome is reachable with `TMUX` set, so the two conditions are no longer interchangeable. Consumers of the `owns_session` input — item 4's window-naming function and [workspace-model.md §4.1 WM-002a] — MUST read it as "the daemon created and owns this session".

   **A daemon that reaches the dispatch loop with `TMUX` unset is NO LONGER a defect.** The prior text made it one. That sentence is withdrawn: the condition it named was never the real dependency (see PL-021a, "the dependency is the tmux BINARY at spawn time"), and the deterministic-session fallback that makes the second outcome safe already existed and was already exercised in production by the supervisor-revive path.
4. **Window naming.** Window names MUST be a deterministic function of `(bead_id, phase, iteration_count, project_hash, owns_session)` per [workspace-model.md §4.1 WM-002a]. The function MUST be replay-stable: replaying a recorded run reproduces the exact window name. In the `owns_session=true` mode the name is `<bead_id>` (the `single` phase) or `<bead_id>/i<n>` / `<bead_id>/r<n>` (the implementer / reviewer phases). In the `owns_session=false` mode — the daemon is spawning into a session it did not create — the same name is prefixed with `hk-<hash6>-`.
...
**Observation-only for the STRUCTURED (Codex) driver path only.** Once the structured agent-input substrate (`specs/agent-input.md` (AIS)) owns a daemon run's input, the direct-tmux substrate carries **no input** for THAT (structured Codex-driver) run — the Codex driver owns the child's stdio directly. This does NOT apply to the **tmux/Claude path**: per COORD c019/c021, tmux paste (`load-buffer`/`paste-buffer`/`send-keys`) is Claude's FIRST-CLASS input transport (a peer input method, not a demoted one), and its positive ack is the Claude-hook-bridge event — `outcome_emitted` (`Stop`) / `agent_ready` (`SessionStart` start/resume) per [claude-hook-bridge.md §4.5 CHB-013], [agent-input.md AIS-003] — never a `capture-pane` scrape. The read verb `tmux capture-pane` MAY survive as a human-facing observation seam only (`internal/lifecycle/tmux/osadapter.go`) and MUST NEVER be read as an ack/liveness signal; the READ boundary of §5 (no `pipe-pane` structured consumption) is unchanged. Inspectability of the **structured Codex driver** (which owns raw stdio, not a TUI pane) is satisfied per the AIS capture-tee (tail the recorded wire) + an optional observation pane; **D5** tmux inspectability holds LITERALLY for Claude, which runs interactive-in-tmux (OQ-AIS-003 resolved).
...
> **DEMOTED (M2 agent-input-substrate).** The `load-buffer` + `paste-buffer` write mechanism is **removed from the daemon RUN input path** — daemon-run task delivery and inter-phase injection are now owned by `specs/agent-input.md` (AIS) (the structured input port + real ack). This clause is **PRESERVED** for exactly two non-run consumers: (a) the **session-keeper** paste path ([session-keeper.md §SK-002], `internal/keeper/injector.go`) per the **D6 carve-out**; and (b) the **interactive-session nudge / boot-seed** paths (`cmd/harmonik/{captain,crew,comms}.go`) that inject into panes they did not spawn via `handler.Substrate`. For those two consumers the `load-buffer` + `paste-buffer` discipline, buffer-name format, delete-buffer cleanup, and `daemon_pane_write` audit **remain NORMATIVE**. For daemon runs, they are superseded — see AIS.

> **Deletion boundary (post-bake, AIS-INV-001 acceptance-gated).** After the AIS bake window passes (C5 fault-harness N-green + output-or-stale oracle), the daemon-run paste-inject stack (`internal/daemon/pasteinject.go`, the input portions of `tmuxsubstrate.go`, and the six type-asserted side-interfaces) is deleted. The tmux write verbs `load-buffer`/`paste-buffer`/`send-keys` in `internal/lifecycle/tmux` are **NOT deleted** while the D6 keeper carve-out and the CLI nudge paths still consume them. `capture-pane` (read) is retained for observation. tmux window **spawn** verbs (`new-window`, `kill-window`, `list-panes`) are retained (shared with crew spawn + M4 remote).

PL-021b §5 forbids the daemon from *reading* pane output via `tmux pipe-pane` or any equivalent channel. This clause addresses the symmetric case — the daemon *writing* content into a pane. For daemon runs, initial-task delivery and inter-phase message injection are now **AIS-owned** (`specs/agent-input.md` (AIS) — the structured input port + AIS-INV-001 bounded-liveness ack); the paste path's live purpose narrows to the session-keeper handoff-cycle nudges and the interactive boot-seed (see [docs/claude-session-comms-audit-2026-05-13.md §6 B2]). **Superseded for daemon runs by `specs/agent-input.md` (AIS) (AIS-INV-001 bounded-liveness ack).**
...
> NON-REGRESSION NOTE (extqueue v0.1). The queue methods of PL-003a
> (`queue-submit`, `queue-append`, `queue-status`, `queue-dry-run`, and queue
> cancellation) ride the same socket and inherit listener-fd passing unchanged.
> Queue state is reconstructed post-exec by PL-005 step 8a's complete
> queue-owned classifier; no second listener, fd, or adoption path exists.
...
(vi) **Whole queue-transaction capability.** Before exec/handoff, the outgoing
daemon MUST inspect the on-disk queue-owned record/schema surface and compare
the target's capability as one unit. The target MUST support: replace-intent
version and exact prior/candidate/temp classification; archive-intent version
and exact linked cancellation handoff; completion-receipt v1 and final-intent
binding; flat receipt-root creation/type/parent-sync; no-replace exact-byte
receipt install; completion-release marker v1 installation/recovery;
trusted-UTC/regressed-clock 720-hour eligibility; queue-ID plus completed-byte
digest CAS; deterministic exact-ID live/receipt status; receipt-first
marker-last GC; and unresolved-state refusal/diagnostics.

Supporting parsers without the matching cleanup/status/retention/ambiguity
behavior is incompatible. A target lacking marker v1 or receipt-first GC MUST
refuse before handoff whenever receipts, markers, or marker temps may exist and
MUST NOT fall back to receipt completion time. An incompatible upgrade,
downgrade, or rollback preserves every record and reports the unsupported
record/version or missing behavior; it does not clean up, reinterpret, consult
JSONL, or replay events.
...
- **`harmonik runner`** — solo-dev convenience command. Executes the following in order: (1) if no daemon is running for the project, start the daemon (equivalent to `harmonik daemon &`); (2) wait for `daemon_ready` via the ready-detection protocol of PL-009b (bounded timeout per OQ-PL-002 defaults); (3) open a tmux session under the project's harmonik naming convention (prefix `harmonik-<project-hash>-`) with one pane for the daemon's log output and additional panes per active handler session (per ntm inspectability — locked decision #4, as narrowed 2026-07-28 per PL-021a: step 3 is the tmux-hosting path, not a precondition on the daemon itself); (4) if the `--orchestrator-agent` flag is supplied, spawn a Claude Code session in a separate tmux pane with the orchestrator-agent prompt and CLI access per §PL-019. On Claude Code unavailable, exit with ON §8 code 23 (`orchestrator-agent-unavailable`). On `ntm` unavailable, exit per PL-021a (ON §8 code 22 `ntm-unavailable`) — an obligation of `harmonik runner`, whose whole purpose is the tmux view, and NOT of `harmonik daemon`, which resolves tmux hosting per PL-028b instead. `harmonik runner` is a distinct entry point with its own exit-code surface; it is NOT a shell alias for `daemon` + `attach`.
- **`hk queue append <group-index> <bead-id ...> [--queue <name>] [--queue-id <uuid>]`** — append through JSON-RPC `queue-append`, preserving shipped `queue_id`, `name`, `group_index`, and `bead_ids`; exact selector precedence and validation are queue-model-owned. Daemon-not-running → exit code 17.
...
- **`hk queue recover [--queue <name>]`** — recover one queue parked at
  `paused-by-failure` through JSON-RPC `queue-recover`. The daemon re-arms the
  failed items and returns the queue to `active` per [queue-model.md §8.3b
  QM-052b]; recovery-error codes are `-32030..-32036`. It is NOT `hk queue
  resume`, which releases a drain pause only. Daemon-not-running → exit code 17.
...
> NOTE. The `harmonik enqueue` subcommand and its JSON-RPC `enqueue` method are
> retired. The live-daemon `hk queue <verb>` surfaces are submit, append,
> status, dry-run, cancel, and recover. `recover` is the failed-queue recovery
> verb of [queue-model.md §8.3b QM-052b]; it is not a mode of `resume`, which
> releases a drain pause only. remove/clear stay deferred.

Tags: mechanism

#### PL-028 refinement — `hk tmux-start` subcommand replaces `harmonik runner` tmux duties

The `harmonik runner` four-step lifecycle of PL-028 obligates the daemon (or runner wrapper) to open a tmux session in step 3. This obligation is satisfied by a distinct subcommand `hk tmux-start`:
...
1. **Trigger conditions.** `hk tmux-start` is invoked by the operator explicitly when starting work from a non-tmux shell. MUST NOT be invoked automatically by the daemon. When the operator is already inside a tmux session (`$TMUX` set), `hk tmux-start` MUST refuse with a friendly message (exit code 0) and SHOULD print the session name they are already in.
   ...
   **It is a convenience, not a precondition (AMENDED v0.6.2).** `hk tmux-start` gives the operator an attached shell to work from. It is NOT required before `harmonik daemon`: as of the PL-028b amendment the daemon creates and owns its own `harmonik-<project_hash>-default` session when no usable ambient session exists (PL-021b item 3), which is the same session name step 2.ii computes. An operator who only wants to watch may skip this command entirely and run `tmux attach -t harmonik-<project_hash>-default` against the daemon's session.
...
3. **`hk` started inside an `hk tmux-start`-created session.** When the operator runs `hk` from inside the session created by step 2.iv, `$TMUX` is set. `hk` therefore takes the PL-021b item-3 ambient-session path, creates handler windows in that same session. This is one of three resolution outcomes, not the only supported one — see PL-021b item 3.
4. **Relationship to PL-028 `harmonik runner`.** `harmonik runner` step-3 obligation is satisfied by `hk tmux-start`; `harmonik runner` MAY be implemented as a convenience or deferred entirely.
...
#### PL-028b — `hk` daemon tmux-hosting resolution and honest degradation at startup

**AMENDED v0.6.2 — this requirement previously mandated the opposite behavior.** It required the daemon to refuse to enter the ready state whenever `$TMUX` was unset, to exit with ON §8 code 24 (`tmux-session-unavailable`, PL-INTERIM, never absorbed into ON §8), and it forbade the daemon from creating its own session. All three obligations are **WITHDRAWN**. The retired text is quoted at the end of this requirement so a reader can tell a deliberate reversal from an omission. Provenance: operator direction 2026-07-28 narrowing locked decision #4; see PL-021a §"Amendment provenance".

**What replaces it.** At `harmonik daemon` startup the daemon MUST *resolve* tmux hosting rather than demand it, per PL-021b item 3, and MUST report the resolved capability honestly:

1. **`$TMUX`-unset is not a failure condition.** It MUST NOT be added to the Cat 0 failure surface, MUST NOT refuse the ready state, and MUST NOT exit. The daemon creates and owns the deterministic `harmonik-<project_hash>-default` session (PL-021b item 3, second outcome) and proceeds normally. `hk tmux-start` is a convenience for the operator who wants an attached shell, not a precondition for the daemon.
2. **Unavailable tmux hosting MUST be announced at startup, before any dispatch.** When PL-021b item 3 resolves to its third outcome, the daemon MUST write an operator-facing diagnostic naming (a) the concrete reason tmux hosting is unavailable and (b) the specific capabilities that are consequently unavailable. Announcing "tmux is missing" alone does not satisfy this; the requirement is the capability list, because that is what the operator has to decide about. At minimum the announcement MUST name the loss of `harmonik supervise start` and therefore of daemon auto-revive, the loss of launch-layer session creation (`harmonik start crew` / `start captain` / keeper nudges, PL-006a), and the loss of attachable agent panes.
3. **Refuse when the loss is total; degrade only when it is partial.** When the selected substrate hosts agents in tmux, the announcement MUST be a refusal and the daemon MUST NOT proceed (PL-021b item 2). When the selected substrate hosts agents outside tmux, the daemon MUST proceed after announcing — tmux absence MUST NOT refuse that boot, per PL-021a — and MUST NOT then present itself as tmux-inspectable.
4. **Never fake, never defer.** The daemon MUST NOT report a capability the resolution did not produce, and MUST NOT allow a missing prerequisite to first surface as a dispatch-time failure. A capability that is genuinely gone is announced loudly at boot and is absent thereafter.

**Socket-bind ordering note (retained, still true where a refusal occurs).** PL-005 step 4 (Cat 0 pre-check) follows step 3a (socket bind), so a refusing daemon will have bound and immediately released its Unix-domain socket. This is acceptable because no handler subprocesses have been spawned and no event-bus state has been written beyond the `daemon_started` envelope. A refusing daemon MUST NOT reach the ready state, MUST NOT emit `daemon_ready`, and MUST NOT advance to PL-005 step 5 or beyond.

**Exit-code consequence.** With the `$TMUX`-unset refusal withdrawn, the daemon no longer has any use for code 24. Code 24 remains claimed only by `hk tmux-start` (PL-028 refinement item 5). It was never absorbed into ON §8 and stays PL-INTERIM. Note that `cmd/harmonik/supervise/start.go` reuses the literal 24 for a different condition (`ExitCodeFlywheelSessionExists`); reconciling that collision belongs to the ON §8 taxonomy pass tracked in `hk-0cjb8`.

> RETIRED TEXT, for reference only. It imposes no obligation.
>
...
The `hk queue` family MUST be dispatched in `cmd/harmonik/main.go` before
`flag.Parse` is called on the global flag set. The dispatch MUST recognize
`os.Args[1] == "queue"` and read `os.Args[2]` as the verb (`submit`, `append`,
`status`, `dry-run`, `cancel`, `recover`). Each verb accepts both `--flag value` and
`--flag=value`. Unrecognized verbs exit 2 and name the supported set. Each verb
routes through `internal/queue/cli` to the target daemon's queue RPC; live
operator cancellation is handled only by the target QueueStore and the linked
replace/archive protocol of [queue-model.md QM-007].

The daemon-not-running path MUST be uniform across all verbs: socket-probe
`ECONNREFUSED` against `.harmonik/daemon.sock` → exit code **17**
(`multi-daemon-target-missing` per ON §8 / PL-008a). In particular,
daemon-unreachable cancellation MUST perform zero local canonical/temp/archive/
intent/legacy/receipt/marker/event writes; no local archive or audit fallback
exists. The single-project remediation prose is owned by
[operator-nfr.md §4.1 ON-004].

For reachable live cancellation, successful RPC response requires durable
cancelled replacement, selected archive and applicable legacy cleanup,
archive-intent absence, and QueueStore/refusal release. Only then may the
daemon attempt the typed Class O `queue_cancelled_operator` audit, followed by
RPC success. Audit failure is diagnostic and MUST NOT roll back archive
success, reacquire ownership, block same-name reuse, or suppress the response.
Graceful shutdown uses the same queue-owned archive primitive but emits no
operator audit. Neither path creates a completion receipt.
...
precedent the daemon already applies to the embedded canonical graph (the `//go:embed
standard-bead.dot` directive on `standardBeadDotSrc` in `internal/daemon/standardgraph.go`); the
init asset bundle is the first directory-form embed in the `cmd/harmonik` package.
...
> documented for the embedded `standard-bead.dot` in the keep-in-sync header comment above
> `standardBeadDotSrc` in `internal/daemon/standardgraph.go`.
>
...
This spec does not own a failure taxonomy. Startup failure modes are cataloged per §PL-008 (obligation owned by [operator-nfr.md §4.1 ON-003]); §PL-008a names the codes this spec consumes from the authoritative ON §8 taxonomy. Codes 22 (`ntm-unavailable`) and 23 (`orchestrator-agent-unavailable`) — declared PL-INTERIM in v0.4.0 — were absorbed into ON §8 in ON v0.4.0 and are now first-class entries; no PL-side interim marker remains. Four residuals across codes 22 and 24 are declared here rather than left to be discovered, and all four are tracked in `hk-0cjb8`: (a) ON §8 titles code 22 `ntm-unavailable` while PL-021b item 2 retitles the same code `tmux-unavailable`, a split ON never absorbed; (b) the implementation returns 1, not 22, on the fatal tmux-hosting branch (PL-021b item 2, declared gap); (c) ON §8's code-22 row still reads tmux-missing → exit 22 unconditionally, whereas PL-021b item 2 now conditions that on the selected substrate — the row inherits the condition by its "per PL-021a" citation, but the ON-side wording has not caught up; (d) code 24 (`tmux-session-unavailable`) was never absorbed into ON §8 and, since the PL-028b amendment withdrew the daemon's `$TMUX`-unset refusal, is claimed only by `hk tmux-start` (PL-028 refinement item 5) — with `cmd/harmonik/supervise/start.go` separately reusing the literal 24 for a different condition. Code 17 (`multi-daemon-target-missing`) is consumed by the `hk queue *` CLI surfaces per PL-028 / PL-028c; the single-project `ECONNREFUSED` remediation prose extension is owned by [operator-nfr.md §4.1 ON-004]. Run-failure taxonomy is owned by [execution-model.md §6.3]. Reconciliation-category taxonomy is owned by [reconciliation/spec.md §8]. Crash semantics (§PL-024, §PL-025, §PL-026) route through those taxonomies rather than defining their own.
...
- **[queue-model.md §2, §3, §6, §7, §8]** — canonical named queues,
  migration-only legacy input, transaction/archive intents, completion
  receipts/release markers, JSON-RPC payloads, validation, cleanup, and
  retention; PL-005 step 8a classifies the complete queue-owned surface and
  PL-028c names the daemon-only CLI transport.
...
- **[workspace-model.md §4.7 WM-026]** — temp+rename+fsync(parent_dir) atomic write discipline; §PL-005 step 0, §PL-027(iv), and QueueStore's canonical named-queue and queue-owned transaction record writes per queue-model.md consume this. Legacy `.harmonik/queue.json` remains migration-only input.
...
> INFORMATIVE: Reverse dependencies are computed on demand from all specs'
> `depends-on` lists. Queue-model relies on PL-003a transport, PL-005 complete
> queue-owned startup recovery, PL-027 capability handoff, and PL-028c
> daemon-only cancellation.
...
- **[agent-input.md]** — structured agent-input substrate (AIS-INV-001 bounded-liveness ack). Superseded for daemon runs by `specs/agent-input.md` (AIS): §PL-021b (observation-only after AIS) and §PL-021d (demoted daemon→pane write mechanism) hand the daemon-run input path to AIS; PL-021d's write discipline is retained only for the session-keeper (SK-002) and interactive-session nudge consumers.
...
**Core.** An implementation conforms to Core when it passes PL-001 through PL-028 (including PL-002a, PL-002b, PL-003a, PL-003b, PL-006a, PL-008a, PL-009a, PL-009b, PL-011a, PL-014a, PL-018a, PL-020a, PL-021a, PL-025a, PL-028c) and satisfies all five invariants (PL-INV-001, -002, -003, -004, -005). PL-013 is retired per extqueue v0.1 and is not a conformance gate. The operator-facing `harmonik upgrade` contract sub-obligations (PL-027 (i)–(v)) depend on operator-nfr ON-020 finalizing for full conformance; the daemon-internal mechanics (i)–(iv) are in-scope for Core as declared here.

**Deferred extensions.** Orchestrator-agent session integration (PL-019) is OPTIONAL. An implementation MAY conform to Core without ever spawning an orchestrator-agent.
...
- **PL-021, PL-021a, PL-022, PL-023** — lint rule: `internal/adapter/ntm` imports only the allowed ntm surface (process/tmux); importing ntm pipeline or SwarmPlan packages is a build failure. Absence-detection test (RESTATED v0.6.2 — the prior wording asserted the unconditional refusal this revision withdrew, and would now be a false green): run the daemon on a PATH without tmux **with the tmux-hosting substrate selected** and assert it refuses to reach the dispatch loop, naming the missing prerequisite; run the same fixture **with a substrate that hosts agents outside tmux** and assert it boots and reaches the dispatch loop after emitting the PL-028b capability-loss announcement. Satisfied by `cmd/harmonik/main_test.go` `TestRunTmuxUnusable_TmuxSubstrateStillRefuses` and `TestRunTmuxEnvUnset_BootsAndReachesDispatchLoop`. The exit-code half of this obligation (code 22 vs the implementation's 1) is a declared gap, not a satisfied assertion — see PL-021b item 2 and `hk-0cjb8`.
- **PL-021b item 3, PL-028b** — resolution and announcement tests covering all three PL-021b item-3 outcomes: (a) `$TMUX` set to a usable session → that session is used and no session is created; (b) no usable ambient session (unset, or naming a supervisor/flywheel session) → the deterministic `harmonik-<project_hash>-default` session is created, kept alive, and `tmux display-message` is NOT consulted when `$TMUX` is unset; (c) tmux unusable → refusal under the tmux substrate, and under a non-tmux substrate a boot announcement whose text names the lost capabilities (supervisor/auto-revive, launch-layer session creation, attachable panes) and not merely the missing binary. Satisfied by `cmd/harmonik/tmuxhosting_test.go` (`TestResolveTmuxHosting_*`, `TestReportNoTmuxHosting_*`) and the `cmd/harmonik/main_test.go` boot tests above.
...
- **PL-027** — upgrade scenario test: retain fd/exec/event assertions and prove whole-capability upgrade succeeds while a target missing linked archive, receipt/release-marker, trusted-clock, exact-ID status, or receipt-first GC behavior refuses upgrade/downgrade/rollback before handoff with byte preservation.
- **PL-028, PL-028c** — CLI-surface tests cover every verb and both flag shapes. Every daemon-down queue verb exits 17; cancellation additionally proves byte-identical canonical/temp/archive/intent/legacy/receipt/marker/event state. Reachable operator cancellation proves durable archive/intent cleanup and release precede a best-effort typed audit and successful response; graceful shutdown emits no audit.
- **PL-004b** — unit tests: `internal/daemon/projectconfig_hkrcp7_test.go` (13 tests) covering: absent daemon block → zero `DaemonConfig` (no error); `workflow_mode: review-loop` and `workflow_mode: dot` parse to the correct enum values; `workflow_mode: single` → `*ErrWorkflowModeFloorViolation` (PL-004a review floor — load-bearing safety assertion per hk-rcp7 acceptance criteria); `workflow_mode: <invalid>` → `*ErrMalformedConfigYAML`; `max_concurrent: N` (N>0) → `MaxConcurrent=N`; `max_concurrent: 0` and `max_concurrent: -1` → `MaxConcurrent=0` (not-configured, defers to flag-or-default); `daemon.target_branch` stored for observability only (does not override `branching.yaml`); `agents:` block coexists with `daemon:` block; empty `daemon:` block → zero value. The flag-over-config precedence path (PL-004b `flag.Visit` probe in `cmd/harmonik/main.go`) is covered by `internal/core/configprecedence_hka8bg38_test.go`.
- **PL-029** — asset-sync guard: `cmd/harmonik/init_skills_sync_test.go` asserts each embedded fleet skill under `cmd/harmonik/assets/skills/` is byte-identical to its canonical counterpart in `.claude/skills/`; the test fails the build when the embedded copy drifts from source. The phantom-target-branch-guard removal (PL-029(f)) is confirmed by the absence of any `--target-branch ≠ main` check in `cmd/harmonik/init_cmd.go` (commit `abec9fec`, hk-7iyh); `harmonik init --target-branch integration` now exits 0 and writes `integration` to `branching.yaml lands_on`. An explicit unit test for `runInit` asserting exit-0 on a non-main `--target-branch` SHOULD be added in the next init test-suite revision (tracked under §10.2 obligations; blocked only by init test scaffolding, not by spec ambiguity).
...
Owner: foundation-author
Blocks: none (Core tests can be authored against the prose obligations in the interim).
...
Blocks: none (the suggested values above are in use).
...
Default-if-unresolved: Operator uses `tmux list-windows` filtered by the `hk-<hash6>-` sentinel to locate harmonik windows. Later, `hk attach --list-windows` or equivalent SHOULD be added to the operator command surface.
...
| 2026-07-30 | 0.7.1 | agent (spec citation cleanup) | **Rotted pointers repaired across `specs/`. No obligation changed by this pass.** Deleted files that were cited as implementation evidence now name the symbol that carries the behavior today. Line-number citations became symbol names, per the repo rule to cite symbols and never line numbers. The retired `review-loop` workflow mode was dropped from every list that presented it as a live selectable mode, because `core.WorkflowMode.Valid()` accepts only `single` and `dot`. Rules that name `review-loop` as a RETIRED value to reject are unchanged, and so are the event `review_loop_cycle_complete` and the review-loop-failure budget, whose symbols still exist. Where a spec named a test as its conformance sensor and that test no longer exists, the text now says so instead of claiming cover it does not have. |
| 2026-07-28 | 0.7.0 | agent (amendment `review-loop-retirement`) | **PL-004a AMENDED: the `workflow_mode` enum narrows to `{single, dot}`, and the review floor's mechanism changes from silent demotion to loud failure.** Companion to [execution-model.md] v0.10.0 (EM-012a-FLOOR amended; EM-015d / EM-015e retired) and [beads-integration.md] v0.9.0. The floor's GUARANTEE is unchanged — a bead resolved from the daemon-level default or the built-in fallback is never dispatched without a review gate — and is delivered by the embedded exemplar's own reviewer node ([workflow-graph.md §17 WG-047..WG-052]). What changes is the failure mechanism: on embedded-graph load failure (parse, missing, schema-incompat, or template-parameter substitution failure) the daemon MUST fail the run with a diagnostic naming the artifact and the error, and MUST NOT dispatch under any other shape; the `dot → review-loop` demotion is WITHDRAWN. **New config obligation:** `workflow_mode: review-loop` in the project config MUST fail at load with an actionable message naming `dot`, NOT be silently coerced. That is deliberately stricter than the per-bead label path (which degrades a retired label to `dot` with a `bead_label_conflict`), and the requirement states why: a config file is operator-authored and read once at boot, so a wrong value there is worth stopping for, while a stale bead label is data the operator may not control and must not wedge the queue. Retired text quoted in full at the end of PL-004a so a reversal is distinguishable from an omission. Restatement-sync: the PL-005 step-0 cached-state inventory (`workflow_mode_default` enum). PL-004b's floor-violation error surface (`ErrWorkflowModeFloorViolation`, which rejects `workflow_mode: single`) is unchanged and now sits alongside the retired-value rejection. No requirement IDs added, renumbered, or retired. |
| 2026-07-28 | 0.6.2 | agent (tmux-optional-boot / hk-0cjb8) | **The daemon may boot without tmux; the hard `$TMUX` fail-fast is withdrawn.** Pays the spec debt named — and deliberately not paid — by commit `1f8781730`, which changed the code and left `specs/` contradicting it. Provenance: operator direction 2026-07-28 narrowing locked decision #4 (*"we should probably be able to run it any way"*), recorded in `plans/2026-07-27-delete-and-rewrite/CHARTER.md` §3 and now in STATUS.md. **Scope, stated because a wider reading is the obvious misreading: tmux is NOT removed.** It stays the default host and the operator's inspection surface; what goes is the fail-fast that made it the only possibility. **AMENDED: PL-021a** (the lead clause, and the one a partial amendment would have left contradicting the rest) — the "MUST fail-fast rather than silently degrading to non-tmux mode" obligation is now conditioned on whether the selected spawn substrate hosts agents in tmux; the prohibition on *silent* degradation is restated verbatim in force and three dishonest behaviors are enumerated so the amendment cannot be read as relaxing them; the real dependency is declared to be the tmux BINARY at spawn time, never an ambient tmux CLIENT at boot; cites the operator-ratified [agent-input.md] OQ-AIS-003 precedent and the [digest-command.md] DC-009 degradation shape. **PL-021a also carries a SCOPE section bounding this whole amendment to the DAEMON boot path**, because "harmonik runs without tmux" is the natural over-reading and is not true of the shipped system: a dated DECLARED SURFACE GAP records that `harmonik run` self-exec-replaces itself with `tmux new-session` when `$TMUX` is unset **ahead of its own daemon-up check**, so even a thin-socket-client invocation against a running daemon refuses to operate outside tmux; the gap section is careful about what that is (working the constraint around silently by relocating the operator's process, the shape PL-021a's silent-degradation prohibition forbids) and what it is not (a violation of PL-028b item 4, whose subject is the daemon and which forbids deferring to dispatch, not refusing up front). The guard's stated rationale ("windows go in the caller's session") is additionally false in code that resolves the deterministic session and ignores the ambient one. A sweep of `cmd/` confirms it is the ONLY such pre-daemon gate. Recorded, not fixed (`hk-o3aj5`); a future amendment extending these clauses to the CLI surface MUST close it rather than inherit it. **AMENDED: PL-021b item 2** — code-22 probe-failure fatality scoped to the tmux-substrate-selected case, with a dated DECLARED IMPLEMENTATION GAP recording that the implementation returns 1, not 22 (inherited from the deleted guard, not introduced here; `hk-0cjb8`). **AMENDED: PL-021b item 3** — session resolution rewritten as exactly three outcomes (usable ambient session / no usable ambient session + working tmux → the deterministic daemon-owned `harmonik-<project_hash>-default` session, a first-class boot path rather than a degradation / tmux unusable); the `tmux display-message` consultation is FORBIDDEN when `$TMUX` is unset (with no client it returns the server's most-recently-current session, possibly a crew or captain session); `owns_session` is declared a function of who created the session, not of whether `$TMUX` was set, because the new middle outcome is reachable with `$TMUX` set; the "a daemon that reaches the dispatch loop without `TMUX` is a defect" sentence is WITHDRAWN. **AMENDED + RETITLED: PL-028b** — was "`hk` daemon refusal when `$TMUX` is unset", now "`hk` daemon tmux-hosting resolution and honest degradation at startup". All three of its obligations (refuse the ready state, exit 24, never self-create a session) are withdrawn, with the retired text quoted in full so a reversal is distinguishable from an omission. Replaced by four obligations: `$TMUX`-unset is not a Cat 0 failure; unavailable tmux hosting MUST be announced at startup naming the specific lost CAPABILITIES (supervisor and therefore auto-revive, launch-layer session creation, attachable panes) rather than only the missing binary; refuse when the loss is total and degrade only when it is partial; never fake a capability and never defer discovery to the first dispatch. **AMENDED: PL-028 `harmonik runner` step 3** and **PL-028 refinement items 1 and 3** — the locked-decision-#4 parenthetical is narrowed, the code-22 exit is confirmed as `harmonik runner`'s obligation and not `harmonik daemon`'s, and `hk tmux-start` is declared a convenience rather than a precondition (it computes the same session name the daemon now creates for itself). **§8** declares four exit-code residuals (the ON-vs-PL two-name split on code 22, the implementation's 1, ON §8's still-unconditional code-22 row, and code 24's orphaning) rather than leaving them to be discovered. **§10.2** restates the PL-021/PL-021a absence-detection test, which asserted the removed refusal and would now be a false green, and adds a PL-021b-item-3 / PL-028b bullet covering all three resolution outcomes and the announcement's content. No requirement IDs added or renumbered; §10.1 Core profile unchanged (PL-021a remains a gate, on its amended text). Cross-spec: [workspace-model.md] WM-002a, [execution-model.md] EM-015d-RIA, [cognition-loop.md] CL-081 amended in the same pass. Refs: `1f8781730`, `hk-0cjb8`. |
| 2026-07-27 | 0.6.1 | agent (`queue-transaction-contract`) | PL-005 now establishes the flat receipt root and classifies canonical/legacy queues, replace/archive intents, completion receipts, and completion-release markers before readiness without JSONL recovery. PL-027 negotiates the complete queue transaction/marker/clock/GC capability and refuses incompatible upgrade, downgrade, or rollback before handoff. PL-028c adds daemon-only queue cancellation, exit-17 zero-write behavior, and post-release best-effort operator audit ordering. |
...
| 2026-06-13 | 0.5.2 | agent (fleet-portability / hk-hwe) | **Fleet-portability spec amendments: PL-004b, PL-006a amend, PL-006d generalize, PL-029, PL-030, PL-031.** Integrates the reviewed fleet-portability spec-draft clauses (kerf bench) into the canonical specs/ files. **PL-004b** (NEW after PL-004a): Operational config resolution for `workflow_mode`, `max_concurrent`, and `target_branch` — flag > `config.yaml daemon:` block > built-in default; closes the PL-004a spec/code gap where the daemon never read the `daemon:` block. **PL-006a** (AMENDED): Enumerates the launch-layer tmux session names that MUST carry the `harmonik-<project_hash>-` prefix: crew sessions (`harmonik-<project_hash>-crew-<name>`), captain (`harmonik-<project_hash>-captain`), and keeper (`harmonik-<project_hash>-keeper-<role>`). The supervisor session uses the distinct `hk-<project_hash>-daemon-supervise` prefix (outside the swept namespace; no PL-006d sentinel exemption required). Adds a window-naming carve-out note (the `hk-<hash6>-` WINDOW-name sentinel of PL-021b is SESSION-INTERNAL and unaffected). References the new `harmonik project-hash` subcommand per §PL-031. **PL-006d** (GENERALIZED, retitled "Orphan-sweep exclusion for live-owned launch-layer sessions"): the exclusion now covers all live-owned launch-layer sessions via three owner-proof mechanisms: (i) supervisor/flywheel sentinel (UNCHANGED), (ii) captain/keeper `captain.sentinel` + PID-live (NEW), (iii) crew registry-record + live-pane-PID probe (NEW). Adds `crew_sessions_skipped` and `captain_sessions_skipped` integer fields to the `daemon_orphan_sweep_completed` payload (additive-tolerance per EV-029). **C3 NOTE** (added after PL-019): `harmonik supervise` is the canonical per-project supervisor; the hand-authored `/tmp/hk-daemon-supervise.sh` is retired; `scripts/hk-supervise.sh` is the supported out-of-band shell fallback; both derive session names from the PL-006a `project_hash`. **PL-029** (NEW §4.11): Portable project initialization contract — `harmonik init` against a foreign repo provisions 8 fleet skills from `embed.FS`, renders `AGENTS.md` from embedded template, writes 3 scaffold files, creates runtime dirs, is idempotent, and removes the phantom target-branch guard. **PL-030** (NEW §4.11): Portable skill-resolution convention — published skills/docs/scripts MUST resolve the project root from `$HARMONIK_PROJECT`, MUST NOT contain a literal hard-coded absolute path (e.g. `/Users/gb/github/harmonik`); project-local-vs-global disambiguation MUST survive as prose. All `.claude/skills/` files verified to carry no literal `/Users/gb/github/harmonik` after commit `e3847c94`. **PL-031** (NEW, adjacent to PL-006a in §4.2): Read-only `harmonik project-hash [--project DIR]` subcommand — prints exactly the 12-hex PL-006a `project_hash` + newline; delegates to `lifecycle.ComputeProjectHash` (the identical accessor the Go core uses); side-effect-free (no daemon, no `$TMUX`); exits non-zero with empty stdout on error for shell-guard degradation. Implementation verified to use `lifecycle.ComputeProjectHash` at `cmd/harmonik/project_hash_cmd.go`. Refs: hk-hwe, hk-5u2b (fleet-portability), e4e167b8, b5b20847, 18ece144, e3847c94. |
| 2026-06-11 | 0.5.1 | agent (standard-bead-dot / hk-o7j) | **PL-004a default-workflow-mode flip: `single` → `dot` with `review-loop` floor.** Consistency amendment landing the `standard-bead-dot` kerf work's flip of the default `workflow_mode`. **PL-004a** rewritten: when the project-config `workflow_mode` field is absent the daemon default MUST now be `dot` (resolving to the embedded `standard-bead.dot` canonical graph at `internal/daemon/standard-bead.dot`, registered per [execution-model.md §7.5.1 EM-055] / [§7.5.5 EM-059]) — NOT the historical `single`. Added the review-floor: on embedded-graph load failure (parse / missing / schema-incompat per [execution-model.md §7.5.3 EM-057]) the default MUST fall back to `review-loop`, NEVER `single`; `single` is reachable ONLY via an explicit per-bead `workflow:single` label (audited via `review_bypassed`), never from the daemon-level default or built-in fallback. The four-tier precedence restatement's tier-4 built-in fallback updated from `single` to `dot` (with the [execution-model.md §4.3 EM-012a-FLOOR] `review-loop` floor); cross-reference updated to [execution-model.md §4.3 EM-012a], which cites PL-004a by name as its tier-3 daemon-level config anchor — the two now agree. **PL-005 step 0** coupled restatement synced: absence of the field defaults the cached `workflow_mode_default` to `dot` with the `review-loop` floor (was `single`). No requirement IDs renumbered; all other PL requirements unchanged. Refs: hk-o7j (standard-bead-dot epic). |
| 2026-05-30 | 0.5.0 | agent (flywheel spec-bundle hk-j7o3i) | **PL-019 promotion + PL-006d + PL-028d/digest additions (flywheel design).** (1) **PL-019** expanded from an informative placeholder to a full normative section (a–h) specifying the supervisor/cognition process: process separation, spawn surface (`harmonik supervise start`), singleton flock at `.harmonik/cognition/supervisor.lock`, pidfile discipline, config JSON with `daemon_instance_id`, tmux session `harmonik-<project_hash>-flywheel` with `remain-on-exit`, file-surface inventory under `.harmonik/cognition/`, PL-018 relationship confirmation (daemon LLM-free invariant unaffected). Daemon-side obligations: refuse to start the supervisor, do NOT read/write cognition subtree beyond sentinel probe per PL-006d. (2) **PL-006d** (new, after PL-006c): orphan-sweep exclusion for coordinator tmux sessions via `.harmonik/cognition/supervisor.sentinel` file; sentinel-present+PID-live → SKIP with structured-log `orphan_sweep_skipped_coordinator_session`; stale-sentinel → kill + remove sentinel; adds `coordinator_sessions_skipped` integer to `daemon_orphan_sweep_completed` payload per [event-model.md §8.7.14] additive-tolerance precedent. Refs: hk-hc3qq. (3) **PL-028** gains `harmonik supervise <verb>` and `harmonik digest` bullets. **PL-028d** (new, after PL-028c): full verb contract for `supervise` family — pre-`flag.Parse` dispatch on `os.Args[1]=="supervise"`; 6 verbs (`start`, `stop`, `status`, `attach`, `restart`, `logs`) with per-verb obligations; exit codes 17/25/0; daemon-not-running path. §3 Glossary gains `supervisor process`, `cognition subtree`, `coordinator sentinel`. PL-004 file-surface inventory gains cognition-subtree exclusion note. §10.1 conformance profile: supervisor integration is deferred; PL-006d sentinel-probe MUST be present even with no supervisor (degrades gracefully). Refs: hk-j7o3i, hk-hc3qq (orphan sweep kills flywheel pane). |
