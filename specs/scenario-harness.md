...
The harness's read of the captured JSONL is governed by [event-model.md §6.2]'s post-fsync torn-tail rule: a torn tail at the end of the file (well-formed lines followed by a partial trailing record after the last fsync) MUST be skipped silently per EV §6.2 — this is normal post-fsync behavior, not a corruption signal. The harness MUST classify the scenario as `verdict=error` with `failure_class=harness-internal-error` (NOT as `assertion-failed`) when, and only when, the log read fails for one of: (i) the event-log directory or file does not exist; (ii) a permissions error prevents reading; (iii) a JSON-parse error occurs at a position other than the post-fsync tail (mid-file corruption); (iv) a disk-full or other I/O error during read. The harness MUST NOT silently treat absent events as "not emitted" when the log itself is corrupt or incomplete.

> RATIONALE — the shed detector is gone, and what it would take to bring it back. This list held a fifth item until 2026-08-05: an observed `bus_overflow` event meant the bus had dropped events during the scenario, so assertion completeness was defeated. The item could not fire. [event-model.md §4.7 EV-011a] specifies a bounded queue for each consumer that sheds on saturation and emits `bus_overflow`, and `internal/eventbus` does not implement any of it — each asynchronous or observer delivery starts its own goroutine with no queue in front of it. Nothing in the tree emits `bus_overflow`, so the detector claimed a protection the harness did not have. The unbuilt shed path is tracked separately. If EV-011a is built, this item MUST come back to the closed list and to `ReadEventLog`, because a shed event is invisible to every other item here: the log stays well-formed and simply holds less than it should.
...
Emitted when the twin encounters an **unrecoverable internal error** that must be visible on the wire stream. Examples include: `call_stop_hook` invoked without a prior settings load, `commit_on_cue` invoked without `--worktree-path`, malformed `settings.json`, or `signal_interrupt` invoked with missing required fields. The twin exits with code 1 immediately after emitting this message.

`twin_error` is a terminal event for the twin process: exactly one `twin_error` is emitted per fatal condition, and no further script steps are attempted after it is emitted.

| Field | Type | Description |
|---|---|---|
| `type` | `string` | Always `"twin_error"`. |
| `reason` | `string` | Short human-readable description of the unrecoverable error. Identifies the script step that failed and the specific condition (e.g., `"call_stop_hook: settings not loaded (--worktree-path was not supplied)"`). |

Assertion scenarios MAY use `event_present` with `type: twin_error` to assert that a specific internal error was surfaced, or use the absence of this message (via `event_absent`) to verify a clean execution path.
...
|---|---|---|---|
| 2026-08-05 | 0.2.4 | bravo | Retire SH-024 item (v), the observed-`bus_overflow` detector, from the closed list, from the §8.7 detection list and from the §10 test list. The detector had no failing input: [event-model.md §4.7 EV-011a] specifies the shed path that emits `bus_overflow` and `internal/eventbus` implements none of it, so nothing in the tree can emit the event. A new RATIONALE under SH-024 records why the item is gone and states that building EV-011a MUST bring it back, because a shed event is invisible to the four remaining items. No requirement IDs added, renumbered or retired; SH-024 keeps its ID and its normative force. Code change is the matching branch in `internal/scenario/asserteval.go` `ReadEventLog`. Refs: hk-oxbv9, hk-5pj8r. |
