...
## Controlled readiness procedure

The readiness procedure MUST fetch, check out, verify, and record the full
candidate commit before build. It MUST retain an evidence manifest containing
the candidate, effective configuration, controlled-load result, Step 9 result,
core-loop result, daemon log, complete event trace, batch JSON, and assessor
report. It MUST submit exactly one declared local stream item at concurrency
one and reject append, remote, Pi, cross-repository, and wave input. A passing
readiness procedure MUST NOT call `feedback`.
